...
footerContent18+[ComponentContent](ComponentContent.md)否是
...
2. [scroller](#ZH-CN_TOPIC_0000002497604876__waterflowoptions对象说明)的[currentOffset](zh-cn_topic_0000002497444896.html#ZH-CN_TOPIC_0000002497444896__currentoffset)接口返回的总偏移量在触发跳转或数据更新后不准确，在回滑到顶部时会重新校准。

3. 如果在同一帧内调用跳转（如无动画的[scrollToIndex](Scroll.md#ZH-CN_TOPIC_0000002497444896__scrolltoindex)、[scrollEdge](Scroll.md#ZH-CN_TOPIC_0000002497444896__scrolledge)）和输入偏移量（如滑动手势或滚动动画），两者都会生效。

4. 调用无动画的[scrollToIndex](Scroll.md#ZH-CN_TOPIC_0000002497444896__scrolltoindex)进行跳转，如果跳转到较远位置（超过视窗内的FlowItem数量的位置）时，移动窗口模式对总偏移量进行估算。
...
参数名类型必填说明value[FlexDirection](枚举说明.md#ZH-CN_TOPIC_0000002529284967__flexdirection)是
...
设置前后两个方向的嵌套滚动模式，实现与父组件的滚动联动。使用方法参考[嵌套滚动实现方式二](Scroll.md#ZH-CN_TOPIC_0000002497444896__示例3嵌套滚动实现方式二)。
...
设置为false时，在首次显示、不带动画[scrollToIndex](Scroll.md#ZH-CN_TOPIC_0000002497444896__scrolltoindex)跳转场景，若当帧布局耗时超过50ms，会将WaterFlow区域内尚未布局的子组件延后到下一帧进行布局。
...
frameNode中[getEvent('WaterFlow')](FrameNode.md#ZH-CN_TOPIC_0000002529284787__geteventwaterflow19)方法的返回值，可用于给WaterFlow节点设置滚动事件。
...
该示例通过[priorityGesture](绑定手势方法.md#ZH-CN_TOPIC_0000002529284849__prioritygesture)和[PinchGesture](PinchGesture.md)实现了双指缩放改变列数效果。
...
从API version 18开始，该示例通过[WaterFlowOptions对象说明](#ZH-CN_TOPIC_0000002497604876__waterflowoptions对象说明)的footerContent接口，实现了WaterFlow组件设置尾部组件。通过[ComponentContent](zh-cn_topic_0000002497444816.html)的update函数更新尾部组件。
...
该示例通过FrameNode中的[getEvent('WaterFlow')](FrameNode.md#ZH-CN_TOPIC_0000002529284787__geteventwaterflow19)获取[UIWaterFlowEvent](#ZH-CN_TOPIC_0000002497604876__uiwaterflowevent19)，并为WaterFlow设置滚动事件回调，用于事件监听方因无法直接修改页面代码而无法使用声明式接口设置回调的场景。
