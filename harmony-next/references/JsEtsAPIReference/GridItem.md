...
- 当Grid中存在大量GridItem时，使用[columnStart](#ZH-CN_TOPIC_0000002497604874__columnstart)/[columnEnd](#ZH-CN_TOPIC_0000002497604874__columnend)、[rowStart](#ZH-CN_TOPIC_0000002497604874__rowstart)/[rowEnd](#ZH-CN_TOPIC_0000002497604874__rowend)设置GridItem大小会导致在使用scrollToIndex滑动到指定Index时，依次遍历GridItem节点，耗时较长。建议使用[GridLayoutOptions](zh-cn_topic_0000002529444839.html#ZH-CN_TOPIC_0000002529444839__gridlayoutoptions10对象说明)布局，以提高查找GridItem位置的效率。最佳实践请参考[优化Grid组件加载慢丢帧问题](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-improve_grid_performance)。
...
参数名类型必填说明value11+[GridItemOptions](#ZH-CN_TOPIC_0000002497604874__griditemoptions11对象说明)否为GridItem提供可选参数，该对象内含有[GridItemStyle](#ZH-CN_TOPIC_0000002497604874__griditemstyle11枚举说明)枚举类型的style参数。
...
需要指定GridItem起始行列号和所占行列数的场景推荐使用[Grid的layoutOptions参数](Grid.md#ZH-CN_TOPIC_0000002529444839__gridlayoutoptions10对象说明)，详细可参考[Grid的示例1](Grid.md#ZH-CN_TOPIC_0000002529444839__示例1固定行列grid)和[Grid的示例3](Grid.md#ZH-CN_TOPIC_0000002529444839__示例3可滚动grid设置跨行跨列节点)。
...
属性设置情况异常类型修正后布局规则仅设置columnsTemplate任意行列异常按一行一列布局。仅设置rowsTemplate任意行列异常按一行一列布局。同时设置rows/columnsTemplaterowStart < rowEnd行跨度 = min(rowEnd-rowStart+1, 总行数)。同时设置rows/columnsTemplaterowStart > rowEnd按一行一列布局。同时设置rows/columnsTemplatecolumnStart < columnEnd列跨度 = min(columnEnd-columnStart+1, 总列数)。同时设置rows/columnsTemplatecolumnStart > columnEnd按一行一列布局。
...
该属性需要在设置[选中态样式](多态样式.md#ZH-CN_TOPIC_0000002497604846__statestyles)前使用才能生效选中态样式。
...
名称类型只读可选说明style[GridItemStyle](#ZH-CN_TOPIC_0000002497604874__griditemstyle11枚举说明)否是
...
名称值说明NONE0无样式。PLAIN1显示Hover、Press态样式。
...
GridItem通过设置合理的ColumnStart、ColumnEnd、RowStart、RowEnd属性来设置自身位置。需要指定GridItem起始行列号和所占行列数的场景推荐使用[Grid的layoutOptions参数](Grid.md#ZH-CN_TOPIC_0000002529444839__gridlayoutoptions10对象说明)，详细可参考[Grid的示例1](Grid.md#ZH-CN_TOPIC_0000002529444839__示例1固定行列grid)和[Grid的示例3](Grid.md#ZH-CN_TOPIC_0000002529444839__示例3可滚动grid设置跨行跨列节点)。
