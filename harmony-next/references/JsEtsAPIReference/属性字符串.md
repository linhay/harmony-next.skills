...
方便灵活应用文本样式的对象，可通过TextController中的[setStyledString](Text.md#ZH-CN_TOPIC_0000002497444914__setstyledstring12)方法与Text组件绑定，可通过RichEditorStyledStringController中的[setStyledString](RichEditor.md#ZH-CN_TOPIC_0000002497604894__setstyledstring12)方法与RichEditor组件绑定。

从API version 12开始支持。后续版本如有新增内容，则采用上角标单独标记该内容的起始版本。

从API version 20开始，支持通过[getParagraphs](Class (MeasureUtils).md#ZH-CN_TOPIC_0000002497604780__getparagraphs20)获取属性字符串的文本布局信息。

属性字符串目前不支持在worker线程中使用。

属性字符串通过controller绑定时，需要等待布局完成后，绑定生效。当[measure](FrameNode.md#ZH-CN_TOPIC_0000002529284787__measure12)和setStyledString同时使用，开发者需要通过[@ohos.arkui.inspector (布局回调)](@ohos.arkui.inspector (布局回调).md)判断布局完成，再绑定属性字符串。

#### 规则说明

- 当组件样式和属性字符串中的样式冲突时，冲突部分以属性字符串设置的样式为准，未冲突部分则生效组件的样式。
- 当属性字符串和[Text](Text.md)子组件冲突时，属性字符串优先级高，即当Text组件中绑定了属性字符串，忽略Text组件下包含[Span](Span.md)等子组件的情况。
- 不支持@State修饰。
- 建议将StyledString定义为成员变量，从而避免应用退后台后被销毁。
- 不支持在[loadContent()](Interface (Window).md#ZH-CN_TOPIC_0000002497604802__loadcontent9)之前创建。
...
以下错误码详细介绍请参考[通用错误码](通用错误码.md)和[属性字符串错误码](属性字符串错误码.md)。
...
被清空样式类型对象属性使用的是对应[Text](Text.md)组件属性的设置值，若Text组件未设置值，则使用对应Text组件属性的默认值。
...
fontStyle[FontStyle](枚举说明.md#ZH-CN_TOPIC_0000002529284967__fontstyle)是是
...
strokeWidth20+[LengthMetrics](Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)否是
...
名称类型只读可选说明type[TextDecorationType](枚举说明.md#ZH-CN_TOPIC_0000002529284967__textdecorationtype)是否
...
style[TextDecorationStyle](枚举说明.md#ZH-CN_TOPIC_0000002529284967__textdecorationstyle12)是是
...
参数名类型必填说明value[LengthMetrics](Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)是文本基线偏移量设置项。如果LengthMetrics的unit值是percent，该设置不生效。
...
参数名类型必填说明value[LengthMetrics](Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)是文本字符间距设置项。如果LengthMetrics的unit值是percent，该设置不生效。
...
参数名类型必填说明lineHeight[LengthMetrics](Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)是文本行高设置项。如果LengthMetrics的value值不大于0时，不限制文本行高，自适应字体大小。
...
名称类型只读可选说明value[PixelMap](Interface (PixelMap).md)是否
...
verticalAlign[ImageSpanAlignment](枚举说明.md#ZH-CN_TOPIC_0000002529284967__imagespanalignment10)是是

获取属性字符串的图片对齐方式。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

objectFit[ImageFit](枚举说明.md#ZH-CN_TOPIC_0000002529284967__imagefit)是是
...
参数名类型必填说明attachmentOptional<[AttachmentType](#ZH-CN_TOPIC_0000002497444920__attachmenttype15)>是PixelMap类型或[ResourceStr](zh-cn_topic_0000002497604974.html#ZH-CN_TOPIC_0000002497604974__resourcestr)类型图片设置项。
...
类型说明[ColorFilter](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__colorfilter9)ColorFilter类型图片颜色滤镜设置项。[DrawingColorFilter](Class (ColorFilter).md)DrawingColorFilter类型图片颜色滤镜设置项。

#### ImageAttachmentInterface对象说明

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

名称类型只读可选说明value[PixelMap](Interface (PixelMap).md)否否设置图片数据源。**元服务API：** 从API version 12开始，该接口支持在元服务中使用。size[SizeOptions](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__sizeoptions)否是
...
名称类型只读可选说明margin[LengthMetrics](Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12) | [Margin](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__margin)否是
...
padding[LengthMetrics](Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12) | [Padding](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__padding)否是
...
borderRadius[LengthMetrics](Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12) | [BorderRadiuses](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__borderradiuses9)否是
...
参数名类型必填说明context[DrawContext](Graphics.md#ZH-CN_TOPIC_0000002529444761__drawcontext)是
...
名称类型只读可选说明textAlign[TextAlign](枚举说明.md#ZH-CN_TOPIC_0000002529284967__textalign)是是
...
overflow[TextOverflow](枚举说明.md#ZH-CN_TOPIC_0000002529284967__textoverflow)是是

获取属性字符串文本段落超长时的显示方式。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

wordBreak[WordBreak](枚举说明.md#ZH-CN_TOPIC_0000002529284967__wordbreak11)是是
...
textIndent[LengthMetrics](Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)否是
...
overflow[TextOverflow](枚举说明.md#ZH-CN_TOPIC_0000002529284967__textoverflow)否是
...
wordBreak[WordBreak](枚举说明.md#ZH-CN_TOPIC_0000002529284967__wordbreak11)否是
...
leadingMargin[LengthMetrics](Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12) | [LeadingMarginPlaceholder](RichEditor.md#ZH-CN_TOPIC_0000002497604894__leadingmarginplaceholder11)否是
...
paragraphSpacing19+[LengthMetrics](Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)否是
...
类型说明[LengthMetrics](Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)
...
名称类型只读可选说明textBackgroundStyle[TextBackgroundStyle](Span.md#ZH-CN_TOPIC_0000002529284887__textbackgroundstyle11对象说明)是否
...
参数名类型必填说明textBackgroundStyle[TextBackgroundStyle](Span.md#ZH-CN_TOPIC_0000002529284887__textbackgroundstyle11对象说明)是
...
从API version 12开始，该示例通过[CustomSpan](#ZH-CN_TOPIC_0000002497444920__customspan)接口和[measureTextSize](zh-cn_topic_0000002497604780.html#ZH-CN_TOPIC_0000002497604780__measuretextsize12)实现属性字符串设置自定义绘制Span。
...
从API version 22开始，该示例通过给[ResourceImageAttachmentOptions](#ZH-CN_TOPIC_0000002497444920__resourceimageattachmentoptions15)设置supportSvg2属性，使[SVG标签解析能力增强功能](zh-cn_topic_0000002529284895.html#ZH-CN_TOPIC_0000002529284895__svg易用性提升)的SVG易用性提升能力生效。
