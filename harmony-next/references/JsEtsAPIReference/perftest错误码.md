...
1. 尝试通过重试解决IPC传输失败问题。
1. 判断PerfTest对象是否已被销毁，若已销毁需要重新[创建PerfTest对象](@ohos.test.PerfTest.md#ZH-CN_TOPIC_0000002529445655__create)。
...
首先保证[PerfTest.run](@ohos.test.PerfTest.md#ZH-CN_TOPIC_0000002529445655__run)接口已执行完成且未抛出异常，确保测试数据已采集完成，再获取指定数据。
