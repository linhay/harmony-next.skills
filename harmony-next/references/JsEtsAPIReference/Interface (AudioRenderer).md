...
在使用AudioRenderer的接口之前，需先通过[createAudioRenderer](Functions.md#ZH-CN_TOPIC_0000002529285689__audiocreateaudiorenderer8)获取AudioRenderer实例。
...
名称类型只读可选说明state8+[AudioState](Enums.md#ZH-CN_TOPIC_0000002529285695__audiostate8)是否音频渲染器的状态。
...
参数名类型必填说明mode[AudioEffectMode](Enums.md#ZH-CN_TOPIC_0000002529285695__audioeffectmode10)是音效模式。callbackAsyncCallback<void>是回调函数。当设置当前音效模式成功，err为undefined，否则为错误对象。
...
参数名类型必填说明mode[AudioEffectMode](Enums.md#ZH-CN_TOPIC_0000002529285695__audioeffectmode10)是音效模式。
...
参数名类型必填说明callbackAsyncCallback<[AudioEffectMode](Enums.md#ZH-CN_TOPIC_0000002529285695__audioeffectmode10)>是回调函数。当获取当前音效模式成功，err为undefined，data为获取到的当前音效模式；否则为错误对象。
...
类型说明Promise<[AudioEffectMode](Enums.md#ZH-CN_TOPIC_0000002529285695__audioeffectmode10)>Promise对象，返回当前音效模式。
...
清空缓冲区（[AudioState](Enums.md#ZH-CN_TOPIC_0000002529285695__audiostate8)为STATE_RUNNING、STATE_PAUSED、STATE_STOPPED状态下可用）。使用Promise异步回调。
...
参数名类型必填说明mode[InterruptMode](Enums.md#ZH-CN_TOPIC_0000002529285695__interruptmode9)是焦点模型。
...
参数名类型必填说明mode[InterruptMode](Enums.md#ZH-CN_TOPIC_0000002529285695__interruptmode9)是焦点模型。callbackAsyncCallback<void>是回调函数。当设置应用的焦点模型成功，err为undefined，否则为错误对象。
...
参数名类型必填说明callbackAsyncCallback<[AudioDeviceDescriptors](Types.md#ZH-CN_TOPIC_0000002497605704__audiodevicedescriptors)>是回调函数。当获取音频流输出设备信息成功，err为undefined，data为获取到的音频流输出设备信息；否则为错误对象。
...
类型说明Promise<[AudioDeviceDescriptors](Types.md#ZH-CN_TOPIC_0000002497605704__audiodevicedescriptors)>Promise对象，返回音频流的输出设备信息。
...
类型说明[AudioDeviceDescriptors](Types.md#ZH-CN_TOPIC_0000002497605704__audiodevicedescriptors)返回音频流的输出设备信息。
...
参数名类型必填说明mode[ChannelBlendMode](Enums.md#ZH-CN_TOPIC_0000002529285695__channelblendmode11)是声道混合模式类型。
...
- 本接口仅适用于[StreamUsage](Enums.md#ZH-CN_TOPIC_0000002529285695__streamusage)为语音消息、VoIP语音通话或者VoIP视频通话的场景，支持听筒、扬声器和系统默认设备。
...
参数名类型必填说明deviceType[DeviceType](Enums.md#ZH-CN_TOPIC_0000002529285695__devicetype)是
...
参数名类型必填说明typestring是事件回调类型，支持的事件为'stateChange'，当AudioRenderer的状态发生变化时，触发该事件。callbackCallback<[AudioState](Enums.md#ZH-CN_TOPIC_0000002529285695__audiostate8)>是回调函数，返回当前音频的状态。
...
参数名类型必填说明typestring是事件回调类型，支持的事件为'stateChange'，当取消监听到达标记事件时，触发该事件。callbackCallback<[AudioState](Enums.md#ZH-CN_TOPIC_0000002529285695__audiostate8)>否回调函数，返回当前音频的状态。
...
参数名类型必填说明typestring是事件回调类型，支持的事件为'outputDeviceChange'，当音频输出设备发生变化时，触发该事件。callbackCallback<[AudioDeviceDescriptors](Types.md#ZH-CN_TOPIC_0000002497605704__audiodevicedescriptors)>是回调函数，返回当前音频流的输出设备描述信息。
...
参数名类型必填说明typestring是事件回调类型，支持的事件为'outputDeviceChange'，当取消监听音频输出设备变化事件时，触发该事件。callbackCallback<[AudioDeviceDescriptors](Types.md#ZH-CN_TOPIC_0000002497605704__audiodevicedescriptors)>否回调函数，返回当前音频流的输出设备描述信息。
...
参数名类型必填说明typestring是事件回调类型，支持的事件为'writeData'，当需要写入音频数据时，触发该事件。callback[AudioRendererWriteDataCallback](Types.md#ZH-CN_TOPIC_0000002497605704__audiorendererwritedatacallback12)是

回调函数，入参代表应用接收待写入的数据缓冲区。

API version 11 不支持返回回调结果，从 API version 12 开始支持返回回调结果[AudioDataCallbackResult](Enums.md#ZH-CN_TOPIC_0000002529285695__audiodatacallbackresult12)。
...
参数名类型必填说明typestring是事件回调类型，支持的事件为'writeData'，当取消监听音频数据写入回调事件时，触发该事件。callback[AudioRendererWriteDataCallback](Types.md#ZH-CN_TOPIC_0000002497605704__audiorendererwritedatacallback12)否
...
参数名类型必填说明rate[AudioRendererRate](Enums.md#ZH-CN_TOPIC_0000002529285695__audiorendererrate8)是渲染的速率。callbackAsyncCallback<void>是回调函数。当设置音频渲染速率成功，err为undefined，否则为错误对象。
...
参数名类型必填说明rate[AudioRendererRate](Enums.md#ZH-CN_TOPIC_0000002529285695__audiorendererrate8)是渲染的速率。
...
参数名类型必填说明callbackAsyncCallback<[AudioRendererRate](Enums.md#ZH-CN_TOPIC_0000002529285695__audiorendererrate8)>是回调函数。当获取当前渲染速率成功，err为undefined，data为获取到的当前渲染速率；否则为错误对象。
...
类型说明Promise<[AudioRendererRate](Enums.md#ZH-CN_TOPIC_0000002529285695__audiorendererrate8)>Promise对象，返回渲染速率。
...
类型说明[AudioRendererRate](Enums.md#ZH-CN_TOPIC_0000002529285695__audiorendererrate8)返回渲染速率。
...
- 该接口仅支持类型为[STREAM_USAGE_MUSIC](Enums.md#ZH-CN_TOPIC_0000002529285695__streamusage)、[STREAM_USAGE_MOVIE](Enums.md#ZH-CN_TOPIC_0000002529285695__streamusage)或[STREAM_USAGE_AUDIOBOOK](Enums.md#ZH-CN_TOPIC_0000002529285695__streamusage)的音频流。
- 该接口不支持高清通路的响度设置。
...
Operation is not supported on this renderer, e.g. the stream usage of this renderer is not one of [STREAM_USAGE_MUSIC](Enums.md#ZH-CN_TOPIC_0000002529285695__streamusage),

[STREAM_USAGE_MOVIE](Enums.md#ZH-CN_TOPIC_0000002529285695__streamusage), or [STREAM_USAGE_AUDIOBOOK](Enums.md#ZH-CN_TOPIC_0000002529285695__streamusage), or this renderer is routed through the high-resolution playback path.
