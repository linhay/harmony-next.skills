...
名称typedef关键字描述[CloudDisk_PathInfo](CloudDisk_PathInfo.md)CloudDisk_PathInfo文件路径信息。[CloudDisk_FileSyncState](CloudDisk_FileSyncState.md)CloudDisk_FileSyncState文件的同步状态。[CloudDisk_ChangeData](CloudDisk_ChangeData.md)CloudDisk_ChangeData定义了同步根路径下单个文件变更事件的数据结构。该结构包含有关文件变更的详细信息，包括唯一ID、父目录的唯一ID、相对路径、变更类型、文件大小和时间戳。[CloudDisk_ChangesResult](CloudDisk_ChangesResult.md)CloudDisk_ChangesResult查询同步根路径中文件变更的结果。该结构体包含同步根路径中文件的变更数据，包括下一个更新序列号、结尾标志以及变更数据项数组。[CloudDisk_FailedList](CloudDisk_FailedList.md)CloudDisk_FailedList同步操作中失败的文件列表信息。该结构包含文件路径信息以及失败的具体错误原因。[CloudDisk_ResultList](CloudDisk_ResultList.md)CloudDisk_ResultList表示一个文件同步操作的结果。该结构体包含文件的绝对路径、同步结果，以及同步状态或失败原因。[CloudDisk_DisplayNameInfo](CloudDisk_DisplayNameInfo.md)CloudDisk_DisplayNameInfo定义同步根路径的显示名称信息。[CloudDisk_SyncFolder](CloudDisk_SyncFolder.md)CloudDisk_SyncFolder同步根属性信息。
...
参数项描述const CloudDisk_SyncFolderPath syncFolderPath查询的同步根路径，参考：{@link CloudDisk_SyncFolderPath}。uint64_t startUsn查询起始的变更序列，范围：[0, 2^64 - 1]。size_t count查询文件变更的数量，范围：[1, 100]。[CloudDisk_ChangesResult](CloudDisk_ChangesResult.md) **changesResult表示查询文件变更的结果。详情请参阅[CloudDisk_ChangesResult](CloudDisk_ChangesResult.md)。
...
参数项描述const CloudDisk_SyncFolderPath syncFolderPath待设置的同步根路径，参考：{@link CloudDisk_SyncFolderPath}。[const CloudDisk_FileSyncState](CloudDisk_FileSyncState.md) fileSyncStates[]指定文件路径及其目标同步状态的[CloudDisk_FileSyncState](CloudDisk_FileSyncState.md)数组。size_t bufferLength待设置同步状态数组的长度，范围：[1, 100]。[CloudDisk_FailedList](CloudDisk_FailedList.md) **failedLists输出参数。返回一个指向[CloudDisk_FailedList](CloudDisk_FailedList.md)数组的指针，该数组包含设置失败的文件。size_t *failedCount输出参数。设置同步状态失败的文件列表数组长度。
...
参数项描述const CloudDisk_SyncFolderPath syncFolderPath待查询的同步根路径，参考：{@link CloudDisk_SyncFolderPath}。[const CloudDisk_PathInfo](CloudDisk_PathInfo.md) paths[]待查询同步状态[CloudDisk_PathInfo](CloudDisk_PathInfo.md)的数组。size_t bufferLength待查询同步状态的数组的长度，范围：[1, 100]。[CloudDisk_ResultList](CloudDisk_ResultList.md) **resultLists输出参数。返回一个查询到的文件同步操作结果，详情可参考：[CloudDisk_ResultList](CloudDisk_ResultList.md)。size_t *resultCount输出参数。返回失败的文件数量。
...
参数项描述[const CloudDisk_SyncFolder](CloudDisk_SyncFolder.md) *syncFolder待注册的同步根路径，参考：[CloudDisk_SyncFolder](CloudDisk_SyncFolder.md)。
...
参数项描述[CloudDisk_SyncFolder](CloudDisk_SyncFolder.md) **syncFolders输出参数。返回同步根路径数组[CloudDisk_SyncFolder](CloudDisk_SyncFolder.md)。size_t *count输出参数。当前网盘注册的所有同步根的数量。
