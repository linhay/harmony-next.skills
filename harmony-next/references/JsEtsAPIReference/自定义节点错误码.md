...
1. 接口支持多线程调用，但是传入的节点不是通过支持多线程的[createNode](ArkUI_NativeNodeAPI_1.md#ZH-CN_TOPIC_0000002497605120__createnode)接口创建的。

**处理步骤**

1. 调整函数调用时机，确保接口在UI线程调用。
1. 将传入的节点从UI主树上卸载后再调用接口。
1. 使用支持多线程调用[createNode](ArkUI_NativeNodeAPI_1.md#ZH-CN_TOPIC_0000002497605120__createnode)接口创建节点后再调用接口。