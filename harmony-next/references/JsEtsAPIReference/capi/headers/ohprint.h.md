...
**相关模块：**[OH_Print](../../topics/misc/OH_Print.md)

#### 汇总

#### 结构体

名称typedef关键字描述[Print_Margin](../../topics/media/Print_Margin.md)Print_Margin打印边距。[Print_PageSize](../../topics/misc/Print_PageSize.md)Print_PageSize纸张大小信息。[Print_Range](../../topics/misc/Print_Range.md)Print_Range打印范围。[Print_PrintAttributes](../../topics/misc/Print_PrintAttributes.md)Print_PrintAttributes打印属性结构体。[Print_PrintDocCallback](../../topics/misc/Print_PrintDocCallback.md)Print_PrintDocCallback打印文档任务回调结构体。
...
参数项描述const char *jobId打印任务id。uint32_t fd回写的文件句柄。[ const Print_PrintAttributes](../../topics/misc/Print_PrintAttributes.md) *oldAttrs用户设置打印参数变化前的参数。[ const Print_PrintAttributes](../../topics/misc/Print_PrintAttributes.md) *newAttrs用户设置打印参数变化后的参数。[ Print_WriteResultCallback](ohprint.h.md#ZH-CN_TOPIC_0000002497445554__print_writeresultcallback) writeCallback使用方回写完文件后调用回调函数通知打印服务。
...
参数项描述const char *printJobName打印任务名称。[Print_PrintDocCallback](../../topics/misc/Print_PrintDocCallback.md) printDocCallback打印文档任务回调结构体。void *context调用接口的ability的上下文。
