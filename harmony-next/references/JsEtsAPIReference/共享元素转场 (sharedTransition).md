...
可以通过设置组件的sharedTransition属性将该元素标记为共享元素并设置对应的共享元素转场动效。sharedTransition仅发生在[页面路由（router）](@ohos.router (页面路由)(不推荐).md)跳转时。
...
curve[Curve](枚举说明.md#ZH-CN_TOPIC_0000002529284967__curve) | string | [ICurve](@ohos.curves (插值计算).md#ZH-CN_TOPIC_0000002497604788__icurve9)否是
...
motionPath[MotionPathOptions](路径动画 (motionPath).md)否是运动路径信息。zIndexnumber否是
...
type[SharedTransitionEffectType](枚举说明.md#ZH-CN_TOPIC_0000002529284967__sharedtransitioneffecttype)否是
