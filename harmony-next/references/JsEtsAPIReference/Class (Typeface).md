...
参数名类型必填说明filePathstring是表示字体资源存放的路径。应用沙箱路径和真实物理路径的对应关系请参考[应用沙箱路径和真实物理路径的对应关系](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-sandbox-directory#应用沙箱路径和真实物理路径的对应关系)。typefaceArguments[TypefaceArguments](zh-cn_topic_0000002497605986.html)是表示字体属性。
...
参数名类型必填说明rawfile[Resource](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resource)是指定字体文件对应的资源对象。当前只支持$rawfile格式引用的资源对象，对应格式写为$rawfile('filePath')，其中filePath为指定字体文件相对于工程中resources/rawfile目录的相对路径。typefaceArguments[TypefaceArguments](Class (TypefaceArguments).md)是表示字体属性。
