...
参数名类型必填说明info[WantAgentInfo](WantAgentInfo.md)是表示创建WantAgent所需的配置信息，包括目标UIAbility、操作类型、请求码等。三方应用在WantAgentInfo中只能设置本应用的UIAbility。callbackAsyncCallback<WantAgent>是回调函数。当创建WantAgent成功，err中code为0，data为创建的WantAgent；否则err会返回对应的错误码和错误信息。
...
参数名类型必填说明info[WantAgentInfo](WantAgentInfo.md)是表示创建WantAgent所需的配置信息，包括目标UIAbility、操作类型、请求码等。三方应用在WantAgentInfo中只能设置本应用的UIAbility。
...
参数名类型必填说明agentWantAgent是WantAgent对象。triggerInfo[TriggerInfo](TriggerInfo.md)是表示触发WantAgent实例时携带的信息，如自定义的extraInfos。callbackAsyncCallback<[CompleteData](#ZH-CN_TOPIC_0000002529444577__completedata)>否主动激发WantAgent实例的回调方法。
...
当两个WantAgent实例由当前用户下的同一应用使用相同的[WantAgentInfo](WantAgentInfo.md)信息创建，并且实例未被[cancel](#ZH-CN_TOPIC_0000002529444577__wantagentcancel)取消，这两个实例相等。在通知（携带WantAgent实例）场景，通知更新时会比较2个通知中的WantAgent实例，不相等时会把旧通知的WantAgent实例删除。
...
当两个WantAgent实例由当前用户下的同一应用使用相同的[WantAgentInfo](WantAgentInfo.md)信息创建，并且实例未被[cancel](#ZH-CN_TOPIC_0000002529444577__wantagentcancel)取消，这两个实例相等。在通知（携带WantAgent实例）场景，通知更新时会比较2个通知中的WantAgent实例，不相等时会把就旧通知的WantAgent实例删除。
...
类型说明[_TriggerInfo](TriggerInfo.md)TriggerInfo对象。
...
类型说明[_WantAgentInfo](WantAgentInfo.md)WantAgentInfo对象。
