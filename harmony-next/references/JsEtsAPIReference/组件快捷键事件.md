...
参数名类型必填说明valuestring | [FunctionKey](枚举说明.md#ZH-CN_TOPIC_0000002529284967__functionkey10)是

热键的单个字符（可以通过键盘输入的字符）或[FunctionKey](枚举说明.md#ZH-CN_TOPIC_0000002529284967__functionkey10)。

空字符串意为取消快捷键绑定。

keysArray<[ModifierKey](枚举说明.md#ZH-CN_TOPIC_0000002529284967__modifierkey10)>是

热键组合。

仅当value为[FunctionKey](枚举说明.md#ZH-CN_TOPIC_0000002529284967__functionkey10)的情况下keys的值可以为空。
...
场景快捷键处理逻辑例子所有支持onClick事件的组件支持自定义组合键无自定义组合键要求控制键Ctrl、Shift、Alt及它们的组合加上热键的单个字符（可以通过键盘输入的字符）或[FunctionKey](枚举说明.md#ZH-CN_TOPIC_0000002529284967__functionkey10)Button('button1').keyboardShortcut('a',[ModifierKey.CTRL])多个不同组件设置相同组合键只响应节点树上的深度最浅的组件，其它组件不响应快捷键
