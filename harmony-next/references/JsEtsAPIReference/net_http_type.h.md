...
名称typedef关键字描述[Http_Buffer](Http_Buffer.md)Http_BufferHTTP缓存结构体。[Http_HeaderValue](Http_HeaderValue.md)Http_HeaderValue请求或者响应的标头映射的值类型。[Http_HeaderEntry](Http_HeaderEntry.md)Http_HeaderEntry请求或者响应的标头的所有键值对。[Http_ClientCert](Http_ClientCert.md)Http_ClientCert发送到服务端的客户端证书配置，服务端将通过客户端证书校验客户端身份。[Http_CustomProxy](Http_CustomProxy.md)Http_CustomProxy用户自定义代理配置。[Http_Proxy](Http_Proxy.md)Http_Proxy代理配置结构体。[Http_PerformanceTiming](Http_PerformanceTiming.md)Http_PerformanceTimingHTTP响应时间信息，会在[Http_Response.performanceTiming](Http_Response.md#ZH-CN_TOPIC_0000002497605476__成员变量)中收集。[Http_RequestOptions](Http_RequestOptions.md)Http_RequestOptions定义HTTP请求配置的结构体。[Http_Response](Http_Response.md)Http_Response定义HTTP响应的结构体。[Http_Request](Http_Request.md)Http_RequestHTTP请求结构体。[Http_EventsHandler](Http_EventsHandler.md)Http_EventsHandler监听不同HTTP事件的回调函数。[Http_Headers](Http_Headers.md)Http_HeadersHTTP请求或者是响应中的标头。
...
参数项描述[struct Http_Response](Http_Response.md) *responseHTTP响应结构体，指向Http_Response的指针，参考[Http_Response](Http_Response.md)。uint32_t errCode响应码。
...
参数项描述[Http_Headers](Http_Headers.md) *headers接收到的请求头，指向Http_Headers的指针，参考[Http_Headers](Http_Headers.md)。
