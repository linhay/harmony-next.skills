...
1. 设备管理器元能力组件不是合法的企业设备管理元能力组件。

**处理步骤**

1. 请检查激活时输入的设备管理器元能力组件name是否存在应用包中。
1. 请检查企业设备管理组件是否继承企业设备管理框架定义的EnterpriseAdminExtensionAbility组件。
...
1. 同时激活多个SDA设备管理器。
1. 重复激活设备管理器，且管理器类型发生变化。
1. 同一个应用多个设备管理器元能力组件激活。

**处理步骤**

1. 检查是否激活多个SDA设备管理器，SDA设备管理器只允许激活一个。
1. 检查指定的设备管理器是否已激活了，且重复激活时管理器类型发生变化，此时需要先解除激活，再重新激活。
1. 检查待激活的设备管理器应用中是否已有设备管理元能力组件激活过，同一个应用只允许激活一个设备管理器元能力组件。
...
1. 检查解除激活时设备管理应用中指定设备管理元能力组件是否激活过。
1. 检查调用者是否解除激活自身，设备管理器应用不能解除激活其他设备管理器应用。
...
1. DA管理器不允许跨用户设置，指定的user ID和调用者user ID不同。

**处理步骤**

1. 检查调用接口中指定的user ID是否是有效的user ID。
1. 检查调用者user ID和指定设置的user ID是否是同一user ID，DA模式下不允许跨用户设置策略。
...
1. 企业设备管理服务没有正常启动。
1. 企业设备管理的RPC对象无法获取。
1. 企业设备管理依赖的其他服务没有正常启动或者RPC对象无法获取。
1. 企业设备管理运行过程中系统异常。
...
1. 当设置了禁止运行应用名单（例如调用[addDisallowedRunningBundlesSync](@ohos.enterprise.applicationManager（应用管理）.md#ZH-CN_TOPIC_0000002522081688__applicationmanageradddisallowedrunningbundlessync)接口），且应用在禁止运行应用名单中时，启动组件会失败。

1. 当设置了允许运行应用名单（例如调用[addAllowedRunningBundles](@ohos.enterprise.applicationManager（应用管理）.md#ZH-CN_TOPIC_0000002522081688__applicationmanageraddallowedrunningbundles21)接口），且应用不在允许运行应用名单中时，启动组件会失败。

**处理步骤**

1. 将应用从禁止运行名单中移除，例如调用[removeDisallowedRunningBundlesSync](@ohos.enterprise.applicationManager（应用管理）.md#ZH-CN_TOPIC_0000002522081688__applicationmanagerremovedisallowedrunningbundlessync)接口。

1. 将应用添加到允许运行名单，例如调用[addAllowedRunningBundles](@ohos.enterprise.applicationManager（应用管理）.md#ZH-CN_TOPIC_0000002522081688__applicationmanageraddallowedrunningbundles21)接口。
...
当已有一个日志收集任务正在执行时，调用了[systemManager.startCollectLog](@ohos.enterprise.systemManager （系统管理）.md#ZH-CN_TOPIC_0000002522081694__systemmanagerstartcollectlog23)接口创建日志收集任务。

**处理步骤**

等前一个日志收集任务完成（即收到[EnterpriseAdminExtensionAbility.onLogCollected](@ohos.enterprise.EnterpriseAdminExtensionAbility（企业设备管理扩展能力）.md#ZH-CN_TOPIC_0000002553201659__enterpriseadminextensionabilityonlogcollected23)回调）后，再调用[systemManager.startCollectLog](@ohos.enterprise.systemManager （系统管理）.md#ZH-CN_TOPIC_0000002522081694__systemmanagerstartcollectlog23)接口创建日志收集任务。
