...
定位请求参数[LocationRequest](@ohos.geoLocationManager (位置服务).md#ZH-CN_TOPIC_0000002553202339__locationrequest)、[CurrentLocationRequest](@ohos.geoLocationManager (位置服务).md#ZH-CN_TOPIC_0000002553202339__currentlocationrequest)中的精度要求(maxAccuracy)值设定过小。

**处理步骤**

请将定位请求参数[LocationRequest](@ohos.geoLocationManager (位置服务).md#ZH-CN_TOPIC_0000002553202339__locationrequest)、[CurrentLocationRequest](@ohos.geoLocationManager (位置服务).md#ZH-CN_TOPIC_0000002553202339__currentlocationrequest)中的精度要求(maxAccuracy)适当放宽，可参考如下场景建议：

-

当[scenario](@ohos.geoLocationManager (位置服务).md#ZH-CN_TOPIC_0000002553202339__locationrequestscenario)为NAVIGATION/TRAJECTORY_TRACKING/CAR_HAILING，或者[priority](@ohos.geoLocationManager (位置服务).md#ZH-CN_TOPIC_0000002553202339__locationrequestpriority)为ACCURACY时，建议设置maxAccuracy为大于10的值。

-

当[scenario](@ohos.geoLocationManager (位置服务).md#ZH-CN_TOPIC_0000002553202339__locationrequestscenario)为DAILY_LIFE_SERVICE/NO_POWER，或者[priority](@ohos.geoLocationManager (位置服务).md#ZH-CN_TOPIC_0000002553202339__locationrequestpriority)为LOW_POWER/FIRST_FIX时，建议设置maxAccuracy为大于100的值。
...
系统未提前将位置信息进行缓存的情况下，需使用[getCurrentLocation](@ohos.geoLocationManager (位置服务).md#ZH-CN_TOPIC_0000002553202339__geolocationmanagergetcurrentlocation)接口获取实时位置信息。
