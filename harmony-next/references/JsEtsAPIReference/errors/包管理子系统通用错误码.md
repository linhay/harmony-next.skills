...
1. 可以使用[查询应用信息命令（dump）](../guides/bm工具.md#查询应用信息命令dump)查看应用是否安装。查看输出的打印信息，应用未安装时，该命令执行会报错。
```ets
...
1. 可以使用[查询应用信息命令（dump）](../guides/bm工具.md#查询应用信息命令dump)查看对应的模块是否安装。查看输出的打印信息中hapModuleNames字段对应的列表是否存在该moduleName，不存在则说明应用未安装该模块。
```ets
# 需要将com.xxx.demo替换为实际查询的bundleName
hdc shell bm dump -n com.xxx.demo
```
...
1. 可以使用[查询应用信息命令（dump）](../guides/bm工具.md#查询应用信息命令dump)查看对应的应用是否存在这个abilityName。查看输出的打印信息中hapModuleInfos字段对应的abilityInfos下是否包含name等于该abilityName，不包含则说明该abilityName不存在。
1. 可以使用[查询应用信息命令（dump）](../guides/bm工具.md#查询应用信息命令dump)查看输出的打印信息中hapModuleNames字段对应的列表是否存在对应的moduleName，不存在则说明应用未安装该模块，对应模块下的ability也不存在。
```ets
# 需要将com.xxx.demo替换为实际查询的bundleName
hdc shell bm dump -n com.xxx.demo
```
...
检查系统中是否存在对应的应用uid值。可以使用[查询应用信息命令（dump）](../guides/bm工具.md#查询应用信息命令dump)查看已安装应用的uid。执行命令行后会输出对应已安装应用的uid，如果输出多个uid，选择大于0的。如果该命令无打印值输出，表示应用未安装。
...
确认指定的ability是否被禁用，可以使用[bm工具](../guides/bm工具.md)查询对应的应用信息。
