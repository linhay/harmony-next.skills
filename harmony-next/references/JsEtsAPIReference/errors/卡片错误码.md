...
检查当前卡片对应[互动卡片配置项sceneAnimationParams](../guides/配置ArkTS卡片的配置文件.md#sceneanimationparams标签)是否正确。
...
检查入参卡片尺寸是否在[FormDimension](@ohos.app.form.formInfo (formInfo).md#ZH-CN_TOPIC_0000002553361175__formdimension)和[supportDimensions](../guides/配置ArkTS卡片的配置文件.md#配置文件字段说明)配置列表。
...
1. 空参数错误，比如传入空字符串（''）。
1. 参数格式错误。
1. 参数值错误。输入参数需要与[app.json5](../guides/app.json5配置文件.md)和[配置ArkTS卡片的配置文件](../guides/配置ArkTS卡片的配置文件.md)中对应的配置保持一致。
