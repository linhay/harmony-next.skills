...
检查当前卡片对应[互动卡片配置项sceneAnimationParams](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-ui-widget-configuration#sceneanimationparams标签)是否正确。
...
检查入参卡片尺寸是否在[FormDimension](../modules/ohos/@ohos.app.form.formInfo (formInfo).md#ZH-CN_TOPIC_0000002497445300__formdimension)和[supportDimensions](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-ui-widget-configuration#配置文件字段说明)配置列表。
...
1. 重启系统后重试。
1. 重启后仍旧失败，请提[在线工单](https://developer.huawei.com/consumer/cn/support/feedback/#)获取帮助。
...
1. 空参数错误，比如传入空字符串（''）。
1. 参数格式错误。
1. 参数值错误。输入参数需要与[app.json5](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-configuration-file)和[配置ArkTS卡片的配置文件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-ui-widget-configuration)中对应的配置保持一致。
