...
1. 检查传入Want中bundleName对应的应用是否安装。可使用如下命令查询已安装的应用列表，若bundleName不在查询结果中，说明应用未安装成功。
```ets
hdc shell bm dump -a
```

1. 多hap应用需确认ability所属的hap是否已被安装。可使用如下命令查询应用的包信息，若安装的应用中没有对应的hap和ability，说明ability所属的hap未被安装。
```ets
...
1. 当目标服务端为AppServiceExtensionAbility类型时，未在module.json5配置文件中配置ACL权限（ohos.permission.SUPPORT_APP_SERVICE_EXTENSION）。

**处理步骤**

1. 检查Want中的bundleName、moduleName和abilityName是否正确。

1. 确认被调用方（服务端）的Ability类型与调用接口是否匹配。对于ServiceExtensionAbility，应使用[connectServiceExtensionAbility()](UIAbilityContext.md#ZH-CN_TOPIC_0000002553200541__connectserviceextensionability)方法连接。同时需要确保[module.json5配置文件](../guides/module.json5配置文件.md)中extensionAbilities的type设置为与接口匹配的service。

1. 若被调用方（服务端）为appService类型，需在服务端的module.json5配置文件中配置ACL权限（ohos.permission.SUPPORT_APP_SERVICE_EXTENSION）。
...
1. [Stage模型](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ability-terminology#stage模型)下，拉起应用时抛出16000004异常，表示被拉应用调用失败，需要检查被拉应用module.json5的Ability字段的[exported](../guides/module.json5配置文件.md#abilities标签)配置是否为true。该配置字段为true，表示可以被其他应用调用；该配置字段为false，表示不可以被其他应用调用。
1. 若应用需要拉起exported为false的ability，请申请ohos.permission.START_INVISIBLE_ABILITY权限（该权限仅系统应用可申请）。
...
使用隐式启动方式或通过[openLink](UIAbilityContext.md#ZH-CN_TOPIC_0000002553200541__openlink12)跳转其他应用。
...
1. 启动Ability时传入的[Want](@ohos.app.ability.Want (Want).md#ZH-CN_TOPIC_0000002553360483__约束限制)数据过大。

1. 设备解锁前拉起非系统应用。

1. 隐式拉起时未安装应用市场App。

1. 开发者无法处理的系统内部错误。包括但不限于：内部对象为空指针、处理超时、IPC跨进程通信失败、包管理获取应用信息失败、系统服务获取失败、启动的Ability实例已达到上限等。

**处理步骤**

1. 对于启动Ability失败时，可以检查传入的[Want](@ohos.app.ability.Want (Want).md#ZH-CN_TOPIC_0000002553360483__约束限制)数据是否过大。

1. 确保在设备解锁前只拉起系统应用，或者延迟拉起非系统应用直到设备解锁。

1. 确保设备上已安装应用市场App，或者在拉起应用前检查应用市场App是否已安装。

1. 对于开发者无法处理的系统内部错误，请尝试重新调用该接口，或者重启设备。
...
1. 请核实应用界面是否完全显示，且未被其他应用窗口遮挡或最小化。
1. 如设备启用了分屏或多窗口模式，请确保当前应用为焦点窗口。
...
1. 调用showAbility/hideAbility时，如果调用方不是以NEW_PROCESS_ATTACH_TO_STATUS_BAR_ITEM或者ATTACH_TO_STATUS_BAR_ITEM模式启动，则返回该错误码。
...
该应用没有在app.json5配置文件[multiAppMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-configuration-file#multiappmode标签)标签中配置应用分身字段，导致该应用不支持分身模式，调用[getCurrentAppCloneIndex](ApplicationContext (应用上下文).md#ZH-CN_TOPIC_0000002553360493__applicationcontextgetcurrentappcloneindex12)接口时返回该错误码。

**处理步骤**

参考[应用多实例的配置方法](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/multiinstance)，在app.json5配置文件中配置multiAppMode标签，开启应用分身功能后，再调用[getCurrentAppCloneIndex](ApplicationContext (应用上下文).md#ZH-CN_TOPIC_0000002553360493__applicationcontextgetcurrentappcloneindex12)接口。
...
调用[startAbility](UIAbilityContext.md#ZH-CN_TOPIC_0000002553200541__startability)、[startAbilityForResult](UIAbilityContext.md#ZH-CN_TOPIC_0000002553200541__startabilityforresult)等启动Ability接口时，如果目标应用不支持应用多开，则返回该错误码。

**处理步骤**

调用[startAbility](UIAbilityContext.md#ZH-CN_TOPIC_0000002553200541__startability)、[startAbilityForResult](UIAbilityContext.md#ZH-CN_TOPIC_0000002553200541__startabilityforresult)等启动Ability接口时，确保目标应用支持应用多开，并在app.json5配置文件中配置[multiAppMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-configuration-file#multiappmode标签)标签开启应用分身功能。
...
1. 确认当前应用声明的linkFeature取值正确，linkFeature描述的功能与应用链接对应的实际功能一致，且应用通过系统审核。
...
指定的[APP_INSTANCE_KEY](@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002553200525__params)不存在时，返回该错误码。

**可能原因**

应用的实例中不存在该[APP_INSTANCE_KEY](@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002553200525__params)指定的实例。

**处理步骤**

确保传入的[APP_INSTANCE_KEY](@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002553200525__params)是一个有效值。
...
[APP_INSTANCE_KEY](@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002553200525__params)和[CREATE_APP_INSTANCE_KEY](@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002553200525__params)不支持同时指定。当指定[CREATE_APP_INSTANCE_KEY](@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002553200525__params)的同时指定[APP_INSTANCE_KEY](@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002553200525__params)，返回该错误码。
...
参数[APP_INSTANCE_KEY](@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002553200525__params)和[CREATE_APP_INSTANCE_KEY](@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002553200525__params)只支持二选一。
...
只允许应用使用[CREATE_APP_INSTANCE_KEY](@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002553200525__params)创建自己的实例，不允许应用间启动时为其他应用创建实例。否则，返回该错误码。
...
删除参数[CREATE_APP_INSTANCE_KEY](@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002553200525__params)。
...
1. 调用方是DelegatorAbility，但是重复调用接口。
...
在[module.json5配置文件](../guides/module.json5配置文件.md)的[abilities标签](../guides/module.json5配置文件.md#abilities标签)中，当前UIAbility的allowSelfRedirect字段取值为“false”。

**处理步骤**

- 如果允许使用App Linking拉起当前UIAbility，开发者需要在[module.json5配置文件](../guides/module.json5配置文件.md)将[abilities标签](../guides/module.json5配置文件.md#abilities标签)的allowSelfRedirect字段设置为true。
...
1. 三方应用设置其他应用的Ability。
1. 其它内部通信错误。

**处理步骤**

1. 检查传入接口的wantAgent对象是否存在。
1. 检查调用方是否为三方应用。不支持三方应用设置其他应用的Ability。
...
调用方不在目标应用的[appIdentifierAllowList](../guides/module.json5配置文件.md#extensionabilities标签)中时，返回该错误码。

**可能原因**

[startAppServiceExtensionAbility](UIAbilityContext.md#ZH-CN_TOPIC_0000002553200541__startappserviceextensionability20)、[stopAppServiceExtensionAbility](UIAbilityContext.md#ZH-CN_TOPIC_0000002553200541__stopappserviceextensionability20)接口调用方的app-identifier不在目标[AppServiceExtensionAbility](@ohos.app.ability.AppServiceExtensionAbility (应用后台服务扩展组件).md)的[appIdentifierAllowList](../guides/module.json5配置文件.md#extensionabilities标签)中。

**处理步骤**

将接口调用方的app-identifier配置在目标[AppServiceExtensionAbility](@ohos.app.ability.AppServiceExtensionAbility (应用后台服务扩展组件).md)的[appIdentifierAllowList](../guides/module.json5配置文件.md#extensionabilities标签)中。
...
使用[connectAppServiceExtensionAbility](UIAbilityContext.md#ZH-CN_TOPIC_0000002553200541__connectappserviceextensionability20)接口时服务端还未启动且当前应用无权限拉起目标服务。

**处理步骤**

1. 等待服务端启动后重新连接。

1. 由当前应用拉起目标服务时，需要将接口调用方的app-identifier配置在目标[AppServiceExtensionAbility](@ohos.app.ability.AppServiceExtensionAbility (应用后台服务扩展组件).md)的[appIdentifierAllowList](../guides/module.json5配置文件.md#extensionabilities标签)中。
...
1. 检查调用context.startAbility时，context对应的ability是否还在运行。若该ability已被析构，会抛出该错误码。

1. 若存在连续调用startAbility和terminateSelf的情况，请确保在收到startAbility成功或失败的回调后，再调用terminateSelf。
...
根据需要调整超时时间。超时时间的设置可参见[设置启动参数](../guides/应用启动框架AppStartup.md#设置启动参数)。
...
1. 进程中存在未完成的[onNewProcessRequest](@ohos.app.ability.AbilityStage (AbilityStage组件管理器).md#ZH-CN_TOPIC_0000002522080526__onnewprocessrequest11)请求。

1. 当启动模式为[specified](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-launch-type#specified启动模式)的UIAbility运行在独立进程时，当前进程中存在未完成的[onAcceptWant](@ohos.app.ability.AbilityStage (AbilityStage组件管理器).md#ZH-CN_TOPIC_0000002522080526__onacceptwant)请求。
