...
1. 检查want中的bundleName、moduleName和abilityName是否正确。
1. 根据Ability类型调用不同接口，如ServiceExtensionAbility应使用[connectServiceExtensionAbility()](../topics/graphics/UIAbilityContext.md#ZH-CN_TOPIC_0000002497604628__connectserviceextensionability)方法连接，并且[module.json5配置文件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file)中extensionAbilities的type需要是与接口匹配的service。
...
使用隐式启动方式或通过[openLink](../topics/graphics/UIAbilityContext.md#ZH-CN_TOPIC_0000002497604628__openlink12)跳转其他应用。
...
1. 确保设备上已安装应用市场应用，或者在拉起应用前检查应用市场是否已安装。
...
该应用没有在app.json5配置文件[multiAppMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-configuration-file#multiappmode标签)标签中配置应用分身字段，导致该应用不支持分身模式，调用[getCurrentAppCloneIndex](../topics/graphics/ApplicationContext (应用上下文).md#ZH-CN_TOPIC_0000002529284613__applicationcontextgetcurrentappcloneindex12)接口时返回该错误码。

**处理步骤**

参考[应用多实例的配置方法](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/multiinstance)，在app.json5配置文件中配置multiAppMode标签，开启应用分身功能后，再调用[getCurrentAppCloneIndex](../topics/graphics/ApplicationContext (应用上下文).md#ZH-CN_TOPIC_0000002529284613__applicationcontextgetcurrentappcloneindex12)接口。
...
指定的[APP_INSTANCE_KEY](../modules/ohos/@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002497604612__params)不存在时，返回该错误码。

**可能原因**

应用的实例中不存在该[APP_INSTANCE_KEY](../modules/ohos/@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002497604612__params)指定的实例。

**处理步骤**

确保传入的[APP_INSTANCE_KEY](../modules/ohos/@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002497604612__params)是一个有效值。
...
[APP_INSTANCE_KEY](../modules/ohos/@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002497604612__params)和[CREATE_APP_INSTANCE_KEY](../modules/ohos/@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002497604612__params)不支持同时指定。当指定[CREATE_APP_INSTANCE_KEY](../modules/ohos/@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002497604612__params)的同时指定[APP_INSTANCE_KEY](../modules/ohos/@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002497604612__params)，返回该错误码。
...
参数[APP_INSTANCE_KEY](../modules/ohos/@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002497604612__params)和[CREATE_APP_INSTANCE_KEY](../modules/ohos/@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002497604612__params)只支持二选一。
...
只允许应用使用[CREATE_APP_INSTANCE_KEY](../modules/ohos/@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002497604612__params)创建自己的实例，不允许应用间启动时为其他应用创建实例。否则，返回该错误码。
...
删除参数[CREATE_APP_INSTANCE_KEY](../modules/ohos/@ohos.app.ability.wantConstant (Want常量).md#ZH-CN_TOPIC_0000002497604612__params)。
...
[startAppServiceExtensionAbility](../topics/graphics/UIAbilityContext.md#ZH-CN_TOPIC_0000002497604628__startappserviceextensionability20)、[stopAppServiceExtensionAbility](../topics/graphics/UIAbilityContext.md#ZH-CN_TOPIC_0000002497604628__stopappserviceextensionability20)接口调用方的app-identifier不在目标[AppServiceExtensionAbility](../modules/ohos/@ohos.app.ability.AppServiceExtensionAbility (应用后台服务扩展组件).md)的[appIdentifierAllowList](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#extensionabilities标签)中。

**处理步骤**

将接口调用方的app-identifier配置在目标[AppServiceExtensionAbility](../modules/ohos/@ohos.app.ability.AppServiceExtensionAbility (应用后台服务扩展组件).md)的[appIdentifierAllowList](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#extensionabilities标签)中。
...
使用[connectAppServiceExtensionAbility](../topics/graphics/UIAbilityContext.md#ZH-CN_TOPIC_0000002497604628__connectappserviceextensionability20)接口时服务端还未启动且当前应用无权限拉起目标服务。

**处理步骤**

1. 等待服务端启动后重新连接。
1. 由当前应用拉起目标服务时，需要将接口调用方的app-identifier配置在目标[AppServiceExtensionAbility](../modules/ohos/@ohos.app.ability.AppServiceExtensionAbility (应用后台服务扩展组件).md)的[appIdentifierAllowList](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#extensionabilities标签)中。
...
1. 若存在连续调用startAbility和terminateSelf的情况，请确认收到startAbility成功或失败的回调后，再调用terminateSelf。
...
1. 进程中存在未完成的[onNewProcessRequest](../modules/ohos/@ohos.app.ability.AbilityStage (AbilityStage组件管理器).md#ZH-CN_TOPIC_0000002529284571__onnewprocessrequest11)请求。
1. 当启动模式为[specified](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-launch-type#specified启动模式)的UIAbility运行在独立进程时，当前进程中存在未完成的[onAcceptWant](../modules/ohos/@ohos.app.ability.AbilityStage (AbilityStage组件管理器).md#ZH-CN_TOPIC_0000002529284571__onacceptwant)请求。
