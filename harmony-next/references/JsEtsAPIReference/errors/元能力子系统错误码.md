...
1. [Stage模型](../guides/Ability Kit术语.md#stage模型)下，拉起应用时抛出16000004异常，表示被拉应用调用失败，需要检查被拉应用module.json5的Ability字段的[exported](../guides/module.json5配置文件.md#abilities标签)配置是否为true。该配置字段为true，表示可以被其他应用调用；该配置字段为false，表示不可以被其他应用调用。
...
1. 查看[对应Extension类型](../guides/ExtensionAbility组件.md)严格模式开启条件。
...
该应用没有在app.json5配置文件[multiAppMode](../guides/app.json5配置文件.md#multiappmode标签)标签中配置应用分身字段，导致该应用不支持分身模式，调用[getCurrentAppCloneIndex](ApplicationContext (应用上下文).md#ZH-CN_TOPIC_0000002553360493__applicationcontextgetcurrentappcloneindex12)接口时返回该错误码。
...
调用[startAbility](UIAbilityContext.md#ZH-CN_TOPIC_0000002553200541__startability)、[startAbilityForResult](UIAbilityContext.md#ZH-CN_TOPIC_0000002553200541__startabilityforresult)等启动Ability接口时，确保目标应用支持应用多开，并在app.json5配置文件中配置[multiAppMode](../guides/app.json5配置文件.md#multiappmode标签)标签开启应用分身功能。
...
1. 当启动模式为[specified](../guides/UIAbility组件启动模式.md#specified启动模式)的UIAbility运行在独立进程时，当前进程中存在未完成的[onAcceptWant](@ohos.app.ability.AbilityStage (AbilityStage组件管理器).md#ZH-CN_TOPIC_0000002522080526__onacceptwant)请求。
