...
安全区域是指页面的显示区域，默认情况下开发者开发的界面都布局在安全区域内，不与系统设置的避让区比如状态栏、导航栏区域重叠。提供属性方法允许开发者设置组件绘制内容突破安全区域的限制，通过[expandSafeArea](#ZH-CN_TOPIC_0000002497444852__expandsafearea)属性支持组件不改变布局情况下扩展其绘制区域至安全区外，通过设置[setKeyboardAvoidMode](#ZH-CN_TOPIC_0000002497444852__setkeyboardavoidmode11)来配置虚拟键盘弹出时页面的避让模式。页面中有标题栏等文字不希望和避让区重叠时，建议对组件设置expandSafeArea属性实现沉浸式效果，也可以直接通过窗口接口[setWindowLayoutFullScreen](zh-cn_topic_0000002497604802.html#ZH-CN_TOPIC_0000002497604802__setwindowlayoutfullscreen9)设置全屏沉浸式。
...
参数名类型必填说明typesArray <[SafeAreaType](#ZH-CN_TOPIC_0000002497444852__safeareatype)>否

配置扩展安全区域的类型。未添加[Metadata](Metadata.md)配置项时，页面不避让挖孔，CUTOUT类型不生效。

默认值：[SafeAreaType.SYSTEM, SafeAreaType.CUTOUT, SafeAreaType.KEYBOARD]

非法值：按默认值处理。

edgesArray <[SafeAreaEdge](#ZH-CN_TOPIC_0000002497444852__safeareaedge)>否
...
类型说明T返回当前组件。
 ...
 滚动类容器内的组件不建议设置expandSafeArea属性，如果设置，需要按照组件嵌套关系，将当前节点到滚动类祖先容器间所有直接节点设置expandSafeArea属性，否则expandSafeArea属性在滚动后可能会失效，写法参考[示例7](#ZH-CN_TOPIC_0000002497444852__示例7滚动类容器扩展安全区)。
...
名称值说明SYSTEM0系统默认非安全区域，包括状态栏、导航栏。CUTOUT1设备的非安全区域，例如刘海屏或挖孔屏区域。KEYBOARD2软键盘区域。
...
名称值说明TOP0上方区域。BOTTOM1下方区域。START2前部区域。END3尾部区域。
...
名称说明[KeyboardAvoidMode](Enums.md#ZH-CN_TOPIC_0000002497604786__keyboardavoidmode11)返回虚拟键盘抬起时的页面避让模式。
...
参数名类型必填说明typesArray <[LayoutSafeAreaType](#ZH-CN_TOPIC_0000002497444852__layoutsafeareatype12)>否

扩展布局安全区域的类型。

默认值：[LayoutSafeAreaType.SYSTEM]，扩展至所有安全区域，比如：状态栏，导航栏和[组件级安全区（safeAreaPadding）](尺寸设置.md#ZH-CN_TOPIC_0000002497444850__safeareapadding14)。

非法值：按默认值处理。

edgesArray <[LayoutSafeAreaEdge](#ZH-CN_TOPIC_0000002497444852__layoutsafeareaedge12)>否
...
忽略布局安全区边缘的组件，如果其宽度或高度设置了[LayoutPolicy.matchParent](尺寸设置.md#ZH-CN_TOPIC_0000002497444850__layoutpolicy15)，其大小和位置都会改变，否则仅改变其位置。
...
名称值说明SYSTEM0设置后，组件的布局范围可扩展至[组件级安全区（safeAreaPadding）](尺寸设置.md#ZH-CN_TOPIC_0000002497444850__safeareapadding14)和页面级安全区（状态栏、导航栏、挖孔区）。
...
该示例利用[ignoreLayoutSafeArea](#ZH-CN_TOPIC_0000002497444852__ignorelayoutsafearea20)改变组件位置。相比未使用该属性，配置ignoreLayoutSafeArea后，Row组件基于Stack内容区、Stack组件级安全区、系统状态栏共同组成的范围，取其左上部分，作左上对齐。
...
该示例利用[ignoreLayoutSafeArea](#ZH-CN_TOPIC_0000002497444852__ignorelayoutsafearea20)和[LayoutPolicy.matchParent](zh-cn_topic_0000002497444850.html#ZH-CN_TOPIC_0000002497444850__layoutpolicy15)同时改变组件大小和位置。相比未使用该属性，配置ignoreLayoutSafeArea后，Row组件基于Stack内容区、Stack组件级安全区，取其右下部分并撑满可用空间。
