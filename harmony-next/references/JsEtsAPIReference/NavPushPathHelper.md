...
当跳转的目标[NavDestination](NavDestination.md)在不同的hsp分包，且未被主包依赖，首次运行元服务只会下载安装主包，需要使用NavPushPathHelper先下载安装相应hsp分包，再将指定的[NavDestination](NavDestination.md)页面信息入栈。使[Navigation](Navigation.md)支持动态加载hsp分包后再跳转。
...
对Navigation路由栈[NavPathStack](Navigation.md#ZH-CN_TOPIC_0000002497444902__navpathstack10)的所有路由跳转接口进行了封装，在NavPushPathHelper中持有一个NavPathStack对象，在封装的跳转接口中，去判断子包是否存在，如果不存在则进行动态下载子包，等结果返回后调用NavPathStack的相应的接口将指定的[NavDestination](NavDestination.md)页面信息入栈。使用示例参见[示例](#ZH-CN_TOPIC_0000002529284937__示例)。
...
参数名类型必填说明navPathStack[NavPathStack](Navigation.md#ZH-CN_TOPIC_0000002497444902__navpathstack10)是[Navigation](Navigation.md)路由栈。

#### pushPath

pushPath(moduleName: string, info: NavPathInfo, animated?: boolean): Promise<void>

先判断分包是否存在，若不存在，则通过moduleName下载分包，再将info指定的[NavDestination](NavDestination.md)页面信息入栈，使用Promise异步回调。
...
参数名类型必填说明moduleNamestring是目标[NavDestination](NavDestination.md)所在分包的moduleName。info[NavPathInfo](Navigation.md#ZH-CN_TOPIC_0000002497444902__navpathinfo10)是[NavDestination](NavDestination.md)页面的信息。animatedboolean否
...
以下错误码的详细介绍请参见[ohos.router(页面路由)](页面路由错误码.md)错误码。
...
具体根据options中指定不同的[LaunchMode](Navigation.md#ZH-CN_TOPIC_0000002497444902__launchmode12枚举说明)，有不同的行为。
...
参数名类型必填说明moduleNamestring是目标[NavDestination](NavDestination.md)所在分包的moduleName。info[NavPathInfo](Navigation.md#ZH-CN_TOPIC_0000002497444902__navpathinfo10)是[NavDestination](NavDestination.md)页面的信息。options[NavigationOptions](Navigation.md#ZH-CN_TOPIC_0000002497444902__navigationoptions12)否页面栈操作选项。
...
先判断分包是否存在，若不存在，则通过moduleName下载分包，再将name指定的[NavDestination](NavDestination.md)页面信息入栈，传递的数据为param，使用Promise异步回调。
...
参数名类型必填说明moduleNamestring是目标[NavDestination](NavDestination.md)所在分包的moduleName。namestring是[NavDestination](NavDestination.md)页面名称。paramObject是[NavDestination](NavDestination.md)页面详细参数。animatedboolean否
...
先判断分包是否存在，若不存在，则通过moduleName下载分包，再将name指定的[NavDestination](NavDestination.md)页面信息入栈，传递的数据为param，添加onPop回调接收入栈页面出栈时的返回结果，并进行处理，使用Promise异步回调。
...
参数名类型必填说明moduleNamestring是目标[NavDestination](NavDestination.md)所在分包的moduleName。namestring是[NavDestination](NavDestination.md)页面名称。paramObject是[NavDestination](NavDestination.md)页面详细参数。onPopCallback<[PopInfo](Navigation.md#ZH-CN_TOPIC_0000002497444902__popinfo11)>是Callback回调，用于页面出栈时触发该回调处理返回结果。animatedboolean否
...
先判断分包是否存在，若不存在，则通过moduleName下载分包，再将name指定的[NavDestination](NavDestination.md)页面信息入栈，传递的数据为param，并且添加用于页面出栈时处理返回结果的OnPop回调，使用Promise异步回调。
...
参数名类型必填说明moduleNamestring是目标[NavDestination](NavDestination.md)所在分包的moduleName。namestring是[NavDestination](NavDestination.md)页面名称。paramObject是[NavDestination](NavDestination.md)页面详细参数。onPopCallback<[PopInfo](Navigation.md#ZH-CN_TOPIC_0000002497444902__popinfo11)>是Callback回调，用于页面出栈时处理返回结果。animatedboolean否
...
先判断分包是否存在，若不存在，则通过moduleName下载分包，再将当前页面栈栈顶退出，将info指定的[NavDestination](NavDestination.md)页面信息入栈，使用Promise异步回调。
...
参数名类型必填说明moduleNamestring是目标[NavDestination](NavDestination.md)所在分包的moduleName。info[NavPathInfo](Navigation.md#ZH-CN_TOPIC_0000002497444902__navpathinfo10)是新栈顶页面参数信息animatedboolean否
...
参数名类型必填说明moduleNamestring是目标[NavDestination](NavDestination.md)所在分包的moduleName。info[NavPathInfo](Navigation.md#ZH-CN_TOPIC_0000002497444902__navpathinfo10)是新栈顶页面参数信息。options[NavigationOptions](Navigation.md#ZH-CN_TOPIC_0000002497444902__navigationoptions12)否页面栈操作选项。
