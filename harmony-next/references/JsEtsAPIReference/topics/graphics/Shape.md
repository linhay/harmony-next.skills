...
包含[Rect](../misc/Rect.md)、[Path](path.md)、[Circle](../misc/circle.md)、[Ellipse](../misc/Ellipse.md)、[Polyline](Polyline.md)、[Polygon](../misc/Polygon.md)、[Image](Image.md)、[Text](Text.md)、[Column](../misc/Column.md)、[Row](../misc/Row.md)和Shape子组件。
...
名称类型只读可选说明x7+[Length](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length)否是
...
y7+[Length](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length)否是
...
width7+[Length](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length)否是
...
参数名类型必填说明value[ViewportRect](Shape.md#ZH-CN_TOPIC_0000002497444944__viewportrect18对象说明)是

Viewport绘制属性。

默认值：{ x: 0, y: 0, width: Shape组件的宽度, height: Shape组件的高度 }
...
设置边框颜色，支持[attributeModifier](../misc/动态属性设置.md#ZH-CN_TOPIC_0000002529284843__attributemodifier)动态设置属性方法，不设置时，默认边框透明度为0，即没有边框。
...
设置边框间隙，支持[attributeModifier](../misc/动态属性设置.md#ZH-CN_TOPIC_0000002529284843__attributemodifier)动态设置属性方法。取值范围为≥0，异常值按照默认值处理。
...
默认值：[stroke](#ZH-CN_TOPIC_0000002497444944__stroke)接口设置的透明度。
...
设置网格效果。将图像分割为（row + 1）* (column + 1)的网格，每个网格交点坐标存储在数组中（每两个元素表示一个交点的x、y坐标）。通过数组value中的坐标值，重新定位网格顶点位置，实现图像局部扭曲。支持[attributeModifier](../misc/动态属性设置.md#ZH-CN_TOPIC_0000002529284843__attributemodifier)动态设置属性方法。

mesh只对shape传入pixelMap时生效，且效果作用于传入的pixelMap。与[绘制模块](../../guides/模块描述.md)的[drawPixelMapMesh12+](../../types/classes/Class (Canvas).md#ZH-CN_TOPIC_0000002497605976__drawpixelmapmesh12)效果一致，建议使用drawPixelMapMesh。
...
异常值undefined、null、NaN和Infinity按照无效值处理。

columnnumber是

mesh矩阵列数。

异常值NaN和Infinity按照无效值处理。
