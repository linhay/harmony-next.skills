...
type为"har"和"shared"类型的Module中推荐使用[ImageSource](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/image-decoding)图片解码方式将资源图片解码为统一的PixelMap加载使用。
...
参数名类型必填说明data[PixelMap](../../types/interfaces/Interface (PixelMap).md)是图片的数据源支持PixelMap对象。
...
用来配置ImageBitmap对象的单位模式，配置后无法动态更改，配置方法同[CanvasRenderingContext2D](../components/CanvasRenderingContext2D.md)。
...
参数名类型必填说明data[PixelMap](../../types/interfaces/Interface (PixelMap).md)是图片的数据源支持PixelMap对象。unit[LengthMetricsUnit](Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetricsunit12)是用来配置ImageBitmap对象的单位模式，配置后无法动态更改，配置方法同[CanvasRenderingContext2D](../components/CanvasRenderingContext2D.md)。
