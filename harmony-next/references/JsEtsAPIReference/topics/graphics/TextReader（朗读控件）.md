...
调用朗读控件接口前，必须先调用[init](#section173751154134515)初始化，否则会报错。
...
[ReaderParam](#section6129163245310)
...
Permission denied. Interface caller does not have permission ohos.permission.KEEP_BACKGROUND_RUNNING.
...
文章id，articleId不传时，默认从文章列表中的第一篇开始播放；传参时，从传入articleId对应的文章开始依次往下播放。（文章id不能重复，否则从列表进入同id的文章，会自动选择同id的第一篇朗读）
...
文章id（文章id不能重复，否则从列表进入同id的文章，会自动选择同id的第一篇文章），articleId不传时，默认从文章列表中的第一篇开始播放；传参时，从传入articleId对应的文章开始依次往下播放

startParams

[StartParams](#section6254122715301)
...
[ResetParamType](#section192921653104)
...
文章内容，content不传或者为空时，播放指定id的文章时，控件内容页显示加载失败；传参时，将指定id文章的内容设置为content。
...
文章信息，仅更新image、imageUrl、bodyInfo、bodyInfoObject；如果文章的ID不在列表中，则不会生效。
...
事件回调类型，支持的事件为'setArticle'，设置文章后，目标文章内容为空（bodyInfo值为空）时，触发该事件。
...
事件回调类型，支持的事件为'setArticle'，设置文章后，触发该事件。
...
需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，若不填，则取消当前应用监听该事件的所有回调函数（当前仅有一个）。
...
事件回调类型，支持的事件为'stateChange'，当前正在播放的文章状态变更时，触发该事件

callback

Callback<[ReadState](#section178743012141)>

是

正在播放的文章状态变更时，执行的回调函数。
...
事件回调类型，支持的事件为'requestMore'，请求更多文章时，触发该事件监听。
...
事件回调类型，支持的事件为'requestMore'，拉到播放列表底端请求更多文章时，触发该事件监听。
...
拉到播放列表底端或播放到文章最后一篇，触发该回调执行。返回的自定义参数在[StartParams](#section6254122715301)的callbackParam属性配置。
...
- 'eventPanel'：播放面板状态变化，其中[PanelEvent](#section13718113213211)中的BPC_10 click事件回调只支持手机

callback

Callback<[NotificationEvent](#section1612119431629) | [PanelEvent](#section13718113213211)>

是

播控中心/播放面板状态发生变化时，触发该回调执行。
...
事件回调类型，支持的事件为'eventReadList'。

callback

Callback<Array<[ListEventState](#section63451441422)>>
...
事件回调类型，支持的事件为'readProgress'。

callback

Callback<[ReadProgress](#section1841441192115)>

是

已播放时长发生相关事件时，触发该回调执行。
...
[BusinessBrandInfo](#section472059101210)
...
如需要设置后台运行功能，请配置[长时任务权限](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/speech-textreader-guide#li56192364816)
...
[MinibarParams](#section1179835142719)
...
[CustomFeature[]](#section6103123551)
...
用于三方自定义场景参数，在requestMore监听中返回，默认空
...
[BarAlignment](#section1031013101325)
...
- 播放面板顶部展示的业务名称，系统字体大小为标准时，英文占1个字符长度，中文占2个字符长度，累计超过24个字符长度时会省略显示（系统字体大小有变动时，显示长度会有变化）
- Pura X设备不显示

panelIcon

[ResourceStr](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#ZH-CN_TOPIC_0000001847209908__resourcestr)
...
[ReadStateCode](../misc/ReadStateCode (播报状态).md)
...
[CustomFeatureType](#section1544534622617)

否

否

自定义特征的类型，系统应用或者用户自定义功能。
...
文章的时间（例:Mon Jul 08 2024 21:25:41 GMT+0800）。
...
[setArticle](#section1658111564332)以及[loadMore](#section1736815283116)接口必填。

实时朗读的正文信息。

从6.0.2(22)版本开始，正文长度支持10000字符以上。

bodyInfoObject

[BodyInfo](#section10646171225714)
...
[CategoryInfo](#section13846143116387)
...
[AudioInfo](#section12182184315243)[]
...
[image.PixelMap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-pixelmap) | string

否

否

文章分类的图片或者图片的url，优先级大于文章封面图片。
...
[BodyInfoType](#section1742241575911)
...
[ReadProgressType](#section6549172112810)
...
触发该事件回调时，用户曾经播放的最大的百分比，而不是当前用户所处于的百分比
...
触发该事件回调时，返回用户当前播放的百分比，取值[0,100]。可用于记录上次播放位置，和[start](#section147237941810)接口配合实现下次继续播放。
...
重置所有本地数据，恢复为初始化配置参数，包括Minibar参数重置，文章列表置空。
