...
名称类型默认值必填描述directionto <side-or-corner> <side-or-corner> = [left | right] || [top | bottom]to bottom (由上到下渐变)否指定过渡方向，如：to left (从右向左渐变) ，或者to bottom right (从左上角到右下角)。angle<deg>180deg否指定过渡方向，以元素几何中心为坐标原点，水平方向为X轴，angle指定了渐变线与Y轴的夹角(顺时针方向)。color<color> [<length>|<percentage>]-是定义使用渐变样式区域内颜色的渐变效果。
