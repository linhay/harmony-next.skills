...
当跳转的目标[NavDestination](../misc/NavDestination.md)在不同的hsp分包，且未被主包依赖，首次运行元服务只会下载安装主包，需要使用NavPushPathHelper先下载安装相应hsp分包，再将指定的[NavDestination](../misc/NavDestination.md)页面信息入栈。使[Navigation](../misc/Navigation.md)支持动态加载hsp分包后再跳转。
...
对Navigation路由栈[NavPathStack](../misc/Navigation.md#ZH-CN_TOPIC_0000002497444902__navpathstack10)的所有路由跳转接口进行了封装，在NavPushPathHelper中持有一个NavPathStack对象，在封装的跳转接口中，去判断子包是否存在，如果不存在则进行动态下载子包，等结果返回后调用NavPathStack的相应的接口将指定的[NavDestination](../misc/NavDestination.md)页面信息入栈。使用示例参见[示例](#ZH-CN_TOPIC_0000002529284937__示例)。
...
参数名类型必填说明navPathStack[NavPathStack](../misc/Navigation.md#ZH-CN_TOPIC_0000002497444902__navpathstack10)是[Navigation](../misc/Navigation.md)路由栈。

#### pushPath

pushPath(moduleName: string, info: NavPathInfo, animated?: boolean): Promise<void>

先判断分包是否存在，若不存在，则通过moduleName下载分包，再将info指定的[NavDestination](../misc/NavDestination.md)页面信息入栈，使用Promise异步回调。
...
参数名类型必填说明moduleNamestring是目标[NavDestination](../misc/NavDestination.md)所在分包的moduleName。info[NavPathInfo](../misc/Navigation.md#ZH-CN_TOPIC_0000002497444902__navpathinfo10)是[NavDestination](../misc/NavDestination.md)页面的信息。animatedboolean否
...
以下错误码的详细介绍请参见[ohos.router(页面路由)](../../errors/页面路由错误码.md)错误码。

错误码ID错误信息300001hsp silent install fail.
...
具体根据options中指定不同的[LaunchMode](../misc/Navigation.md#ZH-CN_TOPIC_0000002497444902__launchmode12枚举说明)，有不同的行为。
...
参数名类型必填说明moduleNamestring是目标[NavDestination](../misc/NavDestination.md)所在分包的moduleName。info[NavPathInfo](../misc/Navigation.md#ZH-CN_TOPIC_0000002497444902__navpathinfo10)是[NavDestination](../misc/NavDestination.md)页面的信息。options[NavigationOptions](../misc/Navigation.md#ZH-CN_TOPIC_0000002497444902__navigationoptions12)否页面栈操作选项。
...
先判断分包是否存在，若不存在，则通过moduleName下载分包，再将name指定的[NavDestination](../misc/NavDestination.md)页面信息入栈，传递的数据为param，使用Promise异步回调。
...
参数名类型必填说明moduleNamestring是目标[NavDestination](../misc/NavDestination.md)所在分包的moduleName。namestring是[NavDestination](../misc/NavDestination.md)页面名称。paramObject是[NavDestination](../misc/NavDestination.md)页面详细参数。animatedboolean否
...
先判断分包是否存在，若不存在，则通过moduleName下载分包，再将name指定的[NavDestination](../misc/NavDestination.md)页面信息入栈，传递的数据为param，添加onPop回调接收入栈页面出栈时的返回结果，并进行处理，使用Promise异步回调。
...
参数名类型必填说明moduleNamestring是目标[NavDestination](../misc/NavDestination.md)所在分包的moduleName。namestring是[NavDestination](../misc/NavDestination.md)页面名称。paramObject是[NavDestination](../misc/NavDestination.md)页面详细参数。onPopCallback<[PopInfo](../misc/Navigation.md#ZH-CN_TOPIC_0000002497444902__popinfo11)>是Callback回调，用于页面出栈时触发该回调处理返回结果。animatedboolean否
...
错误码ID错误信息401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameters types. 3. Parameter verification failed.100001Internal error.100005Builder function not registered.100006NavDestination not found.300001hsp silent install fail.
...
先判断分包是否存在，若不存在，则通过moduleName下载分包，再将name指定的[NavDestination](../misc/NavDestination.md)页面信息入栈，传递的数据为param，并且添加用于页面出栈时处理返回结果的OnPop回调，使用Promise异步回调。
...
参数名类型必填说明moduleNamestring是目标[NavDestination](../misc/NavDestination.md)所在分包的moduleName。namestring是[NavDestination](../misc/NavDestination.md)页面名称。paramObject是[NavDestination](../misc/NavDestination.md)页面详细参数。onPopCallback<[PopInfo](../misc/Navigation.md#ZH-CN_TOPIC_0000002497444902__popinfo11)>是Callback回调，用于页面出栈时处理返回结果。animatedboolean否
...
先判断分包是否存在，若不存在，则通过moduleName下载分包，再将当前页面栈栈顶退出，将info指定的[NavDestination](../misc/NavDestination.md)页面信息入栈，使用Promise异步回调。
...
参数名类型必填说明moduleNamestring是目标[NavDestination](../misc/NavDestination.md)所在分包的moduleName。info[NavPathInfo](../misc/Navigation.md#ZH-CN_TOPIC_0000002497444902__navpathinfo10)是新栈顶页面参数信息animatedboolean否
...
参数名类型必填说明moduleNamestring是目标[NavDestination](../misc/NavDestination.md)所在分包的moduleName。info[NavPathInfo](../misc/Navigation.md#ZH-CN_TOPIC_0000002497444902__navpathinfo10)是新栈顶页面参数信息。options[NavigationOptions](../misc/Navigation.md#ZH-CN_TOPIC_0000002497444902__navigationoptions12)否页面栈操作选项。
