...
| 名称 | typedef关键字 | 描述 |
| --- | --- | --- |
| OH_OnFrameAvailableListener | OH_OnFrameAvailableListener | 一个OH_NativeImage的监听者，通过OH_NativeImage_SetOnFrameAvailableListener接口注册该监听结构体，当有buffer可获取时，将触发回调给用户。 |
...
| int32_t OH_NativeImage_GetBufferMatrix(OH_NativeImage* image, float matrix[16]) | - | 获取根据生产端设置的旋转角度和buffer实际有效内容区域计算出的变换矩阵。 本接口返回一个变换矩阵，该矩阵是OH_NativeImage在消费buffer，即调用OH_NativeImage_UpdateSurfaceImage或者OH_NativeImage_AcquireNativeWindowBuffer时，根据buffer的旋转角度和实际有效内容区域计算所得。 本接口为非线程安全类型接口。 |
...
| OH_NativeImage* OH_NativeImage_CreateWithSingleBufferMode(uint32_t textureId, uint32_t textureTarget, bool singleBufferMode) | - | 使用纹理ID创建一个OH_NativeImage实例，该实例与OpenGL ES的纹理ID和纹理目标相关联，并选择是否设置单buffer模式。 本接口需要与OH_NativeImage_Destroy接口配合使用，否则会存在内存泄露。 本接口为非线程安全类型接口。 |
...
本接口需要与[OH_NativeImage_Destroy](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_destroy)接口配合使用，否则会存在内存泄露。
...
| uint32_t textureTarget | OpenGL ES的纹理目标，取值范围为GL_TEXTURE_2D和GL_TEXTURE_EXTERNAL_OES，具体可见选择纹理类型。 |
...
替代接口： [OH_NativeImage_GetTransformMatrixV2](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_gettransformmatrixv2)
...
matrix在[OH_NativeImage_UpdateSurfaceImage](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_updatesurfaceimage)接口调用后，才会更新。
...
本接口返回一个变换矩阵，该矩阵是[OH_NativeImage](OH_NativeImage.md)在消费buffer，即调用[OH_NativeImage_UpdateSurfaceImage](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_updatesurfaceimage)或者[OH_NativeImage_AcquireNativeWindowBuffer](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_acquirenativewindowbuffer)时，根据buffer的旋转角度和实际有效内容区域计算所得。
...
通过消费端的OH_NativeImage获取一个OHNativeWindowBuffer。本接口不能与[OH_NativeImage_UpdateSurfaceImage](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_updatesurfaceimage)接口同时使用。

本接口将会创建一个OHNativeWindowBuffer。当使用OHNativeWindowBuffer时，用户需要通过[OH_NativeWindow_NativeObjectReference](external_window.h.md#ZH-CN_TOPIC_0000002553202135__oh_nativewindow_nativeobjectreference)接口将其引用计数加一。当OHNativeWindowBuffer使用完，用户需要通过[OH_NativeWindow_NativeObjectUnreference](external_window.h.md#ZH-CN_TOPIC_0000002553202135__oh_nativewindow_nativeobjectunreference)接口将其引用计数减一。

本接口需要和[OH_NativeImage_ReleaseNativeWindowBuffer](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_releasenativewindowbuffer)接口配合使用，否则会存在内存泄露。
...
本接口不能与[OH_NativeImage_UpdateSurfaceImage](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_updatesurfaceimage)接口同时使用。

本接口与[OH_NativeImage_AcquireNativeWindowBuffer](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_acquirenativewindowbuffer)和[OH_NativeImage_ReleaseNativeWindowBuffer](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_releasenativewindowbuffer)配合使用。

本接口需要和[OH_NativeImage_Destroy](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_destroy)接口配合使用，否则会存在内存泄露。
...
此接口建议在[OH_NativeImage_Create](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_create)接口调用后立即调用。

此接口在与[OH_NativeImage_UpdateSurfaceImage](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_updatesurfaceimage)接口一起使用的场景下才会生效。

本接口为非线程安全类型接口。

通过[OH_NativeImage_SetOnFrameAvailableListener](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_setonframeavailablelistener)设置的listener回调不会因为设置了丢帧模式而减少。
...
| --- | --- |
| uint32_t textureId | OpenGL ES的纹理ID，OH_NativeImage实例会与之相关联。 |
| uint32_t textureTarget | OpenGL ES的纹理目标，具体可见选择纹理类型。 |
...
获取最近调用[OH_NativeImage_UpdateSurfaceImage](native_image.h.md#ZH-CN_TOPIC_0000002522082172__oh_nativeimage_updatesurfaceimage)的纹理图像的相关色彩空间。
...
本接口将会创建一个OHNativeWindowBuffer。当使用OHNativeWindowBuffer时，用户需要通过[OH_NativeWindow_NativeObjectReference](external_window.h.md#ZH-CN_TOPIC_0000002553202135__oh_nativewindow_nativeobjectreference)接口将其引用计数加一。

当OHNativeWindowBuffer使用完，用户需要通过[OH_NativeWindow_NativeObjectUnreference](external_window.h.md#ZH-CN_TOPIC_0000002553202135__oh_nativewindow_nativeobjectunreference)接口将其引用计数减一。
...
查询与[OH_NativeImage](OH_NativeImage.md)关联的纹理是否已释放。
...
| 类型 | 说明 |
| --- | --- |
| int32_t | 执行成功时返回NATIVE_ERROR_OK。 image或isReleased为空指针时返回NATIVE_ERROR_INVALID_ARGUMENTS。 |
...
清除所有[OHNativeWindow](NativeWindow.md)的OHNativeWindowBuffer缓存，并将[OH_NativeImage](OH_NativeImage.md)从OpenGL ES上下文中分离。
...
| 类型 | 说明 |
| --- | --- |
| int32_t | 执行成功时返回NATIVE_ERROR_OK。 image为空指针时返回NATIVE_ERROR_INVALID_ARGUMENTS。 |