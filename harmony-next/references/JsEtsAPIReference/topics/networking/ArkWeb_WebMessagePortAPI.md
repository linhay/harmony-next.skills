...
Post Message相关的Native API结构体。在调用接口前建议通过[ARKWEB_MEMBER_MISSING](../../capi/headers/arkweb_type.h.md#ZH-CN_TOPIC_0000002529445189__宏定义)校验该函数结构体是否有对应函数指针，避免SDK与设备ROM不匹配导致crash问题。WebMessagePort相关接口需在UI线程中调用OH_ArkWeb_GetNativeAPI方法获取。
...
类型说明[ArkWeb_ErrorCode](../../capi/headers/arkweb_error_code.h.md#ZH-CN_TOPIC_0000002529445187__arkweb_errorcode)
...
参数项描述const [ArkWeb_WebMessagePortPtr](ArkWeb_WebMessagePort_.md) webMessagePortPost Message端口结构体指针。const char* webTagWeb组件名称。[ArkWeb_OnMessageEventHandler](../../capi/headers/arkweb_type.h.md#ZH-CN_TOPIC_0000002529445189__arkweb_onmessageeventhandler) messageEventHandler处理消息的回调。void* userData用户自定义数据。