...
用户点击设备后，会触发设备选择控制器对应的接口回调，并返回已选择的设备信息；应用可以通过获取到的设备信息处理分布式业务，比如通过startAbility启动远端UIAbility；待启动远端ability成功后，可以通过[updateState](#ZH-CN_TOPIC_0000002553201531__updatestate)接口更新业务状态，界面上将会显示指定的状态。
...
| 类型 | 说明 |
| --- | --- |
| DevicePickerController | 返回设备选择控制器实例，可以与流转控件CollaborationDevicePicker (流转控件)配合使用。 |
...
```
   ...
   ![image](public_sys-resources/zh-cn_image_0000002522085800.webp)![image](public_sys-resources/zh-cn_image_0000002553365713.webp)
...
| --- | --- | --- | --- |
| event | DeviceEvent | 是 | 设备相关的事件，目前支持：设备选择、设备取消选择、已选择设备的下线事件。 |
| callback | Callback<distributedDeviceManager.DeviceBasicInfo> | 否 | 返回发生指定事件的设备信息，与on('deviceSelected' | 'deviceUnselected' | 'selectedDeviceOffline')接口对应，可以在不需要监听设备事件的位置进行取消。比如在组件生命周期的aboutToAppear()调用了on注册接口，则可以在页面aboutToDisappear()调用off取消注册。 |
...
```
   ...
   ![image](public_sys-resources/zh-cn_image_0000002522245790.webp)![image](public_sys-resources/zh-cn_image_0000002553205755.webp)![image](public_sys-resources/zh-cn_image_0000002522085802.webp)![image](public_sys-resources/zh-cn_image_0000002553365715.webp)
...
释放设备选择控制器，与[createDevicePickerController](#ZH-CN_TOPIC_0000002553201531__createdevicepickercontroller)方法对应。

在应用不需要设备选择控制器的时候需要调用release()接口进行释放，比如在应用onCreate()生命周期回调中通过[createDevicePickerController](#ZH-CN_TOPIC_0000002553201531__createdevicepickercontroller)创建了设备选择控制器，可以在onDestroy()生命周期回调中通过release()进行释放。
