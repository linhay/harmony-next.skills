...
| 类型 | 说明 |
| --- | --- |
| Wifi_ResultCode | 返回操作结果，详细定义参见Wifi_ResultCode.  WIFI_SUCCESS 查询WIFI开关状态成功。  WIFI_INVALID_PARAM 入参为空指针。  WIFI_OPERATION_FAILED 服务内部执行错误。 |
...
| 类型 | 说明 |
| --- | --- |
| Wifi_ResultCode | 返回操作结果，详细定义参见Wifi_ResultCode。  WIFI_SUCCESS 成功获取设备Mac地址。  WIFI_PERMISSION_DENIED 权限拒绝。  WIFI_NOT_SUPPORTED 不支持该能力。  WIFI_INVALID_PARAM 输入参数macAddr是空指针。  WIFI_OPERATION_FAILED 内部执行失败。  WIFI_STA_DISABLED Wi-Fi STA模式未启用。 |