...
继承自[GestureInterface<T>](../misc/手势公共接口.md#ZH-CN_TOPIC_0000002497604864__gestureinterfacet11)，设置捏合手势事件。
...
参数名类型必填说明value{ fingers?: number; distance?: number }否
...
设置捏合手势事件。与[PinchGesture](#ZH-CN_TOPIC_0000002497444882__pinchgesture-1)相比，options参数新增isFingerCountLimited，表示是否检查触摸屏幕的手指数量。
...
参数名类型必填说明options[PinchGestureHandlerOptions](../misc/手势处理器.md#ZH-CN_TOPIC_0000002497604858__pinchgesturehandleroptions)否捏合手势处理器配置参数。

#### 事件

 在[GestureEvent](../misc/手势公共接口.md#ZH-CN_TOPIC_0000002497604864__gestureevent对象说明)的fingerList元素中，手指索引编号与位置相对应，即fingerList[index]的id为index。对于先按下但未参与当前手势触发的手指，fingerList中对应的位置为空。建议开发者优先使用fingerInfos。
...
参数名类型必填说明event(event: [GestureEvent](../misc/手势公共接口.md#ZH-CN_TOPIC_0000002497604864__gestureevent对象说明)) => void是手势事件回调函数。
...
Pinch手势识别成功并接收到触摸取消事件的回调。与[onActionCancel](#ZH-CN_TOPIC_0000002497444882__onactioncancel)相比，该回调返回手势事件信息。
...
参数名类型必填说明eventCallback<[GestureEvent](../misc/手势公共接口.md#ZH-CN_TOPIC_0000002497604864__gestureevent对象说明)>是手势事件回调函数。
