...
从API version 20开始，在自定义布局的自定义组件中，子组件若设置了[LayoutPolicy](尺寸设置.md#ZH-CN_TOPIC_0000002553360703__layoutpolicy15)对象的fixAtIdealSize属性，表示尺寸将不受父组件约束，完全按照开发者自定义的尺寸范围布局。

自定义布局内不支持使用懒加载(包含[Repeat](Repeat.md)和[LazyForEach](LazyForEach.md))。
...
示例请参考[自定义布局代码示例](#ZH-CN_TOPIC_0000002553360823__示例)。

**GeometryInfo10+**

父组件（自定义组件）布局信息，继承自[SizeResult](#ZH-CN_TOPIC_0000002553360823__sizeresult10)。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| borderWidth | EdgeWidth | 否 | 否 | 父组件（自定义组件）边框宽度。 单位：vp。 |
| margin | Margin | 否 | 否 | 父组件（自定义组件）margin信息。 单位：vp。 |
| padding | Padding | 否 | 否 | 父组件（自定义组件）padding信息。 单位：vp。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| measureResult | MeasureResult | 否 | 否 | 子组件测量后的尺寸信息。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 单位：vp |
...
测量后的组件布局信息。继承自[SizeResult](#ZH-CN_TOPIC_0000002553360823__sizeresult10)。
...
从API version 9开始支持，从API version 10开始废弃，推荐使用[onPlaceChildren](#ZH-CN_TOPIC_0000002553360823__onplacechildren10)替代。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| children | Array<LayoutChild> | 是 | 子组件布局信息。 |
...
从API version 9开始支持，从API version 10开始废弃，推荐使用[onMeasureSize](#ZH-CN_TOPIC_0000002553360823__onmeasuresize10)替代。
...
从API version 9开始支持，从API version 10开始废弃。建议使用[Measurable](#ZH-CN_TOPIC_0000002553360823__measurable10)或者[Layoutable](#ZH-CN_TOPIC_0000002553360823__layoutable10)替代。
...
| borderInfo | LayoutBorderInfo | 否 | 否 | 子组件border信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| childLayoutInfo | LayoutInfo | 是 | 子组件layout信息。 |
...
从API version 9开始支持，从API version 10开始废弃。建议使用[getBorderWidth](#ZH-CN_TOPIC_0000002553360823__getborderwidth12)，[getMargin](#ZH-CN_TOPIC_0000002553360823__getmargin12)和[getPadding](#ZH-CN_TOPIC_0000002553360823__getpadding12)替代。
...
![image](public_sys-resources/zh-cn_image_0000002553365277.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522245354.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553205317.webp)

**示例4（子组件超过父组件大小约束）**

在自定义布局的自定义组件中，为子组件设置了[LayoutPolicy](尺寸设置.md#ZH-CN_TOPIC_0000002553360703__layoutpolicy15)对象的fixAtIdealSize属性。
...
![image](public_sys-resources/zh-cn_image_0000002522085364.webp)