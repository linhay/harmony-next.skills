...
参数名类型必填说明controller[ArcSwiperController](#ZH-CN_TOPIC_0000002529444841__arcswipercontroller)否给组件绑定一个控制器，用来控制组件翻页。

#### 属性

除支持[通用属性](../misc/通用属性.md)外，还支持以下属性，不支持[Menu控制](../misc/菜单控制.md)。
...
参数名类型必填说明styleOptional<[ArcDotIndicator](#ZH-CN_TOPIC_0000002529444841__arcdotindicator) | boolean>是
...
参数名类型必填说明durationOptional<number>是
...
参数名类型必填说明isVerticalOptional<boolean>是
...
参数名类型必填说明disabledOptional<boolean>是
...
参数名类型必填说明sensitivityOptional<[CrownSensitivity](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__crownsensitivity18)>是
...
设置边缘滑动效果。 目前支持的滑动效果参见[EdgeEffect](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__edgeeffect)的枚举说明。调用控制器接口时回弹不生效。
...
翻至下一页。翻页带动效切换过程，时长通过[duration](#ZH-CN_TOPIC_0000002529444841__duration)指定。
...
翻至上一页。翻页带动效切换过程，时长通过[duration](#ZH-CN_TOPIC_0000002529444841__duration)指定。
...
参数名类型必填说明handler[FinishAnimationHandler](#ZH-CN_TOPIC_0000002529444841__finishanimationhandler)否
...
参数名类型必填说明directionOptional<[ArcDirection](#ZH-CN_TOPIC_0000002529444841__arcdirection-1)>是
...
类型说明[ArcDotIndicator](#ZH-CN_TOPIC_0000002529444841__arcdotindicator)提供弧形圆点指示器属性及功能。
...
参数名类型必填说明colorOptional<[ResourceColor](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcecolor)>是
...
参数名类型必填说明colorOptional<[LinearGradient](DataPanel.md#ZH-CN_TOPIC_0000002497604906__lineargradient10)>是
...
名称值说明THREE_CLOCK_DIRECTION03点钟方向。SIX_CLOCK_DIRECTION16点钟方向。NINE_CLOCK_DIRECTION29点钟方向。
...
参数名类型必填说明indexnumber是当前显示元素的索引。index序列从0开始。
...
参数名类型必填说明indexnumber是当前显示元素的索引，动画开始前的index值（不是最终结束动画的index值）。targetIndexnumber是切换动画目标元素的索引。event[SwiperAnimationEvent](../misc/swiper.md#ZH-CN_TOPIC_0000002529284867__swiperanimationevent10对象说明)是动画相关信息，包括主轴方向上当前显示元素和目标元素相对ArcSwiper起始位置的位移，以及离手速度。
...
参数名类型必填说明indexnumber是当前显示元素的索引。event[SwiperAnimationEvent](../misc/swiper.md#ZH-CN_TOPIC_0000002529284867__swiperanimationevent10对象说明)是动画相关信息，只返回主轴方向上当前显示元素相对于ArcSwiper起始位置的位移。
...
参数名类型必填说明handlerOptional<[IndexChangedHandler](#ZH-CN_TOPIC_0000002529444841__indexchangedhandler)>是当前显示元素的索引回调。
...
参数名类型必填说明handlerOptional<[AnimationStartHandler](#ZH-CN_TOPIC_0000002529444841__animationstarthandler)>是切换动画开始时的回调。
...
当ArcSwiper切换动效结束时触发，包括动画过程中手势中断，通过[SwiperController](../misc/swiper.md#ZH-CN_TOPIC_0000002529284867__swipercontroller)调用finishAnimation。参数为动画结束后的index值，多列ArcSwiper时，index为最左侧组件的索引。
...
参数名类型必填说明handlerOptional<[AnimationEndHandler](#ZH-CN_TOPIC_0000002529444841__animationendhandler)>是切换动画结束时触发该回调。
...
参数名类型必填说明handlerOptional<[GestureSwipeHandler](#ZH-CN_TOPIC_0000002529444841__gestureswipehandler)>是在页面跟手滑动过程中，逐帧触发该回调。
...
在页面跟手滑动和离手后执行切换动画的过程中，会对视窗内所有页面逐帧触发[SwiperContentTransitionProxy](#ZH-CN_TOPIC_0000002529444841__swipercontenttransitionproxy)回调。例如，当视窗内有下标为0、1的两个页面时，会每帧触发两次index值分别为0和1的回调。
...
参数名类型必填说明transitionOptional<[SwiperContentAnimatedTransition](#ZH-CN_TOPIC_0000002529444841__swipercontentanimatedtransition)>是ArcSwiper自定义切换动画相关信息。
...
名称类型只读可选说明timeoutnumber否是ArcSwiper自定义切换动画超时时间。从页面执行默认动画（页面滑动）至移出视窗外的第一帧开始计时，如果到达该时间后，开发者仍未调用[SwiperContentTransitionProxy](#ZH-CN_TOPIC_0000002529444841__swipercontenttransitionproxy)的[finishTransition](#ZH-CN_TOPIC_0000002529444841__finishtransition)接口通知ArcSwiper组件此页面的自定义动画已结束，那么组件就会认为此页面的自定义动画已结束，立即在该页面节点下渲染树。单位ms，默认值为0。transitionCallback<[SwiperContentTransitionProxy](#ZH-CN_TOPIC_0000002529444841__swipercontenttransitionproxy)>否否自定义切换动画具体内容。
...
名称类型只读可选说明selectedIndexnumber否否当前选中页面的索引。indexnumber否否视窗内页面的索引。positionnumber否否index页面相对于ArcSwiper主轴起始位置（selectedIndex对应页面的起始位置）的移动比例。mainAxisLengthnumber否否index对应页面在主轴方向上的长度。
