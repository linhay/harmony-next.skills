...
安全区域是指页面的显示区域，默认情况下开发者开发的界面都布局在安全区域内，不与系统设置的避让区比如状态栏、导航栏区域重叠。提供属性方法允许开发者设置组件绘制内容突破安全区域的限制，通过[expandSafeArea](#ZH-CN_TOPIC_0000002553360705__expandsafearea)属性支持组件不改变布局情况下扩展其绘制区域至安全区外，通过设置[setKeyboardAvoidMode](#ZH-CN_TOPIC_0000002553360705__setkeyboardavoidmode11)来配置虚拟键盘弹出时页面的避让模式。页面中有标题栏等文字不希望和避让区重叠时，建议对组件设置expandSafeArea属性实现沉浸式效果，也可直接通过窗口接口[setWindowLayoutFullScreen](Interface (Window).md#ZH-CN_TOPIC_0000002522080752__setwindowlayoutfullscreen9)实现全屏沉浸式效果。
...
滚动类容器内的组件不建议设置expandSafeArea属性，如果设置，需要按照组件嵌套关系，将当前节点到滚动类祖先容器间所有直接节点设置expandSafeArea属性，否则expandSafeArea属性在滚动后可能会失效，写法参考[示例7](#ZH-CN_TOPIC_0000002553360705__示例7滚动类容器扩展安全区)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| types | Array <LayoutSafeAreaType> | 否 | 扩展布局安全区域的类型。 默认值：[LayoutSafeAreaType.SYSTEM]，扩展至所有安全区域，比如：状态栏，导航栏和组件级安全区（safeAreaPadding）。 非法值：按默认值处理。 |
| edges | Array <LayoutSafeAreaEdge> | 否 | 扩展布局安全区的边缘，并且支持镜像能力。 默认值：[LayoutSafeAreaEdge.ALL]，扩展组件所有边缘。 非法值：按默认值处理。 |
...
忽略布局安全区边缘的组件，如果其宽度或高度设置了[LayoutPolicy.matchParent](尺寸设置.md#ZH-CN_TOPIC_0000002553360703__layoutpolicy15)，其大小和位置都会改变，否则仅改变其位置。
...
![image](public_sys-resources/zh-cn_image_0000002553364679.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522244760.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553204721.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522084768.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553364681.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522244762.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553204723.webp)

**示例8（ignoreLayoutSafeArea延伸组件布局范围）**

该示例利用[ignoreLayoutSafeArea](#ZH-CN_TOPIC_0000002553360705__ignorelayoutsafearea20)改变组件位置。相比未使用该属性，配置ignoreLayoutSafeArea后，Row组件基于Stack内容区、Stack组件级安全区、系统状态栏共同组成的范围，取其左上部分，作左上对齐。
...
![image](public_sys-resources/zh-cn_image_0000002522084770.webp)

**示例9（ignoreLayoutSafeArea配合LayoutPolicy.matchParent延伸组件布局范围）**

该示例利用[ignoreLayoutSafeArea](#ZH-CN_TOPIC_0000002553360705__ignorelayoutsafearea20)和[LayoutPolicy.matchParent](尺寸设置.md#ZH-CN_TOPIC_0000002553360703__layoutpolicy15)同时改变组件大小和位置。相比未使用该属性，配置ignoreLayoutSafeArea后，Row组件基于Stack内容区、Stack组件级安全区，取其右下部分并撑满可用空间。
...
![image](public_sys-resources/zh-cn_image_0000002553364683.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522244764.webp)