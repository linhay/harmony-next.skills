...
设置滚动条的颜色。与[scrollBarColor11+](#ZH-CN_TOPIC_0000002522240828__scrollbarcolor11)相比，color参数开始支持Resource类型。
...
惯性动效触发场景包括：惯性手指快速滑动并离手时，或调用[fling](Scroll.md#ZH-CN_TOPIC_0000002553360749__fling12)方法。

-

使用鼠标滚轮、键盘方向键方式滚动，或通过[scrollTo](Scroll.md#ZH-CN_TOPIC_0000002553360749__scrollto)等方法直接滚动到指定位置，不会产生惯性动效。

-

如果惯性动效通过[fling](Scroll.md#ZH-CN_TOPIC_0000002553360749__fling12)方法触发，则flingSpeedLimit设置不生效。
...
fadingEdge是通过设置[overlay](浮层.md#ZH-CN_TOPIC_0000002522080776__overlay)属性和[blendMode](图像效果.md#ZH-CN_TOPIC_0000002553360709__blendmode11)属性（参数值为BlendMode.SRC_OVER，BlendApplyType.OFFSCREEN）实现的。当fadingEdge生效时，会覆盖原组件的.overlay()属性和.blendMode()属性。

fadingEdge生效时，建议不在设置fadingEdge属性的组件上设置[background](背景设置.md#ZH-CN_TOPIC_0000002553200739__background10)相关属性，会影响渐隐的显示效果。

fadingEdge生效时，设置fadingEdge属性的组件会裁剪到边界，在该组件上设置[clip](形状裁剪.md#ZH-CN_TOPIC_0000002522240784__clip12)属性为false不生效。
...
支持当前页面的滚动组件收到点击状态栏事件后，通过动画回到顶部。点击状态栏后，后台应用的滚动组件不受影响，不做回到顶部的动作。本属性不受[enableScrollInteraction](#ZH-CN_TOPIC_0000002522240828__enablescrollinteraction11)设置的影响。
...
组件收到[表冠事件](表冠事件.md)的前提是该组件获焦，焦点控制可以通过[focusable](焦点控制.md#ZH-CN_TOPIC_0000002553200755__focusable)、[defaultFocus](焦点控制.md#ZH-CN_TOPIC_0000002553200755__defaultfocus9)、[focusOnTouch](焦点控制.md#ZH-CN_TOPIC_0000002553200755__focusontouch9)进行管理。
...
滚动开始时触发。手指拖动滚动组件或拖动滚动组件的滚动条触发的滚动开始时，会触发该事件。使用[Scroller](Scroll.md#ZH-CN_TOPIC_0000002553360749__scroller)滚动控制器触发的带动画的滚动，动画开始时会触发该事件。
...
滚动停止时触发。手拖动滚动组件或拖动滚动组件的滚动条触发的滚动，手离开屏幕后滚动停止时会触发该事件。使用[Scroller](Scroll.md#ZH-CN_TOPIC_0000002553360749__scroller)滚动控制器触发的带动画的滚动，动画停止时会触发该事件。
...
回调当前帧将要滚动的偏移量和当前滚动状态和滚动操作来源，其中回调的偏移量为计算得到的将要滚动的偏移量值，并非最终实际滚动偏移。可以通过该回调返回值指定滚动组件将要滚动的偏移。[Scroll](Scroll.md)组件的[onWillScroll](Scroll.md#ZH-CN_TOPIC_0000002553360749__onwillscroll12)接口的参数类型是[ScrollOnWillScrollCallback](Scroll.md#ZH-CN_TOPIC_0000002553360749__scrollonwillscrollcallback12)。
...
调用不带动画的[ScrollEdge](Scroll.md#ZH-CN_TOPIC_0000002553360749__scrolledge)和[ScrollToIndex](Scroll.md#ZH-CN_TOPIC_0000002553360749__scrolltoindex)时，不触发onWillScroll。
...
从API version 11开始支持，从API version 12开始废弃，[List](List.md)、[Grid](Grid.md)和[WaterFlow](WaterFlow.md)组件的onScroll事件在布局之后触发，建议使用[onDidScroll](#ZH-CN_TOPIC_0000002522240828__ondidscroll12)替代；[Scroll](Scroll.md)组件的onScroll事件在布局之前触发，建议使用[onWillScroll](#ZH-CN_TOPIC_0000002522240828__onwillscroll12)替代。
...
如果惯性动效通过[fling](Scroll.md#ZH-CN_TOPIC_0000002553360749__fling12)方法触发，则onWillStartFling不触发。

-

惯性动效的触发场景参考[flingSpeedLimit](#ZH-CN_TOPIC_0000002522240828__flingspeedlimit11)方法的说明。
...
[nestedScroll](#ZH-CN_TOPIC_0000002522240828__nestedscroll11)属性参数对象。
...
[edgeEffect](#ZH-CN_TOPIC_0000002522240828__edgeeffect11)属性参数对象。
...
[fadingEdge](#ZH-CN_TOPIC_0000002522240828__fadingedge14)属性边缘渐隐参数对象。
...
![image](public_sys-resources/zh-cn_image_0000002522244922.webp)
...
[OnWillScrollCallback](#ZH-CN_TOPIC_0000002522240828__onwillscrollcallback12)返回值对象。
...
设置[onReachStart](滚动组件通用接口.md#ZH-CN_TOPIC_0000002522240828__onreachstart11)事件的回调。
...
设置[onReachEnd](滚动组件通用接口.md#ZH-CN_TOPIC_0000002522240828__onreachend11)事件的回调。
...
设置[onScrollStart](滚动组件通用接口.md#ZH-CN_TOPIC_0000002522240828__onscrollstart11)事件的回调。
...
设置[onScrollStop](滚动组件通用接口.md#ZH-CN_TOPIC_0000002522240828__onscrollstop11)事件的回调。
...
设置[onScrollFrameBegin](Scroll.md#ZH-CN_TOPIC_0000002553360749__onscrollframebegin9)事件的回调。
...
该示例通过设置[enableScrollInteraction](滚动组件通用接口.md#ZH-CN_TOPIC_0000002522240828__enablescrollinteraction11)属性，实现了使用手势滚动纵向列表，并在当前显示界面发生改变时回调索引。

ListDataSource说明及完整代码参考[示例1添加滚动事件](List.md#ZH-CN_TOPIC_0000002522240820__示例1添加滚动事件)。
...
该示例通过设置[fadingEdge](滚动组件通用接口.md#ZH-CN_TOPIC_0000002522240828__fadingedge14)属性，实现了[List](List.md)组件开启边缘渐隐效果并设置边缘渐隐长度。
...
该示例通过设置[clipContent](滚动组件通用接口.md#ZH-CN_TOPIC_0000002522240828__clipcontent14)属性，改变组件的内容层裁剪区域。
...
![image](public_sys-resources/zh-cn_image_0000002553204883.webp)

**示例4（设置滚动条边距）**

从API version 20开始，该示例通过设置[scrollBarMargin](#ZH-CN_TOPIC_0000002522240828__scrollbarmargin20)属性，调整滚动组件的滚动条边距。
...
![image](public_sys-resources/zh-cn_image_0000002522084930.webp)