...
名称类型默认值必填描述valuestring-是用来生成二维码的内容。最大长度为256。idstring-否组件的唯一标识。stylestring-否组件的样式声明。classstring-否组件的样式类，用于引用样式表。refstring-否用来指定指向子元素的引用信息，该引用将注册到父组件的$refs 属性对象上。[]()[]()
...
名称类型默认值必填描述color<color>#000000否二维码颜色。background-color<color>#ffffff否二维码背景颜色。width<length> | <percentage>5+-否
