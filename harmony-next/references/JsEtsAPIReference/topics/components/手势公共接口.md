...
手势识别器对应的滚动类容器组件的信息，继承于[EventTargetInfo](#ZH-CN_TOPIC_0000002522080814__eventtargetinfo12)。
...
在手指全部抬起前阻止手势识别器参与当前手势识别。如果系统已确定该手势识别器的结果（无论成功与否），调用此接口将无效。此方法与GestureRecognizer.[setEnabled](#ZH-CN_TOPIC_0000002522080814__setenabled12)(isEnabled: boolean)不同，[setEnabled](#ZH-CN_TOPIC_0000002522080814__setenabled12)并不会阻止手势识别器对象参与手势识别过程，而只会影响手势对应的回调函数是否执行。
...
点击手势识别器对象，继承自[GestureRecognizer](#ZH-CN_TOPIC_0000002522080814__gesturerecognizer12)。
...
长按手势识别器对象，继承于[GestureRecognizer](#ZH-CN_TOPIC_0000002522080814__gesturerecognizer12)。
...
快滑手势识别器对象，继承于[GestureRecognizer](#ZH-CN_TOPIC_0000002522080814__gesturerecognizer12)。
...
捏合手势识别器对象，继承于[GestureRecognizer](#ZH-CN_TOPIC_0000002522080814__gesturerecognizer12)。
...
旋转手势识别器对象，继承于[GestureRecognizer](#ZH-CN_TOPIC_0000002522080814__gesturerecognizer12)。
...
滑动手势识别器对象，继承于[GestureRecognizer](#ZH-CN_TOPIC_0000002522080814__gesturerecognizer12)。
...
返回滑动手势识别器在不同输入源的情况下触发的最小滑动距离。
 ...
 仅支持对通过Pan手势初始化配置修改的设备类型进行阈值查询。对于默认滑动阈值，可通过查询[SourceTool](绑定手势方法.md#ZH-CN_TOPIC_0000002522240806__sourcetool枚举说明9).Unknown类型获取。其他未主动设置的类型则无法获取。
...
定义手势的事件信息。继承自[BaseEvent](自定义手势判定.md#ZH-CN_TOPIC_0000002522240812__baseevent8)。
...
| tapLocation20+ | EventLocationInfo | 否 | 是 | 用于点击手势中，获取当前手势的坐标信息。  元服务API： 从API version 20开始，该接口支持在元服务中使用。 |
...
基础手势事件类型。继承自[BaseEvent](自定义手势判定.md#ZH-CN_TOPIC_0000002522240812__baseevent8)。
...
继承自[BaseGestureEvent](#ZH-CN_TOPIC_0000002522080814__basegestureevent11对象说明)。可将该对象作为[onGestureJudgeBegin](自定义手势判定.md#ZH-CN_TOPIC_0000002522240812__ongesturejudgebegin)的event参数来传递。
