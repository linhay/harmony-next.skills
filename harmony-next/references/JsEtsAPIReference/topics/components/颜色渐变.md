...
线性渐变。与[linearGradient](#ZH-CN_TOPIC_0000002553200749__lineargradient)相比，options参数新增了对undefined类型的支持。
...
角度渐变。与[sweepGradient](#ZH-CN_TOPIC_0000002553200749__sweepgradient)相比，options参数新增了对undefined类型的支持。
...
| repeating7+ | boolean | 否 | 是 | 为渐变的颜色重复着色。 默认值：false  true：允许为渐变的颜色重复着色。 false：不允许为渐变的颜色重复着色。  元服务API： 从API version 11开始，该接口支持在元服务中使用。 卡片能力： 从API version 9开始，该接口支持在ArkTS卡片中使用。 |
 ...
 [ColorMetrics](Graphics.md#ZH-CN_TOPIC_0000002553200709__colormetrics12)表示填充的颜色，可以使用[colorWithSpace](Graphics.md#ZH-CN_TOPIC_0000002553200709__colorwithspace20)方法构造指定色域属性的颜色。number表示指定颜色所处的位置，取值范围为[0, 1.0]，0表示需要设置渐变色的容器开始处，1.0表示容器的结束处。为了实现多个颜色渐变效果，多个数组中的number类型参数应递增设置。如果后一个数组中的number类型参数小于前一个数组的number类型参数，将按照等于前一个数组number值处理。
...
径向渐变。与[radialGradient](#ZH-CN_TOPIC_0000002553200749__radialgradient)相比，options参数新增了对undefined类型的支持。
...
| repeating7+ | boolean | 否 | 是 | 为渐变的颜色重复着色。默认值：false。  true：允许为渐变的颜色重复着色。 false：不允许为渐变的颜色重复着色。   元服务API： 从API version 11开始，该接口支持在元服务中使用。 卡片能力： 从API version 9开始，该接口支持在ArkTS卡片中使用。 |
 ...
 [ResourceColor](基础类型定义.md#ZH-CN_TOPIC_0000002553360849__resourcecolor)表示填充的颜色，number表示指定颜色所处的位置，取值范围为[0,1.0]，0表示需要设置渐变色的容器的开始处，1.0表示容器的结尾处。想要实现多个颜色渐变效果时，多个数组中number参数建议递增设置，如后一个数组number参数比前一个数组number小的话，按照等于前一个数组number的值处理。

**示例**

**示例1（颜色从右向左线性渐变）**

该示例通过[linearGradient](#ZH-CN_TOPIC_0000002553200749__lineargradient)来实现组件的颜色线性渐变。
...
![image](public_sys-resources/zh-cn_image_0000002553204741.webp)

**示例2（颜色按旋转角度渐变）**

该示例通过[sweepGradient](#ZH-CN_TOPIC_0000002553200749__sweepgradient)来实现组件颜色旋转角度渐变。
...
![image](public_sys-resources/zh-cn_image_0000002522084788.webp)

**示例3（颜色按径向渐变）**

该示例通过[radialGradient](#ZH-CN_TOPIC_0000002553200749__radialgradient)来实现组件颜色径向渐变。
...
![image](public_sys-resources/zh-cn_image_0000002553364701.webp)