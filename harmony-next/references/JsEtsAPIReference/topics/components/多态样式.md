...
多态样式仅支持[通用属性](通用属性.md)。如果多态样式不生效，则该属性可能为组件的私有属性，例如：fontColor、[TextInput](TextInput.md)组件的[backgroundColor](背景设置.md#ZH-CN_TOPIC_0000002553200739__backgroundcolor18)等。此时，可以通过attributeModifier动态设置组件属性来解决此问题。

-

当前多态样式实现依赖于组件自定义节点的刷新机制。因Builder不具备独立的自定义父节点，无法直接触发刷新，致使多态样式无法直接在Builder中生效。解决方法是将多态样式封装至自定义组件内部，再将此组件置于@Builder中，以此来间接实现多态样式。示例代码可参考[示例3设置Builder多态样式](#ZH-CN_TOPIC_0000002522080796__示例3设置builder多态样式)。

-

多态样式的焦点态只有在[焦点激活态](焦点事件.md)开启时生效。
...
当前多态样式的选中状态样式依赖组件选中属性值，可以使用[点击事件](点击事件.md)修改属性值，或使用属性自带[$$](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-two-way-sync)双向绑定功能。
...
![image](public_sys-resources/zh-cn_image_0000002553204769.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522084816.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553364729.webp)