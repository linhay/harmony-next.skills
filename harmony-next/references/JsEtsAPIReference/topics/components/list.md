...
当List和带[virtualScroll](../misc/Repeat.md#ZH-CN_TOPIC_0000002497604972__virtualscroll)的Repeat结合，它的懒加载行为和LazyForEach一致。当List和不带virtualScroll的Repeat结合，它的懒加载行为和ForEach一致。

如果可滚动组件嵌套List组件，并且滚动方向相同，List组件又没有设置主轴尺寸时，List组件会全量加载子组件，导致懒加载失效。该场景推荐使用List嵌套[ListItemGroup](ListItemGroup.md)组件以实现优化性能。

List的预加载是指除了加载显示区域可见区域外子组件外，还支持空闲时隙提前加载部分显示区域外不可见的子组件。使用预加载可以减少滚动丢帧，提升流畅性。预加载需要结合懒加载才会生效。List支持通过[cachedCount](#ZH-CN_TOPIC_0000002529284863__cachedcount)设置预加载的数量。默认会预加载显示区域上下各一屏子节点（最大预加载16行子节点）。List和[ForEach](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-rendering-control-foreach)、[LazyForEach](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-rendering-control-lazyforeach)、[Repeat](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-new-rendering-control-repeat)结合，预加载能力存在差异：
...
当List和带[virtualScroll](../misc/Repeat.md#ZH-CN_TOPIC_0000002497604972__virtualscroll)的Repeat结合，它的预加载行为和LazyForEach一致。当List和不带virtualScroll的Repeat结合，它的预加载行为和ForEach一致。
...
List(options?: [ListOptions](#ZH-CN_TOPIC_0000002529284863__listoptions18对象说明))
...
参数名类型必填说明options[ListOptions](#ZH-CN_TOPIC_0000002529284863__listoptions18对象说明)否设置List组件参数。[]()[]()
...
名称类型只读可选说明initialIndex7+number否是
...
从API version 14开始，如果在List组件创建完成后首次布局前（如List的[onAttach](../misc/挂载卸载事件.md#ZH-CN_TOPIC_0000002529284813__onattach12)事件中），调用Scroller滚动控制器中不带动画的scrollToIndex或scrollEdge方法，会覆盖initialIndex设置的值。

设置了initialIndex后，List从initialIndex对应的子组件开始布局，在这之前的子组件未参与布局，无法计算准确大小，因此通过[currentOffset](Scroll.md#ZH-CN_TOPIC_0000002497444896__currentoffset)接口获取到的List的滚动总偏移量通过估算得出，可能会有误差。可通过设置[childrenMainSize](#ZH-CN_TOPIC_0000002529284863__childrenmainsize12)确保List的滚动总偏移量的准确性。

**卡片能力：** 从API version 9开始，该接口支持在ArkTS卡片中使用。

**元服务API：** 从API version 11开始，该接口支持在元服务中使用。

space7+number | string否是
...
scroller7+[Scroller](Scroll.md#ZH-CN_TOPIC_0000002497444896__scroller)否是
...
除支持[通用属性](通用属性 (ts-component-general-attributes).md)和[滚动组件通用属性](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__属性)外，还支持以下属性：

List组件[通用属性clip](../misc/形状裁剪.md)的默认值为true。
...
参数名类型必填说明value[Axis](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__axis)是
...
divider(value: [ListDividerOptions](#ZH-CN_TOPIC_0000002529284863__listdivideroptions18对象说明) | null)
...
ListItem设置[多态样式](../misc/多态样式.md)时，被按压的子组件上下的分割线不绘制。
...
参数名类型必填说明value[ListDividerOptions](#ZH-CN_TOPIC_0000002529284863__listdivideroptions18对象说明) | null是
...
List设置cachedCount后，显示区域外上下各会预加载并布局cachedCount行ListItem。计算ListItem行数时，会计算ListItemGroup内部的ListItem行数。如果ListItemGroup内没有ListItem，则整个ListItemGroup算一行。配合[裁剪](../misc/形状裁剪.md#ZH-CN_TOPIC_0000002529284827__clip12)或[内容裁剪](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__clipcontent14)属性可以显示出预加载节点。
...
在计算ListItem行数时，会计算ListItemGroup内部的ListItem行数。如果ListItemGroup内没有ListItem，则整个ListItemGroup算一行。配合[clip](../misc/形状裁剪.md#ZH-CN_TOPIC_0000002529284827__clip12)或[clipContent](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__clipcontent14)属性可以显示出预加载节点。
...
参数名类型必填说明countnumber | [CacheCountInfo](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__cachecountinfo22对象说明)是
...
参数名类型必填说明valuenumber | [LengthConstrain](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__lengthconstrain)是
...
gutter10+[Dimension](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__dimension10)否
...
参数名类型必填说明valuenumber | [LengthConstrain](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__lengthconstrain) | [ItemFillPolicy](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__itemfillpolicy22)是
...
设置为ItemFillPolicy类型时，根据List组件宽度对应[断点类型](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-layout-development-grid-layout#栅格容器断点)确定列数，该类型只在List滚动方向为垂直方向时才生效。

gutter[Dimension](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__dimension10)否
...
参数名类型必填说明value[ListItemAlign](#ZH-CN_TOPIC_0000002529284863__listitemalign9枚举说明)是
...
由于浮点数计算精度，设置sticky后，在List滑动过程中小概率产生缝隙，可以通过[pixelRound](../misc/组件级像素取整.md#ZH-CN_TOPIC_0000002529284823__pixelround)指定当前组件向下像素取整解决该问题。
...
参数名类型必填说明value[StickyStyle](#ZH-CN_TOPIC_0000002529284863__stickystyle9枚举说明)是
...
只支持item等高场景限位，不等高场景可能存在不准确的情况。对齐动画期间[onWillScroll](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__onwillscroll12)事件上报的滚动操作来源类型为ScrollSource.FLING。
...
参数名类型必填说明value[ScrollSnapAlign](#ZH-CN_TOPIC_0000002529284863__scrollsnapalign10枚举说明)是
...
参数名类型必填说明speed[ScrollSnapAnimationSpeed](#ZH-CN_TOPIC_0000002529284863__scrollsnapanimationspeed22枚举说明)是
...
设置内容区域起始偏移量。列表滚动到起始位置时，列表内容与列表显示区域边界保留指定距离。与[contentStartOffset11+](#ZH-CN_TOPIC_0000002529284863__contentstartoffset11)相比，参数名改为offset，并开始支持Resource类型。
...
参数名类型必填说明offsetnumber | [Resource](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resource)是
...
设置内容区末尾偏移量。列表滚动到末尾位置时，列表内容与列表显示区域边界保留指定距离。与[contentEndOffset11+](#ZH-CN_TOPIC_0000002529284863__contentendoffset11)相比，参数名改为offset，并开始支持Resource类型。
...
- 该属性通过向List组件提供所有子组件在主轴方向的大小信息，确保在面对子组件主轴大小不一致、增删子组件、使用[scrollToIndex](Scroll.md#ZH-CN_TOPIC_0000002497444896__scrolltoindex)等场景时，List组件能够维护其滑动位置准确性。这样，[scrollTo](Scroll.md#ZH-CN_TOPIC_0000002497444896__scrollto)可以准确的跳转到指定位置，[currentOffset](Scroll.md#ZH-CN_TOPIC_0000002497444896__currentoffset)可以获取到当前准确的滑动位置，内置滚动条可以实现平滑移动无跳变。
...
- 如果子组件有ListItemGroup，必须为每一个ListItemGroup设置[childrenMainSize](ListItemGroup.md#ZH-CN_TOPIC_0000002497604872__childrenmainsize12)属性。List组件和每一个ListItemGroup组件都要通过childrenMainSize属性接口一对一绑定一个ChildrenMainSize对象。
...
参数名类型必填说明value[ChildrenMainSize](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__childrenmainsize12对象说明)是该对象用来维护子组件在主轴方向的大小信息。[]()[]()
...
- maintainVisibleContentPosition属性设置为true后，在显示区域上方插入或删除数据，会触发[onDidScroll](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__ondidscroll12)、onScrollIndex事件。
...
- stackFromEnd属性设置为true后，[ListOptions](#ZH-CN_TOPIC_0000002529284863__listoptions18对象说明)中initialIndex参数默认值为总item个数-1。
...
参数名类型必填说明mode[Optional](../misc/自定义属性设置.md#ZH-CN_TOPIC_0000002529284845__optionalt12)<[FocusWrapMode](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__focuswrapmode20)>是
...
设置当前List组件是否处于可编辑模式。可参考[示例3](#ZH-CN_TOPIC_0000002529284863__示例3设置编辑模式)实现删除选中的list项。
...
名称值说明Start0ListItem在List中，交叉轴方向首部对齐。Center1ListItem在List中，交叉轴方向居中对齐。End2ListItem在List中，交叉轴方向尾部对齐。[]()[]()
...
名称值说明NORMAL0默认列表限位动画速度，通常用于列表项尺寸较大，划一下滚动一个列表项场景。SLOW1列表限位动画速度较慢，通常用于列表项尺寸较小，划一下滚动多个列表项场景。[]()[]()

#### CloseSwipeActionOptions11+对象说明

收起[EXPANDED](ListItem.md#ZH-CN_TOPIC_0000002529444837__swipeactionstate11枚举说明)状态[ListItem](ListItem.md)回调事件集合，用于设置收起动画完成后回调事件。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

名称类型只读可选说明onFinish()=>void否是在收起动画完成后触发。[]()[]()
...
名称类型只读可选说明strokeWidth7+[Length](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length)否否
...
color7+[ResourceColor](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcecolor)否是
...
startMargin7+[Length](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length)否是
...
endMargin7+[Length](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length)否是
...
除支持[通用事件](通用事件 (ts-component-general-events).md)和[滚动组件通用事件](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__事件)外，还支持以下事件：
...
参数名类型必填说明startnumber是List显示区域内第一个子组件的索引值endnumber是List显示区域内最后一个子组件的索引值。center10+number是List显示区域内中间位置子组件的索引值。[]()[]()
...
参数名类型必填说明event() => void是列表到达起始位置时触发的回调。[]()[]()
...
参数名类型必填说明event() => void是列表到达末尾位置时触发的回调。[]()[]()
...
1. List惯性滚动。
1. 调用[fling](Scroll.md#ZH-CN_TOPIC_0000002497444896__fling12)接口触发滚动。

不触发该事件的条件：

1. 调用除[fling](Scroll.md#ZH-CN_TOPIC_0000002497444896__fling12)接口外的其他滚动控制接口。
1. 越界回弹。
1. 拖动滚动条。
...
参数名类型必填说明event[OnScrollFrameBeginCallback](Scroll.md#ZH-CN_TOPIC_0000002497444896__onscrollframebegincallback18)是每帧滚动开始回调函数。[]()[]()

#### onScrollStart9+

onScrollStart(event: () => void)

列表滑动开始时触发。手指拖动列表或列表的滚动条触发的滑动开始时，会触发该事件。使用[Scroller](Scroll.md#ZH-CN_TOPIC_0000002497444896__scroller)滑动控制器触发的带动画的滑动，动画开始时会触发该事件。
...
参数名类型必填说明event() => void是列表滑动开始时触发的回调。[]()[]()

#### onScrollStop

onScrollStop(event: () => void)

列表滑动停止时触发。手拖动列表或列表的滚动条触发的滑动，手离开屏幕后滑动停止时会触发该事件。使用[Scroller](Scroll.md#ZH-CN_TOPIC_0000002497444896__scroller)滑动控制器触发的带动画的滑动，动画停止会触发该事件。
...
参数名类型必填说明event() => void是列表滑动停止时触发的回调。[]()[]()
...
参数名类型必填说明fromnumber是移动前索引值。tonumber是移动后索引值。

**返回值：**

类型说明boolean是否已经移动。返回值为true时列表元素发生移动，返回值为false时列表元素没有移动。[]()[]()
...
不支持拖动到List边缘时触发List的自动滚动，可以使用ForEach、LazyForEach、Repeat的[onMove](../misc/拖拽排序.md#ZH-CN_TOPIC_0000002497444864__onmove)接口实现该效果，参考[示例12（使用OnMove进行拖拽）](#ZH-CN_TOPIC_0000002529284863__示例12使用onmove进行拖拽)。但需注意[onMove](../misc/拖拽排序.md#ZH-CN_TOPIC_0000002497444864__onmove)接口不支持跨ListItemGroup拖拽。

从API version 14开始，该接口支持在[attributeModifier](../misc/动态属性设置.md#ZH-CN_TOPIC_0000002529284843__attributemodifier)中调用。
...
参数名类型必填说明event[ItemDragInfo](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__itemdraginfo对象说明)是拖拽点的信息。itemIndexnumber是被拖拽列表元素索引值。[]()[]()
...
参数名类型必填说明event[ItemDragInfo](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__itemdraginfo对象说明)是拖拽点的信息。[]()[]()
...
参数名类型必填说明event[ItemDragInfo](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__itemdraginfo对象说明)是拖拽点的信息。itemIndexnumber是拖拽起始位置。insertIndexnumber是拖拽插入位置。[]()[]()
...
参数名类型必填说明event[ItemDragInfo](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__itemdraginfo对象说明)是拖拽点的信息。itemIndexnumber是拖拽离开的列表元素索引值。[]()[]()
...
参数名类型必填说明event[ItemDragInfo](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__itemdraginfo对象说明)是拖拽点的信息。itemIndexnumber是拖拽起始位置。insertIndexnumber是拖拽插入位置。isSuccessboolean是是否成功释放。返回值为true时列表元素成功释放，返回值为false时列表元素没有成功释放。[]()[]()
...
参数名类型必填说明handler[OnScrollVisibleContentChangeCallback](#ZH-CN_TOPIC_0000002529284863__onscrollvisiblecontentchangecallback12)是当前显示内容发生改变的时候触发回调。[]()[]()
...
参数名类型必填说明indexnumber是被删除的列表项的索引值。
...
onScroll(event: (scrollOffset: number, scrollState: [ScrollState](#ZH-CN_TOPIC_0000002529284863__scrollstate枚举说明)) => void)

列表滑动时触发。

从API version 7开始支持，从API version 12开始废弃，建议使用[onDidScroll](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__ondidscroll12)替代。
...
名称值说明Idle0空闲状态。滚动状态回归空闲时触发，控制器提供的无动画方法控制滚动时触发。Scroll1滚动状态。手指拖动List，拖动滚动条和滚动鼠标滚轮时触发。Fling2
...
ListScroller继承自[Scroller](Scroll.md#ZH-CN_TOPIC_0000002497444896__scroller)，具有[Scroller](Scroll.md#ZH-CN_TOPIC_0000002497444896__scroller)的全部方法。
...
参数名类型必填说明indexnumber是ListItemGroup在List中的索引值。indexInGroupnumber是ListItem在ListItemGroup中的索引值。
...
类型说明[VisibleListContentInfo](#ZH-CN_TOPIC_0000002529284863__visiblelistcontentinfo12对象说明)入参坐标处的子组件的索引信息。
...
align[ScrollAlign](Scroll.md#ZH-CN_TOPIC_0000002497444896__scrollalign10枚举说明)否
...
将[EXPANDED](ListItem.md#ZH-CN_TOPIC_0000002529444837__swipeactionstate11枚举说明)状态的[ListItem](ListItem.md)收起，并设置回调事件。
...
参数名类型必填说明options[CloseSwipeActionOptions](#ZH-CN_TOPIC_0000002529284863__closeswipeactionoptions11对象说明)否收起[EXPANDED](ListItem.md#ZH-CN_TOPIC_0000002529444837__swipeactionstate11枚举说明)状态的[ListItem](ListItem.md)的回调事件集合。
...
参数名类型必填说明start[VisibleListContentInfo](#ZH-CN_TOPIC_0000002529284863__visiblelistcontentinfo12对象说明)是
...
end[VisibleListContentInfo](#ZH-CN_TOPIC_0000002529284863__visiblelistcontentinfo12对象说明)是
...
名称类型只读可选说明indexnumber否否表示ListItem或ListItemGroup在List中的索引值。itemGroupArea[ListItemGroupArea](#ZH-CN_TOPIC_0000002529284863__listitemgrouparea12枚举说明)否是表示处于ListItemGroup的哪一个区域。itemIndexInGroupnumber否是表示ListItem在ListItemGroup中的索引值。[]()[]()
...
名称值说明NONE0ListItemGroup内部ListItem区域、header区域以及footer区域以外的区域。IN_LIST_ITEM_AREA1ListItemGroup内部ListItem区域。IN_HEADER_AREA2ListItemGroup内部header区域。IN_FOOTER_AREA3ListItemGroup内部footer区域。[]()[]()

#### UIListEvent19+

frameNode中[getEvent('List')](FrameNode.md#ZH-CN_TOPIC_0000002529284787__geteventlist19)方法的返回值，可用于给List节点设置滚动事件。

UIListEvent继承于[UIScrollableCommonEvent](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__uiscrollablecommonevent19)。
...
设置[onWillScroll](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__onwillscroll12)事件的回调。
...
参数名类型必填说明callback[OnWillScrollCallback](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__onwillscrollcallback12) | undefined是onWillScroll事件的回调函数。[]()[]()

#### setOnDidScroll19+

setOnDidScroll(callback: OnScrollCallback | undefined): void

设置[onDidScroll](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__ondidscroll12)事件的回调。
...
参数名类型必填说明callback[OnScrollCallback](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__onscrollcallback12) | undefined是onDidScroll事件的回调函数。[]()[]()

#### setOnScrollIndex19+

setOnScrollIndex(callback: OnListScrollIndexCallback | undefined): void

设置[onScrollIndex](#ZH-CN_TOPIC_0000002529284863__onscrollindex)事件的回调。
...
参数名类型必填说明callback[OnListScrollIndexCallback](#ZH-CN_TOPIC_0000002529284863__onlistscrollindexcallback19) | undefined是onScrollIndex事件的回调函数。[]()[]()
...
设置[onScrollVisibleContentChange](#ZH-CN_TOPIC_0000002529284863__onscrollvisiblecontentchange12)事件的回调。
...
参数名类型必填说明callback[OnScrollVisibleContentChangeCallback](list.md#ZH-CN_TOPIC_0000002529284863__onscrollvisiblecontentchangecallback12) | undefined是onScrollVisibleContentChange事件的回调函数。[]()[]()
...
参数名类型必填说明startnumber是List显示区域内第一个子组件的索引值。endnumber是List显示区域内最后一个子组件的索引值。centernumber是List显示区域内中间位置子组件的索引值。[]()[]()
...
ListDataSource实现了LazyForEach数据源接口[IDataSource](../misc/LazyForEach.md#ZH-CN_TOPIC_0000002529444937__idatasource)，用于通过LazyForEach给List提供子组件。
...
ListDataSource说明及完整代码参考[示例1添加滚动事件](#ZH-CN_TOPIC_0000002529284863__示例1添加滚动事件)。
...
该示例通过设置[childrenMainSize](#ZH-CN_TOPIC_0000002529284863__childrenmainsize12)属性，实现了List在子组件高度不一致时调用scrollTo接口也可以跳转准确。

如果配合状态管理V2使用，详情见：[List与makeObserved](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-v1-v2-migration-application-and-others#滚动组件)。
...
从API version 20开始，该示例通过[focusWrapMode](#ZH-CN_TOPIC_0000002529284863__focuswrapmode20)接口，实现了List组件方向键走焦换行效果。
...
从API version 20开始，该示例展示了通过[scrollBarMargin](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__scrollbarmargin20)属性设置滚动条边距并避让[contentStartOffset](#ZH-CN_TOPIC_0000002529284863__contentstartoffset11)、[contentEndOffset](#ZH-CN_TOPIC_0000002529284863__contentendoffset11)区域的效果。
...
从API version 12开始，该示例展示了使用ForEach的[onMove](../misc/拖拽排序.md#ZH-CN_TOPIC_0000002497444864__onmove)接口进行拖拽排序的效果，支持拖动到List边缘时触发List的自动滚动。
