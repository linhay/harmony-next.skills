...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| offset | number | 是 | 设置渐变点距离起点的位置占总体长度的比例，范围为[0, 1]。 设置offset<0或offset>1无渐变效果。 异常值undefined和null按无效值处理，NaN和Infinity会导致CanvasGradient异常。 |
...
```
 ...
 ![image](public_sys-resources/zh-cn_image_0000002522245190.webp)

**addColorStop20+**

addColorStop(offset: number, color: string | ColorMetrics): void

设置渐变断点值，包括偏移和颜色。支持设置rgb或者argb格式颜色。支持通过传入[ColorMetrics](Graphics.md#ZH-CN_TOPIC_0000002553200709__colormetrics12)类型设置P3色域颜色值，可在支持高色域的设备上获得更丰富的色彩表现。
...
| --- | --- | --- | --- |
| offset | number | 是 | 设置渐变点距离起点的位置占总体长度的比例，范围为[0, 1]。 设置offset<0或offset>1无渐变效果。 异常值undefined和null按无效值处理，NaN和Infinity会导致CanvasGradient异常。 |
| color | string | ColorMetrics | 是 | 设置渐变填充的颜色。 可以使用colorWithSpace方法构造指定色域属性ColorSpace为SRGB或DISPLAY_P3的颜色。每个渐变ColorMetrics的色域属性应当统一，设置不同色域的属性时将抛出异常，错误码：103701。 设置null和undefined无效。 |
...
仅[CanvasRenderingContext2D](CanvasRenderingContext2D.md)对象的[fillStyle](CanvasRenderingContext2D.md#ZH-CN_TOPIC_0000002553360789__fillstyle)和[strokeStyle](CanvasRenderingContext2D.md#ZH-CN_TOPIC_0000002553360789__strokestyle)属性支持设置P3广色域的CanvasGradient对象，且需要将Canvas组件所在窗口的色域模式通过[setWindowColorSpace](Interface (Window).md#ZH-CN_TOPIC_0000002522080752__setwindowcolorspace9)方法设置为广色域模式WIDE_GAMUT。

示例：

通过addColorStop设置指定色域的渐变断点值，包括偏移和颜色。设置窗口色域模式为广色域参照方法[setWindowColorSpace](Interface (Window).md#ZH-CN_TOPIC_0000002522080752__setwindowcolorspace9)。
...
![image](public_sys-resources/zh-cn_image_0000002553205153.webp)