...
DrawModifier可设置遮罩层前景（drawOverlay）、前景（drawForeground）、内容前景（drawFront）、内容（drawContent）和内容背景（drawBehind）的绘制方法，还提供主动触发重绘的方法[invalidate](#ZH-CN_TOPIC_0000002522080802__invalidate)。每个DrawModifier实例只能设置到一个组件上，禁止进行重复设置。

自定义层级示例图

![image](public_sys-resources/zh-cn_image_0000002553364761.webp)
...
请参考[示例1（通过DrawModifier进行自定义绘制）](#ZH-CN_TOPIC_0000002522080802__示例1通过drawmodifier进行自定义绘制)。
...
该接口的[DrawContext](Graphics.md#ZH-CN_TOPIC_0000002553200709__drawcontext)中的Canvas是用于记录指令的临时Canvas，并非节点的真实Canvas。使用请参见[调整自定义绘制Canvas的变换矩阵](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-user-defined-extension-drawmodifier#调整自定义绘制canvas的变换矩阵)。
...
请参考[示例2（通过DrawModifier对容器的前景进行自定义绘制）](#ZH-CN_TOPIC_0000002522080802__示例2通过drawmodifier对容器的前景进行自定义绘制)。
...
![image](public_sys-resources/zh-cn_image_0000002522244840.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553204801.webp)