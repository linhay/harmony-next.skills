...
用于设置组件绑定的手势。可以通过[UIGestureEvent](设置组件绑定的手势.md#ZH-CN_TOPIC_0000002553200771__uigestureevent)对象调用其接口添加或删除手势。
...
点击手势处理器配置参数。继承自[BaseHandlerOptions](#ZH-CN_TOPIC_0000002522080808__basehandleroptions15)。
...
设置长按手势处理器取消回调。长按手势处理器识别成功后，接收到触摸取消事件时触发回调。与[onActionCancel](#ZH-CN_TOPIC_0000002522080808__onactioncancel)接口相比，此接口返回手势事件信息。
...
长按手势处理器配置参数。继承自[BaseHandlerOptions](#ZH-CN_TOPIC_0000002522080808__basehandleroptions15)。
...
| isFingerCountLimited15+ | boolean | 否 | 是 | 是否检查触摸屏幕的手指数量。true表示检查触摸屏幕的手指数量，false表示不检查触摸屏幕的手指数量。若触摸屏幕的手指的数量不等于设置的触发长按的最少手指数（即上述fingers参数），手势识别将失败。 对于已成功识别的手势，后续触摸屏幕的手指数变化，将不触发repeat事件（若触摸屏幕的手指数恢复到设置的触发长按的最少手指数时，可以触发onAction事件），但可以触发onActionEnd事件。 默认值：false 元服务API： 从API version 15开始，该接口支持在元服务中使用。 |
...
设置滑动手势处理器取消回调。滑动手势处理器识别成功后，接收到触摸取消事件时触发回调。与[onActionCancel](#ZH-CN_TOPIC_0000002522080808__onactioncancel-1)接口相比，此接口返回手势事件信息。
...
滑动手势处理器配置参数。继承自[BaseHandlerOptions](#ZH-CN_TOPIC_0000002522080808__basehandleroptions15)。
...
| distance | number | 否 | 是 | 用于指定触发滑动手势事件的最小拖动距离，单位为vp。 手写笔默认值：8，其余输入源默认值：5 说明： Tabs组件滑动与该滑动手势事件同时存在时，可将distance值设为1，使拖动更灵敏，避免造成事件错乱。 取值范围：[0, +∞)，当设定的值小于0时，按默认值处理。 从API version 19开始，手写笔默认值为8，单位为vp。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
| distanceMap19+ | Map<SourceTool, number> | 否 | 是 | 用于指定不同输入源触发滑动手势事件的最小拖动距离，单位为vp。 手写笔默认值：8，其余输入源默认值：5 取值范围：[0, +∞)，当设定的值小于0时，按默认值处理。 元服务API： 从API version 19开始，该接口支持在元服务中使用。 |
...
快滑手势处理器配置参数。继承自[BaseHandlerOptions](#ZH-CN_TOPIC_0000002522080808__basehandleroptions15)。
...
设置捏合手势处理器取消回调。捏合手势处理器识别成功后，接收到触摸取消事件时触发回调。与[onActionCancel](#ZH-CN_TOPIC_0000002522080808__onactioncancel-2)接口相比，此接口返回手势事件信息。
...
捏合手势处理器配置参数。继承自[BaseHandlerOptions](#ZH-CN_TOPIC_0000002522080808__basehandleroptions15)。
...
设置旋转手势处理器取消回调。旋转手势处理器识别成功后，接收到触摸取消事件时触发回调。与[onActionCancel](#ZH-CN_TOPIC_0000002522080808__onactioncancel-3)相比，此接口返回手势事件信息。
...
旋转手势处理器配置参数。继承自[BaseHandlerOptions](#ZH-CN_TOPIC_0000002522080808__basehandleroptions15)。
...
设置手势组处理器取消回调。顺序组合手势（[GestureMode](组合手势.md#ZH-CN_TOPIC_0000002522080812__gesturemode枚举说明).Sequence）取消后触发回调。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| mode | GestureMode | 否 | 否 | 设置组合手势识别模式。 默认值：GestureMode.Sequence |
| gestures | GestureHandler<TapGestureHandler | LongPressGestureHandler | PanGestureHandler | SwipeGestureHandler | PinchGestureHandler | RotationGestureHandler | GestureGroupHandler>[] | 否 | 否 | 手势组下需要包含的手势。 说明：  当需要为一个组件同时添加单击和双击手势时，可在组合手势中添加两个TapGesture，需要双击手势在前，单击手势在后，否则不生效。 |
