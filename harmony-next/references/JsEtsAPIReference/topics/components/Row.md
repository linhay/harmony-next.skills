...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| space | SpaceType | 否 | 是 | 横向布局元素间距。 space为负数或者justifyContent设置为FlexAlign.SpaceBetween、FlexAlign.SpaceAround、FlexAlign.SpaceEvenly时不生效。 默认值：0  单位：vp  非法值：按默认值处理。 说明：  space取值是大于等于0的数字，或者可以转换为数字的字符串，或者可以转换为数字的Resource类型数据。 |
...
| value | FlexAlign | 是 | 子组件在水平方向上的对齐格式。 默认值：FlexAlign.Start |
 ...
 Row布局时若子组件不设置[flexShrink](Flex布局.md#ZH-CN_TOPIC_0000002522080780__flexshrink)则默认不会压缩子组件，即所有子组件主轴大小累加可超过容器主轴，此时FlexAlign.Center和FlexAlign.End会失效。
...
![image](public_sys-resources/zh-cn_image_0000002553364779.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522244858.webp)