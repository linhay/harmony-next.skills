...
设置组件背景。从API version 20开始，content参数新增了对[ResourceColor](基础类型定义.md#ZH-CN_TOPIC_0000002553360849__resourcecolor)类型的支持，并新增了背景向父组件的安全区扩展的能力。

   ![image](public_sys-resources/note_3.0-zh-cn.webp)

-

不支持[onAppear](挂载卸载事件.md#ZH-CN_TOPIC_0000002522240770__onappear)和[onDisAppear](挂载卸载事件.md#ZH-CN_TOPIC_0000002522240770__ondisappear)等和节点挂载/卸载相关的事件。

-

从API version 20开始，该接口仅当content的入参类型为ResourceColor时支持在[attributeModifier](动态属性设置.md#ZH-CN_TOPIC_0000002522240800__attributemodifier)中调用。
...
设置组件背景色。与[backgroundColor](#ZH-CN_TOPIC_0000002553200739__backgroundcolor)相比，color参数新增了对undefined类型的支持。
...
| T | 返回当前组件。 |
 ...
 当通过[backgroundBlurStyle](#ZH-CN_TOPIC_0000002553200739__backgroundblurstyle9)中的inactiveColor指定背景色时，不建议再通过backgroundColor设置背景色。

**backgroundColor20+**

backgroundColor(color: Optional<ResourceColor | ColorMetrics>): T

设置组件背景色。与[backgroundColor](#ZH-CN_TOPIC_0000002553200739__backgroundcolor18)相比，color参数新增了对[ColorMetrics](Graphics.md#ZH-CN_TOPIC_0000002553200709__colormetrics12)类型的支持。
...
设置组件的背景图片。与[backgroundImage](#ZH-CN_TOPIC_0000002553200739__backgroundimage)相比，增加了设置图片同步或异步加载方式的能力。
...
设置组件背景图片的宽度和高度。当未设置backgroundImageSize时，默认组件背景图片宽高效果为[ImageSize.Auto](枚举说明.md#ZH-CN_TOPIC_0000002553200889__imagesize)。
...
为当前组件提供一种背景材质模糊能力，通过枚举值的方式封装了不同的模糊半径、蒙版颜色、蒙版透明度、饱和度、亮度。与[backgroundBlurStyle9+](#ZH-CN_TOPIC_0000002553200739__backgroundblurstyle9)相比，style参数新增了对undefined类型的支持。
...
| T | 返回当前组件。 |
 ...
 当通过backgroundBlurStyle中的inactiveColor指定背景色时，不建议再通过[backgroundColor](#ZH-CN_TOPIC_0000002553200739__backgroundcolor)设置背景色。
...
为当前组件提供一种背景材质模糊能力，通过枚举值的方式封装了不同的模糊半径、蒙版颜色、蒙版透明度、饱和度、亮度。与[backgroundBlurStyle18+](#ZH-CN_TOPIC_0000002553200739__backgroundblurstyle18)相比，新增了sysOptions参数，即支持系统自适应调节参数。
...
为组件添加背景模糊效果，支持自定义设置模糊半径和灰阶参数。与[backdropBlur](#ZH-CN_TOPIC_0000002553200739__backdropblur)相比，radius参数新增了对undefined类型的支持。
...
| T | 返回当前组件。 |
 ...
 blur和backdropBlur是实时模糊接口，会每帧进行实时渲染，性能负载较高。当模糊内容和模糊半径都不需要变化时，建议使用静态模糊接口[blur](@ohos.effectKit (图像效果).md#ZH-CN_TOPIC_0000002553362045__blur)。

**backdropBlur19+**

backdropBlur(radius: Optional<number>, options?: BlurOptions, sysOptions?: SystemAdaptiveOptions): T

为组件添加背景模糊效果，支持自定义设置模糊半径和灰阶参数。与[backdropBlur18+](#ZH-CN_TOPIC_0000002553200739__backdropblur18)相比，新增了sysOptions参数，即支持系统自适应调节参数。
...
| T | 返回当前组件。 |
 ...
 backgroundBlurStyle、blur和backdropBlur为实时接口，每帧执行实时渲染，性能负载较大。当模糊内容与模糊半径均无需变动时，推荐采用静态模糊接口[blur](@ohos.effectKit (图像效果).md#ZH-CN_TOPIC_0000002553362045__blur)。最佳实践请参考[图像模糊动效优化-使用场景](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-fuzzy-scene-performance-optimization#section4945532519)。
...
设置组件背景属性，包括背景模糊半径、亮度、饱和度和颜色等参数。与[backgroundEffect11+](#ZH-CN_TOPIC_0000002553200739__backgroundeffect11)相比，options参数新增了对undefined类型的支持。
...
设置组件背景属性，包括背景模糊半径、亮度、饱和度和颜色等参数。与[backgroundEffect18+](#ZH-CN_TOPIC_0000002553200739__backgroundeffect18)相比，新增了sysOptions参数，即支持系统自适应调节参数。

   ![image](public_sys-resources/note_3.0-zh-cn.webp)

backgroundEffect接口为实时接口，每帧对模糊等效果执行实时渲染，性能负载较大。当组件背景模糊效果无需变动时，推荐采用静态模糊接口[blur](@ohos.effectKit (图像效果).md#ZH-CN_TOPIC_0000002553362045__blur)实现模糊效果。最佳实践请参考：[图像模糊动效优化-使用场景](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-fuzzy-scene-performance-optimization#section4945532519)。
...
设置合法的ResizableOptions时，[backgroundImage](#ZH-CN_TOPIC_0000002553200739__backgroundimage)属性中的repeat参数设置不生效。
...
继承自[BlurStyleOptions](组件内容模糊.md#ZH-CN_TOPIC_0000002522080788__blurstyleoptions)。
...
设置组件背景提亮效果。与[backgroundBrightness12+](#ZH-CN_TOPIC_0000002553200739__backgroundbrightness12)相比，options参数新增了对undefined类型的支持。
...
![image](public_sys-resources/zh-cn_image_0000002522244740.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553204701.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522084748.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553364661.webp)

修改rate和lightUpDegree参数值为0.5,-0.1：

![image](public_sys-resources/zh-cn_image_0000002522244742.webp)

去掉backgroundBrightness的设置，效果如下：

![image](public_sys-resources/zh-cn_image_0000002553204703.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522084750.webp)

**示例6（设置文字异形模糊效果）**

该示例通过[blendMode](图像效果.md#ZH-CN_TOPIC_0000002553360709__blendmode11)和backgroundEffect实现文字异形模糊效果。

如果出现漏线问题，开发者应首先确保两个blendMode所在组件大小严格相同。如果确认相同，可能是组件边界落在浮点数坐标上导致，可尝试设置[pixelRound](组件级像素取整.md#ZH-CN_TOPIC_0000002522240780__pixelround)通用属性，使产生的白线、暗线两侧的组件边界对齐到整数像素坐标上。
...
![image](public_sys-resources/zh-cn_image_0000002553364663.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522244744.webp)

**示例8（设置P3色域背景效果）**

从API version 20开始，该示例通过[backgroundColor](#ZH-CN_TOPIC_0000002553200739__backgroundcolor20)设置P3色域背景效果。
...
![image](public_sys-resources/zh-cn_image_0000002553204705.webp)

**示例9（设置组件背景扩展）**

从API version 20开始，该示例通过[background](#ZH-CN_TOPIC_0000002553200739__background10)实现组件背景扩展到父组件的安全区。
...
![image](public_sys-resources/zh-cn_image_0000002522084752.webp)