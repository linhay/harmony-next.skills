...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | AccessibilityHoverType | 否 | 否 | 无障碍悬浮动作。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AccessibilityTransparentCallback | 是 | 提供开启无障碍模式后未能响应的用户输入的触摸事件，当开启无障碍模式后，单指触摸未能响应无障碍悬浮事件位置时触发该回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | TouchEvent | 是 | 原始touch事件。  说明： TouchEvent对象的触摸事件的类型TouchType为四种无障碍悬浮事件类型中的一种，四种无障碍悬浮事件类型为HOVER_ENTER、HOVER_MOVE、HOVER_EXIT和HOVER_CANCEL。 |
...
从API version 20开始，新增了[onAccessibilityHoverTransparent](#ZH-CN_TOPIC_0000002522080770__onaccessibilityhovertransparent20)入参类型为AccessibilityTransparentCallback的接口。
