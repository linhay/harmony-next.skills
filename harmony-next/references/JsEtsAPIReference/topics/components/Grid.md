...
仅支持[GridItem](GridItem.md)子组件和自定义组件。自定义组件在Grid下使用时，建议使用GridItem作为自定组件的顶层组件，不建议给自定义组件设置属性和事件方法。
...
参数名类型必填说明scroller[Scroller](Scroll.md#ZH-CN_TOPIC_0000002497444896__scroller)否
...
layoutOptions10+[GridLayoutOptions](#ZH-CN_TOPIC_0000002529444839__gridlayoutoptions10对象说明)否Grid布局选项。

#### GridLayoutOptions10+对象说明

Grid布局选项。其中，irregularIndexes和onGetIrregularSizeByIndex可对仅设置rowsTemplate或columnsTemplate的Grid使用，可以指定一个index数组，并为其中的index对应的GridItem设置其占据的行数与列数，使用方法参见[示例3](#ZH-CN_TOPIC_0000002529444839__示例3可滚动grid设置跨行跨列节点)；onGetRectByIndex可对同时设置rowsTemplate和columnsTemplate的Grid使用，为指定的index对应的GridItem设置位置和大小，使用方法参见[示例1](#ZH-CN_TOPIC_0000002529444839__示例1固定行列grid)。
...
onGetRectByIndex11+(index: number) => [number, number,number,number]否是
...
Grid组件使用通用属性[clip12+](../misc/形状裁剪.md#ZH-CN_TOPIC_0000002529284827__clip12)和通用属性[clip18+](../misc/形状裁剪.md#ZH-CN_TOPIC_0000002529284827__clip18)时默认值都为true。
...
使用效果可以参考[示例8](#ZH-CN_TOPIC_0000002529444839__示例8设置自适应列数)。
...
当value设置为string类型时，使用方法参考[columnsTemplate(value: string)](#ZH-CN_TOPIC_0000002529444839__columnstemplate)。
...
参数名类型必填说明valuestring | [ItemFillPolicy](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__itemfillpolicy22)是当前网格组件布局列的数量。
...
参数名类型必填说明valuestring是当前网格布局行的数量或最小行高值。
...
- 此模式下GridItem交叉轴方向尺寸与网格一致，可以通过设置[constraintSize](../misc/尺寸设置.md#ZH-CN_TOPIC_0000002497444850__constraintsize)中的maxWidth或maxHeight限制GridItem交叉轴方向尺寸小于网格。
...
参数名类型必填说明value[Color](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__color) | number | string是
...
设置滚动条的颜色。与[scrollBarColor](#ZH-CN_TOPIC_0000002529444839__scrollbarcolor)相比， 参数名改为color，并开始支持Resource类型。
...
设置缓存后会在Grid显示区域上下各缓存cachedCount*列数个GridItem。配合[裁剪](../misc/形状裁剪.md#ZH-CN_TOPIC_0000002529284827__clip12)或[内容裁剪](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__clipcontent14)属性可以显示出预加载节点。
...
参数名类型必填说明value[GridDirection](#ZH-CN_TOPIC_0000002529444839__griddirection8枚举说明)是
...
设置是否开启鼠标框选。开启框选后，可以配合GridItem的selected属性和onSelect事件获取GridItem的选中状态，还可以设置[选中态样式](../misc/多态样式.md)（无默认选中样式）。
...
supportAnimation动画效果参考[示例5（Grid拖拽场景）](#ZH-CN_TOPIC_0000002529444839__示例5grid拖拽场景)，其他动画效果需要应用自定义拖拽实现。
...
设置嵌套滚动选项。设置前后两个方向的嵌套滚动模式，实现与父组件的滚动联动。当组件内容大小小于组件自身，且[edgeEffect](#ZH-CN_TOPIC_0000002529444839__edgeeffect10)的options为{ alwaysEnabled: false }时，组件自身滑动手势不会触发，嵌套滚动属性不会生效，如果其父滚动组件有滑动手势，则会触发父组件的滑动手势。
...
参数名类型必填说明value[NestedScrollOptions](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__nestedscrolloptions10对象说明)是嵌套滚动选项。
...
设置Grid中GridItem的对齐方式， 使用方法可以参考[示例9](#ZH-CN_TOPIC_0000002529444839__示例9以当前行最高的griditem的高度为其他griditem的高度)。
...
参数名类型必填说明alignment[Optional](../misc/自定义属性设置.md#ZH-CN_TOPIC_0000002529284845__optionalt12)<[GridItemAlignment](#ZH-CN_TOPIC_0000002529444839__griditemalignment12枚举说明)>是
...
名称值说明DEFAULT0使用Grid的默认对齐方式。STRETCH1以一行中的最高的GridItem作为其他GridItem的高度。
...
名称值说明Row0主轴布局方向沿水平方向布局，即自左往右先填满一行，再去填下一行。Column1主轴布局方向沿垂直方向布局，即自上往下先填满一列，再去填下一列。RowReverse2主轴布局方向沿水平方向反向布局，即自右往左先填满一行，再去填下一行。ColumnReverse3主轴布局方向沿垂直方向反向布局，即自下往上先填满一列，再去填下一列。
...
参数名类型必填说明firstnumber是当前显示的网格起始位置的索引值。last10+number是当前显示的网格终止位置的索引值。
...
由于拖拽检测也需要长按，且事件处理机制优先触发子组件事件，GridItem上绑定[LongPressGesture](../input/LongPressGesture.md#ZH-CN_TOPIC_0000002529444825__longpressgesture-1)时无法触发拖拽。如有长按和拖拽同时使用的需求可以使用通用拖拽事件。

拖拽浮起的网格元素可在应用窗口内移动，若需限制移动范围，可通过自定义手势实现，具体参考[示例16（实现GridItem自定义拖拽）](#ZH-CN_TOPIC_0000002529444839__示例16实现griditem自定义拖拽)。

不支持拖动到Grid边缘时自动滚动，可使用通用拖拽实现，具体参考[示例17（通过拖拽事件实现griditem拖拽）](#ZH-CN_TOPIC_0000002529444839__示例17通过拖拽事件实现griditem拖拽)。
...
参数名类型必填说明event[ItemDragInfo](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__itemdraginfo对象说明)是拖拽点的信息。itemIndexnumber是被拖拽网格元素索引值。
...
参数名类型必填说明event[ItemDragInfo](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__itemdraginfo对象说明)是拖拽点的信息。itemIndexnumber是拖拽离开的网格元素索引值。
...
参数名类型必填说明event[ItemDragInfo](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__itemdraginfo对象说明)是拖拽点的信息。itemIndexnumber是拖拽起始位置。insertIndexnumber是拖拽插入位置。isSuccessboolean是
...
参数名类型必填说明indexnumber是当前显示的网格起始位置的索引值。offsetnumber是当前显示的网格起始位置元素相对网格显示起始位置的偏移，单位vp。

**返回值：**

类型说明[ComputedBarAttribute](#ZH-CN_TOPIC_0000002529444839__computedbarattribute10对象说明)滚动条的位置及长度。
...
参数名类型必填说明event() => void是网格到达起始位置时触发的回调。
...
参数名类型必填说明event() => void是网格到达末尾位置时触发的回调。
...
网格滑动开始时触发。手指拖动网格或网格的滚动条触发的滑动开始时，会触发该事件。使用[Scroller](Scroll.md#ZH-CN_TOPIC_0000002497444896__scroller)滑动控制器触发的带动画的滑动，动画开始时会触发该事件。
...
参数名类型必填说明event() => void是网格滑动开始时触发的回调。

#### onScrollStop10+

onScrollStop(event: () => void)

网格滑动停止时触发。手指拖动网格或网格的滚动条触发的滑动，手指离开屏幕后滑动停止时会触发该事件。使用[Scroller](Scroll.md#ZH-CN_TOPIC_0000002497444896__scroller)滑动控制器触发的带动画的滑动，动画停止会触发该事件。
...
参数名类型必填说明event() => void是网格滑动停止时触发的回调。

#### onScroll(deprecated)

onScroll(event: (scrollOffset: number, scrollState: [ScrollState](list.md#ZH-CN_TOPIC_0000002529284863__scrollstate枚举说明)) => void)

网格滑动时触发。

从API version 10开始支持，从API version 12开始废弃，建议使用[onDidScroll](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__ondidscroll12)替代。
...
名称类型只读可选说明totalOffsetnumber否否Grid内容相对显示区域的总偏移，单位px。totalLengthnumber否否Grid内容总长度，单位px。

#### UIGridEvent19+

frameNode中[getEvent('Grid')](FrameNode.md#ZH-CN_TOPIC_0000002529284787__geteventgrid19)方法的返回值，可用于给Grid节点设置滚动事件。

UIGridEvent继承于[UIScrollableCommonEvent](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__uiscrollablecommonevent19)。
...
设置[onScrollIndex](#ZH-CN_TOPIC_0000002529444839__onscrollindex)事件的回调。
...
参数名类型必填说明callback[OnGridScrollIndexCallback](#ZH-CN_TOPIC_0000002529444839__ongridscrollindexcallback19) | undefined是onScrollIndex事件的回调函数。
...
参数名类型必填说明firstnumber是当前显示的Grid起始位置的索引值。lastnumber是当前显示的Grid终止位置的索引值。

#### 示例

#### 示例1（固定行列Grid）

可以使用[GridLayoutOptions](#ZH-CN_TOPIC_0000002529444839__gridlayoutoptions10对象说明)中的onGetRectByIndex指定GridItem的位置和大小。
...
GridDataSource实现了LazyForEach数据源接口[IDataSource](../misc/LazyForEach.md#ZH-CN_TOPIC_0000002529444937__idatasource)，用于通过LazyForEach给Grid提供子组件。
...
[GridLayoutOptions](#ZH-CN_TOPIC_0000002529444839__gridlayoutoptions10对象说明)的使用：irregularIndexes与onGetIrregularSizeByIndex。

GridDataSource说明及完整代码参考[示例2可滚动grid和滚动事件](#ZH-CN_TOPIC_0000002529444839__示例2可滚动grid和滚动事件)。
...
[nestedScroll](#ZH-CN_TOPIC_0000002529444839__nestedscroll10)和[onScrollFrameBegin](#ZH-CN_TOPIC_0000002529444839__onscrollframebegin10)的使用。
...
1. 通过属性[editMode](#ZH-CN_TOPIC_0000002529444839__editmode8)设置Grid是否进入编辑模式，进入编辑模式可以拖拽Grid组件内部GridItem。
1. 在[onItemDragStart](#ZH-CN_TOPIC_0000002529444839__onitemdragstart8)回调中设置拖拽过程中显示的图片。
1. 在[onItemDrop](#ZH-CN_TOPIC_0000002529444839__onitemdrop8)中获取拖拽起始位置，和拖拽插入位置，并在[onItemDrop](#ZH-CN_TOPIC_0000002529444839__onitemdrop8)中完成交换数组位置逻辑。
...
[layoutDirection](#ZH-CN_TOPIC_0000002529444839__layoutdirection8)、[maxCount](#ZH-CN_TOPIC_0000002529444839__maxcount8)、[minCount](#ZH-CN_TOPIC_0000002529444839__mincount8)、[cellLength](#ZH-CN_TOPIC_0000002529444839__celllength8)的使用。
...
属性[columnsTemplate](#ZH-CN_TOPIC_0000002529444839__columnstemplate)中auto-fill、auto-fit和auto-stretch的使用示例。
...
在默认情况下，左右两个GridItem的高度可能是不同的；在设置了Grid的[alignItems](#ZH-CN_TOPIC_0000002529444839__alignitems12)属性为GridItemAlignment.STRETCH后，一行左右两个GridItem中原本高度较小的GridItem会以另一个高度较大的GridItem的高度作为自己的高度。
...
通过[fadingEdge](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__fadingedge14)属性来设置边缘渐隐效果。
...
该示例通过[edgeEffect](#ZH-CN_TOPIC_0000002529444839__edgeeffect10)接口，实现了Grid组件设置单边边缘效果。
...
从API version 20开始，该示例通过[focusWrapMode](#ZH-CN_TOPIC_0000002529444839__focuswrapmode20)接口，实现了Grid组件方向键走焦换行效果。
...
该示例通过FrameNode中的[getEvent('Grid')](FrameNode.md#ZH-CN_TOPIC_0000002529284787__geteventgrid19)获取[UIGridEvent](#ZH-CN_TOPIC_0000002529444839__uigridevent19)，并为Grid设置滚动事件回调，用于事件监听方因无法直接修改页面代码而无法使用声明式接口设置回调的场景。
...
该示例通过[scrollToIndex](Scroll.md#ZH-CN_TOPIC_0000002497444896__scrolltoindex)接口，实现了Grid组件滚动到指定位置。
...
该示例通过[PanGesture](../input/PanGesture.md#ZH-CN_TOPIC_0000002497604860__pangesture-1)接口，实现了Grid组件一边滑动一边选择的效果。
...
该示例通过[gesture](../misc/绑定手势方法.md#ZH-CN_TOPIC_0000002529284849__gesture)接口，实现了GridItem组件自定义拖拽效果。
...
该示例通过[拖拽事件](../misc/拖拽事件.md)实现拖拽GridItem到Grid边缘时Grid自动滚动的功能。
...
GridDataSource说明及完整代码参考[示例2（可滚动Grid和滚动事件）](#ZH-CN_TOPIC_0000002529444839__示例2可滚动grid和滚动事件)。
