...
组件插入显示和删除隐藏的过渡效果。同[transition](#ZH-CN_TOPIC_0000002522080882__transition)相比，增加了转场动画结束的回调。
...
1. 如果在动画范围([animateTo](Class (UIContext).md#ZH-CN_TOPIC_0000002522240732__animateto)、[animation](属性动画 (animation).md))内触发组件的上下树或可见性([visibility](显隐控制.md#ZH-CN_TOPIC_0000002522240774__visibility))改变，而根组件没有配置transition，会给该组件加上默认透明度转场，即TransitionEffect.OPACITY，动画参数跟随所处动画环境的参数。如不需要可通过主动配置TransitionEffect.IDENTITY来禁用，使该组件直接出现或消失。
...
从API version 7开始支持，从API version 10开始废弃，建议使用[TransitionEffect](#ZH-CN_TOPIC_0000002522080882__transitioneffect10对象说明)替代。
...
1. 当使用TransitionOptions类型的入参指定转场效果时，必须配合[animateTo](Class (UIContext).md#ZH-CN_TOPIC_0000002522240732__animateto)使用才有动画效果，动效时长、曲线、延时跟随animateTo中的配置。
...
该示例主要演示如何通过同一[TransitionEffect](#ZH-CN_TOPIC_0000002522080882__transitioneffect10对象说明)来实现图片的出现与消失，出现和消失互为逆过程。
...
![image](public_sys-resources/zh-cn_image_0000002522085296.webp)

**示例2（使用不同接口实现图片出现消失）**

该示例主要演示使用不同[TransitionEffect](#ZH-CN_TOPIC_0000002522080882__transitioneffect10对象说明)来实现图片的出现和消失。
...
![image](public_sys-resources/zh-cn_image_0000002553365213.webp)

**示例3（设置父子组件为transition）**

该示例主要演示通过父子组件都配置[transition](#ZH-CN_TOPIC_0000002522080882__transition)来实现图片的出现和消失。
...
![image](public_sys-resources/zh-cn_image_0000002522245290.webp)