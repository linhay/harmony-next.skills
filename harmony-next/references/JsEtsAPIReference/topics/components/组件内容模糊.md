...
| options | ForegroundBlurStyleOptions | 否 | 内容模糊选项。默认值请参考ForegroundBlurStyleOptions。 |
...
为当前组件提供内容模糊能力。与[foregroundBlurStyle](#ZH-CN_TOPIC_0000002522080788__foregroundblurstyle)相比，style参数新增了对undefined类型的支持。
...
为当前组件提供内容模糊能力。与[foregroundBlurStyle18+](#ZH-CN_TOPIC_0000002522080788__foregroundblurstyle18)相比，新增了sysOptions参数，即支持系统自适应调节参数。

   ![image](public_sys-resources/note_3.0-zh-cn.webp)

 foregroundBlurStyle接口为实时模糊接口，每帧执行实时渲染，性能负载较大。当模糊内容与模糊半径均无需变动时，推荐采用静态模糊接口[blur](@ohos.effectKit (图像效果).md#ZH-CN_TOPIC_0000002553362045__blur)。最佳实践请参考：[图像模糊动效优化-使用场景](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-fuzzy-scene-performance-optimization#section4945532519)。
...
继承自[BlurStyleOptions](#ZH-CN_TOPIC_0000002522080788__blurstyleoptions)，设置内容模糊选项。
...
| blurOptions11+ | BlurOptions | 否 | 是 | 灰阶模糊参数。 默认值：grayscale: [0,0]  元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
...
![image](public_sys-resources/zh-cn_image_0000002553204747.webp)