...
其他支持拖出能力的组件需要开发者将draggable属性设置为true，并在[onDragStart](拖拽事件.md#ZH-CN_TOPIC_0000002553200729__ondragstart)等接口中实现数据传输相关内容，才能正确处理拖拽。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | Array<UniformDataType> | null12+ | Array<string>23+ | 是 | 设置该组件上允许落入的数据类型。从API version 12开始，允许设置成null使该组件不接受所有的数据类型。从API version 23开始，支持设置自定义数据类型Array<string>，自定义数据类型为应用自行定义的数据类型字符串，字符串无明确格式要求，但不应与UniformDataType标准类型格式重复，建议以易记易区分为原则来定义。 |
...
| config | PreviewConfiguration | 否 | 对自定义拖拽过程中的预览图进行配置。 只对dragPreview中的预览生效。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | DragPreviewOptions11+ | 是 | 设置拖拽过程中预览图处理模式及数量角标的显示。 |
| options12+ | DragInteractionOptions12+ | 否 | 设置拖拽过程中预览图浮起的交互模式。 默认值：空 |
...
| modifier12+ | ImageModifier | 否 | 是 | 用于配置拖拽背板图的样式Modifier对象，可使用图片组件所支持的属性和样式来配置背板图样式(参考示例6)，当前支持透明度，阴影，背景模糊度，圆角。文本拖拽只支持默认效果，不支持通过modifier进行自定义。 1.透明度 通过opacity设置不透明度，不透明度的取值范围为0-1。设置0或不设置时采用背板图透明度的默认值0.95，设置1或异常值时不透明。 2.阴影 通过shadow设置阴影。 3.背景模糊度 通过backgroundEffect或backgroundBlurStyle设置背景模糊度，如果两者同时设置，以后设置的属性为准。 4.圆角 通过border或borderRadius设置圆角，当同时在mode和modifier中设置圆角，mode设置的圆角显示优先级低于modifier设置。 默认值：空，无法修改属性。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
...
| ENABLE_MULTI_TILE_EFFECT18+ | 6 | 启用支持多选对象鼠标拖拽不聚拢效果，各拖拽图显示在其原始位置的相对位置，当满足多选的情况下且isMultiSelectionEnabled为true时该参数才生效。不聚拢效果优先级高于dragPreview。不支持二次拖拽、圆角和缩放设置。 元服务API：从API version 18开始，该接口支持在元服务中使用。 |
...
当一个节点上同时设置长按浮起预览（参考[bindContextMenu](菜单控制.md#ZH-CN_TOPIC_0000002522080798__bindcontextmenu12)）与拖拽时，使用该字段设置长按浮起预览图与拖拽预览图过渡动效方式。
...
示例1通过配置[allowDrop](#ZH-CN_TOPIC_0000002522080792__allowdrop)设置组件是否可落入，通过配置[draggable](#ZH-CN_TOPIC_0000002522080792__draggable)设置组件是否可拖拽。
...
![image](public_sys-resources/zh-cn_image_0000002522084806.webp)

**示例2（设置预览图）**

示例2通过配置[dragPreview](#ZH-CN_TOPIC_0000002522080792__dragpreview11)设置拖拽过程的预览图。
...
![image](public_sys-resources/zh-cn_image_0000002553364719.webp)

**示例3（设置背板图样式）**

示例3通过配置[dragPreviewOptions](#ZH-CN_TOPIC_0000002522080792__dragpreviewoptions11)为ENABLE_DEFAULT_SHADOW、ENABLE_DEFAULT_RADIUS设置默认阴影和统一圆角效果。从API version 18开始，通过配置[dragPreviewOptions](#ZH-CN_TOPIC_0000002522080792__dragpreviewoptions11)为ENABLE_DRAG_ITEM_GRAY_EFFECT设置灰显效果。
...
![image](public_sys-resources/zh-cn_image_0000002522244800.webp)

**示例4（设置多选拖拽）**

示例4通过配置[isMultiSelectionEnabled](#ZH-CN_TOPIC_0000002522080792__draginteractionoptions12)实现Grid组件的多选拖拽效果。
...
![image](public_sys-resources/zh-cn_image_0000002553204761.webp)

**示例5（设置默认点按效果）**

示例5通过配置[defaultAnimationBeforeLifting](#ZH-CN_TOPIC_0000002522080792__draginteractionoptions12)实现Grid组件的默认点按效果。
...
![image](public_sys-resources/zh-cn_image_0000002522084808.webp)

**示例6（自定义背板图样式）**

示例6通过配置[ImageModifier](#ZH-CN_TOPIC_0000002522080792__imagemodifier12)实现Image组件的自定义背板图样式。
...
![image](public_sys-resources/zh-cn_image_0000002553364721.webp)

**示例7（图片拖拽设置）**

示例7展示了不同图片（在线图片资源、本地图片资源和PixelMap）在拖拽时组件的设置。

使用网络图片时，需要申请权限ohos.permission.INTERNET。具体申请方式请参考[声明权限](../../guides/声明权限.md)。
...
![image](public_sys-resources/zh-cn_image_0000002522244802.webp)

**示例8（设置图片拖拽震动）**

从API version 18开始，示例8通过设置[enableHapticFeedback](#ZH-CN_TOPIC_0000002522080792__draginteractionoptions12)实现图片拖拽的震动效果。
...
从API version 15开始，示例9通过配置[onlyForLifting](拖拽事件.md#ZH-CN_TOPIC_0000002553200729__previewconfiguration15)实现自定义预览图，仅用于浮起效果以及配置[isLiftingDisabled](#ZH-CN_TOPIC_0000002522080792__draginteractionoptions12)实现禁用浮起效果。
...
![image](public_sys-resources/zh-cn_image_0000002553204763.webp)

自定义预览图禁用浮起效果。

![image](public_sys-resources/zh-cn_image_0000002522084810.webp)

**示例10（以拖拽预览图初始尺寸计算跟手点位置）**

从API version 19开始，示例10通过配置[DragPreviewMode](#ZH-CN_TOPIC_0000002522080792__dragpreviewmode11枚举说明)为ENABLE_TOUCH_POINT_CALCULATION_BASED_ON_FINAL_PREVIEW实现根据拖拽预览图的初始尺寸来计算拖拽过程中跟手点位置。当设置[DragPreviewMode](#ZH-CN_TOPIC_0000002522080792__dragpreviewmode11枚举说明)为ENABLE_MULTI_TILE_EFFECT时，该属性不生效。
...
![image](public_sys-resources/zh-cn_image_0000002553364723.webp)

**示例11（长按浮起预览图与拖拽预览图过渡动效）**

从API version 19开始，示例11通过配置[DraggingSizeChangeEffect](#ZH-CN_TOPIC_0000002522080792__draggingsizechangeeffect19枚举说明)实现不同拖拽过渡效果。
...
![image](public_sys-resources/zh-cn_image_0000002522244804.webp)

**示例12（设置自定义组件落入）**

从API version 23开始，示例12通过组件的[onDragStart](拖拽事件.md#ZH-CN_TOPIC_0000002553200729__ondragstart)接口传递其类型，并在目标组件的[allowDrop](#ZH-CN_TOPIC_0000002522080792__allowdrop)属性中设置允许该类型落入，即可实现自定义组件的拖拽落入功能。
...
![image](public_sys-resources/zh-cn_image_0000002553204765.webp)