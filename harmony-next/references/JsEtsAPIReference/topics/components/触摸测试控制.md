...
设置组件的[触摸测试](../../guides/交互基础机制说明.md#触摸测试)类型。在ArkUI开发框架中，处理触屏事件和鼠标事件时，会在事件触发前进行按压点与组件响应热区的触摸测试，以收集需响应事件的组件。基于测试结果，框架会分发相应的事件。hitTestBehavior属性用于设置不同的触摸测试响应模式，影响触摸测试收集结果及后续事件分发。具体影响参考[HitTestMode](枚举说明.md#ZH-CN_TOPIC_0000002553200889__hittestmode9)枚举说明。影响[点击事件](点击事件.md)、[触摸事件](触摸事件.md)、[拖拽事件](拖拽事件.md)、[鼠标事件](鼠标事件.md)、[轴事件](轴事件.md)、[悬浮事件](悬浮事件.md)、[无障碍悬浮事件](无障碍悬浮事件.md)和[手势事件](绑定手势方法.md)的分发。
...
存在新增节点时，如需该节点响应触摸测试，需要对其设置[HitTestMode](枚举说明.md#ZH-CN_TOPIC_0000002553200889__hittestmode9)。当新增节点存在多层级节点时，仅需设置根节点的HitTestMode。
...
该示例通过设置不同的[HitTestMode](枚举说明.md#ZH-CN_TOPIC_0000002553200889__hittestmode9)值演示了Block和Transparent的触摸测试效果。
...
该示例演示了在Stack组件中存在多节点触摸区域重叠时的触摸测试效果。此时设置[HitTestMode](枚举说明.md#ZH-CN_TOPIC_0000002553200889__hittestmode9)为None时，重叠的背景区域无法响应触摸测试；只有设置为Transparent时，背景区域才能响应触摸测试。
