...
| 类型 | 说明 |
| --- | --- |
| string | 组件属性列表的JSON字符串。 说明：  字符串信息包含组件的tag、id、位置信息(相对于窗口左上角的坐标)以及用于测试检查的组件所包含的相关属性信息。组件中每个字段的含义请参考getInspectorInfo的返回值说明。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | TouchObject | 是 | 触发触摸事件的位置，event参数见TouchObject的介绍。 |
