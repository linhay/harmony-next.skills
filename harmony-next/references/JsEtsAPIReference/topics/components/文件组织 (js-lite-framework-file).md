...
格式支持版本支持的文件类型BMPAPI Version 4+.bmpJPEGAPI Version 4+.jpgPNGAPI Version 4+.png[]()[]()

#### 存储目录定义

从API Version 5开始，[image](image (js-lite-components-basic-image).md)组件支持应用私有目录内的图片资源访问。

目录类型路径前缀访问可见性说明应用私有目录internal://app/仅本应用可见目录随应用卸载删除，路径禁止使用../等方式访问父目录