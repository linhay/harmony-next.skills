...
该组件用来展示列表item分组，宽度默认充满[List](list.md)组件，必须配合List组件来使用。
...
当ListItemGroup和带[virtualScroll](../misc/Repeat.md#ZH-CN_TOPIC_0000002497604972__virtualscroll)的Repeat结合，它的懒加载行为和LazyForEach一致。当ListItemGroup和不带virtualScroll的Repeat结合，它的懒加载行为和ForEach一致。
...
当ListItemGroup和ForEach结合，如果设置了[cachedCount](list.md#ZH-CN_TOPIC_0000002529284863__cachedcount)，除了会布局显示区域内子组件外，还会在空闲时隙根据List组件的cachedCount属性预布局显示区域外cachedCount范围内的子节点。

-

当ListItemGroup和LazyForEach结合，如果设置了[cachedCount](list.md#ZH-CN_TOPIC_0000002529284863__cachedcount)，除了会创建和布局显示区域内子组件外，还会在空闲时隙根据List组件的cachedCount属性预创建和预布局显示区域外cachedCount范围内的子节点。

-

当ListItemGroup和带[virtualScroll](../misc/Repeat.md#ZH-CN_TOPIC_0000002497604972__virtualscroll)的Repeat结合，它的预加载行为和LazyForEach一致。当ListItemGroup和不带virtualScroll的Repeat结合，它的预加载行为和ForEach一致。

- 该组件从API version 9开始支持。后续版本如有新增内容，则采用上角标单独标记该内容的起始版本。
- 该组件的父组件只能是[List](list.md)。
- ListItemGroup组件不支持设置[通用属性aspectRatio](../misc/布局约束.md#ZH-CN_TOPIC_0000002529444795__aspectratio)。
- 当ListItemGroup的父组件List的listDirection属性为Axis.Vertical时，设置[通用属性height](../misc/尺寸设置.md#ZH-CN_TOPIC_0000002497444850__height)属性不生效。ListItemGroup的高度为header高度、footer高度和所有ListItem布局后总高度之和。
- 当父组件List的listDirection属性为Axis.Horizontal时，设置[通用属性width](../misc/尺寸设置.md#ZH-CN_TOPIC_0000002497444850__width)属性不生效。ListItemGroup的宽度为header宽度、footer宽度和所有ListItem布局后总宽度之和。
...
参数名类型必填说明options[ListItemGroupOptions](#ZH-CN_TOPIC_0000002497604872__listitemgroupoptions对象说明)否列表item分组组件参数。
...
名称类型只读可选说明header[CustomBuilder](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__custombuilder8)否是
...
headerComponent13+[ComponentContent](ComponentContent.md)否是
...
footer[CustomBuilder](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__custombuilder8)否是
...
footerComponent13+[ComponentContent](ComponentContent.md)否是

使用ComponentContent类型参数设置ListItemGroup尾部组件。

**说明：**

可以放单个子组件或不放子组件。该参数的优先级高于参数footer。 即同时设置footer和footerComponent时，以footerComponent设置的值为准。

同一个footerComponent不推荐同时给不同的ListItemGroup使用，否则会导致显示问题。

**元服务API：** 从API version 13开始，该接口支持在元服务中使用。

spacenumber | string否是
...
style10+[ListItemGroupStyle](#ZH-CN_TOPIC_0000002497604872__listitemgroupstyle10枚举说明)否是
...
设置为ListItemGroupStyle.CARD时，建议配合[ListItem](ListItem.md)的ListItemStyle.CARD同时使用，显示默认卡片样式。
...
divider(value: [ListDividerOptions](list.md#ZH-CN_TOPIC_0000002529284863__listdivideroptions18对象说明) | null)
...
参数名类型必填说明value[ListDividerOptions](list.md#ZH-CN_TOPIC_0000002529284863__listdivideroptions18对象说明) | null是
...
名称值说明NONE0无样式。CARD1显示默认卡片样式。

#### 示例

#### 示例1（设置吸顶/吸底）

该示例通过[sticky](list.md#ZH-CN_TOPIC_0000002529284863__sticky9)实现了Header吸顶和Footer吸底的效果。

ListDataSource实现了LazyForEach数据源接口[IDataSource](../misc/LazyForEach.md#ZH-CN_TOPIC_0000002529444937__idatasource)，用于通过LazyForEach给List和ListItemGroup提供子组件。
...
该示例通过[ComponentContent](ComponentContent.md#ZH-CN_TOPIC_0000002497444816__componentcontent-1)设置Header/Footer。

ListDataSource说明及完整代码参考[示例1设置吸顶/吸底](#ZH-CN_TOPIC_0000002497604872__示例1设置吸顶吸底)。
...
该示例展示了ListItemGroup在多列布局中的使用，通过设置List组件的[lanes](list.md#ZH-CN_TOPIC_0000002529284863__lanes9)属性实现多列布局。
