...
该组件从API version 12开始支持与垂直滚动的[Swiper](Swiper.md)和[Web](web.md)的联动。当[Swiper](Swiper.md)设置[loop](Swiper.md#ZH-CN_TOPIC_0000002522240824__loop)属性为true时，Refresh无法和[Swiper](Swiper.md)产生联动。

-

Refresh和内容大小小于组件自身的[List](List.md)组件嵌套使用并且中间还有其他组件时，手势可能会被中间组件响应，导致Refresh未产生下拉刷新效果，可以将[alwaysEnabled](滚动组件通用接口.md#ZH-CN_TOPIC_0000002522240828__edgeeffectoptions11对象说明)参数设为true，此时[List](List.md)会响应手势并通过嵌套滚动带动Refresh组件产生下拉刷新效果，具体可以参考[示例9不满一屏实现下拉刷新](#ZH-CN_TOPIC_0000002522080830__示例9不满一屏场景实现下拉刷新)。
...
- 当未设置builder或refreshingContent时，是通过更新子组件的[translate](图形变换.md#ZH-CN_TOPIC_0000002522080784__translate)属性实现的下拉位移效果，下拉位移过程中不会触发子组件的[onAreaChange](组件区域变化事件.md#ZH-CN_TOPIC_0000002553200735__onareachange)事件，子组件设置[translate](图形变换.md#ZH-CN_TOPIC_0000002522080784__translate)属性时不会生效。

- 当设置了builder或refreshingContent时，是通过更新子组件相对于Refresh组件的位置实现的下拉位移效果，下拉位移过程中可以触发子组件的[onAreaChange](组件区域变化事件.md#ZH-CN_TOPIC_0000002553200735__onareachange)事件，子组件设置[position](位置设置.md#ZH-CN_TOPIC_0000002522240778__position)属性时会固定子组件相对于Refresh组件的位置导致子组件不会跟手进行下拉位移。
...
设置当下拉距离超过[refreshOffset](#ZH-CN_TOPIC_0000002522080830__refreshoffset12)时是否能触发刷新。
...
pullDownRatio(ratio: [Optional](自定义属性设置.md#ZH-CN_TOPIC_0000002522240802__optionalt12)<number>)
...
![image](public_sys-resources/zh-cn_image_0000002553204877.webp)

**示例2（设置刷新区域显示文本）**

通过[promptText](#ZH-CN_TOPIC_0000002522080830__refreshoptions对象说明)参数设置刷新区域显示文本。
...
![image](public_sys-resources/zh-cn_image_0000002522084924.webp)

**示例3（自定义刷新区域显示内容-builder）**

通过[builder](#ZH-CN_TOPIC_0000002522080830__refreshoptions对象说明)参数自定义刷新区域显示内容。
...
![image](public_sys-resources/zh-cn_image_0000002553364839.webp)

**示例4（自定义刷新区域显示内容-refreshingContent）**

通过[refreshingContent](#ZH-CN_TOPIC_0000002522080830__refreshoptions对象说明)参数自定义刷新区域显示内容。
...
![image](public_sys-resources/zh-cn_image_0000002522244918.webp)

**示例5（实现最大下拉距离）**

通过[pullDownRatio](#ZH-CN_TOPIC_0000002522080830__pulldownratio12)属性和[onOffsetChange](#ZH-CN_TOPIC_0000002522080830__onoffsetchange12)事件实现最大下拉距离。
...
![image](public_sys-resources/zh-cn_image_0000002553204879.webp)

**示例6（实现下拉刷新上拉加载更多）**

Refresh组件与[List](List.md)组件组合实现下拉刷新上拉加载更多效果。
...
![image](public_sys-resources/zh-cn_image_0000002522084926.webp)

**示例7（设置最大下拉距离）**

从API version 20开始，通过[maxPullDownDistance](#ZH-CN_TOPIC_0000002522080830__maxpulldowndistance20)属性设置最大下拉距离。
...
![image](public_sys-resources/zh-cn_image_0000002553364841.webp)

**示例8（禁止下拉刷新）**

通过[pullDownRatio](#ZH-CN_TOPIC_0000002522080830__pulldownratio12)属性禁止下拉刷新。
...
![image](public_sys-resources/zh-cn_image_0000002522244920.webp)

**示例9（不满一屏场景实现下拉刷新）**

通过设置[edgeEffect](滚动组件通用接口.md#ZH-CN_TOPIC_0000002522240828__edgeeffect11)属性中的alwaysEnabled参数，可以在不满一屏的情况下实现Refresh组件的下拉刷新效果。
...
![image](public_sys-resources/zh-cn_image_0000002553204881.webp)

**示例10（上划不取消刷新）**

该示例通过[pullUpToCancelRefresh](#ZH-CN_TOPIC_0000002522080830__pulluptocancelrefresh23)接口设置上划不取消刷新。
...
![image](public_sys-resources/zh-cn_image_0000002522084928.webp)