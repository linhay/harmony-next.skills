...
自定义组件无获焦能力，当设置[focusable](#ZH-CN_TOPIC_0000002553200755__focusable)、[enabled](禁用控制.md#ZH-CN_TOPIC_0000002522240790__enabled)等属性为false，或者设置[visibility](显隐控制.md#ZH-CN_TOPIC_0000002522240774__visibility)属性为Hidden、None时，也不影响其子组件的获焦。
...
- tabIndex只能够自定义Tab键走焦，若想同时自定义方向键等走焦能力，建议使用[nextFocus](#ZH-CN_TOPIC_0000002553200755__nextfocus18)。
...
如果需要指定组件立刻获焦，推荐使用FocusController中的焦点同步转移接口[requestFocus](Class (FocusController).md#ZH-CN_TOPIC_0000002522240726__requestfocus12)。
...
设置当前组件在指定容器内获焦的优先级。需要配合[focusScopeId](#ZH-CN_TOPIC_0000002553200755__focusscopeid12)一起使用。
...
![image](public_sys-resources/zh-cn_image_0000002522084796.webp)
...
直接使用focusControl可能导致[UI上下文不明确](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-global-interface#ui上下文不明确)的问题，建议使用getUIContext()获取[UIContext](Class (UIContext).md)实例，并使用[getFocusController](Class (UIContext).md#ZH-CN_TOPIC_0000002522240732__getfocuscontroller12)获取绑定实例的focusControl。

**示例**

**示例1（设置组件获焦和走焦的效果）**

该示例通过配置[defaultFocus](#ZH-CN_TOPIC_0000002553200755__defaultfocus9)可以使绑定的组件成为[层级页面](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-common-events-focus-event#基础概念)创建后首次获焦的焦点，配置[groupDefaultFocus](#ZH-CN_TOPIC_0000002553200755__groupdefaultfocus9)可以使绑定的组件成为tabIndex容器创建后首次获焦的焦点，配置[focusOnTouch](#ZH-CN_TOPIC_0000002553200755__focusontouch9)可以使绑定的组件点击后立即获焦。
...
![image](public_sys-resources/zh-cn_image_0000002553364709.webp)

首次按TAB键，焦点切换到tabIndex(1)的容器上，且自动走焦到内部第一个可获焦组件上：

![image](public_sys-resources/zh-cn_image_0000002522244790.webp)

第二次按TAB键，焦点切换到tabIndex(2)的容器上，且自动走到其内部的groupDefaultFocus绑定的组件上：

![image](public_sys-resources/zh-cn_image_0000002553204751.webp)

第三次按TAB键，焦点切换到tabIndex(3)的容器上，且自动走焦到内部配置了DefaultFocus的组件上：

![image](public_sys-resources/zh-cn_image_0000002522084798.webp)

点击绑定了focusOnTouch的组件，组件自身获焦，焦点框被清除，再按下TAB键后，显示焦点框：

![image](public_sys-resources/zh-cn_image_0000002553364711.webp)

**示例2（设置指定组件获焦）**

该示例通过配置[focusControl.requestFocus](#ZH-CN_TOPIC_0000002553200755__requestfocus9)使指定组件获取焦点。
...
![image](public_sys-resources/zh-cn_image_0000002522244792.webp)

申请不可获焦的组件获焦：

![image](public_sys-resources/zh-cn_image_0000002553204753.webp)

申请存在且可获焦的组件获焦：

![image](public_sys-resources/zh-cn_image_0000002522084800.webp)

**示例3（设置焦点框样式）**

该示例通过配置[focusBox](#ZH-CN_TOPIC_0000002553200755__focusbox12)修改组件的焦点框样式。
...
![image](public_sys-resources/zh-cn_image_0000002553364713.webp)

**示例4（设置焦点组走焦）**

该示例通过配置[focusScopePriority](#ZH-CN_TOPIC_0000002553200755__focusscopepriority12)，可以使绑定的组件在所属容器首次获焦时成为焦点，配置[focusScopeId](#ZH-CN_TOPIC_0000002553200755__focusscopeid12)，可以使绑定的容器组件成为焦点组。
...
![image](public_sys-resources/zh-cn_image_0000002522244794.webp)

继续按下TAB键，焦点转移到容器1下一个组件上。

![image](public_sys-resources/zh-cn_image_0000002553204755.webp)

再次按下TAB键，焦点转移到容器1下一个组件上。

![image](public_sys-resources/zh-cn_image_0000002522084802.webp)

继续按下TAB键，焦点转移到容器2中配置了focusScopePriority的组件上。

![image](public_sys-resources/zh-cn_image_0000002553364715.webp)

继续按下TAB键，焦点转移到容器1中名为Group1的组件上。

![image](public_sys-resources/zh-cn_image_0000002522244796.webp)

**示例5（设置tab走焦停留）**

该示例通过配置[tabStop](#ZH-CN_TOPIC_0000002553200755__tabstop14)实现使用tab走焦停留在组件上。
...
![image](public_sys-resources/zh-cn_image_0000002553204757.webp)

接着按下TAB键，焦点转移到配置了tabStop的组件。

![image](public_sys-resources/zh-cn_image_0000002522084804.webp)

再按下Enter键，焦点转移至内部button3上。

![image](public_sys-resources/zh-cn_image_0000002553364717.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522244798.webp)

**示例6（设置自定义走焦）**

从API version 18开始，该示例通过配置[nextFocus](#ZH-CN_TOPIC_0000002553200755__nextfocus18)实现自定义走焦规则。

如果不配置[nextFocus](#ZH-CN_TOPIC_0000002553200755__nextfocus18)，默认的点击tab的走焦顺序为：M->A->B->C；配置了[nextFocus](#ZH-CN_TOPIC_0000002553200755__nextfocus18)以后，走焦顺序变更为：M->D->F->B。
...
![image](public_sys-resources/zh-cn_image_0000002553204759.webp)