...
aboutToAppear函数在创建自定义组件的新实例后，在其build()函数执行前调用。允许在aboutToAppear函数中改变[状态变量](../../modules/ohos/@ohos.arkui.StateManagement (状态管理).md)，更改将在后续执行build()函数中生效。实现[自定义布局](自定义组件的自定义布局.md)的自定义组件的aboutToAppear生命周期在布局过程中触发。具体使用说明，详见[自定义组件生命周期指南](自定义组件的生命周期.md)。
...
onDidBuild函数在自定义组件的build()函数执行后调用，开发者可以在这个阶段实现埋点数据上报等不影响实际UI的功能。具体使用说明，详见[自定义组件生命周期指南](自定义组件的生命周期.md)。
...
aboutToDisappear函数在自定义组件析构销毁时执行。不允许在aboutToDisappear函数中改变状态变量，特别是@Link变量的修改可能会导致应用程序行为不稳定。具体使用说明，详见[自定义组件生命周期指南](自定义组件的生命周期.md)。不建议在aboutToDisappear函数调用后再触发例如[自定义弹窗的创建](自定义弹窗 (CustomDialog).md#ZH-CN_TOPIC_0000002553360809__open)等逻辑，这可能会因为组件树信息丢失导致应用行为异常，例如[@Consume](../../modules/ohos/@ohos.arkui.StateManagement (状态管理).md)找不到对应的[@Provide](../../modules/ohos/@ohos.arkui.StateManagement (状态管理).md)、弹窗内白屏不显示组件等。
...
router路由页面（即[@Entry](自定义组件.md)装饰的自定义组件）每次显示时触发一次，包括路由跳转、应用进入前台等场景。
...
router路由页面（即[@Entry](自定义组件.md)装饰的自定义组件）每次隐藏时触发一次，包括路由跳转、应用进入后台等场景。
...
在router路由页面（即[@Entry](自定义组件.md)装饰的自定义组件）生效，当用户点击返回按钮时触发。返回true表示页面自己处理返回逻辑，不进行页面路由；返回false表示使用默认的路由返回逻辑，不设置返回值按照false处理。
...
![image](public_sys-resources/zh-cn_image_0000002522245352.webp)

**onNewParam19+**

onNewParam?(param: ESObject): void

该回调仅生效于由[@Entry](自定义组件.md)装饰的、作为[router路由](@ohos.router (页面路由)(不推荐).md)页面存在的自定义组件。当之前存在于路由栈中的页面，通过[单实例模式](@ohos.router (页面路由)(不推荐).md#ZH-CN_TOPIC_0000002553200705__routermode9)移动到栈顶时触发该回调。
...
![image](public_sys-resources/zh-cn_image_0000002553205315.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522085362.webp)
...
onFormRecycle回调函数在卡片回收时执行，卡片提供方可以返回需要卡片管理服务代保存的数据，在卡片恢复时通过[onFormRecover](#ZH-CN_TOPIC_0000002553200861__onformrecover11)接口传给卡片提供方。
...
onFormRecover回调函数在卡片恢复时执行，卡片提供方可以拿到卡片回收时卡片管理服务代保存的数据，该数据可以通过[onFormRecycle](#ZH-CN_TOPIC_0000002553200861__onformrecycle11)卡片回收回调函数保存到卡片管理服务。
