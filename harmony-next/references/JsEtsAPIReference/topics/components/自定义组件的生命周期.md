...
aboutToAppear函数在创建自定义组件的新实例后，在其build()函数执行前调用。允许在aboutToAppear函数中改变[状态变量](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-state-management-glossary#状态变量state-variables)，更改将在后续执行build()函数中生效。实现[自定义布局](自定义组件的自定义布局.md)的自定义组件的aboutToAppear生命周期在布局过程中触发。具体使用说明，详见[自定义组件生命周期指南](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-page-custom-components-lifecycle)。
...
该回调仅生效于由[@Entry](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-create-custom-components#entry)装饰的、作为[router路由](@ohos.router (页面路由)(不推荐).md)页面存在的自定义组件。当之前存在于路由栈中的页面，通过[单实例模式](@ohos.router (页面路由)(不推荐).md#ZH-CN_TOPIC_0000002553200705__routermode9)移动到栈顶时触发该回调。
