...
焦点轴事件是指在与游戏手柄交互时，通过十字按键或者操作杆上报的轴事件，此轴事件通过获得焦点的组件分发并回调给应用。若组件默认可获焦，如Button，则不需要额外设置属性。若组件在默认情况下不可获焦，如Text和Image，可以通过将[focusable](焦点控制.md#ZH-CN_TOPIC_0000002553200755__focusable)属性设置为true来启用焦点轴事件。
...
焦点轴事件的对象说明，继承于[BaseEvent](自定义手势判定.md#ZH-CN_TOPIC_0000002522240812__baseevent8)。
...
![image](public_sys-resources/zh-cn_image_0000002553204689.webp)