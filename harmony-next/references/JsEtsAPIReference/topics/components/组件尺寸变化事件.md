...
onSizeChange是布局过程中触发的同步回调，直接在其中更改状态变量存在被纳入动画闭包的风险。具体而言，动画会对比动画前的布局与动画闭包后的布局，若onSizeChange的回调在动画前的布局中同步触发，那么onSizeChange回调中所做的变更将与动画闭包中的变更一同纳入动画过程。为了避免此类问题，可在onSizeChange中使用延迟时间为0的[setTimeout](Timer (定时器).md#ZH-CN_TOPIC_0000002522082610__settimeout)或[postFrameCallback](Class (UIContext).md#ZH-CN_TOPIC_0000002522240732__postframecallback12)，将UI处理逻辑延后至异步执行。
...
![image](public_sys-resources/zh-cn_image_0000002522244738.webp)