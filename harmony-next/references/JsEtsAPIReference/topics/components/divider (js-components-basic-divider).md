...
名称类型默认值必填描述verticalbooleanfalse否使用水平分割线还是垂直分割线，默认水平。
...
名称类型默认值必填描述margin<length>0否使用简写属性设置所有的外边距属性，该属性可以有1到4个值。margin-[left|top|right|bottom]<length>0否使用简写属性设置左、上、右、下外边距属性，类型length，单位px，默认值0。color<color>#08000000否设置分割线颜色。stroke-width<length>1否设置分割线宽度。displaystringflex否确定分割线所产生的框的类型。值flex/none，默认值flex。visibilitystringvisible否是否显示分割线。不可见的框会占用布局。visible代表显示元素，hidden代表不显示元素。line-capstringbutt否
