...
拖拽排序数据移动回调。当父容器组件为[List](List.md)，并且ForEach/LazyForEach/Repeat每次迭代都生成一个ListItem组件时才生效。设置拖拽排序时可以定义不同的拖拽操作，并在响应事件发生时响应。与[onMove](#ZH-CN_TOPIC_0000002553360717__onmove)相比，新增eventHandler参数，可以监听拖拽时上报的回调事件。
...
| --- | --- | --- | --- |
| handler | Optional<OnMoveHandler> | 是 | 拖拽动作。 |
| eventHandler | ItemDragEventHandler | 是 | 拖拽发生时产生的回调。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| onLongPress | Callback<number> | 否 | 是 | 长按时触发的回调。 - 参数index为长按时当前目标的索引号。 |
| onDragStart | Callback<number> | 否 | 是 | 在页面跟手滑动开始时触发的回调。 - 参数index为拖拽开始时当前目标的索引号。 |
| onMoveThrough | OnMoveHandler | 否 | 是 | 在页面跟手滑动过程中经过其他组件时触发的回调。 |
| onDrop | Callback<number> | 否 | 是 | 在页面跟手滑动结束时触发的回调。 - 参数index为拖拽结束时当前目标的索引号。 |
