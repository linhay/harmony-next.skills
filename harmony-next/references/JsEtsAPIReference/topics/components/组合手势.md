...
| gesture | GestureType[] | 否 | 设置一个或者多个基础手势类型时，这些手势会被识别为组合手势。若此参数不填则组合手势识别功能不生效。 说明： 当需要为一个组件同时添加单击和双击手势时，可在组合手势中添加两个TapGesture，需要双击手势在前，单击手势在后，否则不生效。 |
...
![image](public_sys-resources/zh-cn_image_0000002522244848.webp)

按顺序首先触发长按事件，长按事件识别结束之后，触发拖动事件，向右下方拖动：

 ![image](public_sys-resources/zh-cn_image_0000002553204809.webp)