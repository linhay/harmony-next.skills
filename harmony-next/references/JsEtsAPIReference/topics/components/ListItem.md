...
- 该组件的父组件只能是[List](list.md)或者[ListItemGroup](ListItemGroup.md)。
...
参数名类型必填说明value[ListItemOptions](#ZH-CN_TOPIC_0000002529444837__listitemoptions10对象说明)否

为ListItem提供可选参数，该对象内含有[ListItemStyle](#ZH-CN_TOPIC_0000002529444837__listitemstyle10枚举说明)枚举类型的style参数。
...
从API version 7开始支持，从API version 10开始废弃，建议使用[ListItem10+](#ZH-CN_TOPIC_0000002529444837__listitem10)替代。
...
从API version 7开始支持，从API version 9开始废弃，建议使用[List组件sticky属性](list.md#ZH-CN_TOPIC_0000002529284863__sticky9)替代。

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

**参数：**

参数名类型必填说明value[Sticky](#ZH-CN_TOPIC_0000002529444837__stickydeprecated枚举说明)是
...
参数名类型必填说明valueboolean | [EditMode](#ZH-CN_TOPIC_0000002529444837__editmodedeprecated枚举说明)是
...
设置当前ListItem选中状态。该属性支持[$$](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-two-way-sync)双向绑定变量。该属性需要在设置[选中态样式](../misc/多态样式.md#ZH-CN_TOPIC_0000002497604846__statestyles)前使用才能生效选中态样式。
...
参数名类型必填说明value[SwipeActionOptions](#ZH-CN_TOPIC_0000002529444837__swipeactionoptions9对象说明)是ListItem的划出组件。

#### Sticky(deprecated)枚举说明

ListItem吸顶效果枚举。

从API version 7开始支持，从API version 9开始废弃，建议使用[List组件stickyStyle枚举](list.md#ZH-CN_TOPIC_0000002529284863__stickystyle9枚举说明)替代。

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

名称值说明None-无吸顶效果。Normal-当前item吸顶。Opacity-当前item吸顶显示透明度变化效果。
...
名称值说明None-编辑操作不限制。Deletable-可删除。Movable-可移动。
...
名称类型只读可选说明start[CustomBuilder](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__custombuilder8) | [SwipeActionItem](#ZH-CN_TOPIC_0000002529444837__swipeactionitem10对象说明)否是

ListItem向右划动时item左边的组件（List垂直布局时）或ListItem向下划动时item上方的组件（List水平布局时）。

**元服务API：** 从API version 11开始，该接口支持在元服务中使用。

end[CustomBuilder](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__custombuilder8) | [SwipeActionItem](#ZH-CN_TOPIC_0000002529444837__swipeactionitem10对象说明)否是

ListItem向左划动时item右边的组件（List垂直布局时）或ListItem向上划动时item下方的组件（List水平布局时）。

**元服务API：** 从API version 11开始，该接口支持在元服务中使用。

edgeEffect[SwipeEdgeEffect](#ZH-CN_TOPIC_0000002529444837__swipeedgeeffect9枚举说明)否是

滑动效果。

**元服务API：** 从API version 11开始，该接口支持在元服务中使用。

onOffsetChange11+(offset: number) => void否是
...
名称类型只读可选说明actionAreaDistance[Length](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length)否是
...
onEnterActionArea() => void否是

在滑动条目进入删除区域时调用，只触发一次，当再次进入时仍触发。

**元服务API：** 从API version 11开始，该接口支持在元服务中使用。

onExitActionArea() => void否是
...
builderComponent18+[ComponentContent](ComponentContent.md)否是
...
onStateChange11+(state:[SwipeActionState](#ZH-CN_TOPIC_0000002529444837__swipeactionstate11枚举说明)) => void否是
...
名称类型只读可选说明style[ListItemStyle](#ZH-CN_TOPIC_0000002529444837__listitemstyle10枚举说明)否是
...
设置为ListItemStyle.CARD时，建议配合[ListItemGroup](ListItemGroup.md)的ListItemGroupStyle.CARD同时使用，显示默认卡片样式。
...
参数名类型必填说明isSelectedboolean是进入鼠标框选范围即被选中返回true， 移出鼠标框选范围即未被选中返回false。
...
参数名类型必填说明node[FrameNode](FrameNode.md)是ListItem节点对象。direction[ListItemSwipeActionDirection](#ZH-CN_TOPIC_0000002529444837__listitemswipeactiondirection21枚举说明)是ListItem划出菜单的展开方向。

**错误码：**

以下错误码的详细介绍请参见[自定义节点错误码](../../errors/自定义节点错误码.md)。

错误码ID错误信息100023The component type of the node is incorrect.106203The node not mounted to component tree.
...
参数名类型必填说明node[FrameNode](FrameNode.md)是ListItem节点对象。
...
名称值说明START0当列表方向是垂直方向时，LTR模式下表示ListItem的左边，RTL模式下表示ListItem的右边。当列表是水平方向时，表示ListItem的上边。END1当列表方向是垂直方向时，LTR模式下表示ListItem的右边，RTL模式下表示ListItem的左边。当列表是水平方向时，表示ListItem的下边。
...
该示例通过[ComponentContent](ComponentContent.md#ZH-CN_TOPIC_0000002497444816__componentcontent-1)设置ListItem中的划出组件操作时显示的操作项。
...
从API version 21开始，该示例通过[ListItemSwipeActionManager](#ZH-CN_TOPIC_0000002529444837__listitemswipeactionmanager21)管理ListItem的划出菜单。
