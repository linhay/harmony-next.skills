...
名称类型默认值必填描述percentnumber0否当前进度。取值范围为0-100。secondarypercent(Rich)number0否次级进度。取值范围为0-100。
-

类型为ring、scale-ring时，支持如下属性：

名称类型默认值必填描述clockwisebooleantrue否圆环形进度条是否采用顺时针。
-
...
名称类型默认值必填描述color<color>-否loading进度条上的圆点颜色。
