...
如果需要在该组件中实现可嵌入式运行的元服务，必须继承自[EmbeddableUIAbility](../../modules/ohos/@ohos.app.ability.EmbeddableUIAbility (可嵌入式UIAbility组件).md)。否则，系统无法保证元服务功能正常。
...
onError18+[ErrorCallback](../../modules/ohos/@ohos.base (公共回调信息).md#ZH-CN_TOPIC_0000002497445536__errorcallback)否-

被拉起的嵌入式运行元服务在运行过程中发生异常时触发本回调。可通过回调参数中的code、name和message获取错误信息并做处理。

**元服务API：** 从API version 18开始，该接口支持在元服务中使用。

onTerminated18+[Callback](../../modules/ohos/@ohos.base (公共回调信息).md#ZH-CN_TOPIC_0000002497445536__callback)<[TerminationInfo](EmbeddedComponent.md#ZH-CN_TOPIC_0000002497444946__terminationinfo)>否-

被拉起的嵌入式运行元服务通过调用[terminateSelfWithResult](../graphics/UIAbilityContext.md#ZH-CN_TOPIC_0000002497604628__terminateselfwithresult)或者[terminateSelf](../graphics/UIAbilityContext.md#ZH-CN_TOPIC_0000002497604628__terminateself)正常退出时，触发本回调函数。
...
- 若元服务通过调用[terminateSelfWithResult](../graphics/UIAbilityContext.md#ZH-CN_TOPIC_0000002497604628__terminateselfwithresult)退出，其携带的信息会传给回调函数的入参；
- 若元服务通过调用[terminateSelf](../graphics/UIAbilityContext.md#ZH-CN_TOPIC_0000002497604628__terminateself)退出，上述回调函数的入参中，"code"取默认值"0"，"want"为"undefined"。
