...
FullScreenLaunchComponent({ content: Callback<void>, appId: string, options?: AtomicServiceOptions, onError?: ErrorCallback, onTerminated?: Callback<[TerminationInfo](EmbeddedComponent.md#ZH-CN_TOPIC_0000002497444946__terminationinfo)> })
...
| onTerminated18+ | Callback<[TerminationInfo](EmbeddedComponent.md#ZH-CN_TOPIC_0000002497444946__terminationinfo)> | 否 | - | 被拉起的嵌入式运行元服务通过调用[terminateSelfWithResult](../misc/UIAbilityContext.md#ZH-CN_TOPIC_0000002497604628__terminateselfwithresult)或者[terminateSelf](../misc/UIAbilityContext.md#ZH-CN_TOPIC_0000002497604628__terminateself)正常退出时，触发本回调函数。 元服务API： 从API version 18开始，该接口支持在元服务中使用。 |
| onReceive20+ | Callback<Record<string, Object>> | 否 | - | 被拉起的嵌入式运行元服务通过Window调用API时，触发本回调。 元服务API： 从API version 20开始，该接口支持在元服务中使用。 |


- 若元服务通过调用[terminateSelfWithResult](UIAbilityContext.md#ZH-CN_TOPIC_0000002553200541__terminateselfwithresult)退出，其携带的信息会传给回调函数的入参；

- 若元服务通过调用[terminateSelf](UIAbilityContext.md#ZH-CN_TOPIC_0000002553200541__terminateself)退出，上述回调函数的入参中，"code"取默认值"0"，"want"为"undefined"。
