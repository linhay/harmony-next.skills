...
仅支持[MenuPreviewMode](#ZH-CN_TOPIC_0000002522080798__menupreviewmode11)设置为NONE的菜单。
...
[bindContextMenu](#ZH-CN_TOPIC_0000002522080798__bindcontextmenu8)仅支持在子窗中显示，[bindMenu](#ZH-CN_TOPIC_0000002522080798__bindmenu)可以通过配置[MenuOptions](#ZH-CN_TOPIC_0000002522080798__menuoptions10)中的showInSubWindow属性设置是否在子窗中显示。
...
从API version 20开始，该接口仅当content的入参类型为Array<MenuElement>时支持在[attributeModifier](动态属性设置.md#ZH-CN_TOPIC_0000002522240800__attributemodifier)中调用。
...
给组件绑定菜单，控制菜单显隐的触发方式为长按或右键点击，弹出的菜单项需自定义。若需通过代码逻辑控制菜单显隐，请使用[bindContextMenu12+](#ZH-CN_TOPIC_0000002522080798__bindcontextmenu12)。

   ![image](public_sys-resources/note_3.0-zh-cn.webp)

-

不支持在输入法类型窗口中使用bindContextMenu(默认子窗实现)，详情见输入法框架的约束与限制说明[createPanel](@ohos.inputMethodEngine (输入法服务).md#ZH-CN_TOPIC_0000002553201227__createpanel10-1)。
...
不支持在输入法类型窗口中使用bindContextMenuWithResponse（默认子窗实现），详情见输入法框架的约束与限制说明[createPanel](@ohos.inputMethodEngine (输入法服务).md#ZH-CN_TOPIC_0000002553201227__createpanel10-1)。
...
菜单项的信息，继承自[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)。
...
![image](public_sys-resources/zh-cn_image_0000002522084822.webp)
...
type BorderRadiusType = [Length](基础类型定义.md#ZH-CN_TOPIC_0000002553360849__length) | [BorderRadiuses](基础类型定义.md#ZH-CN_TOPIC_0000002553360849__borderradiuses9) | [LocalizedBorderRadiuses](基础类型定义.md#ZH-CN_TOPIC_0000002553360849__localizedborderradiuses12)
...
该示例为bindMenu通过配置[MenuElement](#ZH-CN_TOPIC_0000002522080798__menuelement)弹出普通菜单。
...
![image](public_sys-resources/zh-cn_image_0000002553364735.webp)

**示例2（弹出自定义菜单）**

该示例为bindMenu通过配置CustomBuilder弹出自定义菜单。同时，从API version 18开始支持通过配置[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中的hapticFeedbackMode属性实现菜单弹出时的振动效果。
...
![image](public_sys-resources/zh-cn_image_0000002522244816.webp)

**示例3（长按弹出菜单）**

该示例为bindContextMenu通过配置[responseType](枚举说明.md#ZH-CN_TOPIC_0000002553200889__responsetype8).LongPress弹出菜单。
...
![image](public_sys-resources/zh-cn_image_0000002553204777.webp)

**示例4（右键弹出指向型菜单）**

该示例为bindContextMenu通过配置[responseType](枚举说明.md#ZH-CN_TOPIC_0000002553200889__responsetype8).RightClick和[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中的enableArrow属性弹出指向型菜单。同时，从API version 18开始支持通过配置[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中的hapticFeedbackMode属性实现菜单弹出时的振动效果。
...
![image](public_sys-resources/zh-cn_image_0000002522084824.webp)

**示例5（长按弹出菜单的截图预览样式）**

该示例为bindContextMenu通过配置[responseType](枚举说明.md#ZH-CN_TOPIC_0000002553200889__responsetype8).LongPress和[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中preview属性的[MenuPreviewMode](#ZH-CN_TOPIC_0000002522080798__menupreviewmode11)类型弹出菜单预览样式。
...
![image](public_sys-resources/zh-cn_image_0000002553364737.webp)

**示例6（长按弹出菜单的自定义预览样式）**

该示例为bindContextMenu通过配置[responseType](枚举说明.md#ZH-CN_TOPIC_0000002553200889__responsetype8).LongPress和[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中preview属性的[CustomBuilder](基础类型定义.md#ZH-CN_TOPIC_0000002553360849__custombuilder8)类型弹出菜单自定义预览样式。
...
![image](public_sys-resources/zh-cn_image_0000002522244818.webp)

**示例7（设置状态变量弹出菜单）**

该示例为[bindContextMenu](#ZH-CN_TOPIC_0000002522080798__bindcontextmenu12)通过配置isShown弹出菜单预览样式。
...
该示例为bindContextMenu通过配置[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中的transition属性，实现自定义菜单以及菜单预览时的显示和退出动效。
...
![image](public_sys-resources/zh-cn_image_0000002553204779.webp)

**示例9（设置symbol类型图标）**

该示例为bindMenu通过配置[MenuElement](#ZH-CN_TOPIC_0000002522080798__menuelement)的symbolIcon弹出菜单。
...
![image](public_sys-resources/zh-cn_image_0000002522084826.webp)

**示例10（设置一镜到底动效）**

该示例为bindContextMenu通过配置[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中previewAnimationOptions属性的hoverScale，实现组件截图到自定义预览图的一镜到底过渡动效。
...
![image](public_sys-resources/zh-cn_image_0000002553364739.webp)

**示例11（自定义背景模糊效果参数）**

该示例为bindMenu通过配置[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中的backgroundBlurStyleOptions属性，实现了自定义菜单背景模糊效果。
...
![image](public_sys-resources/zh-cn_image_0000002522244820.webp)

**示例12（自定义背景效果参数）**

该示例为bindMenu通过配置[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中的backgroundEffect属性，实现了自定义菜单背景效果。
...
![image](public_sys-resources/zh-cn_image_0000002553204781.webp)

**示例13（设置一镜到底动效支持抬手打断）**

该示例通过为bindContextMenu配置[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中的previewAnimationOptions属性实现了一镜到底过渡动效的同时，再配置hoverScaleInterruption控制是否允许长按抬手取消菜单弹出。

从API version 20开始，在previewAnimationOptions的类型[ContextMenuAnimationOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuanimationoptions11)中新增了hoverScaleInterruption属性。
...
![image](public_sys-resources/zh-cn_image_0000002522084828.webp)

**示例14（设置预览图边框圆角半径）**

该示例通过bindContextMenu配置[responseType](枚举说明.md#ZH-CN_TOPIC_0000002553200889__responsetype8).LongPress来实现功能。同时，在[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中配置preview属性的[MenuPreviewMode](#ZH-CN_TOPIC_0000002522080798__menupreviewmode11)类型来设置菜单预览样式。最后，通过设置previewBorderRadius来实现预览图边框的圆角半径。

从API version 19开始，在[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中新增了previewBorderRadius属性。
...
![image](public_sys-resources/zh-cn_image_0000002553364741.webp)

**示例15（bindMenu配置生命周期回调）**

该示例为bindMenu配置生命周期回调。

从API version 20开始，在[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中新增了onWillAppear、onDidAppear、onWillDisappear和onDidDisappear属性。
...
![image](public_sys-resources/zh-cn_image_0000002522244822.webp)

**示例16（设置菜单蒙层）**

该示例为bindMenu通过配置mask属性设置菜单蒙层。

从API version 20开始，在[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中新增了mask属性。
...
![image](public_sys-resources/zh-cn_image_0000002553204783.webp)

**示例17（bindMenu设置下拉菜单外描边样式）**

该示例为bindMenu通过配置outlineWidth和outlineColor属性设置下拉菜单外描边样式。

从API version 20开始，在[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中新增了outlineWidth和outlineColor属性。
...
![image](public_sys-resources/zh-cn_image_0000002522084830.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553364743.webp)

**示例19（根据触发方式弹出不同内容的菜单）**

该示例通过在[bindContextMenuWithResponse](#ZH-CN_TOPIC_0000002522080798__bindcontextmenuwithresponse23)中传入CustomBuilderT<ResponseType>给目标组件绑定菜单，组件会在UI函数中返回弹出菜单的触发方式，开发者可根据返回的触发方式实现差异化显示。
...
![image](public_sys-resources/zh-cn_image_0000002522244824.webp)

**示例20（设置菜单避让软键盘）**

该示例通过在bindMenu中配置keyboardAvoidMode设置菜单避让软键盘，通过minKeyboardAvoidDistance设置菜单避让软键盘的最小距离。

从API version 23开始，[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中新增keyboardAvoidMode、minKeyboardAvoidDistance属性。
...
![image](public_sys-resources/zh-cn_image_0000002553204785.webp)

**示例21（设置菜单相对于绑定组件左上角的弹出位置）**

该示例通过设置[ContextMenuOptions](#ZH-CN_TOPIC_0000002522080798__contextmenuoptions10)中的anchorPosition属性，实现了菜单相对于绑定组件左上角弹出的效果。
...
![image](public_sys-resources/zh-cn_image_0000002522084832.webp)