...
EmbeddedComponent用于支持在当前页面嵌入本应用内其他[EmbeddedUIExtensionAbility](../../modules/ohos/@ohos.app.ability.EmbeddedUIExtensionAbility (支持跨进程界面嵌入的ExtensionAbility组件).md)提供的UI。EmbeddedUIExtensionAbility在独立进程中运行，完成页面布局和渲染。
...
参数名类型必填说明loader[Want](../../modules/ohos/@ohos.app.ability.Want (Want).md)是要加载的EmbeddedUIExtensionAbility。type[EmbeddedType](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__embeddedtype12)是提供方的类型。
...
不支持[点击](../misc/点击事件.md)等通用事件。仅支持以下事件：
...
参数名类型必填说明callback[Callback](../../modules/ohos/@ohos.base (公共回调信息).md#ZH-CN_TOPIC_0000002497445536__callback)<[TerminationInfo](#ZH-CN_TOPIC_0000002497444946__terminationinfo)>是回调函数，入参用于接收EmbeddedUIExtensionAbility的返回结果，类型为[TerminationInfo](#ZH-CN_TOPIC_0000002497444946__terminationinfo)。
...
被拉起的EmbeddedUIExtensionAbility在运行过程中发生异常时触发本回调。可通过回调参数中的code、name和message获取错误信息并做处理，业务错误码详细介绍请参见[UIExtension错误码](../../errors/UIExtension错误码.md)。
...
参数名类型必填说明callback[ErrorCallback](../../modules/ohos/@ohos.base (公共回调信息).md#ZH-CN_TOPIC_0000002497445536__errorcallback)是回调函数，入参用于接收异常信息，类型为[BusinessError](../../modules/ohos/@ohos.base (公共回调信息).md#ZH-CN_TOPIC_0000002497445536__businesserror)，可通过参数中的code、name和message获取错误信息并做处理。
...
名称类型只读可选说明codenumber否否被拉起EmbeddedUIExtensionAbility退出时返回的结果码，返回的结果码由terminateSelfWithResult或者terminateSelf被调用时传入的数据决定。want[Want](../../modules/ohos/@ohos.app.ability.Want (Want).md)否是被拉起EmbeddedUIExtensionAbility退出时返回的数据。
