...
sideBarWidth(value: [Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length))
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length) | 是 | 侧边栏的宽度。 默认值：240vp 单位：vp 取值范围：[0, +∞) 说明： API version 9的默认值为200vp，API version 10的默认值为240vp。 |
...
minSideBarWidth(value: [Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length))

设置侧边栏最小宽度。设置为小于0的值时按默认值显示。值不能超过侧边栏容器本身宽度，超过则使用侧边栏容器本身宽度。与[minSideBarWidth](#ZH-CN_TOPIC_0000002522080822__minsidebarwidth)相比，value参数新增了对百分比字符串和其他[像素单位](像素单位.md)的支持。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length) | 是 | 侧边栏最小宽度。 默认值：API version 9及以下版本默认值为200vp，API version 10的默认值为240vp。 取值范围：[0, +∞) |
...
maxSideBarWidth(value: [Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length))

设置侧边栏最大宽度。设置为小于0的值时按默认值显示。值不能超过侧边栏容器本身宽度，超过则使用侧边栏容器本身宽度。与[maxSideBarWidth](#ZH-CN_TOPIC_0000002522080822__maxsidebarwidth)相比，value参数新增了对百分比字符串和其他[像素单位](像素单位.md)的支持。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length) | 是 | 设置侧边栏最大宽度。 默认值：280vp 单位：vp 取值范围：[0, +∞) |
...
minContentWidth(value: [Dimension](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__dimension10))
...
- 如果[autoHide](#ZH-CN_TOPIC_0000002522080822__autohide9)属性为false，则会保持侧边栏宽度[minSideBarWidth](#ZH-CN_TOPIC_0000002522080822__minsidebarwidth)和内容区宽度minContentWidth不变，但内容区会被截断显示；

- 如果autoHide属性为true，则会优先隐藏侧边栏，然后继续缩小至内容区宽度minContentWidth后，内容区宽度保持不变，但内容区会被截断显示。

minContentWidth优先于侧边栏的[maxSideBarWidth](#ZH-CN_TOPIC_0000002522080822__maxsidebarwidth)与sideBarWidth属性，minContentWidth未设置时默认值优先级低于设置的minSideBarWidth与maxSideBarWidth属性。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Dimension](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__dimension10) | 是 | SideBarContainer组件内容区可显示的最小宽度。 默认值：360vp 单位：vp |
...
| icons | ButtonIconOptions18+ | 否 | 是 | 设置侧边栏控制按钮的图标。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| shown8+ | string | PixelMap | [Resource](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resource) | 否 | 否 | 设置侧边栏显示时控制按钮的图标。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
| hidden8+ | string | PixelMap | [Resource](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resource) | 否 | 否 | 设置侧边栏隐藏时控制按钮的图标。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
| switching8+ | string | PixelMap | [Resource](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resource) | 否 | 是 | 设置侧边栏显示和隐藏状态切换时控制按钮的图标。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| strokeWidth | [Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length) | 否 | 否 | 分割线的线宽。 默认值：1vp。 取值范围：[0, +∞)。 说明： 分割线的宽度不支持百分比设置。优先级低于通用属性height，超过通用属性设置大小时，按照通用属性进行裁切。部分设备硬件中存在1像素取整后分割线不显示问题，建议使用2像素。 |
| color | [Resource](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resource)Color | 否 | 是 | 分割线的颜色。 默认值：#000000，3% |
| startMargin | [Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length) | 否 | 是 | 分割线与侧边栏顶端的距离。 默认值：0。 取值范围：[0, +∞)。 |
| endMargin | [Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length) | 否 | 是 | 分割线与侧边栏底端的距离。 默认值：0。 取值范围：[0, +∞)。 |


针对侧边栏子组件设置[通用属性宽高](尺寸设置.md)时，宽高都不生效。

针对侧边栏内容区设置[通用属性宽高](尺寸设置.md)时，宽高都不生效，默认占满SideBarContainer的剩余空间。

当[showSideBar](#ZH-CN_TOPIC_0000002522080822__showsidebar)属性未设置时，依据组件大小进行自动显示：

- 小于[minSideBarWidth](#ZH-CN_TOPIC_0000002522080822__minsidebarwidth) + [minContentWidth](#ZH-CN_TOPIC_0000002522080822__mincontentwidth10)：默认不显示侧边栏。
...
1. 分割线拖拽触发[autoHide](#ZH-CN_TOPIC_0000002522080822__autohide9)时。
...
![image](public_sys-resources/zh-cn_image_0000002522084874.webp)
