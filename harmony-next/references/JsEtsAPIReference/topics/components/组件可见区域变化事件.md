...
不支持兄弟组件对自身节点的遮挡计算，不支持所有祖先的兄弟节点对自身节点的遮挡计算，不支持窗口遮挡计算，不支持组件旋转计算，如[Stack](Stack.md)、[Z序控制](Z序控制.md)、[rotate](图形变换.md#ZH-CN_TOPIC_0000002522080784__rotate)等。

-

不支持非挂树节点的可见面积变化计算。例如，预加载的节点、通过[overlay](浮层.md#ZH-CN_TOPIC_0000002522080776__overlay)能力挂载的自定义节点。

-

不支持[scale](图形变换.md#ZH-CN_TOPIC_0000002522080784__scale)属性，如果想要支持[scale](图形变换.md#ZH-CN_TOPIC_0000002522080784__scale)，则需使用[onVisibleAreaChange22+](#ZH-CN_TOPIC_0000002553360697__onvisibleareachange22)，将measureFromViewport设置为true。
...
| event | VisibleAreaChangeCallback | 是 | 组件可见区域变化事件的回调。 |
| measureFromViewport | boolean | 是 | 设置可见区域计算模式。 当measureFromViewport设置为true时，系统在计算该组件的可见区域时，会考虑父组件的clip 属性设置。如果父组件的clip为false，则认为其内的子组件可以超出其区域进行显示，因此超出父组件的区域也将被视为可见区域纳入计算；如果父组件的clip设置为true，则组件超出父组件的区域会被裁剪，无法显示，因此会被视为不可见区域进行计算。而当measureFromViewport设置为false时，则不考虑clip的影响，直接将组件超出父组件的部分视为不可见区域。 measureFromViewport设置为true时，祖先节点设置scale属性，组件可见比例会被正确计算。 |
...
此接口与[onVisibleAreaChange](组件可见区域变化事件.md#ZH-CN_TOPIC_0000002553360697__onvisibleareachange)接口存在如下差异：onVisibleAreaChange在每一帧都会进行可见区域比例的计算，如果注册节点太多，系统功耗存在劣化。而此接口降低了可见区域比例计算的频度，计算间隔由[VisibleAreaEventOptions](#ZH-CN_TOPIC_0000002553360697__visibleareaeventoptions12)的expectedUpdateInterval参数决定。
...
不支持[scale](图形变换.md#ZH-CN_TOPIC_0000002522080784__scale)属性，如果想要支持[scale](图形变换.md#ZH-CN_TOPIC_0000002522080784__scale)，则需将[VisibleAreaEventOptions](#ZH-CN_TOPIC_0000002553360697__visibleareaeventoptions12)的measureFromViewport设置为true。
...
| expectedUpdateInterval | number | 否 | 是 | 定义了开发者期望的计算间隔，单位为ms。 默认值：1000 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
| measureFromViewport22+ | boolean | 否 | 是 | 设置可见区域计算模式。 当measureFromViewport设置为true时，系统在计算该组件的可见区域时，会考虑父组件的clip 属性设置。如果父组件的clip为false，则认为其内的子组件可以超出其区域进行显示，因此超出父组件的区域也将被视为可见区域纳入计算；如果父组件的clip设置为true，则组件超出父组件的区域会被裁剪，无法显示，因此会被视为不可见区域进行计算。而当measureFromViewport设置为false时，则不考虑clip的影响，直接将组件超出父组件的部分视为不可见区域。 默认值：false measureFromViewport设置为true时，祖先节点设置scale属性，组件可见比例会被正确计算。 元服务API： 从API version 22开始，该接口支持在元服务中使用。 |
...
该示例对组件设置[onVisibleAreaChange](#ZH-CN_TOPIC_0000002553360697__onvisibleareachange)事件，当组件完全显示或者完全消失时触发回调。
...
从API version 17开始，该示例对组件设置[onVisibleAreaApproximateChange](#ZH-CN_TOPIC_0000002553360697__onvisibleareaapproximatechange17)事件，当组件完全显示或者完全消失时触发回调。
...
![image](public_sys-resources/zh-cn_image_0000002553204699.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522084746.webp)