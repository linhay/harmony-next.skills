...
该组件从API version 11开始默认支持安全区避让特性(默认值为：expandSafeArea([SafeAreaType.SYSTEM, SafeAreaType.KEYBOARD, SafeAreaType.CUTOUT], [SafeAreaEdge.TOP, SafeAreaEdge.BOTTOM]))，开发者可以重写该属性覆盖默认行为，API version 11之前的版本需配合[expandSafeArea](安全区域.md#ZH-CN_TOPIC_0000002553360705__expandsafearea)属性实现安全区避让。

-

[NavBar](#ZH-CN_TOPIC_0000002553360755__navbar12)嵌套使用Navigation时，内层NavDestination的生命周期不和外层NavDestination以及[全模态](全屏模态转场.md)的生命周期进行联动。

-

Navigation未设置主副标题（[title](#ZH-CN_TOPIC_0000002553360755__title)或[subTitle](#ZH-CN_TOPIC_0000002553360755__subtitledeprecated)）且[hideBackButton](#ZH-CN_TOPIC_0000002553360755__hidebackbutton)属性设置为true时，不显示标题栏。
...
不建议在[aboutToAppear](自定义组件的生命周期.md#ZH-CN_TOPIC_0000002553200861__abouttoappear)中使用栈操作，此时的页面还未构建完成，会导致白屏或跳转失败等问题。
...
从API version 10开始，推荐使用[NavPathStack](#ZH-CN_TOPIC_0000002553360755__navpathstack10)配合[navDestination](#ZH-CN_TOPIC_0000002553360755__navdestination10)属性进行页面路由。
...
绑定导航控制器到Navigation组件，适用于使用[NavPathStack](#ZH-CN_TOPIC_0000002553360755__navpathstack10)配合[navDestination](#ZH-CN_TOPIC_0000002553360755__navdestination10)属性进行页面路由。
...
绑定路由栈到Navigation组件，并且指定一个NavDestination作为Navigation的导航页（主页），适用于使用[NavPathStack](#ZH-CN_TOPIC_0000002553360755__navpathstack10)配合[navDestination](#ZH-CN_TOPIC_0000002553360755__navdestination10)属性或者系统路由表进行页面路由。使用示例参考[示例16（Navigation使用NavDestination作为导航页）](#ZH-CN_TOPIC_0000002553360755__示例16navigation使用navdestination作为导航页)。
...
设置页面右上角菜单。不设置时不显示菜单项。使用Array<[NavigationMenuItem](#ZH-CN_TOPIC_0000002553360755__navigationmenuitem)> 写法时，竖屏最多支持显示3个图标，横屏最多支持显示5个图标，多余的图标会被放入自动生成的更多图标。
...
设置页面右上角菜单。不设置时不显示菜单项。与[menus](#ZH-CN_TOPIC_0000002553360755__menus)相比，新增菜单选项。使用Array<[NavigationMenuItem](#ZH-CN_TOPIC_0000002553360755__navigationmenuitem)> 写法时，竖屏最多支持显示3个图标，横屏最多支持显示5个图标，多余的图标会被放入自动生成的更多图标。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | Array<ToolbarItem> | CustomBuilder | 是 | 工具栏内容，使用Array<ToolbarItem>设置的工具栏有如下特性： 工具栏所有选项均分底部工具栏，在每个均分内容区布局文本和图标。 竖屏模式最多支持显示5个图标，多余的图标会被放入自动生成的更多图标。横屏模式时，如果为Split模式，仍按照竖屏模式显示，如果为Stack模式需配合menus属性的Array<NavigationMenuItem>使用，底部工具栏会自动隐藏，同时底部工具栏所有选项移动至页面右上角菜单。 使用CustomBuilder写法为用户自定义工具栏选项，不具备以上功能。 |
| options11+ | NavigationToolbarOptions | 否 | 工具栏选项。 包含工具栏背景颜色、工具栏背景模糊样式及模糊选项、工具栏背景属性、工具栏布局方式、是否隐藏工具栏的文本、工具栏更多图标的菜单选项。 |
...
设置是否隐藏工具栏。与[hideToolBar](#ZH-CN_TOPIC_0000002553360755__hidetoolbar)相比，新增工具栏显隐时是否使用动画。
...
设置是否隐藏标题栏。与[hideTitleBar](#ZH-CN_TOPIC_0000002553360755__hidetitlebar)相比，新增标题栏显隐时是否使用动画。
...
设置是否隐藏标题栏中的返回键。返回键仅在[titleMode](#ZH-CN_TOPIC_0000002553360755__titlemode)设置为NavigationTitleMode.Mini时才生效。
...
设置导航页位置。仅在[mode](#ZH-CN_TOPIC_0000002553360755__mode9)设置为NavigationMode.Auto或NavigationMode.Split时生效。
...
组件想要扩展到非安全区域内，需隐藏或者设置标题栏和工具栏为[STACK](Navigation.md#ZH-CN_TOPIC_0000002553360755__barstyle12枚举说明)模式。
...
1. 不建议混合使用systemBarStyle属性和window设置状态栏样式的相关接口，例如：[setWindowSystemBarProperties](Interface (Window).md#ZH-CN_TOPIC_0000002522080752__setwindowsystembarproperties9)。
...
1. [Split](#ZH-CN_TOPIC_0000002553360755__navigationmode9枚举说明)模式下的Navigation，如果内容区没有NavDestination，则遵从Navigation首页的设置，反之则遵从栈顶NavDestination的设置。
...
1. 使用该接口需要先设置Navigation的通用属性[id](组件标识.md#ZH-CN_TOPIC_0000002522080774__id)，否则该接口无效。

1. 该接口需要配合NavDestination的[recoverable](NavDestination.md#ZH-CN_TOPIC_0000002522240830__recoverable14)接口使用。
...
设置是否启用Navigation和NavDestination的工具栏[toolbarConfiguration](#ZH-CN_TOPIC_0000002553360755__toolbarconfiguration10)自适应能力。关闭此能力后，底部工具栏[toolbarConfiguration](#ZH-CN_TOPIC_0000002553360755__toolbarconfiguration10)将不会再移动至页面右上角的菜单中。该接口不适配于自定义菜单，使用该接口需采用[NavigationMenuItem](#ZH-CN_TOPIC_0000002553360755__navigationmenuitem)接口来定义[菜单](#ZH-CN_TOPIC_0000002553360755__menus)。
...
设置是否启用[NavDestination](NavDestination.md)页面[onHidden](NavDestination.md#ZH-CN_TOPIC_0000002522240830__onhidden10)、[onShown](NavDestination.md#ZH-CN_TOPIC_0000002522240830__onshown10)生命周期与全模态的联动触发。
...
从API version 8开始支持，从API version 9开始废弃，建议使用[title](#ZH-CN_TOPIC_0000002553360755__title)替代。
...
从API version 8开始支持，从API version 10开始废弃，建议使用[toolbarConfiguration](#ZH-CN_TOPIC_0000002553360755__toolbarconfiguration10)替代。
...
当[titleMode](#ZH-CN_TOPIC_0000002553360755__titlemode)为NavigationTitleMode.Free时，随着可滚动组件的滑动标题栏模式发生变化时触发此回调。
...
从API version 12开始，NavPathStack允许被继承，派生类对象可以替代基类NavPathStack对象使用。使用示例参见[示例10](#ZH-CN_TOPIC_0000002553360755__示例10定义导航控制器派生类)。
...
将info指定的NavDestination页面信息入栈，具体根据options中指定不同的[LaunchMode](#ZH-CN_TOPIC_0000002553360755__launchmode12枚举说明)，来实现不同的行为。
...
将info指定的NavDestination页面信息入栈，使用Promise异步回调返回接口调用结果，具体根据options中指定不同的[LaunchMode](#ZH-CN_TOPIC_0000002553360755__launchmode12枚举说明)，来实现不同的行为。
...
替换路由栈操作，具体根据options中指定不同的[LaunchMode](#ZH-CN_TOPIC_0000002553360755__launchmode12枚举说明)，来实现不同的行为。
...
替换路由栈操作。使用Promise异步回调返回接口调用结果，具体根据options中指定不同的[LaunchMode](#ZH-CN_TOPIC_0000002553360755__launchmode12枚举说明)，来实现不同的行为。
...
将路由栈内指定navDestinationId的NavDestination页面删除。navDestinationId可以在NavDestination的[onReady](NavDestination.md#ZH-CN_TOPIC_0000002522240830__onready11)回调中获取，也可以在[NavDestinationInfo](@ohos.arkui.observer (无感监听).md#ZH-CN_TOPIC_0000002522240720__navdestinationinfo)中获取。
...
1. 开发者可以通过[NavPathInfo](#ZH-CN_TOPIC_0000002553360755__navpathinfo10)中的页面唯一标识符navDestinationId来操作已有页面，该id由系统默认生成且全局唯一（可以通过[getPathStack](#ZH-CN_TOPIC_0000002553360755__getpathstack19)接口获取，不可主动赋新值）。若该id在当前路由栈中不存在，则表示新增页面，若在当前路由栈中存在，同时对应的name相同，则表示复用已有页面。
...
| onPop11+ | Callback<PopInfo> | 否 | 是 | NavDestination页面触发pop、popToName、popToIndex时返回的回调。仅pop、popToName、popToIndex中设置result参数后触发。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
...
| navDestinationId19+ | string | 否 | 是 | NavDestination页面唯一标识符，该id由系统默认生成且全局唯一，通过getPathStack接口可读取，但不可以主动赋新值。 元服务API： 从API version 19开始，该接口支持在元服务中使用。 |
...
| interception22+ | InterceptionCallback | 否 | 是 | 页面跳转前的回调，允许操作栈，在当前跳转中生效。拦截的页面不会被创建。 元服务API： 从API version 22开始，该接口支持在元服务中使用。 |
...
| symbolIcon12+ | SymbolGlyphModifier | 否 | 是 | 菜单栏单个选项的symbol资源（优先级高于icon）。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
...
| symbolIcon12+ | SymbolGlyphModifier | 否 | 是 | 工具栏单个选项的symbol资源（优先级高于icon）。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
| activeSymbolIcon12+ | SymbolGlyphModifier | 否 | 是 | 工具栏单个选项处于ACTIVE态时的symbol资源（优先级高于activeIcon）。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
...
| SAFE_AREA_PADDING14+ | 2 | 将指定该模式的标题栏或工具栏设置为组件级安全区。 元服务API： 从API version 14开始，该接口支持在元服务中使用。 |
...
| paddingStart12+ | LengthMetrics | 否 | 是 | 标题栏起始端内间距。 仅支持以下任一场景： 1. 显示返回图标，即hideBackButton为false； 2. 使用非自定义标题，即标题value类型为ResourceStr或NavigationCommonTitle。 默认值： LengthMetrics.resource($r('sys.float.margin_left'))。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
| paddingEnd12+ | LengthMetrics | 否 | 是 | 标题栏结束端内间距。 仅支持以下任一场景： 1. 使用非自定义菜单，即菜单value为Array<NavigationMenuItem>； 2. 没有右上角菜单，且使用非自定义标题，即标题value类型为ResourceStr或NavigationCommonTitle。 默认值： LengthMetrics.resource($r('sys.float.margin_right')) 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
...
| moreButtonOptions19+ | MoreButtonOptions | 否 | 是 | 工具栏更多图标的菜单选项。 元服务API： 从API version 19开始，该接口支持在元服务中使用。 |
...
该示例主要演示Navigation页面的布局包括标题栏[title](#ZH-CN_TOPIC_0000002553360755__title)，菜单栏[menus](#ZH-CN_TOPIC_0000002553360755__menus)，内容区和工具栏[toolbarConfiguration](#ZH-CN_TOPIC_0000002553360755__toolbarconfiguration10)。
...
![image](public_sys-resources/zh-cn_image_0000002553204885.webp)

**示例2（使用导航控制器方法）**

该示例主要演示[NavPathStack](#ZH-CN_TOPIC_0000002553360755__navpathstack10)中方法的使用及路由拦截。
...
![image](public_sys-resources/zh-cn_image_0000002522084932.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553364847.webp)

**示例4（Navigation带参返回）**

该示例主要演示Navigation通过[NavPathStack](#ZH-CN_TOPIC_0000002553360755__navpathstack10)提供的接口来实现将设置的参数传给上一级页面。
...
![image](public_sys-resources/zh-cn_image_0000002522244926.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553204887.webp)

**示例6（嵌套场景下获取外层栈）**

该示例主要演示在嵌套Navigation场景下，如何获取父[NavPathStack](#ZH-CN_TOPIC_0000002553360755__navpathstack10)。
...
![image](public_sys-resources/zh-cn_image_0000002522084934.webp)
...
[NavPathStack](#ZH-CN_TOPIC_0000002553360755__navpathstack10)无需声明为状态变量，也可以实现路由栈操作功能。

1.

[NavDestination](NavDestination.md)通过[onReady](NavDestination.md#ZH-CN_TOPIC_0000002522240830__onready11)事件能够拿到对应的[NavPathInfo](#ZH-CN_TOPIC_0000002553360755__navpathinfo10)和所属的[NavPathStack](#ZH-CN_TOPIC_0000002553360755__navpathstack10)。
...
![image](public_sys-resources/zh-cn_image_0000002553364849.webp)

**示例8（NavDestination生命周期时序）**

该示例演示[NavDestination](NavDestination.md)的[onAppear](挂载卸载事件.md#ZH-CN_TOPIC_0000002522240770__onappear)，[onDisAppear](挂载卸载事件.md#ZH-CN_TOPIC_0000002522240770__ondisappear)，[onShown](NavDestination.md#ZH-CN_TOPIC_0000002522240830__onshown10)，[onHidden](NavDestination.md#ZH-CN_TOPIC_0000002522240830__onhidden10)，[onWillAppear](NavDestination.md#ZH-CN_TOPIC_0000002522240830__onwillappear12)，[onWillDisappear](NavDestination.md#ZH-CN_TOPIC_0000002522240830__onwilldisappear12)，[onWillShow](NavDestination.md#ZH-CN_TOPIC_0000002522240830__onwillshow12)，[onWillHide](NavDestination.md#ZH-CN_TOPIC_0000002522240830__onwillhide12)接口的生命周期时序。
...
![image](public_sys-resources/zh-cn_image_0000002522244928.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553204889.webp)

**示例10（定义导航控制器派生类）**

该示例主要演示如何定义[NavPathStack](#ZH-CN_TOPIC_0000002553360755__navpathstack10)的派生类和派生类在Navigation中的基本用法。
...
![image](public_sys-resources/zh-cn_image_0000002522084936.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553364851.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522244930.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553204891.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522084938.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553364853.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522244932.webp)

**示例17（使用新增导航控制器方法）**

该示例通过设置[setInterception](#ZH-CN_TOPIC_0000002553360755__setinterception12)方法来实现路由拦截功能，并在[NavDestinationContext](NavDestination.md#ZH-CN_TOPIC_0000002522240830__navdestinationcontext11)中获取mode。

从API version 22开始，在setInterception的参数类型[NavigationInterception](#ZH-CN_TOPIC_0000002553360755__navigationinterception12)中新增了interception接口。
...
![image](public_sys-resources/zh-cn_image_0000002553204893.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522084940.webp)