...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | PixelMap | 否 | 绘制目标，可将图形绘制在指定的PixelMap对象中，若未设置，则默认在当前绘制目标中进行绘制。 异常值undefined和null按照无效值处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | ViewportRect | 是 | Viewport绘制属性。 默认值：{ x: 0, y: 0, width: Shape组件的宽度, height: Shape组件的高度 } 异常值undefined按照默认值处理。 |
...
设置边框颜色，支持[attributeModifier](动态属性设置.md#ZH-CN_TOPIC_0000002522240800__attributemodifier)动态设置属性方法，不设置时，默认边框透明度为0，即没有边框。
...
设置边框间隙，支持[attributeModifier](动态属性设置.md#ZH-CN_TOPIC_0000002522240800__attributemodifier)动态设置属性方法。取值范围为≥0，异常值按照默认值处理。
...
设置网格效果。将图像分割为（row + 1）* (column + 1)的网格，每个网格交点坐标存储在数组中（每两个元素表示一个交点的x、y坐标）。通过数组value中的坐标值，重新定位网格顶点位置，实现图像局部扭曲。支持[attributeModifier](动态属性设置.md#ZH-CN_TOPIC_0000002522240800__attributemodifier)动态设置属性方法。

   ![image](public_sys-resources/note_3.0-zh-cn.webp)

mesh只对shape传入pixelMap时生效，且效果作用于传入的pixelMap。与[绘制模块](模块描述.md)的[drawPixelMapMesh12+](Class (Canvas).md#ZH-CN_TOPIC_0000002553362049__drawpixelmapmesh12)效果一致，建议使用drawPixelMapMesh。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | Array<any> | 是 | 长度（row + 1）* （column + 1）* 2的数组，记录扭曲后的位图各个顶点位置。 异常值undefined、null、NaN和Infinity按照无效值处理。 |
| column | number | 是 | mesh矩阵列数。 异常值NaN和Infinity按照无效值处理。 |
| row | number | 是 | mesh矩阵行数。 异常值NaN和Infinity按照无效值处理。 |
...
![image](public_sys-resources/zh-cn_image_0000002522245278.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553205241.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522085286.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553365203.webp)