...
FolderStack继承于[Stack](Stack.md)(层叠布局)控件，新增了[折叠屏悬停](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-folded-hover)能力，通过在FolderStack的配置项[FolderStackOptions](#ZH-CN_TOPIC_0000002553360833__folderstackoptions18对象说明)的upperItems数组上设置子组件id，使相应子组件自动避让折叠屏折痕区后移到上半屏。
...
当该组件的父组件为[if/else：条件渲染](../../guides/if-else：条件渲染.md)节点时，折叠屏悬停能力将会失效。
...
设置子组件在容器内的对齐方式。该属性与[align](位置设置.md#ZH-CN_TOPIC_0000002522240778__align)同时设置时，后设置的属性生效。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | boolean | 是 | 是否开启自动旋转。 默认值：true，设置true表示FolderStack在半折叠状态进行布局时开启自动旋转，设置false表示关闭自动旋转。该属性不区分设备类型。 非法值：按默认值处理。 |
...
![image](public_sys-resources/zh-cn_image_0000002553365295.webp)

图2 横屏半折叠

![image](public_sys-resources/zh-cn_image_0000002522245372.webp)
...
![image](public_sys-resources/zh-cn_image_0000002553205335.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522085382.webp)