...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| reasons | Array<ObscuredReasons> | 是 | 设置组件内容的遮罩类型。 默认值：[] 仅支持Image组件、Text组件的隐私遮罩处理。 说明： 如需在图片加载过程中显示隐私遮罩，需要设置Image组件的宽度和高度。 Text组件设置子组件或设置属性字符串时，不支持隐私遮罩。 |
...
![image](public_sys-resources/zh-cn_image_0000002522244750.webp)