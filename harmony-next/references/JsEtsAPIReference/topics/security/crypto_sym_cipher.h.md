...
| 名称 | 描述 |
| --- | --- |
| OH_Crypto_ErrCode OH_CryptoSymCipherParams_Create(OH_CryptoSymCipherParams **params) | 创建对称密钥加解密参数实例。  注意：创建的资源必须通过OH_CryptoSymCipherParams_Destroy销毁。 |
...
| OH_Crypto_ErrCode OH_CryptoSymCipher_Create(const char *algoName, OH_CryptoSymCipher **ctx) | 根据给定的算法名称创建对称密钥加解密实例。  注意：创建的资源必须通过OH_CryptoSymCipher_Destroy销毁。 |
...
| OH_Crypto_ErrCode OH_CryptoSymCipher_Update(OH_CryptoSymCipher *ctx, Crypto_DataBlob *in, Crypto_DataBlob *out) | 更新加密或者解密数据操作。  注意：使用完成后必须通过OH_Crypto_FreeDataBlob释放out内存。 |
| OH_Crypto_ErrCode OH_CryptoSymCipher_Final(OH_CryptoSymCipher *ctx, Crypto_DataBlob *in, Crypto_DataBlob *out) | 输出加/解密（分组模式产生的）剩余数据，最后结束加密或者解密数据操作。  注意：使用完成后必须通过OH_Crypto_FreeDataBlob释放out内存。 |
...
创建对称密钥加解密参数实例。
 ...
 注意：创建的资源必须通过[OH_CryptoSymCipherParams_Destroy](crypto_sym_cipher.h.md#ZH-CN_TOPIC_0000002553201307__oh_cryptosymcipherparams_destroy)销毁。
...
根据给定的算法名称创建对称密钥加解密实例。
 ...
 注意：创建的资源必须通过[OH_CryptoSymCipher_Destroy](crypto_sym_cipher.h.md#ZH-CN_TOPIC_0000002553201307__oh_cryptosymcipher_destroy)销毁。
...
[OH_CryptoSymCipher_Update](crypto_sym_cipher.h.md#ZH-CN_TOPIC_0000002553201307__oh_cryptosymcipher_update)

[OH_CryptoSymCipher_Final](crypto_sym_cipher.h.md#ZH-CN_TOPIC_0000002553201307__oh_cryptosymcipher_final)
...
[OH_CryptoSymCipher_Init](crypto_sym_cipher.h.md#ZH-CN_TOPIC_0000002553201307__oh_cryptosymcipher_init)
