...
以下错误码的详细介绍请参见[账号管理错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-account)和[ArkTS错误码](../../errors/ArkTS错误码 (account-api-error-code).md)。
...
开发者在使用获取手机号一致性状态接口前，需要完成quickLoginMobilePhone（华为账号一键登录）的scope权限申请，详情参见[开发前提](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-phone-unionid-login#section95093591227)。scope权限申请审批未完成或未通过，将报错[1001502014 应用未申请scopes或permissions权限](../../errors/ArkTS错误码.md#section78953611814)。
...
以下错误码的详细介绍请参见[ArkTS错误码](../../errors/ArkTS错误码.md)和[通用错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-universal)。
...
[1001500004](../../errors/ArkTS错误码.md#section13107132603718)

The account does not support this function.

[1001500005](../../errors/ArkTS错误码.md#section612754419379)
...
如果该值为false，且华为账号未登录，调用[executeRequest](#section3975181884311)将返回[1001502001 用户未登录华为账号](../../errors/ArkTS错误码.md#section539558125020)。
...
scope列表，用于获取用户数据。与permissions属性不能同时为空，否则会返回[1001502003 输入参数值无效](../../errors/ArkTS错误码.md#section289120911112)错误码。如果传入不合法的scope（例如空值等）则直接返回OpenID和UnionID。
...
- quickLoginAnonymousPhone：获取华为账号绑定的匿名手机号（该scope只能与openid同时使用，Wearable设备以及海外账号无法获取到手机号，会报[1001500003 不支持该scopes或permissions](../../errors/ArkTS错误码.md#section116574197436)），使用该scope前需要申请账号权限，具体请参考[开发前提](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-phone-unionid-login#section95093591227)。  说明：
...
permission列表，用于获取用户授权临时凭据和用户身份认证信息。与scopes属性不能同时为空，否则会返回[1001502003 输入参数值无效](../../errors/ArkTS错误码.md#section289120911112)错误码。如果传入不合法的permission（例如空值等）则直接返回OpenID和UnionID。
...
如果该值为false并且用户未登录，执行授权请求将返回[1001502001 用户未登录华为账号](../../errors/ArkTS错误码.md#section539558125020)。
...
如果该值为false，执行授权请求将返回[1001500003 不支持该scopes或permissions](../../errors/ArkTS错误码.md#section116574197436)。
...
用户获取的UnionID、OpenID值，传递的类型通过idType属性定义。不可为空，否则会报[1001502003 输入参数值无效](../../errors/ArkTS错误码.md#section289120911112)错误码。长度限制1-256。

UnionID、OpenID值可以通过[LoginWithHuaweiIDResponse](#section15454203811311)、[AuthorizationWithHuaweiIDResponse](#section10418450161620)、[LoginPanel](../components/LoginPanel (华为账号Panel登录组件).md#section72281234181611)或[LoginWithHuaweiIDButton](../components/LoginWithHuaweiIDButton (华为账号Button登录组件).md#section1624716107193)接口获取，具体方法参考其示例代码。
...
通过[LoginWithHuaweiIDButton](../components/LoginWithHuaweiIDButton (华为账号Button登录组件).md#section1624716107193)组件的一键登录功能获取到的手机号，传入完整的手机号需要添加国家码，例如中国境内（香港特别行政区、澳门特别行政区、中国台湾除外）为+86，值不可为空，长度限制1-256。
