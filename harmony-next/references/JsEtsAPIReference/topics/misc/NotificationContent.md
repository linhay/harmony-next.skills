...
名称类型只读可选说明briefTextstring否否通知概要内容，是对通知内容的总结（不可为空字符串，大小不超过1024字节，超出部分会被截断）。expandedTitlestring否否通知展开时的标题（不可为空字符串，大小不超过1024字节，超出部分会被截断）。picture[image.PixelMap](../../types/interfaces/Interface (PixelMap).md)否否通知的图片内容(图像像素的总字节数不能超过2MB)。
...
名称类型只读可选说明titlestring否是胶囊标题。大小不超过200字节，超出部分会被截断。icon[image.PixelMap](../../types/interfaces/Interface (PixelMap).md)否是胶囊图片。backgroundColorstring否是背景颜色。
...
名称类型只读可选说明namesArray<string>否是按钮名称（最多支持3个）。iconsArray<[image.PixelMap](../../types/interfaces/Interface (PixelMap).md)>否是按钮图片（最多支持3个）。iconsResource12+Array<[Resource](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resource)>否是按钮资源（最多支持3个）。
