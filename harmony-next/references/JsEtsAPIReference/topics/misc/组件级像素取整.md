...
参数名类型必填说明value[PixelRoundPolicy](#ZH-CN_TOPIC_0000002529284823__pixelroundpolicy)是
...
名称类型只读可选说明start[PixelRoundCalcPolicy](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__pixelroundcalcpolicy11)否是

组件前部边界取整对齐方式。

设置异常值时按四舍五入规则取整。

top[PixelRoundCalcPolicy](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__pixelroundcalcpolicy11)否是

组件上部边界取整对齐方式。

设置异常值时按四舍五入规则取整。

end[PixelRoundCalcPolicy](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__pixelroundcalcpolicy11)否是

组件尾部边界取整对齐方式。

设置异常值时按四舍五入规则取整。

bottom[PixelRoundCalcPolicy](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__pixelroundcalcpolicy11)否是
...
问题描述解决方法子容器100%填充父容器，在偏移量与大小刚好使父容器向上取整而子组件向下取整时，父容器会露出1px。
...
组件渲染时带有动画且有轻微抖动。关闭相应组件上的像素取整。容器内布局紧凑且子组件大小不一致。关闭相应组件上的像素取整。
