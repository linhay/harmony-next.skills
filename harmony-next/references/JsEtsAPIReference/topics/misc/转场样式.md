...
名称类型默认值描述shareidstring无进行共享元素转场时使用，若不配置，则转场样式不生效。共享元素转场当前支持的组件：[list-item](../components/list-item.md)、[image](../graphics/Image.md)、[text](../graphics/Text.md)、[button](../components/Button.md)、[label](label.md)。
