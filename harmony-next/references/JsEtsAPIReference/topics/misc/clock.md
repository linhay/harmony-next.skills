...
名称类型默认值必填描述clockconfigClockConfig-是
...
digitColor<color>#FF000000否日间时段（6:00-18:00）的表盘文本颜色。digitColorNight<color>与digitColor保持一致否
...
除支持[通用样式](通用样式.md)之外，还支持如下样式：

名称类型默认值必填描述font-family<string>sans-serif否表盘数字的字体列表，用逗号分隔，每个字体用字体名或者字体族名设置。列表中第一个系统中存在的或者通过2.1.6 自定义字体样式指定的字体，会被选中作为文本的字体。

 clock组件会保持显示区域的宽高比为1，最终正方形显示区域的边长为min(width, height)，在width * height的组件区域中居中显示。

#### 事件

名称参数描述hour{hour: number}每个整点触发该事件
