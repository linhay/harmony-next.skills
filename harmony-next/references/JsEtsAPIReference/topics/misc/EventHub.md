...
- 通过[sendableContextManager](../../modules/ohos/@ohos.app.ability.sendableContextManager (sendable上下文管理).md)转换后的Context对象与原先的Context对象属于不同Context对象，不支持其EventHub对象间的数据通信。
...
参数名类型必填说明eventstring是事件名称。callbackFunction是事件回调，事件触发后调用。
...
参数名类型必填说明eventstring是事件名称。...argsObject[]否可变参数，事件触发时，传递给回调函数的参数。
