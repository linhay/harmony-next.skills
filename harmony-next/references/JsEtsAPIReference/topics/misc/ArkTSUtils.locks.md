...
这是一个补充类型别名，表示[lockAsync](#ZH-CN_TOPIC_0000002522080698__lockasync)函数所有重载中的回调。
...
在获取的锁下执行操作。该方法首先获取锁，然后调用回调，最后释放锁。回调在调用[lockAsync](#ZH-CN_TOPIC_0000002522080698__lockasync)的同一线程中以异步方式执行。
...
在获取的锁下执行操作。该方法首先获取锁，然后调用回调，最后释放锁。回调在调用[lockAsync](#ZH-CN_TOPIC_0000002522080698__lockasync)的同一线程中以异步方式执行。在[AsyncLockOptions](#ZH-CN_TOPIC_0000002522080698__asynclockoptions)中可以提供一个可选的超时值。在这种情况下，如果超时前未能获取锁，lockAsync将返回被拒绝的Promise并带上一个BusinessError实例。这种情况下，错误信息将包含持有的锁和等待的锁的信息以及可能的死锁警告。
