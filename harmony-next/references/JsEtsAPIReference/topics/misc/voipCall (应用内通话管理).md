...
获取用户点击事件需使用[voipCall.on](#ZH-CN_TOPIC_0000002553202285__voipcallonvoipcalluievent)在业务流程开始时提前订阅voipCallUiEvent事件，可于业务流程结束后使用[voipCall.off](#ZH-CN_TOPIC_0000002553202285__voipcalloffvoipcalluievent)结束订阅。
...
| userProfile | image.PixelMap | 否 | 否 | 用户头像图片，需导入图像处理模块，详情见PixelMap。 支持传入的最大图片大小为 221 x 221 像素，推荐传入的图片大小为 112 x 112 像素。通过 PixelMap的getPixelBytesNumber接口获取到的图片大小要小于196608。 |
...
通知来电消息，如果应用来电消息建立失败，需调用[reportIncomingCallError](#ZH-CN_TOPIC_0000002553202285__voipcallreportincomingcallerror)通知来电建立失败。需设置通话详细信息，见[VoipCallAttribute](#ZH-CN_TOPIC_0000002553202285__voipcallattribute)。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| voipCallAttribute | VoipCallAttribute | 是 | 应用内通话详细信息如用户头像、用户昵称、通话唯一标识等，详情请参见VoipCallAttribute。（应用上报去电时，VoipCallAttribute中的voipCallState属性必须为VOIP_CALL_STATE_DIALING。） |
...
应用上报去电。需设置通话详细信息，见[VoipCallAttribute](#ZH-CN_TOPIC_0000002553202285__voipcallattribute)。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| voipCallAttribute | VoipCallAttribute | 是 | 应用内通话详细信息如用户头像、用户昵称、通话唯一标识等，详情请参见VoipCallAttribute。 |
...
| callAudioEvent | CallAudioEvent | 是 | VoIP开/关静音、扬声器事件，详情请参见CallAudioEvent。 |
...
该接口不能改变通话类型，例如，语音通话不能升级为视频通话，视频通话也不能降级为语音通话，如需上述升降级操作，请调用[voipCall.reportCallStateChange](#ZH-CN_TOPIC_0000002553202285__voipcallreportcallstatechange-1)。
...
| callState | VoipCallState | 是 | 通话状态，参考VoipCallState。 |
...
| callId | string | 是 | 呼叫ID。 |
| callState | VoipCallState | 是 | 通话状态，参考VoipCallState。 |
| callType | VoipCallType | 是 | 通话类型，参考VoipCallType。 |
