...
聚合查询请求类，继承Omit<[DataReadRequest](#section15461104913472), 'startTime' | 'endTime'>。
...
Partial<Record<keyof T, [AggregateMetricScope](#section246315450395)[]>>
...
[GroupOption](#section17334182555912)
...
调用方的应用的OAuth 2.0客户端ID(client_id)，若未填写，默认为调用方client_id。
...
调用方的应用名称，长度小于256字节。首次若未填写，默认为'CoreServiceExtAbility'，更新需要调用[updateDataSource](#section7156914939)接口。
...
调用方的应用版本信息，长度小于256字节。首次若未填写，默认为空，更新需要调用[updateDataSource](#section7156914939)接口。
...
授权请求参数类型，继承[AuthorizationBase](#section16391736104620)。
...
非数据类型权限，使用scope授权，请参见[OAuth权限](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/health-cloudsync#section1451346172216)，若未填写，默认为空。
...
授权响应返回类型，继承[AuthorizationBase](#section16391736104620)。
...
读取请求参数基类，继承[DataRequest](#section142361026204113)。
...
相对于当前位置的偏移，若未填写，默认为0，无偏移。

sortOrder

[SortOrder](#section1063613217259)
...
[DataSourceOptions](#section2546745691)

否

是

请求关联的数据源信息，若未填写，默认为无数据源限制。
...
数据源的标识，由平台生成，无法更改，长度小于256字节。

deviceInfo

[DeviceInfo](#section9562162482112)
...
[AppInfo](#section19505904120)

是

是

应用信息，若未填写，默认为调用方应用信息。
...
应用的OAuth 2.0客户端ID(client_id)，若未填写，默认为空。
...
数据源的唯一标识（dataSourceId与bundleName、deviceUniqueId不能同时填写）。
...
设备UniqueId（仅当未填写dataSourceId时可填写）。
...
HDK_BLOOD_SUGAR_MONITOR

'086'

血糖仪。

HDK_BLOOD_PRESSURE_MONITOR

'02B'

血压计。

HDK_HEART_RATE_MONITOR
...
设备名称，长度小于256字节。(插入数据源时必填)

category

[DeviceCategory](#section178320191510)

是

是

设备类型，需与productId匹配。(插入数据源时必填)
...
category: [DeviceCategory](#section178320191510).MANUAL_INPUT
...
[SubDataType](#section1928718141247)
...
统计数据，锻炼记录关联的统计数据类型参考[exerciseSequenceHelper](exerciseSequenceHelper(锻炼记录类型常量).md)定义的模型。
...
详情数据，锻炼记录关联的详情数据类型参考[exerciseSequenceHelper](exerciseSequenceHelper(锻炼记录类型常量).md)定义的模型，若未填写，默认为空。

#### ExerciseSequenceDeleteRequest

删除锻炼记录请求类，继承Omit<[DataRequest](#section142361026204113), 'startLocalDate' | 'endLocalDate'>。
...
[SubDataType](#section1928718141247) | [SubDataType](#section1928718141247)[] | null

否

否

锻炼记录子数据类型。若为空时，删除所有类型。
...
读取锻炼记录请求类，继承Omit<[DataReadRequest](#section15461104913472), 'startLocalDate' | 'endLocalDate'>。
...
[SequenceReadOptions](#section83641141804)<DK>

否

是

详情数据选项，若未填写，默认为不查询详情数据。
...
[HealthValueType](#section14966196131311) | [PaceValueType](#section1842384565915)
...
[GroupUnitType](#section6754124511337)
...
每个分组的单元数量，若未填写，默认为1，每个分组仅有一个单元，当前按天聚合只支持duration为1。
...
HealthSequence<K extends Record<string, [HealthValueType](#section14966196131311)> = Record<string, [HealthValueType](#section14966196131311)>,DK extends Record<string, [SequencePoint](#section244154911570)[]> = Record<string, [SequencePoint](#section244154911570)[]>>
...
统计数据，健康记录关联的统计数据类型参考[healthSequenceHelper](healthSequenceHelper(健康记录类型常量).md)定义的模型。
...
详情数据，健康记录关联的详情数据类型参考[healthSequenceHelper](healthSequenceHelper(健康记录类型常量).md)定义的模型，若未填写，默认为空。

#### HealthSequenceDeleteRequest

删除健康记录请求类，继承Omit<[DataRequest](#section142361026204113), 'startLocalDate' | 'endLocalDate'>。
...
healthSequenceDataType

[DataType](#section136147472298) | [DataType](#section136147472298)[]
...
读取健康记录请求类，继承Omit<[DataReadRequest](#section15461104913472), 'startLocalDate' | 'endLocalDate'>。
...
SamplePoint<K extends Record<string, [HealthValueType](#section14966196131311)> = Record<string, [HealthValueType](#section14966196131311)>>
...
数据的字段，数据类型对应的字段参考[samplePointHelper](samplePointHelper(采样数据类型常量).md)定义的模型。

#### SamplePointDeleteRequest

type SamplePointDeleteRequest = UnixTimeBasedDataDeleteRequest

删除数据采样点请求类型，继承Omit<[DataRequest](#section142361026204113), 'startLocalDate' | 'endLocalDate'>。
...
SamplePointReadRequest<FK extends Record<string, [HealthValueType](#section14966196131311)> = Record<string, [HealthValueType](#section14966196131311)>>

读取数据采样点请求类，继承Omit<[DataReadRequest](#section15461104913472), 'startLocalDate' | 'endLocalDate'>。
...
Partial<Pick<FK, keyof FK>>

否

是

要读取的字段列表，若samplePointDataType参数为数组，fields字段不能设置，数据类型对应的字段参考[samplePointHelper](samplePointHelper(采样数据类型常量).md)定义的模型。若为空，则用于读取所有字段。
...
[HealthValueType](#section14966196131311)
...
是否读取全部详情。true为读取全部详情，false为不读取详情，若未填写，则withPartialDetails参数生效。
...
读取部分详情数据类型（若需要读取部分详情，withDetails参数不能填写），锻炼记录与健康记录关联的详情数据类型分别参考[exerciseSequenceHelper](exerciseSequenceHelper(锻炼记录类型常量).md)与[healthSequenceHelper](healthSequenceHelper(健康记录类型常量).md)定义的模型。
...
子数据类型的名称，若未填写，默认匹配id对应的名称。

#### UnixTimeBasedDataDeleteRequest

基于Unix时间戳的删除请求基类，继承Omit<[DataRequest](#section142361026204113), 'startLocalDate' | 'endLocalDate'>。
...
上下文，目前只支持[UIAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-uiability)或[UIExtensionAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-uiextensionability)的上下文环境。
...
以下错误码的详细介绍请参见[运动健康服务ArkTS API错误码](../../errors/ArkTS API错误码.md)。

错误码ID

错误信息

[401](../../errors/ArkTS API错误码.md#section1598111146271)
...
[DataSourceBase](#section137003251628)
...
Promise对象，返回数据源唯一标识符（dataSourceId）。
...
[201](../../errors/ArkTS API错误码.md#section383228112410)
...
[1002700001](../../errors/ArkTS API错误码.md#section153755119333)

System internal error.

[1002700002](../../errors/ArkTS API错误码.md#section1210612513414)

Database processing error.

[1002701001](../../errors/ArkTS API错误码.md#section140917121405)

Network error. The network is unavailable.

[1002702001](../../errors/ArkTS API错误码.md#section673773124213)

Account error. The user has not logged in with HUAWEI ID.

[1002702002](../../errors/ArkTS API错误码.md#section1283610451436)

Account error. Failed to obtain account information with HUAWEI ID.

[1002703001](../../errors/ArkTS API错误码.md#section9390213122310)
...
[DataSourceReadRequest](#section13571101918281)
...
Promise<[DataSource](#section197195291918)[]>

Promise对象，返回[DataSource](#section197195291918)数组。
...
[DataSource](#section197195291918)
...
[SamplePoint](#section341712418323)[] | [SamplePoint](#section341712418323)
...
[ExerciseSequence](#section17704373316)[] | [ExerciseSequence](#section17704373316)
...
[HealthSequence](#section1038711561532)[] | [HealthSequence](#section1038711561532)
...
[SamplePointReadRequest](#section1419811172920)

是

读取数据采样点请求，查询时间跨度范围为31天。
...
Promise对象，T继承[SamplePoint](#section341712418323)，返回数据采样点数组。
...
[ExerciseSequenceReadRequest](#section17500162113363)

是

读取锻炼记录请求，查询时间跨度范围为31天。
...
Promise对象，T继承[ExerciseSequence](#section17704373316)，返回锻炼记录数组。
...
[HealthSequenceReadRequest](#section11615192919718)

是

读取健康记录请求，查询时间跨度范围为31天。
...
Promise对象，T继承[HealthSequence](#section1038711561532)，返回健康记录数组。
...
[SamplePointDeleteRequest](#section1680218365918) | [SamplePointDeleteRequest](#section1680218365918)[]
...
[ExerciseSequenceDeleteRequest](#section207708111867) | [ExerciseSequenceDeleteRequest](#section207708111867)[]
...
[HealthSequenceDeleteRequest](#section1779165141119) | [HealthSequenceDeleteRequest](#section1779165141119)[]
...
[AggregateRequest](#section599483722019) | [AggregateRequest](#section599483722019)[]
...
Promise对象，T继承[AggregateResult](#section19656142151412)，返回聚合查询结果数组。
...
[common.UIAbilityContext](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext#uiabilitycontext-1)
...
[AuthorizationRequest](#section1956619544498)

是

授权请求参数，确保授权参数中的权限已在申请运动健康服务时勾选。
...
Promise<[AuthorizationResponse](#section963483625117)>

Promise对象，返回授权响应结果。

**错误码：**

以下错误码的详细介绍请参见[运动健康服务ArkTS API错误码](../../errors/ArkTS API错误码.md)，其他错误码请参见[华为账号服务ArkTS错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-error-code)。
...
[801](../../errors/ArkTS API错误码.md#section14722630151713)
...
Health Service Kit进程状态的回调类型，支持的事件为：'serviceDie'，当Health Service Kit进程销毁时，触发该事件。
