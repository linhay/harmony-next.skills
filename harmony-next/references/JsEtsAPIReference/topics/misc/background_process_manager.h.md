...
相关模块： [BackgroundProcessManager](BackgroundProcessManager.md)
...
| 名称 | typedef关键字 | 描述 |
| --- | --- | --- |
| BackgroundProcessManager_ProcessPriority | BackgroundProcessManager_ProcessPriority | 子进程压制档位。 |
| BackgroundProcessManager_ErrorCode | BackgroundProcessManager_ErrorCode | 定义后台子进程管控错误码。 |
...
| BackgroundProcessManager_ProcessPriority priority | 压制档位。 |

返回：

| 类型 | 说明 |
| --- | --- |
| int | 返回 ERR_BACKGROUND_PROCESS_MANAGER_SUCCESS，表示压制参数发送成功。  返回 ERR_BACKGROUND_PROCESS_MANAGER_INVALID_PARAM，表示参数检查失败。 |
...
| 类型 | 说明 |
| --- | --- |
| int | 返回 ERR_BACKGROUND_PROCESS_MANAGER_SUCCESS，表示压制参数发送成功。 |