...
域名，仅在企业空间有值，例如“xx.com”，在个人空间中，其值为空。
...
工作空间域账号名称，仅在企业空间中有效，而在个人空间中则为空。
...
域中的账号标识符，仅在企业空间中有效，而在个人空间中则为空。其创建空间时，由系统自动生成的字符串，在[setWorkspaceInfo](#section15245021163318)中可设置为空。
...
工作空间所属域的服务器配置标识符。由创建空间时系统自动生成的，仅在企业空间中有效，例如“xx:test.com”，而在个人空间中则为空。

enterpriseWorkspaceName
...
企业空间名称，企业设备在某个空间下的标签属性。例如，该名称在企业空间为“企业空间”，在个人空间为“个人空间”。
...
工作空间ID。首个空间ID为100，后续创建的工作空间ID逐一递增，例如101、102。
...
工作空间的资料照片，是由本地照片的Base64格式拼接成的JSON字符串。“type”为照片类型，当前仅支持值为0；“defaultImg”为由照片转为Base64格式的字符串。例如，拼接后字符串为“{"type":0,"defaultImg":"data:image/png;base64,iVBO******Jggg==}”。
...
工作空间的最后登录时间，通常为系统生成的10位数字组成的时间戳。
...
[WorkspaceDomainInfo](#section1457564515610)

否

否

工作空间域信息，具体可参见[WorkspaceDomainInfo](#section1457564515610)。
...
不可访问的路径列表，列表只能包含如下列表的子集：['/data/service','/data/app','/storage','/mnt']
...
[EventType](#section3338122511597)

否

否

事件类型，取值可参见[EventType](#section3338122511597)。
...
关闭。关闭后不支持后台应用冻结和公共数据目录加解密。

EFFICIENT

1

效率模式。支持后台应用冻结和公共数据目录加解密。
...
[WorkspaceType](#section3637177173919)

是

工作空间的类型，取值可参见[WorkspaceType](#section3637177173919)。

params

[CreateWorkspaceParams](#section2818219505)

否

创建工作空间配置，具体可参见[CreateWorkspaceParams](#section2818219505)。
...
Promise<[WorkspaceInfo](#section1528202625516)>
...
[QueryType](#section87287517554)

是

工作空间类型，其取值可参见[QueryType](#section87287517554)。
...
设置的工作空间的资料照片，是由本地照片的Base64格式拼接成的JSON字符串。“type”为照片类型，当前仅支持值为0；“defaultImg”为由照片转为Base64格式的字符串。例如，拼接后字符串为“{"type":0,"defaultImg":"data:image/png;base64,iVBO******Jggg==}”。
...
AsyncCallback<[EventData](#section1831382917492)>

是

回调函数，当订阅空间事件成功，[EventData](#section1831382917492)为与空间事件相关的详细信息。
...
[UserDataEnum](#section8847145144416)
...
Promise<[ProcessConfigInfo](#section667711319120)[]>

Promise对象，表示当前配置的管控信息列表。
...
表示用户空间类型，其值参考[UserDataEnum](#section8847145144416)枚举值。
...
表示策略状态。当key为”lockdown“时，取值可参见[LockdownModePolicy](#section58124545010)枚举值。
...
工作空间ID。首个空间ID为100，后续创建的工作空间ID逐一递增，例如101、102。如果未设置，则默认使用调用者所在的工作空间ID。
...
Promise对象，策略状态，具体取值请参见[LockdownModePolicy](#section58124545010)枚举值。
...
表示属于深度冻结豁免名单的应用ID数组。列表，例如“[com.example.test1, com.example.test2]”。
...
表示属于豁免名单的空间ID。首个空间ID为100，后续创建的工作空间ID逐一递增，例如101、102。如果未设置，则默认使用调用者所在的工作空间ID。
