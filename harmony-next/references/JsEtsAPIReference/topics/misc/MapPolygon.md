...
多边形，继承[BaseOverlay](BaseOverlay.md)。多边形可以是凸面或凹面，它可以跨越180子午线并且可以具有未填充的孔。在调用map.[MapComponentController](MapComponentController.md)类的[addPolygon](MapComponentController.md#ZH-CN_TOPIC_0000002553362311__addpolygon)方法时会返回该类型的实例。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| holes | Array<Array<mapCommon.LatLng>> | 是 | 空心洞数组，其中空心洞是LatLng数组。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| jointType | mapCommon.JointType | 是 | 顶点样式，默认值为JointType.DEFAULT。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| patterns | Array<mapCommon.PatternItem> | 是 | PatternItem对象的数组。默认的边框样式为实心。 |
