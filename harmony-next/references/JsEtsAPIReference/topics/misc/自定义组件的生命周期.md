...
aboutToAppear函数在创建自定义组件的新实例后，在其build()函数执行前调用。允许在aboutToAppear函数中改变状态变量，更改将在后续执行build()函数中生效。实现[自定义布局](自定义组件的自定义布局.md)的自定义组件的aboutToAppear生命周期在布局过程中触发。
...
类型说明void | boolean返回按钮动作。返回true表示页面自己处理返回逻辑，不进行页面路由；返回false表示使用默认的路由返回逻辑，不设置返回值按照false处理。
...
该回调仅生效于由[@Entry](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-create-custom-components#entry)装饰的、作为[router路由](../../modules/ohos/@ohos.router (页面路由)(不推荐).md)页面存在的自定义组件。当之前存在于路由栈中的页面，通过[单实例模式](../../modules/ohos/@ohos.router (页面路由)(不推荐).md#ZH-CN_TOPIC_0000002529444757__routermode9)移动到栈顶时触发该回调。
...
参数名类型必填说明paramESObject是路由跳转时传递到目标页面的数据。
...
参数名类型必填说明paramsRecord<string, Object | undefined | null>是自定义组件的构造参数。
...
参数名类型必填说明theme[Theme](#ZH-CN_TOPIC_0000002529444913__theme12)是自定义组件当前生效的Theme对象。
...
类型说明[Theme](../../modules/ohos/@ohos.arkui.theme(主题换肤).md#ZH-CN_TOPIC_0000002529284765__theme)自定义组件当前生效的Theme对象。
...
onFormRecycle回调函数在卡片回收时执行，卡片提供方可以返回需要卡片管理服务代保存的数据，在卡片恢复时通过[onFormRecover](#ZH-CN_TOPIC_0000002529444913__onformrecover11)接口传给卡片提供方。
...
类型说明string返回卡片提供方需要卡片管理服务代保存的数据。
...
onFormRecover回调函数在卡片恢复时执行，卡片提供方可以拿到卡片回收时卡片管理服务代保存的数据，该数据可以通过[onFormRecycle](#ZH-CN_TOPIC_0000002529444913__onformrecycle11)卡片回收回调函数保存到卡片管理服务。
...
参数名类型必填说明statusDatastring是卡片回收时卡片管理服务代保存的数据。
