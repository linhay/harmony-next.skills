...
应用可以通过[receiveMessage](pushService（推送服务基础能力）.md#ZH-CN_TOPIC_0000002553202477__pushservicereceivemessage)()获取场景化消息数据的参数定义。
...
| data | string | 否 | 否 | 传递给Ability的数据。  type为'IM'时，参见data取值样例（格式化后）。  type为 'VoIP' | 'BACKGROUND' | 'EMERGENCY'时，参见data取值样例（格式化后）。  type为 'ALERT'时，无需额外配置data字段： notification完整字段参考AlertPayload 通知消息中[Notification](请求体参数说明.md#section17371529101117)结构体。 参见样例。 |
...
#### Remote[Notification](请求体参数说明.md#section17371529101117)Info

扩展通知数据，继承[PushPayload](#ZH-CN_TOPIC_0000002522242512__pushpayload)。
...
| --- | --- | --- | --- | --- |
| id | number | 否 | 否 | 通知id。 |
| type | string | 否 | 否 | 继承自PushPayload，表示传递给Remote[Notification](请求体参数说明.md#section17371529101117)ExtensionAbility的消息类型。 |
| data | string | 否 | 否 | 继承自PushPayload，表示传递给Remote[Notification](请求体参数说明.md#section17371529101117)ExtensionAbility的数据。 |

#### Remote[Notification](请求体参数说明.md#section17371529101117)Content
...
应用内通话消息数据，继承[PushPayload](#ZH-CN_TOPIC_0000002522242512__pushpayload)。
...
| type | string | 否 | 否 | 继承自PushPayload，表示传递给[VoIPExtensionAbility](VoIPExtensionAbility（应用内通话消息扩展Ability）.md)的消息类型。 |
| data | string | 否 | 否 | 继承自PushPayload，表示传递给[VoIPExtensionAbility](VoIPExtensionAbility（应用内通话消息扩展Ability）.md)的数据。 |