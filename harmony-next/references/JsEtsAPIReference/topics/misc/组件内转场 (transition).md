...
1. 如果在动画范围([animateTo](../../types/classes/Class (UIContext).md#ZH-CN_TOPIC_0000002529444749__animateto)、[animation](属性动画 (animation).md))内触发组件的上下树或可见性([Visibility](显隐控制.md))改变，而根组件没有配置transition，会给该组件加上默认透明度转场，即TransitionEffect.OPACITY，动画参数跟随所处动画环境的参数。如不需要可通过主动配置TransitionEffect.IDENTITY来禁用，使该组件直接出现或消失。
...
参数名类型必填说明type[Type](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__transitiontype)是转场类型。effect[Effect](#ZH-CN_TOPIC_0000002497604930__transitioneffect10对象说明)是转场参数。
...
名称类型只读可选说明type[TransitionType](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__transitiontype)否是
...
1. 当使用TransitionOptions类型的入参指定转场效果时，**必须**配合[animateTo](../../types/classes/Class (UIContext).md#ZH-CN_TOPIC_0000002529444749__animateto)使用才有动画效果，动效时长、曲线、延时跟随animateTo中的配置。
