...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| maxSize | Size | 否 | 否 | 最大尺寸。 |
| minSize | Size | 否 | 否 | 最小尺寸。 |
| percentReference | Size | 否 | 否 | 子节点计算百分比时的尺寸基准。 |
...
当UI状态发生变化时触发的回调。接收回调触发时的[UIState](#ZH-CN_TOPIC_0000002522240744__uistate20)状态，该参数的取值为UIState状态枚举值或其运算结果。
...
请参考[节点操作示例](#ZH-CN_TOPIC_0000002522240744__节点操作示例)。

**appendChild12+**

appendChild(node: FrameNode): void

在FrameNode最后一个子节点后添加新的子节点。当前FrameNode如果不可修改，抛出异常信息。[typeNode](#ZH-CN_TOPIC_0000002522240744__typenode12)在appendChild时会校验子组件类型或个数，不满足时抛出异常信息，限制情况请查看[typeNode](#ZH-CN_TOPIC_0000002522240744__typenode12)描述。
...
以下错误码的详细介绍请参见[自定义节点错误码](自定义节点错误码.md)。
...
请参考[LazyForEach场景节点操作示例](#ZH-CN_TOPIC_0000002522240744__lazyforeach场景节点操作示例)。
...
请参考[节点操作示例](#ZH-CN_TOPIC_0000002522240744__节点操作示例)和[获取根节点示例](#ZH-CN_TOPIC_0000002522240744__获取根节点示例)。
...
将当前FrameNode移动到目标FrameNode的指定位置。当前FrameNode如果不可修改，抛出异常信息。targetParent为[typeNode](#ZH-CN_TOPIC_0000002522240744__typenode12)时会校验子组件类型或个数，不满足时抛出异常信息，限制情况请查看[typeNode](#ZH-CN_TOPIC_0000002522240744__typenode12)描述。

   ![image](public_sys-resources/note_3.0-zh-cn.webp)

当前仅支持以下类型的[TypedFrameNode](#ZH-CN_TOPIC_0000002522240744__typedframenode12)进行移动操作：[Stack](#ZH-CN_TOPIC_0000002522240744__stack12)、[XComponent](#ZH-CN_TOPIC_0000002522240744__xcomponent12)。对于其他类型的节点，移动操作不会生效。

当前仅支持根节点为以下类型组件的[BuilderNode](BuilderNode.md#ZH-CN_TOPIC_0000002522080744__buildernode-1)进行移动操作：[Stack](Stack.md)、[XComponent](XComponent.md)、[EmbeddedComponent](EmbeddedComponent.md)。对于其他类型的组件，移动操作不会生效。
...
获取FrameNode相对于父组件带有绘制属性的位置偏移，单位为VP，绘制属性比如[transform](图形变换.md#ZH-CN_TOPIC_0000002522080784__transform), [translate](图形变换.md#ZH-CN_TOPIC_0000002522080784__translate)等，返回的坐标是组件布局时左上角变换后的坐标。
...
获取FrameNode相对于窗口带有绘制属性的位置偏移，单位为VP，绘制属性比如[transform](图形变换.md#ZH-CN_TOPIC_0000002522080784__transform), [translate](图形变换.md#ZH-CN_TOPIC_0000002522080784__translate)等，返回的坐标是组件布局时左上角变换后的坐标。
...
获取FrameNode相对于屏幕带有绘制属性的位置偏移，单位为VP，绘制属性比如[transform](图形变换.md#ZH-CN_TOPIC_0000002522080784__transform), [translate](图形变换.md#ZH-CN_TOPIC_0000002522080784__translate)等，返回的坐标是组件布局时左上角变换后的坐标。
...
获取用户设置的节点ID（通用属性设置的[ID](组件标识.md)）。
...
获取节点是否是剪裁到组件区域。当调用[dispose](#ZH-CN_TOPIC_0000002522240744__dispose12)解除对实体FrameNode节点的引用关系之后，返回值为true。
...
请参考[检验FrameNode是否有效示例](#ZH-CN_TOPIC_0000002522240744__检验framenode是否有效示例)。
...
以查询[Button](Button.md)组件节点为例获取到的Object结果部分值如下。
...
FrameNode对象调用dispose后，由于不对应任何实体FrameNode节点，在调用部分查询接口([getMeasuredSize](#ZH-CN_TOPIC_0000002522240744__getmeasuredsize12)、[getLayoutPosition](#ZH-CN_TOPIC_0000002522240744__getlayoutposition12))的时候会导致应用出现jscrash。

-

通过[getUniqueId](#ZH-CN_TOPIC_0000002522240744__getuniqueid12)可以判断当前FrameNode是否对应一个实体FrameNode节点。当UniqueId大于0时表示该对象对应一个实体FrameNode节点。
...
![image](public_sys-resources/zh-cn_image_0000002522084722.webp)

**commonAttribute12+**

get commonAttribute(): CommonAttribute

获取FrameNode中持有的CommonAttribute接口，用于设置[通用属性](通用属性.md)和[通用事件](通用事件.md)。

仅可以修改自定义节点的属性。

   ![image](public_sys-resources/note_3.0-zh-cn.webp)

FrameNode的效果参考对齐方式为顶部起始端的[Stack](Stack.md)容器组件。
...
请参考[基础事件示例](#ZH-CN_TOPIC_0000002522240744__基础事件示例)。
...
请参考[基础事件示例](#ZH-CN_TOPIC_0000002522240744__基础事件示例)和[LazyForEach场景基础事件使用示例](#ZH-CN_TOPIC_0000002522240744__lazyforeach场景基础事件使用示例)。

**gestureEvent14+**

get gestureEvent(): UIGestureEvent

获取FrameNode中持有的UIGestureEvent对象，用于设置组件绑定的手势事件。通过gestureEvent接口设置的手势不会覆盖通过[声明式手势接口](绑定手势方法.md)绑定的手势，两者同时设置了手势时，优先回调声明式接口设置的手势事件。
...
请参考[手势事件示例](#ZH-CN_TOPIC_0000002522240744__手势事件示例)。
...
该接口的[DrawContext](Graphics.md#ZH-CN_TOPIC_0000002553200709__drawcontext)中的Canvas是用于记录指令的临时Canvas，并非节点的真实Canvas。使用请参见[调整自定义绘制Canvas的变换矩阵](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-user-defined-arktsnode-framenode#调整自定义绘制canvas的变换矩阵)。
...
请参考[节点自定义示例](#ZH-CN_TOPIC_0000002522240744__节点自定义示例)。
...
调用FrameNode的测量方法，根据父容器的布局约束，对FrameNode进行测量，计算出尺寸，如果测量方法被重写，则调用重写的方法。建议在[onMeasure](#ZH-CN_TOPIC_0000002522240744__onmeasure12)方法中调用。
...
调用FrameNode的布局方法，为FrameNode及其子节点指定布局位置，如果布局方法被重写，则调用重写的方法。建议在[onLayout](#ZH-CN_TOPIC_0000002522240744__onlayout12)方法中调用。
...
支持添加ComponentContent类型的组件内容。要求当前节点是一个可修改的节点，即[isModifiable](#ZH-CN_TOPIC_0000002522240744__ismodifiable12)的返回值为true，否则抛出异常信息。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| content | ComponentContent<T> | ReactiveComponentContent<T>22+ | 是 | FrameNode节点中显示的组件内容。 |
...
![image](public_sys-resources/zh-cn_image_0000002553364635.webp)
...
当前仅支持[Scroll](#ZH-CN_TOPIC_0000002522240744__scroll12), [Swiper](#ZH-CN_TOPIC_0000002522240744__swiper12)，[List](#ZH-CN_TOPIC_0000002522240744__list12)，[ListItem](#ZH-CN_TOPIC_0000002522240744__listitem12)，[ListItemGroup](#ZH-CN_TOPIC_0000002522240744__listitemgroup12)，[WaterFlow](#ZH-CN_TOPIC_0000002522240744__waterflow12)，[FlowItem](#ZH-CN_TOPIC_0000002522240744__flowitem12)，[Grid](#ZH-CN_TOPIC_0000002522240744__grid14)，[GridTime](#ZH-CN_TOPIC_0000002522240744__griditem14)，[TextInput](#ZH-CN_TOPIC_0000002522240744__textinput12)，[TextArea](#ZH-CN_TOPIC_0000002522240744__textarea14)，[Column](#ZH-CN_TOPIC_0000002522240744__column12)，[Row](#ZH-CN_TOPIC_0000002522240744__row12)，[Stack](#ZH-CN_TOPIC_0000002522240744__stack12)，[Flex](#ZH-CN_TOPIC_0000002522240744__flex12)，[RelativeContainer](#ZH-CN_TOPIC_0000002522240744__relativecontainer12)，[Progress](#ZH-CN_TOPIC_0000002522240744__progress12)，[LoadingProgress](#ZH-CN_TOPIC_0000002522240744__loadingprogress12)，[Image](#ZH-CN_TOPIC_0000002522240744__image12)，[Button](#ZH-CN_TOPIC_0000002522240744__button12)，[CheckBox](#ZH-CN_TOPIC_0000002522240744__checkbox18)，[Radio](#ZH-CN_TOPIC_0000002522240744__radio18)，[Slider](#ZH-CN_TOPIC_0000002522240744__slider18)，[Toggle](#ZH-CN_TOPIC_0000002522240744__toggle18)，[XComponent](#ZH-CN_TOPIC_0000002522240744__xcomponent12)类型的[TypedFrameNode](#ZH-CN_TOPIC_0000002522240744__typedframenode12)设置跨ArkTS语言访问选项。
...
请参考[节点复用回收使用示例](#ZH-CN_TOPIC_0000002522240744__节点复用回收使用示例)。
...
请参考[组件设置和删除多态样式状态示例](#ZH-CN_TOPIC_0000002522240744__组件设置和删除多态样式状态示例)。
...
完整示例请参考[接纳为附属节点示例](#ZH-CN_TOPIC_0000002522240744__接纳为附属节点示例)。
...
请参考[局部与窗口坐标转化示例](#ZH-CN_TOPIC_0000002522240744__局部与窗口坐标转化示例)。
...
TypedFrameNode继承自[FrameNode](#ZH-CN_TOPIC_0000002522240744__framenode-1)，用于声明具体类型的FrameNode。
...
[commonAttribute](#ZH-CN_TOPIC_0000002522240744__commonattribute12)仅在CustomFrameNode上生效，TypedFrameNode上commonAttribute行为未定义。建议使用[attribute](#ZH-CN_TOPIC_0000002522240744__属性)接口而非[commonAttribute](#ZH-CN_TOPIC_0000002522240744__commonattribute12)接口进行通用属性设置，如node.attribute.backgroundColor(Color.Pink)。

**typeNode12+**

typeNode提供创建具体类型的FrameNode能力，可通过FrameNode的基础接口进行自定义的挂载，使用占位容器进行显示。

使用typeNode创建[Text](Text.md)、[Image](Image.md)、[Select](Select.md)、[Toggle](Toggle.md)节点时，当传入的[UIContext](Class (UIContext).md)对应的UI实例销毁后，调用该接口会返回一个无效的FrameNode节点，无法正常挂载和显示。

示例：

请参考[自定义具体类型节点示例](#ZH-CN_TOPIC_0000002522240744__自定义具体类型节点示例)。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<TextInterface, TextAttribute> | 提供Text类型FrameNode节点。 TextInterface用于TypedFrameNode的initialize接口的入参，入参为Text组件的构造函数类型。 TextAttribute用于TypedFrameNode的attribute接口的返回值，返回Text组件的属性设置对象。 |
...
将文本控制器[TextController](Text.md#ZH-CN_TOPIC_0000002553360767__textcontroller11)绑定到[Text](#ZH-CN_TOPIC_0000002522240744__text12)节点。若该节点非ArkTS语言创建，则需要设置是否支持跨语言访问，如果不支持跨语言访问，则抛出异常。该接口不支持声明式方式创建的节点。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<ColumnInterface, ColumnAttribute> | 提供Column类型FrameNode节点。 ColumnInterface用于TypedFrameNode的initialize接口的入参，入参为Column组件的构造函数类型。 ColumnAttribute用于TypedFrameNode的attribute接口的返回值，返回Column组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<RowInterface, RowAttribute> | 提供Row类型FrameNode节点。 RowInterface用于TypedFrameNode的initialize接口的入参，入参为Row组件的构造函数类型。 RowAttribute用于TypedFrameNode的attribute接口的返回值，返回Row组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<StackInterface, StackAttribute> | 提供Stack类型FrameNode节点。 StackInterface用于TypedFrameNode的initialize接口的入参，入参为Stack组件的构造函数类型。 StackAttribute用于TypedFrameNode的attribute接口的返回值，返回Stack组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<GridRowInterface, GridRowAttribute> | 提供GridRow类型FrameNode节点。 GridRowInterface用于TypedFrameNode的initialize接口的入参，入参为GridRow组件的构造函数类型。 GridRowAttribute用于TypedFrameNode的attribute接口的返回值，返回GridRow组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<GridColInterface, GridColAttribute> | 提供GridCol类型FrameNode节点。 GridColInterface用于TypedFrameNode的initialize接口的入参，入参为GridCol组件的构造函数类型。 GridColAttribute用于TypedFrameNode的attribute接口的返回值，返回GridCol组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<FlexInterface, FlexAttribute> | 提供Flex类型FrameNode节点。 FlexInterface用于TypedFrameNode的initialize接口的入参，入参为Flex组件的构造函数类型。 FlexAttribute用于TypedFrameNode的attribute接口的返回值，返回Flex组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<SwiperInterface, SwiperAttribute> | 提供Swiper类型FrameNode节点。 SwiperInterface用于TypedFrameNode的initialize接口的入参，入参为Swiper组件的构造函数类型。 SwiperAttribute用于TypedFrameNode的attribute接口的返回值，返回Swiper组件的属性设置对象。 |
...
请参考[createNode('Swiper')12+示例](#ZH-CN_TOPIC_0000002522240744__createnodeswiper12)。

**bindController('Swiper')20+**

bindController(node: FrameNode, controller: SwiperController, nodeType: 'Swiper'): void

将控制器[SwiperController](Swiper.md#ZH-CN_TOPIC_0000002522240824__swipercontroller)绑定到[Swiper](#ZH-CN_TOPIC_0000002522240744__swiper12)节点。若该节点非ArkTS语言创建，则需要设置是否支持跨语言访问，如果不支持跨语言访问，则抛出异常。该接口不支持声明式方式创建的节点。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<ProgressInterface, ProgressAttribute> | 提供Progress类型FrameNode节点。 ProgressInterface用于TypedFrameNode的initialize接口的入参，入参为Progress组件的构造函数类型。 ProgressAttribute用于TypedFrameNode的attribute接口的返回值，返回Progress组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<ScrollInterface, ScrollAttribute> | 提供Scroll类型FrameNode节点。 ScrollInterface用于TypedFrameNode的initialize接口的入参，入参为Scroll组件的构造函数类型。 ScrollAttribute用于TypedFrameNode的attribute接口的返回值，返回Scroll组件的属性设置对象。 |
...
完整示例请参考[createNode('Scroll')](#ZH-CN_TOPIC_0000002522240744__createnodescroll12)的示例。
...
完整示例请参考[滚动事件示例](#ZH-CN_TOPIC_0000002522240744__滚动事件示例)。

**bindController('Scroll')15+**

bindController(node: FrameNode, controller: Scroller, nodeType: 'Scroll'): void

将滚动控制器[Scroller](Scroll.md#ZH-CN_TOPIC_0000002553360749__scroller)绑定到[Scroll](#ZH-CN_TOPIC_0000002522240744__scroll12)节点。若该节点非ArkTS语言创建，则需要设置是否支持跨语言访问，如果不支持跨语言访问，则抛出异常。该接口不支持声明式方式创建的节点。
...
以下错误码的详细介绍请参见[自定义节点错误码](自定义节点错误码.md)和[通用错误码](通用错误码.md)。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<RelativeContainerInterface, RelativeContainerAttribute> | 提供RelativeContainer类型FrameNode节点。 RelativeContainerInterface用于TypedFrameNode的initialize接口的入参，入参为RelativeContainer组件的构造函数类型。 RelativeContainerAttribute用于TypedFrameNode的attribute接口的返回值，返回RelativeContainer组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<DividerInterface, DividerAttribute> | 提供Divider类型FrameNode节点。 DividerInterface用于TypedFrameNode的initialize接口的入参，入参为RelativeContainer组件的构造函数类型。 DividerAttribute用于TypedFrameNode的attribute接口的返回值，返回Divider组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<LoadingProgressInterface, LoadingProgressAttribute> | 提供LoadingProgress类型FrameNode节点。 LoadingProgressInterface用于TypedFrameNode的initialize接口的入参，入参为LoadingProgress组件的构造函数类型。 LoadingProgressAttribute用于TypedFrameNode的attribute接口的返回值，返回LoadingProgress组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<SearchInterface, SearchAttribute> | 提供Search类型FrameNode节点。 SearchInterface用于TypedFrameNode的initialize接口的入参，入参为Search组件的构造函数类型。 SearchAttribute用于TypedFrameNode的attribute接口的返回值，返回Search组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<BlankInterface, BlankAttribute> | 提供Blank类型FrameNode节点。 BlankInterface用于TypedFrameNode的initialize接口的入参，入参为Blank组件的构造函数类型。 BlankAttribute用于TypedFrameNode的attribute接口的返回值，返回Blank组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<ImageInterface, ImageAttribute> | 提供Image类型FrameNode节点。 ImageInterface用于TypedFrameNode的initialize接口的入参，入参为Image组件的构造函数类型。 ImageAttribute用于TypedFrameNode的attribute接口的返回值，返回Image组件的属性设置对象。 |
...
List类型的FrameNode节点类型。只允许添加[ListItem](#ZH-CN_TOPIC_0000002522240744__listitem12)、[ListItemGroup](#ZH-CN_TOPIC_0000002522240744__listitemgroup12)类型子组件。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<ListInterface, ListAttribute> | 提供List类型FrameNode节点。 ListInterface用于TypedFrameNode的initialize接口的入参，入参为List组件的构造函数类型。 ListAttribute用于TypedFrameNode的attribute接口的返回值，返回List组件的属性设置对象。 |
...
完整示例请参考[createNode('List')](#ZH-CN_TOPIC_0000002522240744__createnodelist12)的示例。

**bindController('List')20+**

bindController(node: FrameNode, controller: Scroller, nodeType: 'List'): void

将滚动控制器[Scroller](Scroll.md#ZH-CN_TOPIC_0000002553360749__scroller)绑定到[List](#ZH-CN_TOPIC_0000002522240744__list12)节点。若该节点非ArkTS语言创建，则需要设置是否支持跨语言访问，如果不支持跨语言访问，则抛出异常。该接口不支持声明式方式创建的节点。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<ListItemInterface, ListItemAttribute> | 提供ListItem类型FrameNode节点。 ListItemInterface用于TypedFrameNode的initialize接口的入参，入参为ListItem组件的构造函数类型。 ListItemAttribute用于TypedFrameNode的attribute接口的返回值，返回ListItem组件的属性设置对象。 |
...
参考[createNode('List')](#ZH-CN_TOPIC_0000002522240744__createnodelist12)示例。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<TextInputInterface, TextInputAttribute> | 提供TextInput类型FrameNode节点。 TextInputInterface用于TypedFrameNode的initialize接口的入参，入参为TextInput组件的构造函数类型。 TextInputAttribute用于TypedFrameNode的attribute接口的返回值，返回TextInput组件的属性设置对象。 |
...
将输入框控制器[TextInputController](TextInput.md#ZH-CN_TOPIC_0000002553200807__textinputcontroller8)绑定到[TextInput](#ZH-CN_TOPIC_0000002522240744__textinput12)节点。若该节点非ArkTS语言创建，则需要设置是否支持跨语言访问，如果不支持跨语言访问，则抛出异常。该接口不支持声明式方式创建的节点。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<ListItemGroupInterface, ListItemGroupAttribute> | 提供ListItemGroup类型FrameNode节点。 ListItemGroupInterface用于TypedFrameNode的initialize接口的入参，入参为ListItemGroup组件的构造函数类型。 ListItemGroupAttribute用于TypedFrameNode的attribute接口的返回值，返回ListItemGroup组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<WaterFlowInterface, WaterFlowAttribute> | 提供WaterFlow类型FrameNode节点。 WaterFlowInterface用于TypedFrameNode的initialize接口的入参，入参为WaterFlow组件的构造函数类型。 WaterFlowAttribute用于TypedFrameNode的attribute接口的返回值，返回WaterFlow组件的属性设置对象。 |
...
获取[WaterFlow](#ZH-CN_TOPIC_0000002522240744__waterflow12)节点中持有的UIWaterFlowEvent对象，用于设置滚动事件。设置的滚动事件与声明式定义的事件平行；设置的滚动事件不覆盖原有的声明式事件。同时设置两个事件回调的时候，优先回调声明式事件。
...
完整示例请参考[createNode('WaterFlow')](#ZH-CN_TOPIC_0000002522240744__createnodewaterflow12)的示例。

**bindController('WaterFlow')20+**

bindController(node: FrameNode, controller: Scroller, nodeType: 'WaterFlow'): void

将滚动控制器[Scroller](Scroll.md#ZH-CN_TOPIC_0000002553360749__scroller)绑定到[WaterFlow](#ZH-CN_TOPIC_0000002522240744__waterflow12)节点。若该节点非ArkTS语言创建，则需要设置是否支持跨语言访问，如果不支持跨语言访问，则抛出异常。该接口不支持声明式方式创建的节点。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<FlowItemInterface, FlowItemAttribute> | 提供FlowItem类型FrameNode节点。 FlowItemInterface用于TypedFrameNode的initialize接口的入参，入参为FlowItem组件的构造函数类型。 FlowItemAttribute用于TypedFrameNode的attribute接口的返回值，返回FlowItem组件的属性设置对象。 |
...
参考[createNode('WaterFlow')](#ZH-CN_TOPIC_0000002522240744__createnodewaterflow12)示例。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<XComponentInterface, XComponentAttribute> | 提供XComponent类型FrameNode节点。 XComponentInterface用于TypedFrameNode的initialize接口的入参，入参为XComponent组件的构造函数类型。 XComponentAttribute用于TypedFrameNode的attribute接口的返回值，返回XComponent组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<QRCodeInterface, QRCodeAttribute> | 提供QRCode类型FrameNode节点。 QRCodeInterface用于TypedFrameNode的initialize接口的入参，入参为QRCode组件的构造函数类型。 QRCodeAttribute用于TypedFrameNode的attribute接口的返回值，返回QRCode组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<BadgeInterface, BadgeAttribute> | 提供Badge类型FrameNode节点。 BadgeInterface用于TypedFrameNode的initialize接口的入参，入参为Badge组件的构造函数类型。 BadgeAttribute用于TypedFrameNode的attribute接口的返回值，返回Badge组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<GridInterface, GridAttribute> | 提供Grid类型FrameNode节点。 GridInterface用于TypedFrameNode的initialize接口的入参，入参为Grid组件的构造函数类型。 GridAttribute用于TypedFrameNode的attribute接口的返回值，返回Grid组件的属性设置对象。 |
...
完整示例请参考[createNode('Grid')](#ZH-CN_TOPIC_0000002522240744__createnodegrid14)的示例。

**bindController('Grid')20+**

bindController(node: FrameNode, controller: Scroller, nodeType: 'Grid'): void

将滚动控制器[Scroller](Scroll.md#ZH-CN_TOPIC_0000002553360749__scroller)绑定到[Grid](#ZH-CN_TOPIC_0000002522240744__grid14)节点。若该节点非ArkTS语言创建，则需要设置是否支持跨语言访问，如果不支持跨语言访问，则抛出异常。该接口不支持声明式方式创建的节点。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<GridItemInterface, GridItemAttribute> | 提供GridItem类型FrameNode节点。 GridItemInterface用于TypedFrameNode的initialize接口的入参，入参为GridItem组件的构造函数类型。 GridItemAttribute用于TypedFrameNode的attribute接口的返回值，返回GridItem组件的属性设置对象。 |
...
参考[createNode('Grid')](#ZH-CN_TOPIC_0000002522240744__createnodegrid14)示例。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<TextClockInterface, TextClockAttribute> | 提供TextClock类型FrameNode节点。 TextClockInterface用于TypedFrameNode的initialize接口的入参，入参为TextClock组件的构造函数类型。 TextClockAttribute用于TypedFrameNode的attribute接口的返回值，返回TextClock组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<TextTimerInterface, TextTimerAttribute> | 提供TextTimer类型FrameNode节点。 TextTimerInterface用于TypedFrameNode的initialize接口的入参，入参为TextTimer组件的构造函数类型。 TextTimerAttribute用于TypedFrameNode的attribute接口的返回值，返回TextTimer组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<MarqueeInterface, MarqueeAttribute> | 提供Marquee类型FrameNode节点。 MarqueeInterface用于TypedFrameNode的initialize接口的入参，入参为Marquee组件的构造函数类型。 MarqueeAttribute用于TypedFrameNode的attribute接口的返回值，返回Marquee组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<TextAreaInterface, TextAreaAttribute> | 提供TextArea类型FrameNode节点。 TextAreaInterface用于TypedFrameNode的initialize接口的入参，入参为TextArea组件的构造函数类型。 TextAreaAttribute用于TypedFrameNode的attribute接口的返回值，返回TextArea组件的属性设置对象。 |
...
将输入框控制器[TextAreaController](TextArea.md#ZH-CN_TOPIC_0000002522240842__textareacontroller8)绑定到[TextArea](#ZH-CN_TOPIC_0000002522240744__textarea14)节点。若该节点非ArkTS语言创建，则需要设置是否支持跨语言访问，如果不支持跨语言访问，则抛出异常。该接口不支持声明式方式创建的节点。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<SymbolGlyphInterface, SymbolGlyphAttribute> | 提供SymbolGlyph类型FrameNode节点。 SymbolGlyphInterface用于TypedFrameNode的initialize接口的入参，入参为SymbolGlyph组件的构造函数类型。 SymbolGlyphAttribute用于TypedFrameNode的attribute接口的返回值，返回SymbolGlyph组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<CheckboxInterface, CheckboxAttribute> | 提供Checkbox类型FrameNode节点。 CheckboxInterface用于TypedFrameNode的initialize接口的入参，入参为Checkbox组件的构造函数类型。 CheckboxAttribute用于TypedFrameNode的attribute接口的返回值，返回Checkbox组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<CheckboxGroupInterface, CheckboxGroupAttribute> | 提供CheckboxGroup类型FrameNode节点。 CheckboxGroupInterface用于TypedFrameNode的initialize接口的入参，入参为CheckboxGroup组件的构造函数类型。 CheckboxGroupAttribute用于TypedFrameNode的attribute接口的返回值，返回CheckboxGroup组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<RatingInterface, RatingAttribute> | 提供Rating类型FrameNode节点。 RatingInterface用于TypedFrameNode的initialize接口的入参，入参为Rating组件的构造函数类型。 RatingAttribute用于TypedFrameNode的attribute接口的返回值，返回Rating组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<RadioInterface, RadioAttribute> | 提供Radio类型FrameNode节点。 RadioInterface用于TypedFrameNode的initialize接口的入参，入参为Radio组件的构造函数类型。 RadioAttribute用于TypedFrameNode的attribute接口的返回值，返回Radio组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<SliderInterface, SliderAttribute> | 提供Slider类型FrameNode节点。 SliderInterface用于TypedFrameNode的initialize接口的入参，入参为Slider组件的构造函数类型。 SliderAttribute用于TypedFrameNode的attribute接口的返回值，返回Slider组件的属性设置对象。 |
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<SelectInterface, SelectAttribute> | 提供Select类型FrameNode节点。 SelectInterface用于TypedFrameNode的initialize接口的入参，入参为Select组件的构造函数类型。 SelectAttribute用于TypedFrameNode的attribute接口的返回值，返回Select组件的属性设置对象。 |
...
type Toggle = TypedFrameNode<[ToggleInterface](Toggle.md#ZH-CN_TOPIC_0000002553360759__接口), [ToggleAttribute](Toggle.md#ZH-CN_TOPIC_0000002553360759__属性)>

[Toggle](Toggle.md)类型的FrameNode节点类型。
...
| 类型 | 说明 |
| --- | --- |
| TypedFrameNode<ToggleInterface, ToggleAttribute> | 提供Toggle类型FrameNode节点。 ToggleInterface用于TypedFrameNode的initialize接口的入参，入参为Toggle组件的构造函数类型。 ToggleAttribute用于TypedFrameNode的attribute接口的返回值，返回Toggle组件的属性设置对象。 |
...
NodeAdapter提供FrameNode的数据懒加载能力，通过[LazyForEach](LazyForEach.md)实现接口功能。
...
请参考[NodeAdapter使用示例](#ZH-CN_TOPIC_0000002522240744__nodeadapter使用示例)。
...
重新加载全部数据操作。实际调用了LazyForEach中的[OnDataReloaded](LazyForEach.md#ZH-CN_TOPIC_0000002522080924__ondatareloaded)接口通知组件重新加载所有数据。
...
请参考[检验NodeAdapter是否有效示例](#ZH-CN_TOPIC_0000002522240744__检验nodeadapter是否有效示例)。
...
![image](public_sys-resources/zh-cn_image_0000002522244716.webp)
...
该示例说明在FrameNode上[createAnimation](#ZH-CN_TOPIC_0000002522240744__createanimation20)、[cancelAnimations](#ZH-CN_TOPIC_0000002522240744__cancelanimations20)、[getNodePropertyValue](#ZH-CN_TOPIC_0000002522240744__getnodepropertyvalue20)接口的用法。
...
![image](public_sys-resources/zh-cn_image_0000002553204677.webp)
...
该示例演示了FrameNode释放节点前后分别使用[isDisposed](#ZH-CN_TOPIC_0000002522240744__isdisposed20)接口验证节点的状态，释放节点前节点调用isDisposed接口返回true，释放节点后节点调用isDisposed接口返回false。
...
![image](public_sys-resources/zh-cn_image_0000002522084724.webp)

**检验NodeAdapter是否有效示例**

该示例演示了[NodeAdapter](#ZH-CN_TOPIC_0000002522240744__nodeadapter12)释放节点前后分别使用[isDisposed](#ZH-CN_TOPIC_0000002522240744__isdisposed20)接口验证节点的状态，释放节点前节点调用isDisposed接口返回true，释放节点后节点调用isDisposed接口返回false。
...
![image](public_sys-resources/zh-cn_image_0000002553364637.webp)

**获取根节点示例**

该示例演示了如何通过FrameNode的[getParent](#ZH-CN_TOPIC_0000002522240744__getparent12)接口获取当前页面根节点。
...
从API version 22开始，该示例演示了如何通过FrameNode的[adoptChild](#ZH-CN_TOPIC_0000002522240744__adoptchild22)和[removeAdoptedChild](#ZH-CN_TOPIC_0000002522240744__removeadoptedchild22)接口进行接纳为附属节点的相关操作。
...
该示例演示了如何通过FrameNode的[convertPositionToWindow](#ZH-CN_TOPIC_0000002522240744__convertpositiontowindow23)和[convertPositionFromWindow](#ZH-CN_TOPIC_0000002522240744__convertpositionfromwindow23)接口进行局部与窗口坐标转化。
