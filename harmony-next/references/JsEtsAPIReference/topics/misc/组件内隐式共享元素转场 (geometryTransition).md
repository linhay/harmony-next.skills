...
[geometryTransition](组件内隐式共享元素转场 (geometryTransition).md)必须配合[animateTo](../../types/classes/Class (UIContext).md#ZH-CN_TOPIC_0000002529444749__animateto)使用才有动画效果，动效时长、曲线跟随[animateTo](../../types/classes/Class (UIContext).md#ZH-CN_TOPIC_0000002529444749__animateto)中的配置，不支持[animation](属性动画 (animation).md)动画。
...
参数名类型必填说明idstring是用于设置绑定关系，id置空字符串清除绑定关系避免参与共享行为，id可更换重新建立绑定关系。同一个id只能有两个组件绑定且是in/out不同类型角色，不能多个组件绑定同一个id。
...
参数名类型必填说明idstring是用于设置绑定关系，id置空字符串清除绑定关系避免参与共享行为，id可更换重新建立绑定关系。同一个id只能有两个组件绑定且是in/out不同类型角色，不能多个组件绑定同一个id。options[GeometryTransitionOptions](#ZH-CN_TOPIC_0000002529284923__geometrytransitionoptions11)否组件内共享元素转场动画参数。
