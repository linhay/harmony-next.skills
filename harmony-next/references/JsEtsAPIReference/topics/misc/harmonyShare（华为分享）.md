...
华为分享事件窗口注册配置项。继承[BaseCapabilityRegistry](#ZH-CN_TOPIC_0000002553362465__basecapabilityregistry)。
...
沙箱接收事件窗口注册配置项。继承[BaseCapabilityRegistry](#ZH-CN_TOPIC_0000002553362465__basecapabilityregistry)。
...
沙箱接收传输完成后回调函数。可用此方法监听完成事件，并处理接收后的数据。继承[TransferBaseResults](#ZH-CN_TOPIC_0000002553362465__transferbaseresults)。
