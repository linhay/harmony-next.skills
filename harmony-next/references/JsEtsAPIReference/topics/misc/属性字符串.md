...
方便灵活应用文本样式的对象，可通过TextController中的[setStyledString](../graphics/Text.md#ZH-CN_TOPIC_0000002497444914__setstyledstring12)方法与Text组件绑定，可通过RichEditorStyledStringController中的[setStyledString](RichEditor.md#ZH-CN_TOPIC_0000002497604894__setstyledstring12)方法与RichEditor组件绑定。

从API version 12开始支持。后续版本如有新增内容，则采用上角标单独标记该内容的起始版本。

从API version 20开始，支持通过[getParagraphs](../../types/classes/Class (MeasureUtils).md#ZH-CN_TOPIC_0000002497604780__getparagraphs20)获取属性字符串的文本布局信息。

属性字符串目前不支持在worker线程中使用。

属性字符串通过controller绑定时，需要等待布局完成后，绑定生效。当[measure](../components/FrameNode.md#ZH-CN_TOPIC_0000002529284787__measure12)和setStyledString同时使用，开发者需要通过[@ohos.arkui.inspector (布局回调)](../../modules/ohos/@ohos.arkui.inspector (布局回调).md)判断布局完成，再绑定属性字符串。

#### 规则说明

- 当组件样式和属性字符串中的样式冲突时，冲突部分以属性字符串设置的样式为准，未冲突部分则生效组件的样式。
- 当属性字符串和[Text](../graphics/Text.md)子组件冲突时，属性字符串优先级高，即当Text组件中绑定了属性字符串，忽略Text组件下包含[Span](../components/span.md)等子组件的情况。
- 不支持@State修饰。
- 建议将StyledString定义为成员变量，从而避免应用退后台后被销毁。
- 不支持在[loadContent()](../../types/interfaces/Interface (Window).md#ZH-CN_TOPIC_0000002497604802__loadcontent9)之前创建。
...
参数名类型必填说明valuestring | [ImageAttachment](#ZH-CN_TOPIC_0000002497444920__imageattachment) | [CustomSpan](#ZH-CN_TOPIC_0000002497444920__customspan)是
...
需要设置styles时，通过[setStyle](#ZH-CN_TOPIC_0000002497444920__setstyle)等方法实现。

stylesArray<[StyleOptions](#ZH-CN_TOPIC_0000002497444920__styleoptions对象说明)>否
...
当属性字符串中包含图片或[CustomSpan](#ZH-CN_TOPIC_0000002497444920__customspan)时，其返回的结果用空格表示。
...
参数名类型必填说明other[StyledString](#ZH-CN_TOPIC_0000002497444920__styledstring)是StyledString类型的比较对象。
...
不比较GestureStyle，当属性字符串配置了不同事件，文本和其他样式相同时，亦视为相等。

当比较CustomSpan或[LeadingMarginSpan](#ZH-CN_TOPIC_0000002497444920__leadingmarginspan22)时，比较的是地址，地址相等，视为相等。
...
参数名类型必填说明startnumber是子属性字符串开始位置的下标。lengthnumber否子属性字符串的长度。

**返回值：**

类型说明[StyledString](#ZH-CN_TOPIC_0000002497444920__styledstring)
...
参数名类型必填说明startnumber是指定范围属性字符串的下标。lengthnumber是指定范围属性字符串的长度。styledKey[StyledStringKey](#ZH-CN_TOPIC_0000002497444920__styledstringkey枚举说明)否指定范围属性字符串样式的枚举值。

**返回值：**

类型说明Array<[SpanStyle](#ZH-CN_TOPIC_0000002497444920__spanstyle对象说明)>
...
使用方法参考[示例12（fromHtml和toHtml互相转换）](#ZH-CN_TOPIC_0000002497444920__示例12fromhtml和tohtml互相转换)。

标签名称说明<p>段落，分隔文本段落<span>行内文本，支持样式设置。API version 17及之前，<span>设置的background-color属性转换不生效。<img>插入图片<strong>加粗文本<br>20+换行<b>20+加粗文本<a>20+超链接<i>20+斜体文本<em>20+斜体文本<s>20+删除线（中划线）<u>20+下划线<del>20+删除线（中划线）<sup>20+上标文本<sub>20+下标文本
...
参数名类型必填说明htmlstring是html格式的字符串。

**返回值：**

类型说明Promise<[StyledString](#ZH-CN_TOPIC_0000002497444920__styledstring)>属性字符串。

**错误码**：

以下错误码详细介绍请参考[通用错误码](../../errors/通用错误码.md)和[属性字符串错误码](../../errors/属性字符串错误码.md)。

错误码ID错误信息401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified; 2.Incorrect parameters types; 3. Parameter verification failed.170001Convert Error.

#### toHtml14+

static toHtml(styledString: StyledString): string

将属性字符串转换成HTML格式字符串。支持转换的属性字符串[StyledStringKey](#ZH-CN_TOPIC_0000002497444920__styledstringkey枚举说明)包括：StyledStringKey.FONT、StyledStringKey.DECORATION、StyledStringKey.LETTER_SPACING、StyledStringKey.TEXT_SHADOW、StyledStringKey.LINE_HEIGHT、StyledStringKey.IMAGE。
...
参数名类型必填说明styledStringStyledString是属性字符串。
...
继承于[StyledString](#ZH-CN_TOPIC_0000002497444920__styledstring)类。
...
参数名类型必填说明startnumber是指定范围的下标。lengthnumber是指定范围的长度。otherstring是
...
参数名类型必填说明startnumber是插入位置的下标。otherstring是
...
当属性字符串中包含图片或[CustomSpan](#ZH-CN_TOPIC_0000002497444920__customspan)时，同样生效。
...
参数名类型必填说明startnumber是指定范围的下标。lengthnumber是指定范围的长度。
...
参数名类型必填说明spanStyle[SpanStyle](#ZH-CN_TOPIC_0000002497444920__spanstyle对象说明)是
...
被清空样式类型对象属性使用的是对应[Text](../graphics/Text.md)组件属性的设置值，若Text组件未设置值，则使用对应Text组件属性的默认值。
...
参数名类型必填说明startnumber是指定范围开始位置的下标。lengthnumber是指定范围的长度。styledKey[StyledStringKey](#ZH-CN_TOPIC_0000002497444920__styledstringkey枚举说明)是样式类型枚举值。
...
参数名类型必填说明startnumber是指定范围开始位置的下标。lengthnumber是指定范围的长度。
...
参数名类型必填说明startnumber是指定范围开始位置的下标。lengthnumber是指定范围的长度。other[StyledString](#ZH-CN_TOPIC_0000002497444920__styledstring)是新的属性字符串对象。
...
参数名类型必填说明startnumber是开始插入位置的下标。other[StyledString](#ZH-CN_TOPIC_0000002497444920__styledstring)是新的属性字符串对象。
...
参数名类型必填说明other[StyledString](#ZH-CN_TOPIC_0000002497444920__styledstring)是新的属性字符串对象。
...
类型说明[TextStyle](#ZH-CN_TOPIC_0000002497444920__textstyle)文本字体样式。[DecorationStyle](#ZH-CN_TOPIC_0000002497444920__decorationstyle)文本装饰线样式。[BaselineOffsetStyle](#ZH-CN_TOPIC_0000002497444920__baselineoffsetstyle)文本基线偏移量样式。[LetterSpacingStyle](#ZH-CN_TOPIC_0000002497444920__letterspacingstyle)文本字符间距样式。[LineHeightStyle](#ZH-CN_TOPIC_0000002497444920__lineheightstyle)文本行高样式。[TextShadowStyle](#ZH-CN_TOPIC_0000002497444920__textshadowstyle)文本阴影样式。[GestureStyle](#ZH-CN_TOPIC_0000002497444920__gesturestyle)事件手势样式。[ParagraphStyle](#ZH-CN_TOPIC_0000002497444920__paragraphstyle)文本段落样式。[ImageAttachment](#ZH-CN_TOPIC_0000002497444920__imageattachment)图片样式。[CustomSpan](#ZH-CN_TOPIC_0000002497444920__customspan)自定义绘制Span样式。[UserDataSpan](#ZH-CN_TOPIC_0000002497444920__userdataspan)UserDataSpan样式。[UrlStyle](#ZH-CN_TOPIC_0000002497444920__urlstyle14)超链接样式。[BackgroundColorStyle](#ZH-CN_TOPIC_0000002497444920__backgroundcolorstyle14)文本背景颜色样式。
...
styledKey[StyledStringKey](#ZH-CN_TOPIC_0000002497444920__styledstringkey枚举说明)否否样式类型的枚举值。styledValue[StyledStringValue](#ZH-CN_TOPIC_0000002497444920__styledstringvalue)否否样式对象。
...
名称类型只读可选说明startnumber否否匹配属性字符串样式的开始位置。lengthnumber否否匹配属性字符串样式的长度。styledKey[StyledStringKey](#ZH-CN_TOPIC_0000002497444920__styledstringkey枚举说明)否否样式类型的枚举值。styledValue[StyledStringValue](#ZH-CN_TOPIC_0000002497444920__styledstringvalue)否否样式对象。
...
名称类型只读可选说明fontColor[ResourceColor](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcecolor)是是
...
fontStyle[FontStyle](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__fontstyle)是是

获取属性字符串的文本字体样式。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

strokeWidth20+number是是

获取属性字符串的文本描边宽度。

默认返回0，单位为[vp](像素单位.md)。

**元服务API：** 从API version 20开始，该接口支持在元服务中使用。

strokeColor20+[ResourceColor](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcecolor)是是
...
superscript20+[SuperscriptStyle](文本组件公共接口.md#ZH-CN_TOPIC_0000002529444865__superscriptstyle20枚举说明)是是
...
参数返回值100020013002400350046005700680079008FontWeight.Bold (or 'bold')9FontWeight.Normal (or 'normal')10FontWeight.Bolder (or 'bolder')11FontWeight.Lighter (or 'lighter')12FontWeight.Medium (or 'medium')13FontWeight.Regular (or 'regular')14
...
参数名类型必填说明value[TextStyleInterface](#ZH-CN_TOPIC_0000002497444920__textstyleinterface对象说明)否字体样式设置项。
...
strokeWidth20+[LengthMetrics](../graphics/Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)否是
...
strokeColor20+[ResourceColor](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcecolor)否是
...
superscript20+[SuperscriptStyle](文本组件公共接口.md#ZH-CN_TOPIC_0000002529444865__superscriptstyle20枚举说明)否是
...
参数名类型必填说明value[GestureStyleInterface](#ZH-CN_TOPIC_0000002497444920__gesturestyleinterface对象说明)否事件设置项。
...
onTouch20+Callback<[TouchEvent](触摸事件.md#ZH-CN_TOPIC_0000002529444777__touchevent对象说明)>否是
...
名称类型只读可选说明enableMultiTypeboolean否是
...
名称类型只读可选说明type[TextDecorationType](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__textdecorationtype)是否

获取属性字符串的文本装饰线类型。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

color[ResourceColor](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcecolor)是是

获取属性字符串的文本装饰线颜色。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

style[TextDecorationStyle](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__textdecorationstyle12)是是

获取属性字符串的文本装饰线样式。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

thicknessScale20+number是是

获取属性字符串的文本装饰线粗细缩放值。

**元服务API：** 从API version 20开始，该接口支持在元服务中使用。

options20+[DecorationOptions](#ZH-CN_TOPIC_0000002497444920__decorationoptions20)是是
...
参数名类型必填说明value[DecorationStyleInterface](#ZH-CN_TOPIC_0000002497444920__decorationstyleinterface)是
...
options[DecorationOptions](#ZH-CN_TOPIC_0000002497444920__decorationoptions20)否
...
名称类型只读可选说明type[TextDecorationType](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__textdecorationtype)否否
...
style[TextDecorationStyle](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__textdecorationstyle12)否是
...
名称类型只读可选说明baselineOffsetnumber是否
...
参数名类型必填说明value[LengthMetrics](../graphics/Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)是文本基线偏移量设置项。如果LengthMetrics的unit值是percent，该设置不生效。
...
名称类型只读可选说明letterSpacingnumber是否
...
参数名类型必填说明value[LengthMetrics](../graphics/Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)是文本字符间距设置项。如果LengthMetrics的unit值是percent，该设置不生效。
...
参数名类型必填说明lineHeight[LengthMetrics](../graphics/Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)是文本行高设置项。如果LengthMetrics的value值不大于0时，不限制文本行高，自适应字体大小。
...
名称类型只读可选说明textShadowArray<[ShadowOptions](图像效果.md#ZH-CN_TOPIC_0000002497444856__shadowoptions对象说明)>是否获取属性字符串的文本阴影。
...
参数名类型必填说明value[ShadowOptions](图像效果.md#ZH-CN_TOPIC_0000002497444856__shadowoptions对象说明) | Array<[ShadowOptions](图像效果.md#ZH-CN_TOPIC_0000002497444856__shadowoptions对象说明)>是文本阴影设置项。
...
名称类型只读可选说明value[PixelMap](../../types/interfaces/Interface (PixelMap).md)是否

获取属性字符串的图片数据源。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

size[SizeOptions](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__sizeoptions)是是
...
sizeInVp21+[SizeOptions](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__sizeoptions)是是
...
verticalAlign[ImageSpanAlignment](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__imagespanalignment10)是是

获取属性字符串的图片对齐方式。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

objectFit[ImageFit](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__imagefit)是是

获取属性字符串的图片缩放类型。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

layoutStyle[ImageAttachmentLayoutStyle](#ZH-CN_TOPIC_0000002497444920__imageattachmentlayoutstyle对象说明)是是

获取属性字符串的图片布局。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

colorFilter15+[ColorFilterType](#ZH-CN_TOPIC_0000002497444920__colorfiltertype15)是是

获取属性字符串的图片颜色滤镜效果。

**元服务API：** 从API version 15开始，该接口支持在元服务中使用。

supportSvg222+boolean是是

获取属性字符串是否开启[SVG标签解析能力增强功能](SVG标签解析能力增强.md)。
...
参数名类型必填说明value[ImageAttachmentInterface](#ZH-CN_TOPIC_0000002497444920__imageattachmentinterface对象说明)是图片设置项。

#### constructor15+

constructor(attachment: Optional<AttachmentType>)

图片对象的构造函数。与value类型入参构造函数相比，attachment参数增加了对undefined类型和[ResourceStr](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcestr)类型图片的支持。
...
参数名类型必填说明attachmentOptional<[AttachmentType](#ZH-CN_TOPIC_0000002497444920__attachmenttype15)>是PixelMap类型或[ResourceStr](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcestr)类型图片设置项。

#### AttachmentType15+

type AttachmentType = ImageAttachmentInterface | ResourceImageAttachmentOptions

图片设置项类型，用于设置属性字符串PixelMap类型或[ResourceStr](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcestr)类型图片。

**元服务API：** 从API version 15开始，该接口支持在元服务中使用。

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

类型说明[ImageAttachmentInterface](#ZH-CN_TOPIC_0000002497444920__imageattachmentinterface对象说明)PixelMap类型图片设置项。[ResourceImageAttachmentOptions](#ZH-CN_TOPIC_0000002497444920__resourceimageattachmentoptions15)ResourceStr类型图片设置项。
...
类型说明[ColorFilter](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__colorfilter9)ColorFilter类型图片颜色滤镜设置项。[DrawingColorFilter](../../types/classes/Class (ColorFilter).md)DrawingColorFilter类型图片颜色滤镜设置项。

#### ImageAttachmentInterface对象说明

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

名称类型只读可选说明value[PixelMap](../../types/interfaces/Interface (PixelMap).md)否否设置图片数据源。**元服务API：** 从API version 12开始，该接口支持在元服务中使用。size[SizeOptions](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__sizeoptions)否是

设置图片大小，不支持百分比。 **元服务API：** 从API version 12开始，该接口支持在元服务中使用。
...
设置图片基于文本的对齐方式。**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

默认值：ImageSpanAlignment.BOTTOM

objectFit[ImageFit](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__imagefit)否是

设置图片的缩放类型，当前枚举类型不支持ImageFit.MATRIX。**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

默认值：ImageFit.Cover

layoutStyle[ImageAttachmentLayoutStyle](#ZH-CN_TOPIC_0000002497444920__imageattachmentlayoutstyle对象说明)否是设置图片布局。**元服务API：** 从API version 12开始，该接口支持在元服务中使用。colorFilter15+[ColorFilterType](#ZH-CN_TOPIC_0000002497444920__colorfiltertype15)否是设置属性字符串的图片颜色滤镜效果。**元服务API：** 从API version 15开始，该接口支持在元服务中使用。
...
名称类型只读可选说明margin[LengthMetrics](../graphics/Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12) | [Margin](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__margin)否是
...
padding[LengthMetrics](../graphics/Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12) | [Padding](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__padding)否是
...
borderRadius[LengthMetrics](../graphics/Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12) | [BorderRadiuses](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__borderradiuses9)否是
...
名称类型只读可选说明resourceValueOptional<[ResourceStr](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcestr)>否否

设置图片数据源。

**元服务API：** 从API version 15开始，该接口支持在元服务中使用。

size[SizeOptions](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__sizeoptions)否是
...
layoutStyle[ImageAttachmentLayoutStyle](#ZH-CN_TOPIC_0000002497444920__imageattachmentlayoutstyle对象说明)否是

设置图片布局。

**元服务API：** 从API version 15开始，该接口支持在元服务中使用。

colorFilter[ColorFilterType](#ZH-CN_TOPIC_0000002497444920__colorfiltertype15)否是
...
参数名类型必填说明measureInfo[CustomSpanMeasureInfo](#ZH-CN_TOPIC_0000002497444920__customspanmeasureinfo对象说明)是文本的字体大小。

**返回值：**

类型说明[CustomSpanMetrics](#ZH-CN_TOPIC_0000002497444920__customspanmetrics对象说明)
...
参数名类型必填说明context[DrawContext](../graphics/Graphics.md#ZH-CN_TOPIC_0000002529444761__drawcontext)是
...
drawInfo[CustomSpanDrawInfo](#ZH-CN_TOPIC_0000002497444920__customspandrawinfo对象说明)是自定义绘制Span的绘制信息。
...
名称类型只读可选说明textAlign[TextAlign](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__textalign)是是
...
overflow[TextOverflow](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__textoverflow)是是

获取属性字符串文本段落超长时的显示方式。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

wordBreak[WordBreak](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__wordbreak11)是是

获取属性字符串文本段落的断行规则。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

leadingMarginnumber | [LeadingMarginPlaceholder](RichEditor.md#ZH-CN_TOPIC_0000002497604894__leadingmarginplaceholder11)是是
...
textVerticalAlign20+[TextVerticalAlign](文本组件公共接口.md#ZH-CN_TOPIC_0000002529444865__textverticalalign20)是是

获取属性字符串文本段落在垂直方向的对齐方式。

**元服务API：** 从API version 20开始，该接口支持在元服务中使用。

leadingMarginSpan22+[LeadingMarginSpan](#ZH-CN_TOPIC_0000002497444920__leadingmarginspan22)是是
...
参数名类型必填说明value[ParagraphStyleInterface](#ZH-CN_TOPIC_0000002497444920__paragraphstyleinterface对象说明)否段落样式设置项。
...
textIndent[LengthMetrics](../graphics/Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)否是
...
overflow[TextOverflow](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__textoverflow)否是
...
wordBreak[WordBreak](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__wordbreak11)否是
...
leadingMargin[LengthMetrics](../graphics/Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12) | [LeadingMarginPlaceholder](RichEditor.md#ZH-CN_TOPIC_0000002497604894__leadingmarginplaceholder11)否是
...
paragraphSpacing19+[LengthMetrics](../graphics/Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)否是
...
leadingMarginSpan22+[LeadingMarginSpan](#ZH-CN_TOPIC_0000002497444920__leadingmarginspan22)否是
...
drawInfo[LeadingMarginSpanDrawInfo](#ZH-CN_TOPIC_0000002497444920__leadingmarginspandrawinfo22对象说明)是自定义绘制信息。
...
类型说明[LengthMetrics](../graphics/Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12)
...
direction[TextDirection](文本组件公共接口.md#ZH-CN_TOPIC_0000002529444865__textdirection22)否否文本内容的方向。startnumber否否
...
字体样式键。[TextStyle](属性字符串.md#ZH-CN_TOPIC_0000002497444920__textstyle)所属键。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

DECORATION1

文本装饰线样式键。[DecorationStyle](属性字符串.md#ZH-CN_TOPIC_0000002497444920__decorationstyle)所属键。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

BASELINE_OFFSET2

文本基线偏移量样式键。[BaselineOffsetStyle](属性字符串.md#ZH-CN_TOPIC_0000002497444920__baselineoffsetstyle)所属键。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

LETTER_SPACING3

文本字符间距样式键。[LetterSpacingStyle](属性字符串.md#ZH-CN_TOPIC_0000002497444920__letterspacingstyle)所属键。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

TEXT_SHADOW4

文本阴影样式键。[TextShadowStyle](属性字符串.md#ZH-CN_TOPIC_0000002497444920__textshadowstyle)所属键。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

LINE_HEIGHT5

文本行高样式键。[LineHeightStyle](属性字符串.md#ZH-CN_TOPIC_0000002497444920__lineheightstyle)所属键。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

BACKGROUND_COLOR14+6

文本背景色样式键。[BackgroundColorStyle](属性字符串.md#ZH-CN_TOPIC_0000002497444920__backgroundcolorstyle14)所属键。

**元服务API：** 从API version 14开始，该接口支持在元服务中使用。

URL14+7

超链接样式键。[UrlStyle](属性字符串.md#ZH-CN_TOPIC_0000002497444920__urlstyle14)所属键。

**元服务API：** 从API version 14开始，该接口支持在元服务中使用。

GESTURE100

事件手势键。[GestureStyle](属性字符串.md#ZH-CN_TOPIC_0000002497444920__gesturestyle)所属键。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

PARAGRAPH_STYLE200

段落样式键。[ParagraphStyle](属性字符串.md#ZH-CN_TOPIC_0000002497444920__paragraphstyle)所属键。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

IMAGE300

图片键。[ImageAttachment](属性字符串.md#ZH-CN_TOPIC_0000002497444920__imageattachment)所属键。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

CUSTOM_SPAN400

自定义绘制Span键。[CustomSpan](属性字符串.md#ZH-CN_TOPIC_0000002497444920__customspan)所属键。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

USER_DATA500

UserDataSpan键。[UserDataSpan](属性字符串.md#ZH-CN_TOPIC_0000002497444920__userdataspan)所属键。
...
名称类型只读可选说明textBackgroundStyle[TextBackgroundStyle](../components/span.md#ZH-CN_TOPIC_0000002529284887__textbackgroundstyle11对象说明)是否
...
参数名类型必填说明textBackgroundStyle[TextBackgroundStyle](../components/span.md#ZH-CN_TOPIC_0000002529284887__textbackgroundstyle11对象说明)是
...
名称类型只读可选说明urlstring是否获取属性字符串的超链接内容。
...
参数名类型必填说明urlstring是超链接设置项。

#### 示例

#### 示例1（属性字符串处理）

从API version 12开始，该示例通过[insertString](#ZH-CN_TOPIC_0000002497444920__insertstring)、[removeStyles](#ZH-CN_TOPIC_0000002497444920__removestyles)、[replaceStyle](#ZH-CN_TOPIC_0000002497444920__replacestyle)、[getStyles](#ZH-CN_TOPIC_0000002497444920__getstyles)接口实现属性字符串的插入、删除、替换、查看。
...
从API version 12开始，该示例通过[StyleOptions](#ZH-CN_TOPIC_0000002497444920__styleoptions对象说明)中的styledKey、styledValue接口实现属性字符串绑定事件。
...
从API version 12开始，该示例通过[getStyles](#ZH-CN_TOPIC_0000002497444920__getstyles)、[setStyle](#ZH-CN_TOPIC_0000002497444920__setstyle)接口实现属性字符串查询和设置样式。
...
从API version 12开始，该示例通过[ImageAttachment](#ZH-CN_TOPIC_0000002497444920__imageattachmentinterface对象说明)接口实现属性字符串设置图片。
...
从API version 12开始，该示例通过[LineHeightStyle](#ZH-CN_TOPIC_0000002497444920__lineheightstyle)、[ParagraphStyle](#ZH-CN_TOPIC_0000002497444920__paragraphstyle)接口实现属性字符串设置文本行高和段落样式。
...
从API version 12开始，该示例通过[CustomSpan](#ZH-CN_TOPIC_0000002497444920__customspan)接口和[measureTextSize](../../types/classes/Class (MeasureUtils).md#ZH-CN_TOPIC_0000002497604780__measuretextsize12)实现属性字符串设置自定义绘制Span。
...
从API version 12开始，该示例通过[UserDataSpan](#ZH-CN_TOPIC_0000002497444920__userdataspan)接口实现属性字符串支持存储自定义扩展信息的功能。
...
从API version 14开始，该示例通过[UrlStyle](#ZH-CN_TOPIC_0000002497444920__urlstyle14)接口，实现了对属性字符串中超链接设置的支持。
...
从API version 15开始，该示例通过给[ImageAttachment](#ZH-CN_TOPIC_0000002497444920__imageattachmentinterface对象说明)设置colorFilter实现了给图像设置颜色滤镜效果。
...
从API version 12开始，该示例通过[subStyledString](#ZH-CN_TOPIC_0000002497444920__substyledstring)、[removeString](#ZH-CN_TOPIC_0000002497444920__removestring)、[removeStyle](#ZH-CN_TOPIC_0000002497444920__removestyle)、[clearStyles](#ZH-CN_TOPIC_0000002497444920__clearstyles)、[replaceStyledString](#ZH-CN_TOPIC_0000002497444920__replacestyledstring)、[insertStyledString](#ZH-CN_TOPIC_0000002497444920__insertstyledstring)接口实现属性字符串的插入、删除、替换。
...
从API version 20开始，该示例通过[TextStyle](#ZH-CN_TOPIC_0000002497444920__textstyle)设置strokeWidth和strokeColor接口实现属性字符串的文本描边。
...
该示例通过[fromHtml](#ZH-CN_TOPIC_0000002497444920__fromhtml)（从API version 12开始）、[toHtml](#ZH-CN_TOPIC_0000002497444920__tohtml14)（从API version 14开始）接口，将HTML中strong、b20+、em20+、i20+、u20+、del20+、s20+、a20+、sub20+、sup20+标签及其style属性中的background-color转换为属性字符串并转回HTML。
...
从API version 20开始，该示例通过[DecorationStyle](#ZH-CN_TOPIC_0000002497444920__decorationstyle)中设置enableMultiType、thicknessScale接口，实现多装饰线显示与加粗装饰线的效果。
...
从API version 21开始，该示例通过[ImageAttachmentInterface](#ZH-CN_TOPIC_0000002497444920__imageattachmentinterface对象说明)实现属性字符串设置图片，并且获取该图片以vp为单位的尺寸。
...
从API version 22开始，该示例通过[LeadingMarginSpan](#ZH-CN_TOPIC_0000002497444920__leadingmarginspan22)设置段落缩进，并且自定义缩进图案。
...
从API version 22开始，该示例通过给[ResourceImageAttachmentOptions](#ZH-CN_TOPIC_0000002497444920__resourceimageattachmentoptions15)设置supportSvg2属性，使[SVG标签解析能力增强功能](SVG标签解析能力增强.md#ZH-CN_TOPIC_0000002529284895__svg易用性提升)的SVG易用性提升能力生效。
