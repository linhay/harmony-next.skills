...
| code | 是 | String | 当“grant_type”为“authorization_code”时，此参数必选。获取方式详见LoginWithHuaweiIDResponse或AuthorizationWithHuaweiIDResponse。 |
...
| id_token | 是 | String | ID Token（JWT格式），详细信息请参见[验证ID Token有效性](验证ID Token有效性.md)中ID Token描述。 |
...
| 1101 | 20182 | grant_type值不正确。 | grant_type可选值如下： - “authorization_code”：该场景用于[获取用户级凭证](获取用户级凭证.md)。 - “refresh_token”： 该场景用于[刷新用户级凭证](刷新用户级凭证.md)。 - “client_credentials”：该场景用于[获取应用级凭证](获取应用级凭证.md)。 |
