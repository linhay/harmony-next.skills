...
可填写16字节星闪服务UUID，或填写2字节支持数传的星闪标准服务UUID。UUID格式参考“[星闪标准服务标识](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/nearlink-faq#section1263634155618)”。
...
可填写16字节星闪服务UUID，或填写2字节支持数传的星闪标准服务UUID。UUID格式参考“[星闪标准服务](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/nearlink-faq#section1263634155618)”。
...
[ConnectionParams](#section85111632162516)
...
固定填写"connectionStateChanged"字符串，表示端口通道连接状态变更事件。

callback

Callback<[ConnectionResult](#section149561050194010)>

是

表示端口通道连接状态变化回调函数的入参，回调函数由用户创建通过该接口注册。
...
[ConnectionStateParams](#section1230716832813)
...
[DataParams](#section181874974014)

是

指明发送数据的参数，包含远端设备地址、服务UUID以及发送的数据包。
...
固定填写"readData"字符串，表示端口通道数据接收事件。

callback

Callback<[DataParams](#section181874974014)>

是

表示端口通道数据接收回调函数的入参。回调函数由用户创建通过该接口注册。
...
星闪服务UUID，例如：37bea880-fc70-11ea-b720-000000004386。UUID格式参考[星闪标准服务UUID](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/nearlink-faq#section1263634155618)。
...
期望发送数据的包长，单位为byte。范围[0, 65535]，默认值为512。

transferMode

[TransferMode](#section757023015351)
