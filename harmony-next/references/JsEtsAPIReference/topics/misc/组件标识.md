...
参数名类型必填说明valuestring是组件的唯一标识，唯一性由使用者保证。
...
**说明**：
 ...
 字符串信息包含组件的tag、id、位置信息(相对于窗口左上角的坐标)以及用于测试检查的组件所包含的相关属性信息。组件中每个字段的含义请参考[getInspectorInfo](../components/FrameNode.md#ZH-CN_TOPIC_0000002529284787__getinspectorinfo12)的返回值说明。
...
类型说明Object组件树及组件属性列表的JSON对象。组件中每个字段的含义请参考[getInspectorInfo](../components/FrameNode.md#ZH-CN_TOPIC_0000002529284787__getinspectorinfo12)的返回值说明。
...
参数名类型必填说明idstring是要触发事件的组件的id。actionnumber是
...
paramsstring是事件参数，无参数传空字符串 ""。

**返回值:**

类型说明boolean找不到指定id的组件时返回false，其余情况返回true。
...
参数名类型必填说明event[TouchObject](触摸事件.md#ZH-CN_TOPIC_0000002529444777__touchobject)是触发触摸事件的位置，event参数见[TouchObject](触摸事件.md#ZH-CN_TOPIC_0000002529444777__touchobject)的介绍。

**返回值:**

类型说明boolean事件发送失败时返回false，其余情况返回true。
...
参数名类型必填说明event[KeyEvent](按键事件.md#ZH-CN_TOPIC_0000002529284805__keyevent对象说明)是按键事件，event参数见[KeyEvent](按键事件.md#ZH-CN_TOPIC_0000002529284805__keyevent对象说明)介绍。
...
参数名类型必填说明event[MouseEvent](鼠标事件.md#ZH-CN_TOPIC_0000002497604812__mouseevent对象说明)是鼠标事件，event参数见[MouseEvent](鼠标事件.md#ZH-CN_TOPIC_0000002497604812__mouseevent对象说明)介绍。
