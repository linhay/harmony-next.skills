...
您需要在AppGallery Connect上开通“消息回执”权益，如何开通请参见[开通回执权益](../../guides/（可选）开发消息回执.md#开通回执权益)。
...
| pushType | 是 | Integer | 消息类型，详情见[push-type](请求体结构说明.md#section20573634202313)。 |
| appPackageName | 是 | String | 消息发送方的应用包名。 |
| [biTag](请求体参数说明.md#section418321011212) | 否 | String | 您在发送消息时携带的biTag。如果下发推送消息未设置biTag字段，则该字段内容为空。 |
...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| formId | 是 | Long | 服务卡片的实例ID，当卡片onAddForm（卡片使用方添加卡片至桌面）时获取。最大值为2^31-1。 |
| result | 是 | Integer | 消息到达的回执状态码，请参见回执状态码。 |
| timestamp | 是 | Long | 消息成功到达客户端的时间戳（毫秒级时间戳），失败则为回执消息生成的时间戳。 |
