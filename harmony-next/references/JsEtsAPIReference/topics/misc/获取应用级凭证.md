...
应用服务端调用此接口[获取应用级凭证](获取应用级凭证.md)Access Token。
...
| 1101 | 20182 | grant_type值不正确。 | grant_type可选值如下： - “authorization_code”：该场景用于[获取用户级凭证](获取用户级凭证.md)。 - “refresh_token”： 该场景用于[刷新用户级凭证](刷新用户级凭证.md)。 - “client_credentials”：该场景用于获取应用级凭证。 |
