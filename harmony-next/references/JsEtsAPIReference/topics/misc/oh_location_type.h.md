...
| 名称 | typedef关键字 | 描述 |
| --- | --- | --- |
| Location_BasicInfo | Location_BasicInfo | 定义位置基本信息的结构体。 |
| Location_Info | Location_Info | 定义位置信息的结构体。 |
| Location_RequestConfig | Location_RequestConfig | 定义位置请求参数的结构体。 |

**枚举**

| 名称 | typedef关键字 | 描述 |
| --- | --- | --- |
| Location_ResultCode | Location_ResultCode | 定义位置服务的错误码。 |
| Location_UseScene | Location_UseScene | 定义位置请求中的用户活动场景类型。 |
| Location_PowerConsumptionScene | Location_PowerConsumptionScene | 定义位置请求中的功耗场景类型。 |
| Location_SourceType | Location_SourceType | 定义位置信息的来源。 |
...
| 类型 | 说明 |
| --- | --- |
| Location_BasicInfo | 返回位置基本信息结构体。详细定义请参考Location_BasicInfo。 |
...
| 类型 | 说明 |
| --- | --- |
| Location_ResultCode | 返回操作结果，详细定义参见Location_ResultCode。  LOCATION_SUCCESS 获取附加信息成功。  LOCATION_INVALID_PARAM 1. 入参location或additionalInfo是空指针。  2. 入参length太小，也就是additionalInfo指向的内存太小导致无法保存完整的附加信息字符串。 |
...
| 参数项 | 描述 |
| --- | --- |
| Location_Info* location | 指向Location_Info实例的指针，携带最新的位置信息。  location实例的内存会在Location_InfoCallback结束时回收，请在此之前调用OH_LocationInfo_GetBasicInfo等接口获取位置信息。 |
| void* userData | 指向调用者数据结构或对象的指针，该参数是通过OH_LocationRequestConfig_SetCallback传入的。 |
...
| 类型 | 说明 |
| --- | --- |
| Location_RequestConfig* | 返回指向Location_RequestConfig实例的指针。  如果返回NULL表示创建失败，可能的原因是应用地址空间满，导致空间分配不出来。 |
...
| 参数项 | 描述 |
| --- | --- |
| Location_RequestConfig* requestConfig | 指向Location_RequestConfig实例的指针。  该实例是由OH_Location_CreateRequestConfig创建的。 |
...
设置位置请求参数中的用户活动场景。
 ...
 如果两个参数都未设置，则默认useScene为[LOCATION_USE_SCENE_DAILY_LIFE_SERVICE](oh_location_type.h.md#ZH-CN_TOPIC_0000002522242380__location_usescene)，powerConsumptionScene参数无效。
...
| --- | --- |
| Location_RequestConfig* requestConfig | 指向Location_RequestConfig实例的指针。  该实例是由OH_Location_CreateRequestConfig创建的。 |
| Location_UseScene useScene | 表示位置请求时的用户活动场景。  默认值是LOCATION_USE_SCENE_DAILY_LIFE_SERVICE。  详细定义见Location_UseScene。 |
...
| --- | --- |
| Location_RequestConfig* requestConfig | 指向Location_RequestConfig实例的指针。  该实例是由OH_Location_CreateRequestConfig创建的。 |
| Location_PowerConsumptionScene powerConsumptionScene | 表示位置请求的功耗场景。  默认值是LOCATION_LOW_POWER_CONSUMPTION。  详细定义见Location_PowerConsumptionScene。 |
...
| --- | --- |
| Location_RequestConfig* requestConfig | 指向Location_RequestConfig实例的指针。  该实例是由OH_Location_CreateRequestConfig创建的。 |
| Location_InfoCallback callback | 指向回调函数的指针，该回调函数用于接收位置信息变化。  详细定义请参考Location_InfoCallback。 |
