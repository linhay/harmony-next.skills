...
![image](public_sys-resources/zh-cn_image_0000002553366087.webp)
...
![image](public_sys-resources/zh-cn_image_0000002522246164.webp)
...
| 'language' | 系统语言。 |
| 'osFullName' | 系统版本。 |
| 'fontSizeSetting' | 显示设备逻辑像素的密度，代表物理像素与逻辑像素的缩放系数，计算方式为： 该参数为浮点数，受densityDPI范围限制，取值范围在[0.5，4.0]。一般取值1.0、3.0等，实际取值取决于不同设备提供的densityDPI。 |
...
| 类型 | 说明 |
| --- | --- |
| SystemInfo | 返回SystemInfo对象。 说明： getSystemInfoSync接口不支持获取windowWidth、windowHeight、statusBarHeight和screenSafeArea属性，如需获取可使用getSystemInfo接口。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<SystemInfo> | Promise对象，返回SystemInfo对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| properties | Array<SystemSettingType> | 否 | 获取蓝牙、定位、Wi-Fi开关，以及设备方向等系统信息的请求对象。 说明： - 使用5.0.1(13)及以上版本时不需要配置Wi-Fi权限、蓝牙权限。 - 获取Wi-Fi信息需要配置权限ohos.permission.GET_WIFI_INFO。 - 获取蓝牙开启状态需要配置权限ohos.permission.ACCESS_BLUETOOTH。 - 获取用户程序访问控制权限可参考程序访问控制管理。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| SystemSettingInfo | 返回SystemSettingInfo对象。 |
