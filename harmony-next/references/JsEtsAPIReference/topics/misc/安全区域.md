...
setKeyboardAvoidMode传入异常值时，该属性设置不生效。
 ...
 setKeyboardAvoidMode针对页面生效，对于弹窗类组件不生效，比如Dialog、Popup、Menu、BindSheet、BindContentCover、Toast、OverlayManager。弹窗类组件的避让模式可以参考[CustomDialogControllerOptions对象说明](../components/自定义弹窗 (CustomDialog).md#ZH-CN_TOPIC_0000002497444956__customdialogcontrolleroptions对象说明)。
...
该示例利用[ignoreLayoutSafeArea](#ZH-CN_TOPIC_0000002497444852__ignorelayoutsafearea20)和[LayoutPolicy.matchParent](尺寸设置.md#ZH-CN_TOPIC_0000002497444850__layoutpolicy15)同时改变组件大小和位置。相比未使用该属性，配置ignoreLayoutSafeArea后，Row组件基于Stack内容区、Stack组件级安全区，取其右下部分并撑满可用空间。
