...
从API version 10开始，尺寸设置内部分属性支持使用calc计算特性，具体支持属性请参考对应的属性说明。calc计算特性是一种动态计算长度值的函数，常用于灵活设置布局尺寸（如宽度、高度、边距等）。它允许通过数学表达式组合不同单位和数值，实现动态响应式设计。注意，在使用calc时，运算符与数值之间需要使用空格隔开。具体使用场景可见[示例1](#ZH-CN_TOPIC_0000002497444850__示例1设置组件的宽高和边距)。
...
在[TextInput](../graphics/TextInput.md)组件中，width设置auto表示自适应文本宽度。

-

在[AlphabetIndexer](AlphabetIndexer.md)组件中，width设置auto表示自适应宽度最大索引项的宽度。
...
类型说明T返回当前组件。
 ...
 在[Row](Row.md)、[Column](Column.md)、[RelativeContainer](../components/RelativeContainer.md)组件中，width、height设置auto表示自适应子组件。
...
参数名类型必填说明widthValue[Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length) | [LayoutPolicy](尺寸设置.md#ZH-CN_TOPIC_0000002497444850__layoutpolicy15)是
...
参数名类型必填说明heightValue[Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length) | [LayoutPolicy](尺寸设置.md#ZH-CN_TOPIC_0000002497444850__layoutpolicy15)是
...
参数名类型必填说明value[SizeOptions](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__sizeoptions)是
...
参数名类型必填说明value[Padding](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__padding) | [Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length) | [LocalizedPadding](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__localizedpadding12)12+是
...
参数名类型必填说明value[Margin](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__margin) | [Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length) | [LocalizedMargin](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__localizedmargin12)12+ 是
...
margin设置百分比时，上下左右外边距均以父容器的width作为基础值。在[Row](Row.md)、[Column](Column.md)、[Flex](Flex.md)交叉轴上布局时，子组件交叉轴的大小与margin的和为整体。
...
设置安全区边距属性。允许容器向自身添加组件级安全区域，供子组件延伸，支持[attributeModifier](动态属性设置.md#ZH-CN_TOPIC_0000002529284843__attributemodifier)动态设置属性方法。
...
参数名类型必填说明paddingValue[Padding](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__padding) | [LengthMetrics](../graphics/Graphics.md#ZH-CN_TOPIC_0000002529444761__lengthmetrics12) | [LocalizedPadding](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__localizedpadding12)是
...
当父辈和祖先容器设置了组件级安全区域时，子组件可以感知并利用该区域，称该区域为累计安全区延伸（accumulatedSafeAreaExpand，下文简称SAE），表示子组件在四个方向上各可延伸的长度。当祖辈与更上一级祖辈的safeAreaPadding相邻接（即未被margin、border、padding分隔）时，SAE将递归地向外累积，直至不存在相邻的更外层safeAreaPadding或递归至页面容器外。系统级避让区域（如状态栏、导航条、挖孔区等，详情参见[expandSafeArea](安全区域.md)中的说明）可视为页面容器特有的safeAreaPadding，同样参与该延伸范围的计算。

通过与其他属性配合使用，可对上述计算得到的组件级安全区区域加以利用。例如，对子组件设置[ignoreLayoutSafeArea](安全区域.md#ZH-CN_TOPIC_0000002497444852__ignorelayoutsafearea20)属性，即可利用SAE延伸组件的布局范围。

#### layoutWeight

layoutWeight(value: number | string): T

设置组件的布局权重，使组件在父容器（[Row](Row.md)/[Column](Column.md)/[Flex](Flex.md)）的主轴方向按照权重分配尺寸。
...
仅在[Row](Row.md)/[Column](Column.md)/[Flex](Flex.md)布局中生效。

可选值为大于等于0的数字，或者可以转换为数字的字符串。

如果容器中有子元素设置了layoutWeight属性，且设置的属性值大于0，则所有子元素不会再基于[flexShrink](Flex布局.md#ZH-CN_TOPIC_0000002497604830__flexshrink)和[flexGrow](Flex布局.md#ZH-CN_TOPIC_0000002497604830__flexgrow)布局。
...
width与minWidth与maxWidth使用父容器传递的布局限制进行布局。height与minHeight与maxHeight使用父容器传递的布局限制进行布局。
...
名称类型只读可选说明matchParent[LayoutPolicy](尺寸设置.md#ZH-CN_TOPIC_0000002497444850__layoutpolicy15)是否
...
wrapContent20+[LayoutPolicy](尺寸设置.md#ZH-CN_TOPIC_0000002497444850__layoutpolicy15)是否
...
fixAtIdealSize20+[LayoutPolicy](尺寸设置.md#ZH-CN_TOPIC_0000002497444850__layoutpolicy15)是否
...
LayoutPolicy支持设置三种布局策略：matchParent（自适应父组件布局）、wrapContent（根据内容自适应但不超过父组件尺寸的布局）和fixAtIdealSize（根据内容自适应，可能超过父组件尺寸的布局）。具体示例代码参见[设置布局策略](尺寸设置.md#ZH-CN_TOPIC_0000002497444850__示例5设置布局策略)。
