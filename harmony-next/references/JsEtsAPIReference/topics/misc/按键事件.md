...
按键事件是指组件与物理键盘、遥控器等按键设备交互时触发的事件，适用于所有可获焦组件，例如Button。对于默认不可获焦的组件，如Text，Image等，可以将[focusable](焦点控制.md#ZH-CN_TOPIC_0000002529444807__focusable)属性设置为true后使用按键事件。
...
参数名类型必填说明event(event: [KeyEvent](#ZH-CN_TOPIC_0000002529284805__keyevent对象说明)) => void是获得KeyEvent对象。
...
参数名类型必填说明event[Callback](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__callback12)<[KeyEvent](#ZH-CN_TOPIC_0000002529284805__keyevent对象说明), boolean>是按键事件的回调。
...
参数名类型必填说明event[Callback](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__callback12)<[KeyEvent](#ZH-CN_TOPIC_0000002529284805__keyevent对象说明), boolean>是处理按键事件的回调。
...
参数名类型必填说明event[Callback](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__callback12)<[KeyEvent](#ZH-CN_TOPIC_0000002529284805__keyevent对象说明), boolean>是处理按键事件分发的回调。
...
名称类型只读可选说明type[KeyType](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__keytype)否否

按键的类型。

**元服务API：** 从API version 11开始，该接口支持在元服务中使用。

[keyCode](../../modules/ohos/@ohos.multimodalInput.keyCode (键值).md#ZH-CN_TOPIC_0000002529285559__keycode)number否否
...
keySource[KeySource](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__keysource)否否
...
阻塞[事件冒泡](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-interaction-basic-principles#事件冒泡)传递。

**元服务API：** 从API version 11开始，该接口支持在元服务中使用。

intentionCode10+[IntentionCode](#ZH-CN_TOPIC_0000002529284805__intentioncode10)否否
...
isNumLockOn19+boolean否是

NumLock是否锁定（true: 锁定；false: 解锁）。

**元服务API：** 从API version 19开始，该接口支持在元服务中使用。

isCapsLockOn19+boolean否是

CapsLock是否锁定（true: 锁定；false: 解锁）。

**元服务API：** 从API version 19开始，该接口支持在元服务中使用。

isScrollLockOn19+boolean否是
...
类型说明boolean功能键是否被按下。true表示被按下，false表示未被按下。
...
类型说明[IntentionCode](../../modules/ohos/@ohos.multimodalInput.intentionCode (意图事件).md)按键对应的意图。
...
1. 事件冒泡：在一个树形结构中，当子节点处理完一个事件后，再将该事件交给它的父节点处理。
1. 可以在[onKeyEvent15+](#ZH-CN_TOPIC_0000002529284805__onkeyevent15)中，通过返回true消费按键事件阻止冒泡，效果等同于stopPropagation。
