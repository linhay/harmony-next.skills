...
| sub | 是 | String | 用户的UnionID。具体格式要求请参考OpenID和UnionID的格式说明。 |
...
| nonce | 否 | String | 防重放攻击随机值。详情请参考LoginWithHuaweiIDRequest或AuthorizationWithHuaweiIDRequest的nonce字段说明。 |
...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| anonymized_login_mobile_number | 否 | String | 匿名化的华为账号绑定手机号码，详见华为账号一键登录（获取手机号和UnionID/OpenID）。 |
