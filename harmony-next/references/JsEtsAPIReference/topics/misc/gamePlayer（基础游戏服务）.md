...
- 0：gamePlayerId与openId、playerId不兼容，即调用[getLocalPlayer](#section17453143012584)接口时，玩家首次登录游戏生成的玩家标识；teamPlayerId与unionId不兼容，即调用[unionLogin](#section157848375136)接口时，玩家首次登录游戏未选择转移APK游戏数据生成的玩家标识。
...
玩家本次可玩时长，单位：分钟。**此参数为预留参数，当前固定返回-1。**
...
[ThirdUserAgeRange](#section12735946153615)
...
Array<[ThirdAccountInfo](#section412355692217)>
...
- true：调用[unionLogin](#section157848375136)接口时，强制弹出联合登录面板。  注意：

当需要切换账号或进行其他操作时，请将此值设置为true。

- false：调用[unionLogin](#section157848375136)接口时，优先使用玩家上一次的选择，不强制弹出联合登录面板。

默认为false。

loginPanelType

[LoginPanelType](#section20129173413913)
...
[Resource](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#ZH-CN_TOPIC_0000001847209908__resource)
...
[BoundPlayerInfo](#section331911518595)
...
[GSKLocalPlayer](#section1033653365917)
...
[PlayerChangedEvent](#section57008633419)
...
[UnionLoginParam](#section1030452961619)
...
Promise<[UnionLoginResult](#section556415719255)>
...
Promise<[GSKLocalPlayer](#section1033653365917)>
...
AsyncCallback<[GSKLocalPlayer](#section1033653365917)>
...
[ThirdUserInfo](#section1036174617108)
...
[GSKPlayerRole](#section116223815415)
...
Callback<[PlayerChangedResult](#section7712953312)>
...
try {
  // 参数this.onPlayerChangedEventCallback为gamePlayer.on('playerChanged', this.onPlayerChangedEventCallback)中第二个参数
  gamePlayer.off('playerChanged', this.onPlayerChangedEventCallback);
} catch (error) {
  let err = error as BusinessError;
  hilog.error(0x0000, 'testTag', `Failed to unregister. Code: ${err.code}, message: ${err.message}`);
}
...
try {
  // 参数this.miniGameAddictionPreventedCallback为gamePlayer.on('miniGameAddictionPrevented', this.miniGameAddictionPreventedCallback)中第二个参数
  gamePlayer.off('miniGameAddictionPrevented', this.miniGameAddictionPreventedCallback);
} catch (error) {
  let err = error as BusinessError;
  hilog.error(0x0000, 'testTag', `Failed to unregister. Code: ${err.code}, message: ${err.message}`);
}
...
[MiniGameLoginParam](#section176210299469)
...
Promise<[MiniGamePlayer](#section859174864711)>
...
[ProductType](#section183551656122815)
