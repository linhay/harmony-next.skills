...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| ARENGINE_FEATURE_TYPE_SLAM | 0 | 平板查找特性。 |
| ARENGINE_FEATURE_TYPE_DEPTH | 1 | 深度特性。 |
| ARENGINE_FEATURE_TYPE_MESH | 2 | Mesh特性。 |
| ARENGINE_FEATURE_TYPE_IMAGE | 3 | 图像识别特性。 |
| ARENGINE_FEATURE_TYPE_SEMANTIC_DENSE | 4 | 高精几何特性。 |
| ARENGINE_FEATURE_TYPE_SEMANTIC | 5 | 语义识别特性。 |
| ARENGINE_FEATURE_TYPE_FACE | 6 | 人脸特性。 |
| ARENGINE_FEATURE_TYPE_BODY | 7 | 人体骨骼特性。 |
...
| --- | --- | --- |
| DISABLED | 0 | 关闭深度模式。 |
| AUTOMATIC | 1 | 开启深度模式。 AR Engine会自动尝试从多种深度源获取深度信息。 说明： 通常有两种深度源，运动算法和硬件深度传感器 (Time of Flight，简称ToF)。目前仅支持运动算法。 |
...
| --- | --- | --- |
| UNKNOWN | 0 | 无效的图像格式。 |
| YUV_420_888 | 2 | YUV_420_888格式，适用于处理高分辨率的图像数据。 |
| Y_8 | 3 | Y_8格式，适用于对图像数据要求较低的精度或存储空间的场景。 |
| Y_16 | 4 | Y_16格式，适用于对图像数据精度要求较高的场景。 |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| GRAVITY | 0 | 由重力确定。 世界坐标系的Y轴与重力方向一致，原点为设备所在的原始位置。参见AR Engine重力对齐世界坐标系。 |
| GRAVITY_AND_HEADING | 1 | 由重力和指南针确定。 世界坐标系的Y轴与重力方向一致，X轴指向指南针北向，原点为设备所在的原始位置。参见AR Engine重力对齐北向坐标系。 说明： 该选项当前仅支持省电模式，请将ARPowerMode置为POWER_SAVING以启动该选项。 |
...
设备行为差异： 该接口在部分手机和部分平板设备中可以正常调用，在其他设备中无法正常调用。开发者可以通过使用[arViewController.isARTypeSupported](arViewController（AR场景管理能力）.md#ZH-CN_TOPIC_0000002522082116__arviewcontrollerisartypesupported)接口查询能力是否支持。
...
以下错误码的详细介绍请参见[通用错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-universal)和[AR Engine错误码](AR Engine错误码.md)。
...
将图像添加到图像数据库，并输出对应图像的索引。使用Promise异步回调。
...
释放增强图像数据库对象[ARAugmentedImageDatabase](#ZH-CN_TOPIC_0000002553202077__araugmentedimagedatabase)占用的内存。使用Promise异步回调。
...
[ARSession](#ZH-CN_TOPIC_0000002553202077__arsession)的功能配置参数。
...
由于此函数并没有释放锚点[ARAnchor](#ZH-CN_TOPIC_0000002553202077__aranchor)，开发者需要通过调用[ARAnchor.release](#ZH-CN_TOPIC_0000002553202077__aranchorrelease)来释放锚点。
...
释放管理AR Engine系统状态对象[ARSession](#ZH-CN_TOPIC_0000002553202077__arsession)占用的内存。使用Promise异步回调。
...
| state | number | 否 | 否 | 添加增强图像的结果状态。成功为0，否则为对应错误码。 |
...
这个函数不会释放锚点，开发者需要通过调用[ARAnchor.release](#ZH-CN_TOPIC_0000002553202077__aranchorrelease)方法来释放锚点。
...
释放锚点对象[ARAnchor](#ZH-CN_TOPIC_0000002553202077__aranchor)占用的内存。使用Promise异步回调。
...
这个类继承自[ARTrackable](#ZH-CN_TOPIC_0000002553202077__artrackable)类。
...
let data: ArrayBuffer = faceBlendShapes.getData();
```
...
let face: arEngine.ARFace = trackables[0] as arEngine.ARFace;
let faceBlendShapes: arEngine.ARBlendShapes = face.getBlendShapes();
let types: Array<arEngine.ARBlendShapeType> = faceBlendShapes.getTypes();
```
...
释放[ARBlendShapes](#ZH-CN_TOPIC_0000002553202077__arblendshapes)对象。使用Promise异步回调。
...
let data: ArrayBuffer = faceLandmark.getVertices2D();
```
...
let face: arEngine.ARFace = trackables[0] as arEngine.ARFace;
let faceLandmark: arEngine.ARLandmark = face.getLandmark();
let data: ArrayBuffer = faceLandmark.getVertices3D();
```
...
释放[ARLandmark](#ZH-CN_TOPIC_0000002553202077__arlandmark)对象。使用Promise异步回调。
...
这个类继承自[ARAnchor](#ZH-CN_TOPIC_0000002553202077__aranchor)类。
...
if (arbodys.length > 0) {
    const landmarks: arEngine.ARBodyLandmark2D[] = arbodys[0].getLandmarks2D();
    const trakeId: number = arbodys[0].trackId;
}
```
...
acquireBodySkeleton(): Array<[ARBody](#ZH-CN_TOPIC_0000002553202077__arbody)>
...
// arSession创建参考ARSession.getFrame接口示例代码
let frame: arEngine.ARFrame = arSession.getFrame();
let arBodys: arEngine.ARBody[] = frame.acquireBodySkeleton();
```
...
释放帧数据对象[ARFrame](#ZH-CN_TOPIC_0000002553202077__arframe)占用的内存。使用Promise异步回调。
...
let face: arEngine.ARFace = trackables[0] as arEngine.ARFace;
let faceGeometry: arEngine.ARGeometry = face.getGeometry();
let vertices = faceGeometry.getVertices();
```
...
let face: arEngine.ARFace = trackables[0] as arEngine.ARFace;
let faceGeometry: arEngine.ARGeometry = face.getGeometry();
let texCoord: ArrayBuffer = faceGeometry.getTexCoord();
```
...
let face: arEngine.ARFace = trackables[0] as arEngine.ARFace;
let faceGeometry: arEngine.ARGeometry = face.getGeometry();
let indices: ArrayBuffer = faceGeometry.getIndices();
```
...
let face: arEngine.ARFace = trackables[0] as arEngine.ARFace;
let faceGeometry: arEngine.ARGeometry = face.getGeometry();
let triangleLabels: ArrayBuffer = faceGeometry.getTriangleLabels();
```
...
释放[ARGeometry](#ZH-CN_TOPIC_0000002553202077__argeometry)对象。使用Promise异步回调。
...
如果[ARPointOrientationMode](#ZH-CN_TOPIC_0000002553202077__arpointorientationmode)是ESTIMATED_SURFACE_NORMAL，则 X+ 是射线的垂直向量，也就是平面的平行向量，Y+ 是平面的法向量，Z+ 是平面的平行向量，大致指向相机。

如果[ARPointOrientationMode](#ZH-CN_TOPIC_0000002553202077__arpointorientationmode)是INITIALIZED_TO_IDENTITY，则坐标的旋转不会随着平面的角度变化而变化。X+ 是射线的垂直向量，指向右侧（从设备的视角），Y+ 指向上方，Z+ 大致指向相机。
...
释放命中检测结果对象[ARHitResult](#ZH-CN_TOPIC_0000002553202077__arhitresult)占用的内存。使用Promise异步回调。
...
| width | number | 是 | 否 | 当前帧的图像宽度，以像素为单位。其数值范围与设备相机像素有关。 |
| height | number | 是 | 否 | 当前帧的图像高度，以像素为单位。其数值范围与设备相机像素有关。 |
...
释放相机视频流帧对象[ARImage](#ZH-CN_TOPIC_0000002553202077__arimage)占用的内存。使用Promise异步回调。
...
释放位姿对象[ARPose](#ZH-CN_TOPIC_0000002553202077__arpose)占用的内存。使用Promise异步回调。
...
释放环境网格数据对象[ARSceneMesh](#ZH-CN_TOPIC_0000002553202077__arscenemesh)占用的内存。使用Promise异步回调。
...
释放高精几何重建对象数据[ARSemanticDenseData](#ZH-CN_TOPIC_0000002553202077__arsemanticdensedata)占用的内存。使用Promise异步回调。
...
释放被追踪对象[ARTrackable](#ZH-CN_TOPIC_0000002553202077__artrackable)占用的内存。使用Promise异步回调。
...
在AR Engine中创建和初始化一个新的[ARPose](#ZH-CN_TOPIC_0000002553202077__arpose)对象并返回。
