...
| 名称 | 描述 |
| --- | --- |
| NN_QuantParam *OH_NNQuantParam_Create() | 创建一个NN_QuantParam量化参数实例。 |
| OH_NN_ReturnCode OH_NNQuantParam_SetScales(NN_QuantParam *quantParams, const double *scales, size_t quantCount) | 设置NN_QuantParam的缩放系数。 参数quantCount是张量中量化参数的数量，例如对于per-channel量化，quantCount就是通道数量。 |
| OH_NN_ReturnCode OH_NNQuantParam_SetZeroPoints(NN_QuantParam *quantParams, const int32_t *zeroPoints, size_t quantCount) | 设置NN_QuantParam的零点。 参数quantCount是张量中量化参数的数量，例如对于per-channel量化，quantCount就是通道数量。 |
| OH_NN_ReturnCode OH_NNQuantParam_SetNumBits(NN_QuantParam *quantParams, const uint32_t *numBits, size_t quantCount) | 设置NN_QuantParam的量化位数。 参数quantCount是张量中量化参数的数量，例如对于per-channel量化，quantCount就是通道数量。 |
| OH_NN_ReturnCode OH_NNQuantParam_Destroy(NN_QuantParam **quantParams) | 销毁NN_QuantParam实例。  当设置NN_QuantParam实例到一个NN_Tensor中后，如果不再使用该实例，需要销毁它以避免内存泄漏。  如果quantParams或*quantParams是空指针，那么该接口仅打印警告日志，不会执行销毁操作。 |
| OH_NNModel *OH_NNModel_Construct(void) | 创建OH_NNModel类型的模型实例，搭配OH_NNModel模块提供的其他接口，完成模型实例的构造。 |
...
| OH_NN_ReturnCode OH_NNModel_SetTensorData(OH_NNModel *model, uint32_t index, const void *dataBuffer, size_t length) | 设置张量的数值。 对于具有常量值的张量（如模型的权重），需要在构图阶段使用该接口设置数值。张量的索引值根据张量添加进模型的顺序决定，张量的添加参考OH_NNModel_AddTensorToModel。 |
| OH_NN_ReturnCode OH_NNModel_SetTensorQuantParams(OH_NNModel *model, uint32_t index, NN_QuantParam *quantParam) | 设置张量的量化参数，参考NN_QuantParam。 |
| OH_NN_ReturnCode OH_NNModel_SetTensorType(OH_NNModel *model, uint32_t index, OH_NN_TensorType tensorType) | 设置张量的类型，参考OH_NN_TensorType。 |
...
| void OH_NNModel_Destroy(OH_NNModel **model) | 销毁模型实例。调用OH_NNModel_Construct创建的模型实例需要调用该接口主动销毁，否则将造成内存泄漏。 如果model为空指针或者*model为空指针，该接口仅打印警告日志，不执行销毁操作。 |
...
| void OH_NNExecutor_DestroyInputMemory(OH_NNExecutor *executor, uint32_t inputIndex, OH_NN_Memory **memory) | 释放OH_NN_Memory实例指向的输入内存。 调用OH_NNExecutor_AllocateInputMemory创建的内存实例，需要主动调用该接口进行释放，否则将造成内存泄漏。 |
| void OH_NNExecutor_DestroyOutputMemory(OH_NNExecutor *executor, uint32_t outputIndex, OH_NN_Memory **memory) | 释放OH_NN_Memory实例指向的输出内存。 调用OH_NNExecutor_AllocateOutputMemory创建的内存实例，需要主动调用该接口进行释放，否则将造成内存泄漏。outputIndex和memory的对应关系需要和创建内存实例时保持一致。 如果memory或*memory为空指针，该接口仅打印警告日志，不执行释放操作。 |
| OH_NN_ReturnCode OH_NNExecutor_SetInputWithMemory(OH_NNExecutor *executor,uint32_t inputIndex,const OH_NN_Tensor *tensor,const OH_NN_Memory *memory) | 将OH_NN_Memory实例指向的硬件共享内存，并指定为单个输入使用的内存。 |
| OH_NN_ReturnCode OH_NNExecutor_SetOutputWithMemory(OH_NNExecutor *executor,uint32_t outputIndex,const OH_NN_Memory *memory) | 将OH_NN_Memory实例指向的硬件共享内存，并指定为单个输出使用的内存。 |
...
创建[NN_QuantParam](NN_QuantParam.md)量化参数实例后，调用[OH_NNQuantParam_SetScales](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnquantparam_setscales)、[OH_NNQuantParam_SetZeroPoints](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnquantparam_setzeropoints)或[OH_NNQuantParam_SetNumBits](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnquantparam_setnumbits)设置它的属性值，并调用[OH_NNModel_SetTensorQuantParams](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_settensorquantparams)将它设置到[NN_Tensor](NN_Tensor.md)中。最后再调用[OH_NNQuantParam_Destroy](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnquantparam_destroy)销毁它，以避免内存泄露。
...
在开始构图前，先调用[OH_NNModel_Construct](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_construct)创建模型实例，根据模型的拓扑结构，调用[OH_NNModel_AddTensorToModel](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_addtensortomodel)、[OH_NNModel_AddOperation](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_addoperation)和[OH_NNModel_SetTensorData](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_settensordata)方法，填充模型的数据节点和算子节点；然后调用[OH_NNModel_SpecifyInputsAndOutputs](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_specifyinputsandoutputs)指定模型的输入和输出；当构造完模型的拓扑结构，调用[OH_NNModel_Finish](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_finish)完成模型的构建。

模型实例使用完毕后，需要调用[OH_NNModel_Destroy](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_destroy)销毁模型实例，避免内存泄漏。
...
该接口根据[NN_TensorDesc](NN_TensorDesc.md)向model实例中添加张量，张量添加的顺序是模型中记录张量的索引值。[OH_NNModel_SetTensorData](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_settensordata)、[OH_NNModel_AddOperation](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_addoperation)和[OH_NNModel_SpecifyInputsAndOutputs](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_specifyinputsandoutputs)接口根据该索引值，指定不同的张量。
...
张量的索引值根据张量添加进模型的顺序决定，张量的添加参考[OH_NNModel_AddTensorToModel](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_addtensortomodel)。
...
设置张量的类型，参考[OH_NN_TensorType](neural_network_runtime_type.h.md#ZH-CN_TOPIC_0000002522242588__oh_nn_tensortype)。
...
该接口将对算子参数的属性和输入、输出张量的数量进行校验，这些属性需要在调用[OH_NNModel_AddTensorToModel](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_addtensortomodel)添加张量时正确设置。

每个算子期望的参数、输入和输出属性请参考[OH_NN_OperationType](neural_network_runtime_type.h.md#ZH-CN_TOPIC_0000002522242588__oh_nn_operationtype)。

paramIndices、inputIndices和outputIndices中存储的是张量的索引值，每个索引值根据张量添加进模型的顺序决定，正确设置并添加算子要求准确设置每个张量的索引值。

张量的添加参考[OH_NNModel_AddTensorToModel](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_addtensortomodel)。

如果添加算子时，添加了额外的参数（非算子需要的参数），该接口返回[OH_NN_INVALID_PARAMETER](neural_network_runtime_type.h.md#ZH-CN_TOPIC_0000002522242588__oh_nn_returncode)；如果没有设置算子参数，则算子按默认值设置缺省的参数，默认值请参考[OH_NN_OperationType](neural_network_runtime_type.h.md#ZH-CN_TOPIC_0000002522242588__oh_nn_operationtype)。
...
模型实例需要指定张量作为端到端的输入和输出张量。设置一个张量为输入或输出张量后，就不能再通过调用[OH_NNModel_SetTensorData](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_settensordata)设置张量数据，而需要在执行阶段调用OH_NNExecutor的方法设置输入或输出张量数据。

张量的索引值根据张量添加进模型的顺序决定，张量的添加参考[OH_NNModel_AddTensorToModel](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_addtensortomodel)。 暂时不支持异步设置模型输入和输出张量。
...
在调用该接口后，无法进行额外的构图操作，调用[OH_NNModel_AddTensorToModel](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_addtensortomodel)、[OH_NNModel_AddOperation](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_addoperation)、[OH_NNModel_SetTensorData](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_settensordata)和[OH_NNModel_SpecifyInputsAndOutputs](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_specifyinputsandoutputs)将返回[OH_NN_OPERATION_FORBIDDEN](neural_network_runtime_type.h.md#ZH-CN_TOPIC_0000002522242588__oh_nn_returncode)。

在调用[OH_NNModel_GetAvailableOperations](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_getavailableoperations)和[OH_NNCompilation_Construct](neural_network_core.h.md#ZH-CN_TOPIC_0000002522082592__oh_nncompilation_construct)之前，必须先调用该接口完成构图。
...
调用[OH_NNModel_Construct](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_construct)创建的模型实例需要调用该接口主动销毁，否则将造成内存泄漏。
...
替代接口： [OH_NNModel_AddTensorToModel](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_addtensortomodel)
...
- 固定形状输入的场景：tensor各属性必须和构图阶段调用[OH_NNModel_AddTensor](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnmodel_addtensor)添加的张量保持一致；

-

动态形状输入的场景：在构图阶段，由于动态输入的形状不确定，调用该接口时，要求tensor.dimensions中的每个值必须大于0，以确定执行计算阶段输入的形状。设置形状时，只允许调整数值为-1的维度。

假设在构图阶段，输入A的维度为[-1, 224, 224, 3]，调用该接口时，只能调整第一个维度的尺寸，如：[3, 224, 224, 3]。调整其他维度将返回[OH_NN_INVALID_PARAMETER](neural_network_runtime_type.h.md#ZH-CN_TOPIC_0000002522242588__oh_nn_returncode)。

起始版本： 9

废弃版本： 11

替代接口： [OH_NNExecutor_RunSync](neural_network_core.h.md#ZH-CN_TOPIC_0000002522082592__oh_nnexecutor_runsync)
...
调用[OH_NNExecutor_Run](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnexecutor_run)完成单次模型推理后，Neural Network Runtime将比对dataBuffer指向的内存与输出数据的长度，根据不同情况，返回不同结果：

- 如果内存大小大于或等于数据长度：则推理后的结果将拷贝至内存，并返回[OH_NN_SUCCESS](neural_network_runtime_type.h.md#ZH-CN_TOPIC_0000002522242588__oh_nn_returncode)，可以通过访问dataBuffer读取推理结果。

- 如果内存大小小于数据长度：则[OH_NNExecutor_Run](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnexecutor_run)将返回[OH_NN_INVALID_PARAMETER](neural_network_runtime_type.h.md#ZH-CN_TOPIC_0000002522242588__oh_nn_returncode)，并输出日志告知内存太小的信息。
...
替代接口： [OH_NNTensor_CreateWithSize](neural_network_core.h.md#ZH-CN_TOPIC_0000002522082592__oh_nntensor_createwithsize)
...
调用[OH_NNExecutor_AllocateInputMemory](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnexecutor_allocateinputmemory)创建的内存实例，需要主动调用该接口进行释放，否则将造成内存泄漏。
...
替代接口： [OH_NNTensor_Destroy](neural_network_core.h.md#ZH-CN_TOPIC_0000002522082592__oh_nntensor_destroy)
...
调用[OH_NNExecutor_AllocateOutputMemory](neural_network_runtime.h.md#ZH-CN_TOPIC_0000002553362513__oh_nnexecutor_allocateoutputmemory)创建的内存实例，需要主动调用该接口进行释放，否则将造成内存泄漏。
