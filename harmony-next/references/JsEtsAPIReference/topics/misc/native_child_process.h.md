...
| Ability_NativeChildProcess_ErrCode OH_Ability_RegisterNativeChildProcessExitCallback(OH_Ability_OnNativeChildProcessExit onProcessExit) | - | 注册子进程退出回调。只有OH_Ability_StartNativeChildProcess、OH_Ability_StartNativeChildProcessWithConfigs和@ohos.app.ability.childProcessManager的startNativeChildProcess启动的子进程退出时才会触发所注册的回调函数。重复注册同一个回调函数只会保留一个。 |
...
创建一个子进程配置信息对象。创建对象成功后需要通过调用[OH_Ability_DestroyChildProcessConfigs](native_child_process.h.md#ZH-CN_TOPIC_0000002553200571__oh_ability_destroychildprocessconfigs)来销毁对象从而避免内存泄漏。
...
设置子进程配置信息对象的数据沙箱与网络环境的共享模式，详见[NativeChildProcess_IsolationMode](native_child_process.h.md#ZH-CN_TOPIC_0000002553200571__nativechildprocess_isolationmode)。该设置仅当调用[OH_Ability_StartNativeChildProcessWithConfigs](native_child_process.h.md#ZH-CN_TOPIC_0000002553200571__oh_ability_startnativechildprocesswithconfigs)、[OH_Ability_CreateNativeChildProcessWithConfigs](#ZH-CN_TOPIC_0000002553200571__oh_ability_createnativechildprocesswithconfigs)接口时生效。
...
[OH_IPCRemoteProxy_Destroy](ipc_cremote_object.h.md#ZH-CN_TOPIC_0000002553361205__oh_ipcremoteproxy_destroy)
...
设备行为差异： 对于API version 13及之前版本，该接口在PC/2in1中可正常使用，在其他设备中返回[NCP_ERR_NOT_SUPPORTED](#ZH-CN_TOPIC_0000002553200571__ability_nativechildprocess_errcode)错误码。对于API version 14及之后版本，该接口在PC/2in1、Tablet设备可正常使用，在其他设备中返回[NCP_ERR_NOT_SUPPORTED](#ZH-CN_TOPIC_0000002553200571__ability_nativechildprocess_errcode)错误码。
...
[OH_Ability_OnNativeChildProcessStarted](native_child_process.h.md#ZH-CN_TOPIC_0000002553200571__oh_ability_onnativechildprocessstarted)
...
设备行为差异： 该接口在PC/2in1、Tablet设备中可正常调用，在其他设备中返回[NCP_ERR_NOT_SUPPORTED](#ZH-CN_TOPIC_0000002553200571__ability_nativechildprocess_errcode)错误码。
...
设备行为差异： 对于API 13及之前版本，该接口在PC/2in1设备中可正常使用，在其他设备类型中返回[NCP_ERR_NOT_SUPPORTED](#ZH-CN_TOPIC_0000002553200571__ability_nativechildprocess_errcode)错误码。对于API 14及之后版本，该接口在PC/2in1、Tablet中可正常使用，在其他设备类型中返回[NCP_ERR_NOT_SUPPORTED](#ZH-CN_TOPIC_0000002553200571__ability_nativechildprocess_errcode)错误码。
...
设备行为差异： 该接口在PC/2in1、Tablet中可正常调用，在其他设备类型中返回[NCP_ERR_NOT_SUPPORTED](#ZH-CN_TOPIC_0000002553200571__ability_nativechildprocess_errcode)错误码。
...
通过[OH_Ability_StartNativeChildProcess](#ZH-CN_TOPIC_0000002553200571__oh_ability_startnativechildprocess)启动子进程后，子进程能够在任意so和任意子线程中获取启动参数[NativeChildProcess_Args](NativeChildProcess_Args.md)。
...
[OH_Ability_RegisterNativeChildProcessExitCallback](#ZH-CN_TOPIC_0000002553200571__oh_ability_registernativechildprocessexitcallback)

[OH_Ability_UnregisterNativeChildProcessExitCallback](#ZH-CN_TOPIC_0000002553200571__oh_ability_unregisternativechildprocessexitcallback)
...
注册Native子进程退出回调函数。只有[OH_Ability_StartNativeChildProcess](#ZH-CN_TOPIC_0000002553200571__oh_ability_startnativechildprocess)、[OH_Ability_StartNativeChildProcessWithConfigs](#ZH-CN_TOPIC_0000002553200571__oh_ability_startnativechildprocesswithconfigs)和[@ohos.app.ability.childProcessManager的startNativeChildProcess](@ohos.app.ability.childProcessManager (子进程管理).md#ZH-CN_TOPIC_0000002522080530__childprocessmanagerstartnativechildprocess13)启动的子进程退出时才会触发所注册的回调函数。当重复注册同一个回调函数时，子进程退出时只会执行一次回调函数。

参数必须实现[OH_Ability_OnNativeChildProcessExit](#ZH-CN_TOPIC_0000002553200571__oh_ability_onnativechildprocessexit)入口函数。详见[注册Native子进程退出回调](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/capi-nativechildprocess-exit-info)。
...
参数必须实现[OH_Ability_OnNativeChildProcessExit](#ZH-CN_TOPIC_0000002553200571__oh_ability_onnativechildprocessexit)入口函数。详见[解注册Native子进程退出回调](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/capi-nativechildprocess-exit-info)。
...
调用该接口无法终止[childProcessManager.startChildProcess](@ohos.app.ability.childProcessManager (子进程管理).md#ZH-CN_TOPIC_0000002522080530__childprocessmanagerstartchildprocess)接口在SELF_FORK模式下启动的子进程。
