...
https://hag.cloud.huawei.com/open-ability/v2/service-events/revoke
...
应用级AccessToken，前面需要加上Bearer，通过调用Oauth2.0的接口获得，接口请求https://oauth-login.cloud.huawei.com/oauth2/v3/token。
...
接口请求UTC时间戳，格式yyyymmddhhmmssSSS
...
华为分配的openId，openid推送的事件需要填openid——openId与sid不可同时为空，至少填入一项
...
华为分配的sid，sid推送的事件需要填sid——openId与sid不可同时为空，至少填入一项
