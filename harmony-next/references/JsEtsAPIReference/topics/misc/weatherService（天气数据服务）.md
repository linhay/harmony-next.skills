...
| city | City | 否 | 是 | 请求经纬度对应的城市信息，若设置了limitedDatasets且数组中不包含Dataset.CITY，该字段不返回。  起始版本： 6.1.0(23) 模型约束： 此接口仅可在Stage模型下使用。 |
...
| alertTitle | string | 否 | 是 | 预警标题。  起始版本： 6.1.0(23) 模型约束： 此接口仅可在Stage模型下使用。 |
...
| INDICES | 5 | 天气指数。 |
| TIDES | 6 | 潮汐。 |
| CITY | 7 | 城市信息。  起始版本： 6.1.0(23) 模型约束： 此接口仅可在Stage模型下使用。 |
