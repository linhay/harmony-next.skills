...
还提供实体抽取功能，通过对用户输入的文本进行实体识别。然后依据Kit中的实体类别来进行分类，其中用户可以根据实体类别列表中的类别来进行选择。输出结果中包含实体的类别、实体在原文本中的位置、实体的原文本以及实体解析后的其他字段。实体字段内容可参考[EntityType](#ZH-CN_TOPIC_0000002522242570__entitytype)详情。
...
| --- | --- | --- | --- | --- |
| word | string | 否 | 否 | 词语。 |
| wordTag | string | 否 | 否 | 词性。词性分类参考wordTag。 |
...
| jsonObject | string | 否 | 否 | 实体的其他字段。详情参考jsonObject。 |
