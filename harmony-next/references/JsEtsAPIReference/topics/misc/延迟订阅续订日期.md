...
1. 不支持对享受免费试用优惠期内的订阅延迟续订日期。
1. 过去365天内每个用户的同一个订阅最多延迟2次。
1. 每次调用API的订阅的下一个续订日期最短可延迟1天，最长为90天。
...
请求Id。开发者自行生成用于跟踪每个订阅续订日期延长请求唯一标识符的字符串。
...
延迟后的订阅有效期时间，标准时间戳，如：1737354632136。
