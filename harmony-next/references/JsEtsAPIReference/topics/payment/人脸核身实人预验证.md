...
| PayDevAuth | 是 | String | 取值为：PayDevAuth的JSON字符串 |
...
| credentialIdNo | 是 | String | 证件编号。最大长度为128。 说明： 敏感字段使用SM加密，具体参见上传开发者公钥及下载华为公钥-SM2加密示例代码参考。 |
| realName | 是 | String | 用户姓名。最大长度为128。 说明： 敏感字段使用SM加密，具体参见上传开发者公钥及下载华为公钥-SM2加密示例代码参考。 |
| openId | 是 | String | 用户对外展示的ID（获取方式请参见获取用户信息）。长度范围为1-512。 说明： 入参中的实名信息，需要和做人脸核身的用户保持一致。 不要求与openId用户的实名信息保持一致。 |
