...
开发者接入合单支付，首先需要调用该接口获取到预支付ID（prepayId），用prepayId构建[orderStr](数据模型说明.md#ZH-CN_TOPIC_0000002553362425__orderstr)参数返回给客户端调用支付接口拉起华为支付收银台。
...
说明：元服务预下单接口请使用[https://petalpay-developer.cloud.huawei.com.cn/api/v2/partner/combined/preorder/create/fa](https://petalpay-developer.cloud.huawei.com.cn/api/v2/partner/combined/preorder/create/fa)
...
| expireTime | 否 | String | 交易过期时间。此时间必须为准确的UTC时间。 格式要求："yyyy-MM-dd'T'HH:mm:ss.SSSZ" 。 说明： - 下单过期时间，不传默认2个小时，如果传递则最小值无限制，最大180天，超过180天系统会报错。 - 传已过时间可能会导致订单因过期、超时等原因异常关闭。 - 开发者可以参考如下函数获取对应的UTC过期时间示例。 |
