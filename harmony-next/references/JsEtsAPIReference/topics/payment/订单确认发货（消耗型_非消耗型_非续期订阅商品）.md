...
开发者服务器收到IAP服务端关键事件通知后，调用[订单状态查询（消耗型/非消耗型/非续期订阅型商品）](订单状态查询（消耗型_非消耗型_非续期订阅商品）.md)接口获取订单的最新状态，再根据订单状态发放权益，具体请参见[确保权益发放](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/iap-delivering-products#确保权益发放)。
