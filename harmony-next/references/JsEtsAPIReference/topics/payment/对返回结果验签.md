...
从header的x5c中获取证书链（依次是叶子证书、中间证书、根证书），使用[Huawei CBG Root CA G2](https://pki.consumer.huawei.com/ca/cer/RootCaG2Ecdsa.cer)证书对证书链进行验证。
...
通过Header指定的算法和PublicKey进行JWT验签，可以使用各种开源库来验签，具体请参见[jwt.io](https://jwt.io/)。
