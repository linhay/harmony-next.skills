...
Context上下文，仅支持包含connectServiceExtensionAbility方法的Context（例：[UIAbilityContext](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext#uiabilitycontext-1)）。
...
[AuthClient](#section876918413377)

权限管理客户端，存储了权限模块的相关方法。

**错误码：**

以下错误码的详细介绍请参见[Wear Engine ArkTS API错误码](../../errors/ArkTS API错误码.md)。

错误码ID

错误信息

[401](../../errors/ArkTS API错误码.md#section201741341307)

Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameter types.

[801](../../errors/ArkTS API错误码.md#section5941173162512)

Capability not supported.

[1008509999](../../errors/ArkTS API错误码.md#section93011432734)
...
[AuthorizationRequest](#section12513135291215)
...
Promise<[AuthorizationResponse](#section1536013509131)>
...
[1008500001](../../errors/ArkTS API错误码.md#section1047474644710)

Network error. The network is unavailable.

[1008500004](../../errors/ArkTS API错误码.md#section485113712218)

App has not applied for the Wear Engine service.

[1008500006](../../errors/ArkTS API错误码.md#section1461311549277)

User privacy is not agreed.

[1008500007](../../errors/ArkTS API错误码.md#section15917165718219)

The device capability is not supported.

[1008500008](../../errors/ArkTS API错误码.md#section612518413314)

Account error. The user has not logged in with HUAWEI ID.

[1008500009](../../errors/ArkTS API错误码.md#section94850224314)
...
获取用户状态权限。如：穿戴设备的佩戴状态。

MOTION_SENSOR

3

获取对端设备运动传感器数据权限。如：加速度传感器数据。

HEALTH_SENSOR

4

获取对端设备人体传感器数据权限。如：心率传感器数据。
...
[DeviceClient](#section126321419369)

Device客户端，存储了Device模块的相关方法。
...
Promise<[Device](#section385118379447)[]>
...
[DeviceCategory](#section145591125155713)
...
设备是否为智能表。true：是，false：不是。

osCategory

[OsCategory](#section8322015161010)
...
[WearEngineCapability](#section1639211301479)
...
[1008500002](../../errors/ArkTS API错误码.md#section18859424615)

No device is bound.

[1008500003](../../errors/ArkTS API错误码.md#section11616131326)
...
[DeviceCapability](#section0965105510366)
...
[1008500005](../../errors/ArkTS API错误码.md#section665613120197)
...
[MonitorClient](#section178254313113)

Monitor客户端，存储了Monitor模块的相关方法。
...
[Device](#section385118379447)的随机标识符，用于指定本次查询的设备。

item

[MonitorItem](#section16422462613)

是

可查询的设备状态枚举，用于指定本次查询的状态。
...
Promise<[MonitorData](#section12024822410)>

Promise对象，返回查询的结果。不同状态返回值的含义请参考[MonitorItem](#section16422462613)。
...
[1008500010](../../errors/ArkTS API错误码.md#section411611281734)
...
[Device](#section385118379447)的随机标识符，用于指定本次订阅的设备。

type

[MonitorEvent](#section0407141762619)

是

可订阅的设备状态枚举，用于指定本次订阅监听的设备状态。

callback

Callback<[MonitorEventData](#section38571246164212)>

是

回调函数，状态发生变化后执行，传入[MonitorEventData](#section38571246164212)类。建议保证回调函数的生命周期延长至取消监听时，详情请见[unsubscribeEvent](#section182736075018)。
...
[1008500012](../../errors/ArkTS API错误码.md#section1599615141614)
...
[Device](#section385118379447)的随机标识符，用于指定本次取消订阅的设备。
...
回调函数。此处需保证传入的对象与调用[subscribeEvent](#section74368231248)接口时传入的回调函数为同一个对象。
...
当使用[queryStatus](#section3383841153218)接口查询设备状态时，返回值含义请见[MonitorItem](#section16422462613)。

当使用[subscribeEvent](#section74368231248)接口订阅监听时，返回值含义请见[MonitorEvent](#section0407141762619)。
...
[MonitorData](#section12024822410)
...
[P2pClient](#section10175450143316)

P2p客户端，存储了P2p模块的相关方法。
...
Promise对象。表示设备侧应用是否安装。true：已安装，false：未安装。
...
Promise<[P2pResult](#section75108813115)>

Promise对象。属性中的code字段表示本次拉起应用的结果。
...
[P2pAppParam](#section7874153142020)
...
[P2pMessage](#section188531948121319)

是

需要传输的消息内容，取值范围：[1，4096)，单位字节。
...
Promise对象。返回[P2pResult](#section75108813115)对象，其属性中的code字段表示本次消息发送的结果。
...
AsyncCallback<[P2pResult](#section75108813115)>
...
[1008500011](../../errors/ArkTS API错误码.md#section4583155021811)
...
Promise对象。属性中的code字段表示本次取消文件发送的结果。
...
Callback<[P2pMessage](#section188531948121319)>

是

接收到设备侧应用发送的消息后执行的回调函数。
...
Callback<[P2pFile](#section1223384152)>

是

接收到设备侧应用发送的文件后执行的回调函数。
...
[应用指纹，用于标识应用的唯一身份。](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/wearengine_faq-9)
...
REMOTE_APP_NOT_INSTALLED

200

设备侧应用未安装。

REMOTE_APP_NOT_RUNNING
...
用于返回除文件传输外的P2p通信结果，返回值含义见[P2pResultCode](#section34326155711)。
...
传输消息的内容，格式为Uint8Array（二进制字节数组）类型数据。
...
[fs.File](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-file-fs#file)
...
[NotifyClient](#section82221754103913)

模板化通知客户端，存储了发送模板化通知的相关方法。
...
[NotificationOptions](#section0751457344)
...
[Notification](#section33797461644)
...
[NotificationFeedback](#section498821414418)
...
[NotificationType](#section45813154136)
...
通知的内容，取值范围：[1，400)，单位字节。

buttons

[NotificationButton](#section7336124218819)[]
...
NOTIFICATION_WITH_ONE_BUTTON

51

拥有一个按钮的通知类型。

NOTIFICATION_WITH_TWO_BUTTONS

52

拥有两个按钮的通知类型。

NOTIFICATION_WITH_THREE_BUTTONS
...
[NotificationAction](#section924615208466)
...
错误码，含义请见[NotificationErrorCode](#section9782193965320)。
...
NOTIFICATION_SWITCHED_TO_BACKGROUND

0

用户使用Home键退出或熄屏，通知退回后台。
...
Wear Engine内部错误。通过[在线提单](https://developer.huawei.com/consumer/cn/support/feedback/#/)提交问题，华为支持人员会及时处理。
...
[SensorClient](#section1743102372015)

Sensor客户端，存储了Sensor模块的相关方法。
...
[Device](#section385118379447)的随机标识符，用于指定设备。
...
Promise<[Sensor](#section19142203612579)[]>

Promise对象，返回设备侧可用的传感器列表。
...
[SensorType](#section2420171225210)
...
Callback<[SensorResult](#section19118123861519)>
...
回调函数，用于处理传感器上报的数据，需要同订阅监听时的回调函数为同一个对象。
...
ECG（electrocardiograph）传感器。

PHOTOPLETHYSMOGRAPHY

1

PPG（photoplethysmogram）传感器。
...
传感器分辨率，当前仅作为Sensor对象的返回值信息。

isUtcTimestampSupported
...
传感器是否支持UTC（ Coordinated Universal Time）时间戳。true：支持，false：不支持。
...
数据内容，格式及含义请参考[穿戴设备传感器数据格式及样例](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/device_sensor#section18811142832216)。
...
错误码，含义请见[SensorErrorCode](#section173105551894)。
