...
创建OH_AVBuffer实例。需要注意的是，返回值指向的创建OH_AVBuffer的实例需要开发者主动调用接口释放，请参阅[OH_AVBuffer_Destroy](#ZH-CN_TOPIC_0000002522241858__oh_avbuffer_destroy)。
...
获取除基础属性外的其他参数，信息在OH_AVFormat中承载。需要注意的是，返回值指向的创建OH_AVFormat的实例需要开发者主动释放，请参阅[OH_AVFormat_Destroy](native_avformat.h.md#ZH-CN_TOPIC_0000002553361783__oh_avformat_destroy)。
...
获取OH_NativeBuffer实例的指针。需要注意的是，返回值指向的创建OH_NativeBuffer的实例需要开发者主动调用接口释放，请参阅[OH_NativeBuffer_Unreference](native_buffer.h.md#ZH-CN_TOPIC_0000002522082148__oh_nativebuffer_unreference)。
