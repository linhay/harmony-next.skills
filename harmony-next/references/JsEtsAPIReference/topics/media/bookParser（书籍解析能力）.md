...
本模块提供书籍解析的能力，支持对txt、epub、mobi、azw、azw3格式的书籍文件进行解析。通过提前导入到[应用沙箱目录](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-sandbox-directory)中的书籍文件，初始化[BookParserHandler](#section59035422210)。将书籍基本信息、[书脊](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/reader-introduction#section2215512152119)内容列表、目录列表和章节内容解析出来。
...
书籍内容的字符集，根据书籍编码格式决定，例如："utf-8"。具体支持格式可参考[TextEncoder](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-util#textencoder)定义。（预留字段，暂不支持。）
...
书籍资源请求回调接口，在排版引擎渲染界面时调用，需配合ReaderComponentController的注册接口[on('resourceRequest')](../misc/readerCore（阅读核心能力）.md#section169541417131914)使用。如果有自定义背景及字体资源，需要在此返回对应资源。
...
本地书籍文件路径（仅支持当前应用下的[应用沙箱目录](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-sandbox-directory)）。
...
Promise<[BookParserHandler](#section59035422210)>
...
[1017000999](../../errors/ArkTS API错误码.md#section31031138161116)

Other error.

[1017010003](../../errors/ArkTS API错误码.md#section1855956131312)

Book file format is unexpected.

[1017010004](../../errors/ArkTS API错误码.md#section692725412147)
...
[BookInfo](#section38181821125613)
...
[1017000001](../../errors/ArkTS API错误码.md#section1427915529104)
...
[1017010002](../../errors/ArkTS API错误码.md#section1115195931210)
...
[CatalogItem](#section12325515195)[]
...
[SpineItem](#section1029864651619)[]
...
内容资源索引[SpineItem](#section1029864651619).index。
...
[1017010001](../../errors/ArkTS API错误码.md#section19552151617126)
...
开发者通过此接口可获取书封资源。同时排版引擎获取书籍里的图片等资源时，会优先调用该方法，如果获取不到资源会继续调用[on('resourceRequest')](../misc/readerCore（阅读核心能力）.md#section169541417131914)获取资源。
...
内容资源索引[SpineItem](#section1029864651619).index。如果传负数，如-1，代表获取书封。
...
目录节点带锚点的内容资源路径[CatalogItem](#section12325515195).href。
