...
作为Ability Kit的基础定义模块，AbilityBase提供了组件启动参数[Want](../../capi/headers/want.h.md)的定义与接口，可以用于应用组件间的信息传递。
