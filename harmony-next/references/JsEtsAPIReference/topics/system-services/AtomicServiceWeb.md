...
名称类型必填装饰器类型说明src[ResourceStr](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcestr)是-网页资源地址，访问网络资源需要在AGC配置业务域名，访问本地资源仅支持包内文件（$rawfile）。不支持通过状态变量（例如@State）动态更新地址。加载的网页中支持通过JS SDK提供的接口调用系统能力，具体以JS SDK为准。controller[AtomicServiceWebController](#ZH-CN_TOPIC_0000002529444909__atomicservicewebcontroller)是@ObjectLink通过AtomicServiceWebController可以控制AtomicServiceWeb组件各种行为。navPathStack[NavPathStack](../misc/Navigation.md#ZH-CN_TOPIC_0000002497444902__navpathstack10)否-路由栈信息。当使用NavDestination作为页面的根容器时，需传入NavDestination容器对应的NavPathStack处理页面路由。mixedMode[MixedMode](../misc/Enums.md#ZH-CN_TOPIC_0000002497605218__mixedmode)否@Prop设置是否允许加载超文本传输协议（HTTP）和超文本传输安全协议（HTTPS）混合内容，默认不允许加载HTTP和HTTPS混合内容。darkMode[WebDarkMode](../misc/Enums.md#ZH-CN_TOPIC_0000002497605218__webdarkmode9)否@Prop设置Web深色模式，默认关闭。forceDarkAccessboolean否@Prop设置网页是否开启强制深色模式。true表示设置网页开启强制深色模式，false表示设置网页不开启强制深色模式。默认值：false。该属性仅在darkMode开启深色模式时生效。nestedScroll15+[NestedScrollOptions](../misc/滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__nestedscrolloptions10对象说明) | [NestedScrollOptionsExt](../../types/interfaces/Interfaces（其他）.md#ZH-CN_TOPIC_0000002529445183__nestedscrolloptionsext14)否@Prop

设置嵌套滚动选项。

**元服务API：** 从API version 15开始，该接口支持在元服务中使用。

onMessageCallback<[OnMessageEvent](#ZH-CN_TOPIC_0000002529444909__onmessageevent)>否-H5页面通过JS SDK的postMessage()发送消息后，Web组件对应的页面返回或销毁时，触发该回调。onErrorReceiveCallback<[OnErrorReceiveEvent](#ZH-CN_TOPIC_0000002529444909__onerrorreceiveevent)>否-网页加载遇到错误时触发该回调。出于性能考虑，建议此回调中尽量执行简单逻辑。在无网络的情况下，触发此回调。onHttpErrorReceiveCallback<[OnHttpErrorReceiveEvent](#ZH-CN_TOPIC_0000002529444909__onhttperrorreceiveevent)>否-网页加载资源遇到的HTTP错误（响应码>=400)时触发该回调。onPageBeginCallback<[OnPageBeginEvent](#ZH-CN_TOPIC_0000002529444909__onpagebeginevent)>否-网页开始加载时触发该回调，且只在主frame触发，iframe或者frameset的内容加载时不会触发此回调。onPageEndCallback<[OnPageEndEvent](#ZH-CN_TOPIC_0000002529444909__onpageendevent)>否-网页加载完成时触发该回调，且只在主frame触发。onControllerAttachedCallback<void>否-当Controller成功绑定到Web组件时触发该回调。onLoadIntercept[OnLoadInterceptCallback](#ZH-CN_TOPIC_0000002529444909__onloadinterceptcallback)否-当Web组件加载url之前触发该回调，用于判断是否阻止此次访问。默认允许加载。onProgressChangeCallback<[OnProgressChangeEvent](../../types/interfaces/Interfaces（其他）.md#ZH-CN_TOPIC_0000002529445183__onprogresschangeevent12)>否-网页加载进度变化时触发该回调。
...
以下错误码的详细介绍请参见[错误码](../../errors/Webview错误码.md)。
...
参数名类型必填说明urlstring | [Resource](../misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resource)是需要加载的 URL。headersArray<[WebHeader](#ZH-CN_TOPIC_0000002529444909__webheader)>否URL的附加HTTP请求头。
...
名称类型只读可选说明request[WebResourceRequest](../../types/classes/Class (WebResourceRequest).md)否否网页请求的封装信息。error[WebResourceError](../../types/classes/Class (WebResourceError).md)否否网页加载资源错误的封装信息 。
...
名称类型只读可选说明request[WebResourceRequest](../../types/classes/Class (WebResourceRequest).md)否否网页请求的封装信息。response[WebResourceResponse](../../types/classes/Class (WebResourceResponse).md)否否资源响应的封装信息。
...
名称类型只读可选说明data[WebResourceRequest](../../types/classes/Class (WebResourceRequest).md)否否网页请求的封装信息。
