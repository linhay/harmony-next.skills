...
pushCommon.[AppProfileType](../misc/pushCommon（推送服务公共信息）.md#section411410371767)
...
账号匿名标识，配置为调用[bindAppProfileId](#section20575105524912)()时设置的appProfileId。最大长度为64。
...
- pushService.PushType中DEFAULT（通知消息场景）起始版本：5.0.2(14)。调用示例代码请参考[应用在前台时处理通知消息](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/push-send-alert#section99251359545)中步骤3。
- IM（通知扩展消息场景）调用示例代码请参考[开发步骤](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/push-send-extend-noti#section7799156112814)中步骤3。
- VoIP（应用内通话消息场景）

调用示例代码请参考[开发步骤](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/push-voip#section41452724812)中步骤2。

- BACKGROUND（后台消息场景）

调用示例代码请参考[开发步骤](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/push-background#section8986919144115)中步骤5。

ability

[Ability](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-ability)
...
Callback<pushCommon.[PushPayload](../misc/pushCommon（推送服务公共信息）.md#section16335431712)>
...
若调用接收场景化消息接口（[pushService.receiveMessage](#section125883044911)）和注册分布式消息事件在同一个UIAbility中，则action.ohos.push.distribute.listener和action.ohos.push.listener定义在同一个actions数组中，否则分别定义在对应UIAbility中的actions中。
...
[Callee](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-uiability#callee)

是

通用组件服务端注册和解除客户端[Caller](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-uiability#caller)通知送信的callback接口，从UIAbility中获取。

callback

[DistributedMessageCallback](#section411410371767)
...
pushCommon.[PushPayload](../misc/pushCommon（推送服务公共信息）.md#section16335431712)
...
Promise对象。返回DistributedMessageResult对象，见[DistributedMessageResult](#section015518411017)说明。
...
[ResultCode](#section016942713918)

否

否

回调函数执行结果，枚举类型，见枚举[ResultCode](#section016942713918)说明。

#### ResultCode

表示[DistributedMessageCallback](#section411410371767)回调函数执行结果的枚举。
