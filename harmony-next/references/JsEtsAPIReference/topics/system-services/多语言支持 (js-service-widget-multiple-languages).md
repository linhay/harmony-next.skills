...
开发者仅需要通过[定义资源文件](#ZH-CN_TOPIC_0000002529445019__定义资源文件)和[引用资源](#ZH-CN_TOPIC_0000002529445019__引用资源)两个步骤，就可以使用开发框架的多语言能力。
...
在[文件组织](文件组织 (js-service-widget-file).md)中指定的i18n文件夹内放置每个语言地区下的资源定义文件即可，资源文件命名为“语言-地区.json”格式，例如英文（美国）的资源文件命名为en-US.json。当开发框架无法在应用中找到系统语言的资源文件时，默认使用en-US.json中的资源内容。
...
属性类型参数必填描述$tFunction请见$t参数说明是根据系统语言完成简单的替换：this.$t('strings.hello')

**表2** $t参数说明

参数类型必填描述pathstring是资源路径
-
...
属性类型参数必填描述$tcFunction请见$tc参数说明是
...
参数类型必填描述pathstring是资源路径countnumber是要表达的值
-
