...
通过[获取用户级凭证](获取用户级凭证.md)获取的Access Token。
...
到华为开发者联盟[API Console](https://developer.huawei.com/consumer/cn/console/overview)上，在“凭证”菜单中，选择创建服务账号密钥时会生成密钥文件，打开密钥文件中有sub_account 信息，该参数为密钥文件中的sub_account 的值。
...
public class GetDetailInfoDemo {
    public static void main(String[] args) throws IOException {
        ...
        // 替换为您获取到的Access Token
        String accessToken = "*<Access Token>*";
        // 替换为您创建服务账号时分配的私钥（秘钥文件中private_key的值，不含前缀"-----BEGIN PRIVATE KEY-----\n"，不含后缀"\n-----END PRIVATE KEY-----\n"）
        String privateKey = "*<Private Key>*";
        JSONObject result = getDetailInfo(url, accessToken);
        // 解析获取encryptedInfo
...
请根据[使用约束](#section74906714178)章节进行检查。
