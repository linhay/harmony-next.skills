...
应用针对不同模式使能HDR效果、设置的色彩空间以及设置相机输出流[Profile](Interfaces (其他).md#ZH-CN_TOPIC_0000002497605794__profile)中的[CameraFormat](Enums.md#ZH-CN_TOPIC_0000002497445814__cameraformat)一一对应关系可参考下表。例如，在录像模式下若需要选择HDR拍摄，相机预览输出流和录像输出流[Profile](Interfaces (其他).md#ZH-CN_TOPIC_0000002497605794__profile)中的[CameraFormat](Enums.md#ZH-CN_TOPIC_0000002497445814__cameraformat)可选择CAMERA_FORMAT_YCRCB_P010，色彩空间ColorSpace可选择设置2020_HLG_LIMIT。
...
参数名类型必填说明colorSpace[colorSpaceManager.ColorSpace](@ohos.graphics.colorSpaceManager (色彩管理).md#ZH-CN_TOPIC_0000002497445992__colorspace)是色彩空间，通过[getSupportedColorSpaces](Interface (ColorManagementQuery).md#ZH-CN_TOPIC_0000002497605780__getsupportedcolorspaces12)接口获取。
...
类型说明[colorSpaceManager.ColorSpace](@ohos.graphics.colorSpaceManager (色彩管理).md#ZH-CN_TOPIC_0000002497445992__colorspace)当前设置的色彩空间。
