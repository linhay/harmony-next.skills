...
参数项描述uint64_t deviceId设备ID，代表要获取描述符的设备。[struct UsbDeviceDescriptor](UsbDeviceDescriptor.md) *desc设备描述符，详细定义请参考[UsbDeviceDescriptor](UsbDeviceDescriptor.md)。
...
参数项描述uint64_t deviceId设备ID，代表要获取配置描述符的设备。uint8_t configIndex配置id，对应USB协议中的{@link bConfigurationValue}。struct [UsbDdkConfigDescriptor](UsbDdkConfigDescriptor.md) ** const config配置描述符，包含USB协议中定义的标准配置描述符，以及与其关联的接口描述符和端点描述符。
...
参数项描述const struct [UsbDdkConfigDescriptor](UsbDdkConfigDescriptor.md) * const config配置描述符，通过[OH_Usb_GetConfigDescriptor](usb_ddk_api.h.md#ZH-CN_TOPIC_0000002529285603__oh_usb_getconfigdescriptor)获得的配置描述符。
...
参数项描述uint64_t deviceId设备ID，代表要操作的设备。uint8_t interfaceIndex接口索引，对应USB协议中的[bInterfaceNumber](UsbInterfaceDescriptor.md)。uint64_t *interfaceHandle接口操作句柄，接口声明成功后，该参数将会被赋值。
...
参数项描述uint64_t interfaceHandle接口操作句柄，代表要操作的接口。[const struct UsbControlRequestSetup](UsbControlRequestSetup.md) *setup请求相关的参数，详细定义请参考[UsbControlRequestSetup](UsbControlRequestSetup.md)。uint32_t timeout超时时间，单位为毫秒。uint8_t *data要传输的数据。uint32_t *dataLen表示data的数据长度，在函数返回后，表示实际读取到的数据的长度。
...
参数项描述uint64_t interfaceHandle接口操作句柄，代表要操作的接口。[const struct UsbControlRequestSetup](UsbControlRequestSetup.md) *setup请求相关的参数，详细定义请参考[UsbControlRequestSetup](UsbControlRequestSetup.md)。uint32_t timeout超时时间，单位为毫秒。const uint8_t *data要传输的数据。uint32_t dataLen表示data数据长度。
...
参数项描述[const struct UsbRequestPipe](UsbRequestPipe.md) *pipe要传输数据的管道信息。[UsbDeviceMemMap](UsbDeviceMemMap.md) *devMmap数据缓冲区，可以通过[OH_Usb_CreateDeviceMemMap](usb_ddk_api.h.md#ZH-CN_TOPIC_0000002529285603__oh_usb_createdevicememmap)获得。
...
参数项描述[const struct UsbRequestPipe](UsbRequestPipe.md) *pipe要传输数据的管道信息。[DDK_Ashmem](DDK_Ashmem.md) *ashmem共享内存，可以通过[OH_DDK_CreateAshmem](ddk_api.h.md#ZH-CN_TOPIC_0000002497605608__oh_ddk_createashmem)获得。
...
参数项描述uint64_t deviceId设备ID，代表要创建缓冲区的设备。size_t size缓冲区的大小。[UsbDeviceMemMap](UsbDeviceMemMap.md) **devMmap创建的缓冲区通过该参数返回给调用者。
...
参数项描述[UsbDeviceMemMap](UsbDeviceMemMap.md) *devMmap销毁由[OH_Usb_CreateDeviceMemMap](usb_ddk_api.h.md#ZH-CN_TOPIC_0000002529285603__oh_usb_createdevicememmap)创建的缓冲区。
...
参数项描述[struct Usb_DeviceArray](Usb_DeviceArray.md) *devices已申请好的设备内存地址，用于存放获取到的设备ID列表及数量。
