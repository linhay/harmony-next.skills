...
以下错误码的详细介绍请参见[全局快捷键管理错误码](全局快捷键管理错误码.md)和[通用错误码](通用错误码.md)。
...
参数名类型必填说明typestring是事件类型，固定取值为'keyPressed'。options[KeyPressedConfig](#ZH-CN_TOPIC_0000002497445592__keypressedconfig16)是按键事件消费设置。callbackCallback<[KeyEvent](zh-cn_topic_0000002529445533.html#ZH-CN_TOPIC_0000002529445533__keyevent)>是回调函数，用于返回按键事件。订阅不同的按键事件需要使用不同的callback，否则订阅不生效。
