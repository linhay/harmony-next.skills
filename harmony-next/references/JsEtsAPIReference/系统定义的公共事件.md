...
公共事件类型定义在[ohos.commonEventManager模块的Support枚举](@ohos.commonEventManager (公共事件模块).md#ZH-CN_TOPIC_0000002497445532__support)中。
...
与这个公共事件相关的接口：setOsAccountName、setOsAccountProfilePhoto, 这些为系统API，setOsAccountDistributedInfon为公共API，具体参看[系统账号接口文档](@ohos.account.osAccount (系统账号管理).md)、[分布式账号接口文档](@ohos.account.distributedAccount (分布式账号管理).md)。
