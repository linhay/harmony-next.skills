...
参数项描述const [OH_Drawing_Rect](OH_Drawing_Rect.md)* rect指向矩形对象的指针。float xRadX轴上的圆角半径，小于或等于0时无效。float yRadY轴上的圆角半径，小于或等于0时无效。

**返回：**

类型说明[OH_Drawing_RoundRect](OH_Drawing_RoundRect.md)*函数会返回一个指针，指针指向创建的圆角矩形对象。
...
参数项描述const [OH_Drawing_RoundRect](OH_Drawing_RoundRect.md)* roundRect指向用于拷贝的圆角矩形对象[OH_Drawing_RoundRect](OH_Drawing_RoundRect.md)的指针。

**返回：**

类型说明[OH_Drawing_RoundRect](OH_Drawing_RoundRect.md)*函数会返回一个指针，指针指向创建的新圆角矩形对象。
...
参数项描述[OH_Drawing_RoundRect](OH_Drawing_RoundRect.md)* roundRect指向圆角矩形对象的指针。[OH_Drawing_CornerPos](#ZH-CN_TOPIC_0000002497606010__oh_drawing_cornerpos) pos圆角位置的枚举，支持类型可见[OH_Drawing_CornerPos](zh-cn_topic_0000002497606010.html#ZH-CN_TOPIC_0000002497606010__oh_drawing_cornerpos)。OH_Drawing_Corner_Radii radii圆角半径结构体OH_Drawing_Corner_Radii，其中包含x轴方向和y轴方向上的半径，半径小于等于0时无效。
...
参数项描述[OH_Drawing_RoundRect](OH_Drawing_RoundRect.md)* roundRect指向圆角矩形对象的指针。[OH_Drawing_CornerPos](#ZH-CN_TOPIC_0000002497606010__oh_drawing_cornerpos) pos圆角位置的枚举，支持类型可见[OH_Drawing_CornerPos](zh-cn_topic_0000002497606010.html#ZH-CN_TOPIC_0000002497606010__oh_drawing_cornerpos)。
...
参数项描述[OH_Drawing_RoundRect](OH_Drawing_RoundRect.md)* roundRect指向圆角矩形对象的指针。
...
参数项描述[OH_Drawing_RoundRect](OH_Drawing_RoundRect.md)* roundRect指向圆角矩形对象[OH_Drawing_Point2D](OH_Drawing_Point2D.md)的指针。float dxx轴方向偏移量。float dyy轴方向偏移量。
