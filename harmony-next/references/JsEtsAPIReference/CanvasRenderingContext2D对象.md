...
名称类型描述[fillStyle](#ZH-CN_TOPIC_0000002529285005__fillstyle)<color> | [CanvasGradient](zh-cn_topic_0000002497605014.html) | [CanvasPattern](CanvasPattern.md)
...
[lineWidth](#ZH-CN_TOPIC_0000002529285005__linewidth)number设置绘制线条的宽度。[strokeStyle](#ZH-CN_TOPIC_0000002529285005__strokestyle)<color> | [CanvasGradient](zh-cn_topic_0000002497605014.html) | CanvasPattern
...
参数名类型必填说明imageImage是图源对象，具体参考[Image对象](Image对象.md)。repetitionstring是设置图像重复的方式，取值为：'repeat'、'repeat-x'、 'repeat-y'、'no-repeat'。
...
参数名类型必填说明imageImage | PixelMap9+是图片资源，请参考[Image对象](Image对象.md) 或[PixelMap对象](Interface (PixelMap).md)。sxnumber是裁切源图像时距离源图像左上角的x坐标值。synumber是裁切源图像时距离源图像左上角的y坐标值。sWidthnumber是裁切源图像时需要裁切的宽度。sHeightnumber是裁切源图像时需要裁切的高度。dxnumber是绘制区域左上角在x轴的位置。dynumber是绘制区域左上角在y 轴的位置。dWidthnumber是绘制区域的宽度。dHeightnumber是绘制区域的高度。
...
创建一个线性渐变色，返回CanvasGradient对象，请参考[CanvasGradient对象](CanvasGradient对象.md)。
...
创建新的、空白的、指定大小的ImageData对象，请参考[ImageData对象](ImageData对象.md)。
...
类型说明[ImageData](ImageData对象.md)返回创建的ImageData对象。
...
根据一个现有的ImageData对象，重新创建一个宽、高相同但不会复制图像数据的ImageData对象，请参考[ImageData对象](ImageData对象.md)。

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

**参数：**

参数名类型必填说明imageData[ImageData](ImageData对象.md)是复制现有的ImageData对象。
...
以当前canvas指定区域内的像素创建[ImageData对象](ImageData对象.md)。
...
类型说明[ImageData](ImageData对象.md)返回包含指定区域像素的ImageData对象。
...
参数名类型必填说明imageData[ImageData](ImageData对象.md)是包含像素值的ImageData对象。dxnumber是填充区域在x轴方向的偏移量。dynumber是填充区域在y轴方向的偏移量。dirtyXnumber是源图像数据矩形裁切范围左上角距离源图像左上角的x轴偏移量。dirtyYnumber是源图像数据矩形裁切范围左上角距离源图像左上角的y轴偏移量。dirtyWidthnumber是源图像数据矩形裁切范围的宽度。dirtyHeightnumber是源图像数据矩形裁切范围的高度。
...
参数名类型必填说明imageData[ImageData](ImageData对象.md)是包含像素值的ImageData对象。dxnumber是填充区域在x轴方向的偏移量。dynumber是填充区域在y轴方向的偏移量。
...
类型说明[PixelMap](Interface (PixelMap).md)返回包含指定区域像素的PixelMap对象。
...
显示给定的[ImageBitmap对象](ImageBitmap对象.md)。

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

**参数：**

参数名类型必填说明bitmap[ImageBitmap](ImageBitmap对象.md)是待显示的ImageBitmap对象。
