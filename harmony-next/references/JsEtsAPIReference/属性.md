...
此接口只支持注册一个对象，若需要注册多个对象请使用[registerJavaScriptProxy9+](Class (WebviewController).md#ZH-CN_TOPIC_0000002497605192__registerjavascriptproxy)。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明javaScriptProxy[JavaScriptProxy](Interfaces（其他）.md#ZH-CN_TOPIC_0000002529445183__javascriptproxy12)是
...
- 通过@State变量控制横向滚动条的隐藏/显示后，需要调用[controller.refresh()](Class (WebviewController).md#ZH-CN_TOPIC_0000002497605192__refresh)生效。
...
- 该媒体播放策略将同时管控有声视频。
- 建议为所有Web组件设置相同的[audioExclusive](Interfaces（其他）.md#ZH-CN_TOPIC_0000002529445183__webmediaoptions10)值。
...
参数名类型必填说明options[WebMediaOptions](Interfaces（其他）.md#ZH-CN_TOPIC_0000002529445183__webmediaoptions10)是
...
参数名类型必填说明scriptsArray<[ScriptItem](Interfaces（其他）.md#ZH-CN_TOPIC_0000002529445183__scriptitem11)>是
...
- 支持嵌套滚动的容器：[Grid](Grid.md)、[List](List.md)、[Scroll](Scroll.md)、[Swiper](Swiper.md)、[Tabs](Tabs.md)、[WaterFlow](WaterFlow.md)、[Refresh](Refresh.md)、[bindSheet](半模态转场.md#ZH-CN_TOPIC_0000002497604850__bindsheet)。
...
参数名类型必填说明value[NestedScrollOptions](滚动组件通用接口.md#ZH-CN_TOPIC_0000002529284871__nestedscrolloptions10对象说明) | [NestedScrollOptionsExt](Interfaces（其他）.md#ZH-CN_TOPIC_0000002529445183__nestedscrolloptionsext14)14+是

可滚动组件滚动时的嵌套滚动选项。

value为NestedScrollOptions（向前、向后两个方向）类型时，scrollForward、scrollBackward默认滚动选项为[NestedScrollMode.SELF_FIRST](枚举说明.md#ZH-CN_TOPIC_0000002529284967__nestedscrollmode10)。
...
参数名类型必填说明config[NativeMediaPlayerConfig](Interfaces（其他）.md#ZH-CN_TOPIC_0000002529445183__nativemediaplayerconfig12)是
...
菜单项数量，及菜单的content大小、icon图标尺寸，与ArkUI [Menu](Menu.md)组件保持一致。
...
参数名类型必填说明options[EmbedOptions](Interfaces（其他）.md#ZH-CN_TOPIC_0000002529445183__embedoptions16)否
...
当enableDataDetector设置为true且[copyOptions](#ZH-CN_TOPIC_0000002497605206__copyoptions11)设置为CopyOptions.LocalDevice时，AI菜单功能将被激活。此时，在网页中选中文本后，文本选择菜单能够展示对应的AI菜单项，包括[TextMenuItemId](zh-cn_topic_0000002529444865.html#ZH-CN_TOPIC_0000002529444865__textmenuitemid12)中的url（打开连接）、email（新建邮件）、phoneNumber（呼叫）、address（导航至该位置）、dateTime（新建日程提醒）。
...
参数名类型必填说明detectConfig[BlankScreenDetectionConfig](Interfaces（其他）.md#ZH-CN_TOPIC_0000002529445183__blankscreendetectionconfig22)是白屏检测的策略配置。
...
从API version 8开始支持，从API version 10开始废弃。建议使用[setCustomUserAgent](Class (WebviewController).md#ZH-CN_TOPIC_0000002497605192__setcustomuseragent10)10+替代。
...
参数名类型必填说明expandedMenuOptionsArray<[ExpandedMenuItemOptions](Interfaces（其他）.md#ZH-CN_TOPIC_0000002529445183__expandedmenuitemoptionsdeprecated)>是

扩展菜单选项。

菜单项数量，及菜单的content大小、startIcon图标尺寸，与ArkUI [Menu](Menu.md)组件保持一致。
