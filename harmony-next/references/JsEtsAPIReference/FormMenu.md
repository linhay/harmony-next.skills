...
参数名类型必填说明want[Want](@ohos.app.ability.Want (Want).md#ZH-CN_TOPIC_0000002529284603__want)是待发布功能组件的want信息。componentIdstring是应用内功能组件ID，组件ID对应的界面与待添加的服务卡片界面相似。options[AddFormOptions](#ZH-CN_TOPIC_0000002529284951__addformoptions)否添加卡片选项。
...
名称类型只读可选说明formBindingData[formBindingData.FormBindingData](@ohos.app.form.formBindingData (卡片数据绑定类).md#ZH-CN_TOPIC_0000002529445243__formbindingdata)否是卡片数据。callbackAsyncCallback<string>否是返回添加卡片是否成功的结果回调。返回为0表示卡片添加成功，非0表示卡片添加失败，失败时请参考[卡片错误码信息](卡片错误码.md)进行排查。style[FormMenuItemStyle](#ZH-CN_TOPIC_0000002529284951__formmenuitemstyle)否是菜单自定义样式信息。
