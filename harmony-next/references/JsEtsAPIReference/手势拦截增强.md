...
提供系统内置手势与响应链上其他组件的手势设置并行关系的回调事件。此接口对应的C API接口为[setInnerGestureParallelTo](ArkUI_NativeGestureAPI_1.md#ZH-CN_TOPIC_0000002497605100__setinnergestureparallelto)。
...
当前仅支持[Tabs](Tabs.md)，其他组件请不要设置此参数。
...
从API version 20开始，该示例通过配置[onTouchTestDone](#ZH-CN_TOPIC_0000002529444829__ontouchtestdone20)指定手势识别器不参与后续手势处理，触发回调时，调用[preventBegin](zh-cn_topic_0000002497604864.html#ZH-CN_TOPIC_0000002497604864__preventbegin20)阻止手势识别器参与后续处理。点击Tap2和Tap1的重合区域，不调用preventBegin时，触发Tap2对应的手势；调用preventBegin阻止Tap2时，触发Tap1对应的手势。
