...
参数项描述[ArkUI_NodeHandle](ArkUI_Node_.md) node需要被设置手势打断回调的ArkUI节点。void* userData用户自定义数据。interrupter打断回调，info返回手势打断数据。interrupter 返回 GESTURE_INTERRUPT_RESULT_CONTINUE，手势正常进行； 返回 GESTURE_INTERRUPT_RESULT_REJECT 手势打断。
