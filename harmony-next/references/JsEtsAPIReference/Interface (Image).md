...
参数名类型必填说明componentType[ComponentType](Enums.md#ZH-CN_TOPIC_0000002529285837__componenttype9)是图像的组件类型。（目前仅支持 ComponentType:JPEG，实际返回格式由生产者决定，如相机）callbackAsyncCallback<[Component](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__component9)>是回调函数，当返回组件缓冲区成功，err为undefined，data为获取到的组件缓冲区；否则为错误对象。
...
参数名类型必填说明componentType[ComponentType](Enums.md#ZH-CN_TOPIC_0000002529285837__componenttype9)是图像的组件类型。（目前仅支持 ComponentType:JPEG，实际返回格式由生产者决定，如相机）。
