...
音视频录制管理类，用于音视频媒体录制。在调用AVRecorder的方法前，需要先通过[createAVRecorder()](Functions.md#ZH-CN_TOPIC_0000002529445861__mediacreateavrecorder9)构建一个AVRecorder实例。
...
名称类型只读可选说明state9+[AVRecorderState](Types.md#ZH-CN_TOPIC_0000002529285893__avrecorderstate9)是否
...
参数名类型必填说明typestring是状态机切换事件回调类型，支持的事件：'stateChange'，用户操作和系统都会触发此事件。callback[OnAVRecorderStateChangeHandler](Types.md#ZH-CN_TOPIC_0000002529285893__onavrecorderstatechangehandler12)是回调函数，返回状态机切换事件。
...
取消订阅播放状态机[AVRecorderState](Types.md#ZH-CN_TOPIC_0000002529285893__avrecorderstate9)切换的事件。使用callback异步回调。
...
参数名类型必填说明typestring是状态机切换事件回调类型，支持的事件：'stateChange'，用户操作和系统都会触发此事件。callback[OnAVRecorderStateChangeHandler](Types.md#ZH-CN_TOPIC_0000002529285893__onavrecorderstatechangehandler12)否
...
订阅AVRecorder的错误事件，该事件仅用于错误提示，不需要用户停止播控动作。如果此时[AVRecorderState](Types.md#ZH-CN_TOPIC_0000002529285893__avrecorderstate9)也切至error状态，用户需要通过[reset()](#ZH-CN_TOPIC_0000002529445863__reset9)或者[release()](#ZH-CN_TOPIC_0000002529445863__release9)退出录制操作。使用callback异步回调。
...
callback[ErrorCallback](@ohos.base (公共回调信息).md#ZH-CN_TOPIC_0000002497445536__errorcallback)是回调函数，返回录制错误事件。
...
callback[ErrorCallback](@ohos.base (公共回调信息).md#ZH-CN_TOPIC_0000002497445536__errorcallback)否
...
订阅媒体资源回调事件，当[FileGenerationMode](Enums.md#ZH-CN_TOPIC_0000002497445922__filegenerationmode12)枚举设置为系统创建媒体文件时，会在[stop](#ZH-CN_TOPIC_0000002529445863__stop9)操作结束后把[PhotoAsset](zh-cn_topic_0000002497605954.html)对象回调给应用。使用callback异步回调。
