...
调用文本计算接口时，不推荐同时用[ApplicationContext.setFontSizeScale](ApplicationContext (应用上下文).md#ZH-CN_TOPIC_0000002529284613__applicationcontextsetfontsizescale13)设置应用字体大小缩放比例。为了确保时序正确性，建议开发者自行监听字体缩放变化，以保证测算结果的准确性。
...
参数名类型必填说明options[MeasureOptions](@ohos.measure (文本计算).md#ZH-CN_TOPIC_0000002529444755__measureoptions)是被计算文本描述信息。
...
通过MeasureUtils的measureTextSize方法和unicode码点计算，手动实现文本截断。与设置[maxLines](Text.md#ZH-CN_TOPIC_0000002497444914__maxlines)、[textOverflow](Text.md#ZH-CN_TOPIC_0000002497444914__textoverflow)实现同样的效果。
...
将属性字符串根据文本布局选项转换成对应的[Paragraph](@ohos.graphics.text (文本模块).md#ZH-CN_TOPIC_0000002497605988__paragraph)数组。

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

**参数：**

参数名类型必填说明styledString[StyledString](属性字符串.md#ZH-CN_TOPIC_0000002497444920__styledstring)是待转换的属性字符串。options[TextLayoutOptions](文本组件公共接口.md#ZH-CN_TOPIC_0000002529444865__textlayoutoptions对象说明20)否文本布局选项。

**返回值：**

类型说明Array<[Paragraph](@ohos.graphics.text (文本模块).md#ZH-CN_TOPIC_0000002497605988__paragraph)>[Paragraph](@ohos.graphics.text (文本模块).md#ZH-CN_TOPIC_0000002497605988__paragraph)的数组。
