...
该回调仅生效于由[@Entry](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-create-custom-components#entry)装饰的、作为[router路由](zh-cn_topic_0000002529444757.html)页面存在的自定义组件。当之前存在于路由栈中的页面，通过[单实例模式](@ohos.router (页面路由)(不推荐).md#ZH-CN_TOPIC_0000002529444757__routermode9)移动到栈顶时触发该回调。
...
类型说明[Theme](@ohos.arkui.theme(主题换肤).md#ZH-CN_TOPIC_0000002529284765__theme)自定义组件当前生效的Theme对象。
