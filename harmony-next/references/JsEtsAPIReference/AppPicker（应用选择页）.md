...
[guardService.AppInfo](GuardService（屏幕时间守护服务）.md#section76401996131)
...
Promise对象。返回string[]，用户勾选的应用的token数组。
        ...
        Button("TestStartAppPicker")
          .onClick(async () => {
            try {
              ...
              console.error('startAppPicker invoke failure. code: ' + e.code + ' message: ' + e.message);
            }
        })
      }
  }
...
许可应用跳转页的子标题。该字符串支持的最大长度为200个字符。
  ...
  build() {
      Column() {
        Button("TestStartAppForm")
          .onClick(async () => {
            try {
              await appPicker.startAppForm(this.getUIContext().getHostContext(), { appTokens: [] }, "TestStartAppForm", false);
              console.info('startAppForm invoke success');
            } catch (e) {
              console.error('startAppForm invoke failure. code: ' + e.code + ' message: ' + e.message);
            }
        })
      }
  }
