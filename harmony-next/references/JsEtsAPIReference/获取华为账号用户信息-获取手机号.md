...
通过[获取用户级凭证](获取用户级凭证.md)获取的Access Token。获取凭证时Authorization Code需包含phone scope的授权。
...
用户选择授权给应用的手机号（华为账号绑定号码或用户选择的其他手机号），详见[获取手机号](https://developer.huawei.com/consumer/cn/doc/atomic-guides/account-guide-atomic-get-phone)。
...
不带国家码的手机号，此处为去除国际冠码与国际电话区号的手机号形式。
