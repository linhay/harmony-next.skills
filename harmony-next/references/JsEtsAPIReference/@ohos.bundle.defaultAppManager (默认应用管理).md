...
参数名类型必填说明typestring是要查询的应用类型，取[ApplicationType](#ZH-CN_TOPIC_0000002529284605__applicationtype)或者[UniformDataType](zh-cn_topic_0000002497444710.html)类型中的值。
...
参数名类型必填说明typestring是要查询的应用类型，取[ApplicationType](#ZH-CN_TOPIC_0000002529284605__applicationtype)或者[UniformDataType](zh-cn_topic_0000002497444710.html)类型中的值。callbackAsyncCallback<boolean>是[回调函数](@ohos.base (公共回调信息).md#ZH-CN_TOPIC_0000002497445536__asynccallback)，当获取成功时，err为null，data为bool值，true表示是默认应用，false表示不是默认应用；否则为错误对象。
