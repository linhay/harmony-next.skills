...
参数名类型必填说明value[AnimationStatus](枚举说明.md#ZH-CN_TOPIC_0000002529284967__animationstatus)是
...
参数名类型必填说明value[FillMode](枚举说明.md#ZH-CN_TOPIC_0000002529284967__fillmode)是
...
设置组件是否通过系统[onVisibleAreaChange](组件可见区域变化事件.md#ZH-CN_TOPIC_0000002497444844__onvisibleareachange)的可见性判定，控制组件的暂停和播放。
...
当设置为true时，组件将基于系统的[onVisibleAreaChange](组件可见区域变化事件.md#ZH-CN_TOPIC_0000002497444844__onvisibleareachange)可见性判定，控制组件的暂停和播放。

 当组件的运行状态为[AnimationStatus](枚举说明.md#ZH-CN_TOPIC_0000002529284967__animationstatus)的Running时，若判定组件不可见，则自动执行暂停操作；若判定为可见，则自动恢复播放。

默认值：false

**说明：**

当该属性由true动态修改为false时，组件将依据当前的[AnimationStatus](枚举说明.md#ZH-CN_TOPIC_0000002529284967__animationstatus)状态进行处理。

 例如，若当前状态为Running且因[onVisibleAreaChange](组件可见区域变化事件.md#ZH-CN_TOPIC_0000002497444844__onvisibleareachange)的不可见回调暂停，则在属性由true改为false后，组件会从上次暂停的位置重新开始播放。
...
名称类型只读可选说明srcstring | [Resource](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resource)9+ | [PixelMap](图像类型定义.md#ZH-CN_TOPIC_0000002497604902__pixelmap)12+否否

图片路径，图片格式为jpg、jpeg、svg、png、bmp、webp、ico和heif，从API Version9开始支持[Resource](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resource)类型的路径，从API version 12开始支持[PixelMap](图像类型定义.md#ZH-CN_TOPIC_0000002497604902__pixelmap)类型。
