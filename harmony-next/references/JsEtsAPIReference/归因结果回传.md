...
unix时间戳，毫秒，回传发生的时间戳，建议校验时判断与当前时间相差不超过 5min。
...
签名值。接收方应使用应用归因服务提供的[公钥](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/store-attribution-receive#section15571357102111)，对该签名值进行验签，具体参考[验签计算规则](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/store-attribution-receive#section15571357102111)、[回传结果验签方法](#section114341820105218)。
...
安装来源媒体应用id。（主要面向安装后转化事件，回传本次发生转化事件的开发者App的安装归因媒体应用id）
...
安装来源分发平台id。（主要面向安装后转化事件，回传本次发生转化事件的开发者App的安装分发平台id）
...
打开来源媒体应用id。（主要面向安装后转化事件，回传本次开发者App的打开来源媒体应用id）
...
1. 您需要在pom.xml文件中引入最新版本的开源密码算法包[Bouncy Castle](https://www.bouncycastle.org/latest_releases.md)。
