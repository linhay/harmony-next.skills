...
本模块首批接口从API version 7开始支持，从API version 9废弃，替换模块为[@ohos.app.ability.wantAgent](@ohos.app.ability.wantAgent (WantAgent模块).md)。后续版本的新增接口，采用上角标单独标记接口的起始版本。
...
参数名类型必填说明info[WantAgentInfo](WantAgentInfo.md)是WantAgent信息。callbackAsyncCallback<WantAgent>是创建WantAgent的回调方法。
...
参数名类型必填说明info[WantAgentInfo](WantAgentInfo.md)是WantAgent信息。
...
参数名类型必填说明agentWantAgent是WantAgent对象。triggerInfo[TriggerInfo](TriggerInfo.md)是TriggerInfo对象。callbackCallback<CompleteData>否主动激发WantAgent实例的回调方法。
