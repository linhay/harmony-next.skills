...
名称typedef关键字描述[OH_NN_UInt32Array](OH_NN_UInt32Array.md)OH_NN_UInt32Array该结构体用于存储32位无符号整型数组。[OH_NN_QuantParam](OH_NN_QuantParam.md)OH_NN_QuantParam量化信息。[OH_NN_Tensor](OH_NN_Tensor.md)OH_NN_Tensor

张量结构体。

通常用于构造模型图中的数据节点和算子参数，在构造张量时需要明确数据类型、维数、维度信息和量化信息。

[OH_NN_Memory](OH_NN_Memory.md)OH_NN_Memory内存结构体。[OH_NNModel](OH_NNModel.md)OH_NNModel模型句柄。[OH_NNCompilation](OH_NNCompilation.md)OH_NNCompilation编译器句柄。[OH_NNExecutor](OH_NNExecutor.md)OH_NNExecutor执行器句柄。[NN_QuantParam](NN_QuantParam.md)NN_QuantParam量化参数的句柄。[NN_TensorDesc](NN_TensorDesc.md)NN_TensorDescTensor描述的句柄。[NN_Tensor](NN_Tensor.md)NN_TensorTensor句柄。
