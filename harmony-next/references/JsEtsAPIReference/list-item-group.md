...
<[list](list.md)>的子组件，用来展示分组，宽度默认充满list组件。
...
名称类型默认值必填描述typestringdefault否list-item-group类型，同一list支持多种type的list-item-group，相同type的list-item-group需要确保渲染后的视图布局也完全相同，当type固定时，使用show属性代替if属性，确保视图布局不变。
...
名称参数描述groupclick{ groupid: string }
