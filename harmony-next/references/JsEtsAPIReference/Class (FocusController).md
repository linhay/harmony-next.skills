...
参数名类型必填说明keystring是节点对应的[组件标识](组件标识.md)。

**错误码：**

以下错误码的详细介绍请参见[焦点错误码](焦点错误码.md)。
...
参数名类型必填说明isAutoFocusTransferboolean是设置页面切换时，新的页面是否需要主动获取焦点，例如[Router](@ohos.router (页面路由)(不推荐).md#ZH-CN_TOPIC_0000002529444757__routerpushurldeprecated)、[Navigation](Navigation.md)、[Menu](Menu.md)、[Dialog](弹出框 (Dialog).md)、[Popup](Popup.md)等。true表示需要主动获取焦点，false表示不需要主动获取焦点。默认值为true。
...
参数名类型必填说明mode[KeyProcessingMode](焦点控制.md#ZH-CN_TOPIC_0000002529444807__keyprocessingmode15)是按键处理模式。
