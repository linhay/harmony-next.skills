...
设置组件由缩放大小或位移变化引起的运动过程中的动态模糊效果。需要与动画的[AnimateParam](显式动画 (animateTo).md#ZH-CN_TOPIC_0000002497444950__animateparam对象说明)的onFinish参数配合使用。
...
参数名类型必填说明value[MotionBlurOptions](#ZH-CN_TOPIC_0000002497444860__motionbluroptions对象说明)是定义运动模糊参数。
...
在当前组件由缩放大小或位移变化引起的运动过程中，增加动态模糊效果。与[motionBlur](#ZH-CN_TOPIC_0000002497444860__motionblur)相比，motionBlur参数新增了对undefined类型的支持。
...
参数名类型必填说明motionBlurOptional<[MotionBlurOptions](#ZH-CN_TOPIC_0000002497444860__motionbluroptions对象说明)>是
...
名称类型只读可选说明radiusnumber否否模糊半径，取值范围[0.0, ∞)，建议设置1.0以内。anchor[MotionBlurAnchor](#ZH-CN_TOPIC_0000002497444860__motionbluranchor对象说明)否否运动模糊锚点坐标。运动模糊锚点坐标设置时需要与动画缩放的锚点保持一致设置。
...
名称类型只读可选说明xnumber否否锚点坐标x值，取值范围[0.0, 1.0]。ynumber否否锚点坐标y值，取值范围[0.0, 1.0]。
