...
参数名类型必填说明type[SideBarContainerType](#ZH-CN_TOPIC_0000002497604870__sidebarcontainertype枚举说明)否
...
Overlay1侧边栏浮在内容区上面，不会影响内容区的大小。AUTO10+2
...
参数名类型必填说明value[ButtonStyle](#ZH-CN_TOPIC_0000002497604870__buttonstyle对象说明)是侧边栏控制按钮的属性。
...
设置侧边栏的宽度。设置为小于0的值时按默认值显示。受最小宽度和最大宽度限制，不在限制区域内取最近的点。与[sideBarWidth](#ZH-CN_TOPIC_0000002497604870__sidebarwidth)相比，value参数新增了对百分比字符串和其他[像素单位](zh-cn_topic_0000002497444996.html)的支持。
...
设置侧边栏最小宽度。设置为小于0的值时按默认值显示。值不能超过侧边栏容器本身宽度，超过则使用侧边栏容器本身宽度。与[minSideBarWidth](#ZH-CN_TOPIC_0000002497604870__minsidebarwidth)相比，value参数新增了对百分比字符串和其他[像素单位](zh-cn_topic_0000002497444996.html)的支持。
...
设置侧边栏最大宽度。设置为小于0的值时按默认值显示。值不能超过侧边栏容器本身宽度，超过则使用侧边栏容器本身宽度。与[maxSideBarWidth](#ZH-CN_TOPIC_0000002497604870__maxsidebarwidth)相比，value参数新增了对百分比字符串和其他[像素单位](zh-cn_topic_0000002497444996.html)的支持。
...
参数名类型必填说明value[SideBarPosition](#ZH-CN_TOPIC_0000002497604870__sidebarposition9枚举说明)是
...
参数名类型必填说明value[DividerStyle](#ZH-CN_TOPIC_0000002497604870__dividerstyle10对象说明) | null是
...
icons[ButtonIconOptions18+](#ZH-CN_TOPIC_0000002497604870__buttoniconoptions18对象说明)否是设置侧边栏控制按钮的图标。
...
名称类型只读可选说明shown8+string | [PixelMap](Interface (PixelMap).md) | [Resource](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resource)否否

设置侧边栏显示时控制按钮的图标。

**元服务API：** 从API version 11开始，该接口支持在元服务中使用。

hidden8+string | [PixelMap](Interface (PixelMap).md) | [Resource](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resource)否否

设置侧边栏隐藏时控制按钮的图标。

**元服务API：** 从API version 11开始，该接口支持在元服务中使用。

switching8+string | [PixelMap](Interface (PixelMap).md) | [Resource](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resource)否是
...
名称说明Start侧边栏位于容器左侧。End侧边栏位于容器右侧。
...
分割线的宽度不支持百分比设置。优先级低于[通用属性height](尺寸设置.md#ZH-CN_TOPIC_0000002497444850__height)，超过通用属性设置大小时，按照通用属性进行裁切。部分设备硬件中存在1像素取整后分割线不显示问题，建议使用2像素。
...
参数名类型必填说明valueboolean是true表示显示，false表示隐藏。
