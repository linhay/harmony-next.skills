...
参数项描述float sigmaX表示沿x轴方向上高斯模糊的标准差，必须大于0。float sigmaY表示沿y轴方向上高斯模糊的标准差，必须大于0。[OH_Drawing_TileMode](drawing_shader_effect.h.md#ZH-CN_TOPIC_0000002529286001__oh_drawing_tilemode) tileMode图像滤波器效果平铺模式类型，支持可选的具体模式可见[OH_Drawing_TileMode](drawing_shader_effect.h.md#ZH-CN_TOPIC_0000002529286001__oh_drawing_tilemode)枚举。[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)* imageFilter表示将要和当前图像滤波器叠加的输入滤波器, 如果为NULL，表示直接将当前图像滤波器作用于原始图像。

**返回：**

类型说明[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)*函数会返回一个指针，指针指向创建的图像滤波器对象[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)。如果对象返回NULL，表示创建失败；可能的原因是可用内存为空。
...
参数项描述float sigmaX表示沿x轴方向上高斯模糊的标准差，必须大于0.0。float sigmaY表示沿y轴方向上高斯模糊的标准差，必须大于0.0。[OH_Drawing_TileMode](drawing_shader_effect.h.md#ZH-CN_TOPIC_0000002529286001__oh_drawing_tilemode) tileMode图像滤波器效果平铺模式类型，支持可选的具体模式可见[OH_Drawing_TileMode](drawing_shader_effect.h.md#ZH-CN_TOPIC_0000002529286001__oh_drawing_tilemode)枚举。[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)* input表示将要和当前图像滤波器叠加的输入滤波器, 如果为NULL，表示直接将当前图像滤波器作用于原始图像。[const OH_Drawing_Rect](OH_Drawing_Rect.md)* rect表示裁剪的矩形区域，如果为NULL，表示直接将模糊效果作用于整个图像。

**返回：**

类型说明OH_Drawing_ImageFilter*函数会返回一个指针，指针指向创建的图像滤波器对象[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)。如果对象返回NULL，表示创建失败；可能的原因是可用内存为空。
...
参数项描述[OH_Drawing_ColorFilter](OH_Drawing_ColorFilter.md)* colorFilter指向具有颜色变换效果的颜色滤波器对象[OH_Drawing_ColorFilter](OH_Drawing_ColorFilter.md)。[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)* imageFilter表示将要和当前图像滤波器叠加的输入滤波器, 如果为NULL，表示直接将当前图像滤波器作用于原始图像。

**返回：**

类型说明[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)*函数会返回一个指针，指针指向创建的图像滤波器对象[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)。如果对象返回NULL，表示创建失败；可能的原因是可用内存为空，或者是colorFilter为NULL。
...
参数项描述float x水平方向的平移距离。float y竖直方向的平移距离。[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)* imageFilter需进行平移的滤波器，如果为空，则将无滤波效果的绘制结果进行平移。

**返回：**

类型说明[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)*函数会返回一个指针，指针指向创建的图像滤波器对象[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)。如果对象返回NULL，表示创建失败，可能原因为可用内存为空。
...
参数项描述[OH_Drawing_ShaderEffect](OH_Drawing_ShaderEffect.md)* shaderEffect表示要应用于图像的着色器效果。

**返回：**

类型说明[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)*函数会返回一个指针，指针指向创建的图像滤波器对象[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)。如果结果返回NULL，表示创建失败，可能原因为可用内存为空。
...
参数项描述[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)* imageFilter指向图像滤波器对象[OH_Drawing_ImageFilter](OH_Drawing_ImageFilter.md)的指针。