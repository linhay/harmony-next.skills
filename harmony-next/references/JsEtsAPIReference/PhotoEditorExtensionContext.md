...
参数名类型必填说明uristring是编辑后图片的[uri](@ohos.file.fileuri (文件URI).md)，格式为file://<bundleName>/<sandboxPath>。

**返回值：**

类型说明Promise<AbilityResult>Promise对象，返回AbilityResult对象，编辑过的图片uri存在want.uri中，[uri](@ohos.file.fileuri (文件URI).md)格式为file://<bundleName>/<sandboxPath>。

**错误码：**

以下错误码详细介绍参考[通用错误码](通用错误码.md)和[元能力子系统错误码](元能力子系统错误码.md)。
...
参数名类型必填说明pixeMap[image.PixelMap](Interface (PixelMap).md)是编辑过的图片image.PixelMap。option[image.PackingOption](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__packingoption)是设置打包参数。
