...
类型说明Promise<[ColumnType](Enums.md#ZH-CN_TOPIC_0000002497604686__columntype18)>Promise对象。返回指定列的数据类型。当结果集中包含重名列时，通过列名获取的结果会不符合预期。
...
类型说明[ColumnType](Enums.md#ZH-CN_TOPIC_0000002497604686__columntype18)返回指定列的数据类型。当结果集中包含重名列时，通过列名获取的结果会不符合预期。
...
类型说明[ValueType](Types.md#ZH-CN_TOPIC_0000002497444708__valuetype)表示允许的数据字段类型。
...
以[Assets](Types.md#ZH-CN_TOPIC_0000002497444708__assets10)形式获取当前行中指定列的值，如果当前列的数据类型为Assets类型，会以Assets类型返回指定值，如果当前列中的值为null时，会返回null，其他类型则返回14800000。
...
类型说明[Assets](Types.md#ZH-CN_TOPIC_0000002497444708__assets10)以Assets形式返回指定列的值。
...
类型说明[ValuesBucket](Types.md#ZH-CN_TOPIC_0000002497444708__valuesbucket)返回指定行的值。当结果集中包含重名列时，返回值会不符合预期。
...
类型说明Promise<Array<[ValuesBucket](Types.md#ZH-CN_TOPIC_0000002497444708__valuesbucket)>>返回maxCount条数据，剩余数据不足maxCount条则返回剩余数据，返回空数组时代表已经遍历到结果集的末尾。当结果集中包含重名列时，返回值会不符合预期。
...
类型说明[sendableRelationalStore.ValuesBucket](@ohos.data.sendableRelationalStore (共享关系型数据库).md#ZH-CN_TOPIC_0000002529284679__valuesbucket)当前行数据的sendable形式，用于跨线程传递。
