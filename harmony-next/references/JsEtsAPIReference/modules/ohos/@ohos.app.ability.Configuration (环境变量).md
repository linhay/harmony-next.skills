...
该环境变量支持在[UIAbility](@ohos.app.ability.UIAbility (带界面的应用组件).md)组件和[UIExtensionAbility](@ohos.app.ability.UIExtensionAbility (带界面的ExtensionAbility组件).md)组件中订阅，不支持在[ApplicationContext](../../topics/graphics/ApplicationContext (应用上下文).md)和[AbilityStage](@ohos.app.ability.AbilityStage (AbilityStage组件管理器).md)组件容器中订阅。
