...
- 本模块接口在[单元测试框架](../../guides/单元测试框架使用指导.md)中使用。
- 本模块接口不支持并发调用。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| metrics | Array<PerfMetric> | 否 | 否 | 被测性能指标列表。 |
...
其中，回调函数的参数为boolean类型，true代表代码段执行符合预期，false代表代码段执行不符合预期。[代码示例](#ZH-CN_TOPIC_0000002522081810__create)。
...
以下错误码的详细介绍请参见[perftest错误码]([perftest错误码](../../errors/perftest错误码.md).md)。
