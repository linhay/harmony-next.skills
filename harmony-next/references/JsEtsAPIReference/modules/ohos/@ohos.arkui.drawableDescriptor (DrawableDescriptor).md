...
父类对象提供可重写的方法，包含：获取[PixelMap](../../types/interfaces/Interface (PixelMap).md)实例，图片资源加载能力。
...
类型说明[image.PixelMap](../../types/interfaces/Interface (PixelMap).md)PixelMap
...
类型说明[DrawableDescriptorLoadedResult](#ZH-CN_TOPIC_0000002497604770__drawabledescriptorloadedresult21)图片资源的加载结果。

**错误码：**

以下错误码的详细介绍请参见[DrawableDescriptor错误码](../../errors/DrawableDescriptor错误码.md)。

错误码ID错误信息111001resource loading failed.
...
类型说明Promise<[DrawableDescriptorLoadedResult](#ZH-CN_TOPIC_0000002497604770__drawabledescriptorloadedresult21)>图片资源的加载结果。
...
支持通过传入PixelMap创建PixelMapDrawableDescriptor对象。继承自[DrawableDescriptor](#ZH-CN_TOPIC_0000002497604770__drawabledescriptor)。
...
参数名类型必填说明src[image.PixelMap](../../types/interfaces/Interface (PixelMap).md)否PixelMap类型参数，存储 PixelMap 图片数据。

#### LayeredDrawableDescriptor

当传入资源id或name为包含前景和背景资源的json文件时，生成LayeredDrawableDescriptor对象。继承自[DrawableDescriptor](#ZH-CN_TOPIC_0000002497604770__drawabledescriptor)。
...
参数名类型必填说明foreground[DrawableDescriptor](#ZH-CN_TOPIC_0000002497604770__drawabledescriptor)否分层图标的前景图片选项。background[DrawableDescriptor](#ZH-CN_TOPIC_0000002497604770__drawabledescriptor)否分层图标的背景图片选项。mask[DrawableDescriptor](#ZH-CN_TOPIC_0000002497604770__drawabledescriptor)否分层图标的遮罩选项。
...
类型说明[DrawableDescriptor](#ZH-CN_TOPIC_0000002497604770__drawabledescriptor)DrawableDescriptor对象。
...
frameDurations21+Array<number>否是
...
使用[Image](../../topics/graphics/Image.md)组件播放PixelMap数组或动图资源时传入AnimatedDrawableDescriptor对象，该对象继承自[DrawableDescriptor](#ZH-CN_TOPIC_0000002497604770__drawabledescriptor)。
...
参数名类型必填说明pixelMapsArray<[image.PixelMap](../../types/interfaces/Interface (PixelMap).md)>是PixelMap 数组类型参数，存储 PixelMap 图片数据。options[AnimationOptions](#ZH-CN_TOPIC_0000002497604770__animationoptions12)否动画控制选项。
...
参数名类型必填说明src[ResourceStr](../../topics/misc/基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcestr) | Array<[image.PixelMap](../../types/interfaces/Interface (PixelMap).md)>是

动图资源地址或者[PixelMap](../../types/interfaces/Interface (PixelMap).md)对象构成的的数组。

 ResourceStr当前支持的范围：应用资源Resource，沙箱路径（file://<bundleName>/<sandboxPath>），BASE64字符串。

options[AnimationOptions](#ZH-CN_TOPIC_0000002497604770__animationoptions12)否动画控制参数。
...
当[Image](../../topics/graphics/Image.md)组件与AnimatedDrawableDescriptor确保1比1持有（仅传入一个[Image](../../topics/graphics/Image.md)组件）时，id非必填；

若同一AnimatedDrawableDescriptor需绑定多个[Image](../../topics/graphics/Image.md)组件，则必须设置唯一id以准确获取对应组件的动画控制器（唯一性由开发者保证）。

此规则基于动画系统设计原则：动画数据可多组件共享，但各组件动画独立运行，AnimationController与组件严格1比1持有关系（一个组件一个AnimationController对象）。

另外，[AnimatedDrawableDescriptor](#ZH-CN_TOPIC_0000002497604770__animateddrawabledescriptor12)支持不可见时自动暂停播放功能，详见[onVisibleAreaChange](../../topics/misc/组件可见区域变化事件.md#ZH-CN_TOPIC_0000002497444844__onvisibleareachange)。

**返回值：**

类型说明[AnimationController](#ZH-CN_TOPIC_0000002497604770__animationcontroller21) | undefined动画控制器对象。

**示例：**

[Image](../../topics/graphics/Image.md)组件与AnimatedDrawableDescriptor保持1比1持有关系，示例代码如下。
...
[Image](../../topics/graphics/Image.md)组件与AnimatedDrawableDescriptor保持1比N持有关系，示例代码如下。
...
类型说明[AnimationStatus](../../guides/枚举说明.md#ZH-CN_TOPIC_0000002529284967__animationstatus)动图的播放状态。包含4种状态：初始态、播放态、暂停态、停止态。
