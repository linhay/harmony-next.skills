...
由于图片占用内存较大，所以当PixelMap对象使用完成后，应主动调用[release](#ZH-CN_TOPIC_0000002553201941__release)方法及时释放内存。释放时应确保该对象的所有异步方法均执行完成，且后续不再使用该对象。
...
| options | image.InitializationOptions | 是 | 创建像素的属性，包括透明度，尺寸，缩放值，像素格式和是否可编辑。 |
...
根据Surface ID和区域信息创建一个PixelMap对象。该区域的大小由[Region](Interfaces (其他).md#ZH-CN_TOPIC_0000002553201939__region8).size指定。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| surfaceId | string | 是 | 对应Surface的ID，可通过预览组件获取，如[XComponent](../../topics/components/XComponent.md)组件。 |
| region | image.Region | 是 | 截取的画面区域。仅支持从画面左上角开始截取部分或整个画面，即Region中的x和y必须为0，Region.size中width和height的取值范围分别为[1, 预览流宽度]和[1, 预览流高度]。如需截取任意区域，可先获取整个画面，再使用crop截取所需区域。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| pixelmap | image.PixelMap | 是 | image下的非sendable的PixelMap。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| PixelMap | 成功同步返回sendable的PixelMap对象，失败抛出异常。 |
...
由于图片占用内存较大，所以当PixelMap对象使用完成后，应主动调用[release](Interface (PixelMap).md#ZH-CN_TOPIC_0000002522241972__release7)方法及时释放内存。释放时应确保该对象的所有异步方法均执行完成，且后续不再使用该对象。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| pixelmap | PixelMap | 是 | sendableImage下的PixelMap。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| image.PixelMap | 成功同步返回image下的非sendable的PixelMap对象，失败抛出异常。 |
...
图像像素类，用于读取或写入图像数据以及获取图像信息。在调用PixelMap的方法前，需要先通过[createPixelMap](#ZH-CN_TOPIC_0000002553201941__sendableimagecreatepixelmap)创建一个PixelMap实例。目前pixelmap序列化大小最大128MB，超过会送显失败。大小计算方式为(宽*高*每像素占用字节数)。

sendableImage下的PixelMap支持sendable属性，支持worker线程共享。sendableImage下的PixelMap，可以利用[Convert](#ZH-CN_TOPIC_0000002553201941__sendableimageconverttopixelmap)方法与image下的PixelMap进行互相转换。转换后，原对象的方法均不允许再调用，否则将报错501 无法调用接口。跨线程处理PixelMap时，需要考虑多线程问题。

在调用PixelMap的方法前，需要先通过[sendableImage.createPixelMap](#ZH-CN_TOPIC_0000002553201941__sendableimagecreatepixelmap)构建一个PixelMap对象。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| area | image.PositionArea | 是 | 区域大小，根据区域读取。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| area | image.PositionArea | 是 | 区域，根据区域写入。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<image.ImageInfo> | Promise实例，用于异步获取图像像素信息，失败时返回错误信息。 |
...
如需使用同步方式创建PixelMap可使用：[createPixelMapFromParcel](#ZH-CN_TOPIC_0000002553201941__sendableimagecreatepixelmapfromparcel)。
...
由于图片占用内存较大，所以当ImageSource实例使用完成后，应主动调用[release](#ZH-CN_TOPIC_0000002553201941__release-1)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| uri | string | 是 | 图片路径，当前仅支持应用沙箱路径。 当前支持格式有：.jpg .png .gif .bmp .webp .dng [SVG](../../topics/misc/Functions.md#ZH-CN_TOPIC_0000002529445805__svg标签说明) .ico。 |
...
通过缓冲区创建ImageSource实例。buf数据是未解码的数据，不可以传入类似于RBGA，YUV的像素buffer数据，如果想通过像素buffer数据创建pixelMap，可以调用[sendableImage.createPixelMap](#ZH-CN_TOPIC_0000002553201941__sendableimagecreatepixelmap)这一类方法。
...
createImageReceiver(size: image.Size, format: [image.ImageFormat](../../types/enums/Enums.md#ZH-CN_TOPIC_0000002529285837__imageformat9), capacity: number): ImageReceiver

通过图片大小、图片格式、容量创建ImageReceiver实例。

由于图片占用内存较大，所以当ImageReceiver实例使用完成后，应主动调用[release](#ZH-CN_TOPIC_0000002553201941__release-3)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。
...
| format | [image.ImageFormat](../../types/enums/Enums.md#ZH-CN_TOPIC_0000002529285837__imageformat9) | 是 | 图像格式，取值为image.ImageFormat常量，目前仅支持 ImageFormat:JPEG。 |
...
ImageSource类，用于获取图片相关信息。在调用ImageSource的方法前，需要先通过[sendableImage.createImageSource](#ZH-CN_TOPIC_0000002553201941__sendableimagecreateimagesource)构建一个ImageSource实例。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| clipRect | Region | 否 | 否 | 要裁剪的图像区域。 |
| size | Size | 是 | 否 | 图像大小。 如果Image对象所存储的是相机预览流数据（YUV图像数据），那么获取到的size中的宽和高分别对应YUV图像的宽和高。 如果Image对象所存储的是相机拍照流数据（JPEG图像数据），由于已是编码后的文件，size中的宽等于JPEG文件大小，高等于1。 Image对象所存储的数据是预览流还是拍照流，取决于应用将receiver中的surfaceId传给相机的是previewOutput还是captureOutput。 相机预览与拍照最佳实践请参考双路预览(ArkTS)与拍照实践(ArkTS)。 |
| format | number | 是 | 否 | 图像格式，参考[OH_NativeBuffer_Format](../../topics/graphics/buffer_common.h.md#ZH-CN_TOPIC_0000002529285985__oh_nativebuffer_format)。 |
...
getComponent(componentType: [image.ComponentType](../../types/enums/Enums.md#ZH-CN_TOPIC_0000002529285837__componenttype9)): Promise<image.Component>
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| componentType | [image.ComponentType](../../types/enums/Enums.md#ZH-CN_TOPIC_0000002529285837__componenttype9) | 是 | 图像的组件类型。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| size | image.Size | 是 | 否 | 图片大小。 |
| capacity | number | 是 | 否 | 同时访问的图像数。该参数仅作为期望值，实际capacity由设备硬件决定。 |
| format | [image.ImageFormat](../../types/enums/Enums.md#ZH-CN_TOPIC_0000002529285837__imageformat9) | 是 | 否 | 图像格式。 |
...
此接口需要在[on](#ZH-CN_TOPIC_0000002553201941__on)回调触发后调用，才能正常的接收到数据。且此接口返回的[Image](#ZH-CN_TOPIC_0000002553201941__image)对象使用完毕后需要调用[release](#ZH-CN_TOPIC_0000002553201941__release-2)方法释放，释放后才可以继续接收新的数据。
