...
证书算法库框架提供证书相关接口。其中，依赖加解密算法库框架的基础算法能力的部分，详细接口说明可参考[cryptoFramework API参考](@ohos.security.cryptoFramework (加解密算法库框架).md)。
...
ERR_OUT_OF_MEMORY19020001

内存错误。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

ERR_RUNTIME_ERROR19020002

运行时外部错误。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

ERR_PARAMETER_CHECK_FAILED20+19020003

参数检查失败。

**元服务API：** 从API version 20开始，该接口支持在元服务中使用。

ERR_CRYPTO_OPERATION19030001

调用三方算法库API出错。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

ERR_CERT_SIGNATURE_FAILURE19030002

证书签名验证错误。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

ERR_CERT_NOT_YET_VALID19030003

证书尚未生效。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

ERR_CERT_HAS_EXPIRED19030004

证书过期。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

ERR_UNABLE_TO_GET_ISSUER_CERT_LOCALLY19030005

无法获取证书的颁发者。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

ERR_KEYUSAGE_NO_CERTSIGN19030006

证书的密钥用途不含证书签名。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

ERR_KEYUSAGE_NO_DIGITAL_SIGNATURE19030007

证书的密钥用途不含数字签名。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

ERR_MAYBE_WRONG_PASSWORD18+19030008
...
名称类型只读可选说明dataArray<Uint8Array>否否数据列表。
...
名称值说明FORMAT_DER0DER格式。FORMAT_PEM1PEM格式。FORMAT_PKCS711+2PKCS7格式。
...
名称类型只读可选说明typestring否否openssl指定的扩展类型。valuestring否否扩展值。
...
名称类型只读可选说明subject[X500DistinguishedName](#ZH-CN_TOPIC_0000002529285375__x500distinguishedname12)否否X509定义的Name类型的对象。mdNamestring否否摘要算法名。attributesArray<[CsrAttribute](#ZH-CN_TOPIC_0000002529285375__csrattribute18)>否是扩展。outFormat[EncodingBaseFormat](#ZH-CN_TOPIC_0000002529285375__encodingbaseformat18)否是输出类型。
...
名称值说明CERT_ITEM_TYPE_TBS0表示获取证书的待签名信息。CERT_ITEM_TYPE_PUBLIC_KEY1表示获取证书的公钥信息。CERT_ITEM_TYPE_ISSUER_UNIQUE_ID2表示获取证书的颁发者唯一编号。CERT_ITEM_TYPE_SUBJECT_UNIQUE_ID3表示获取证书的主体唯一编号。CERT_ITEM_TYPE_EXTENSIONS4表示获取证书的扩展域信息。
...
名称值说明EXTENSION_OID_TYPE_ALL0表示获取扩展域中所有的对象标识符。EXTENSION_OID_TYPE_CRITICAL1表示获取扩展域中critical为true的对象标识符。EXTENSION_OID_TYPE_UNCRITICAL2表示获取扩展域中critical为false的对象标识符。
...
名称值说明EXTENSION_ENTRY_TYPE_ENTRY0表示获取整个对象。EXTENSION_ENTRY_TYPE_ENTRY_CRITICAL1表示获取对象的critical属性。EXTENSION_ENTRY_TYPE_ENTRY_VALUE2表示获取对象的数据。
...
名称类型只读可选说明dataUint8Array否否传入的证书数据。encodingFormat[EncodingFormat](#ZH-CN_TOPIC_0000002529285375__encodingformat)否否指明证书编码格式。
...
名称类型只读可选说明dataUint8Array否否证书数据，按照长度（2字节）-数据的形式传入。如：08ABCDEFGH07ABCDEFG，第一本证书，前2个字节表示证书的长度为8字节，后面附加8字节的证书数据；第2本证书前2个字节表示证书的长度为7字节，后面附加7字节的证书数据。countnumber否否传入的数据中，包含的证书数量。encodingFormat[EncodingFormat](#ZH-CN_TOPIC_0000002529285375__encodingformat)否否指明证书编码格式。
...
名称值说明GENERAL_NAME_TYPE_OTHER_NAME0表示其他名称。GENERAL_NAME_TYPE_RFC822_NAME1表示电子邮件地址。GENERAL_NAME_TYPE_DNS_NAME2表示一个DNS名称。GENERAL_NAME_TYPE_X400_ADDRESS3表示X.400地址。GENERAL_NAME_TYPE_DIRECTORY_NAME4表示一个目录名称。GENERAL_NAME_TYPE_EDI_PARTY_NAME5表示特定的EDI实体。GENERAL_NAME_TYPE_UNIFORM_RESOURCE_ID6表示一个统一资源标识符。GENERAL_NAME_TYPE_IP_ADDRESS7表示一个IP地址。GENERAL_NAME_TYPE_REGISTERED_ID8表示一个已注册的对象标识符。
...
名称类型只读可选说明type[GeneralNameType](#ZH-CN_TOPIC_0000002529285375__generalname12)否否指定具体的证书主体类型。nameUint8Array否是指定具体的证书主体DER格式内容。
...
名称类型只读可选说明x509Cert[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)否是指定具体的证书对象。validDatestring否是指定证书有效期。issuerUint8Array否是指定证书颁发者，为DER编码格式。keyUsageArray<boolean>否是指定是否需要匹配密钥用途。true为需要，false为不需要。serialNumberbigint否是指定证书的序列号。subjectUint8Array否是指定证书主题，DER编码格式。publicKey[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)否是指定证书公钥，DER编码格式。publicKeyAlgIDstring否是指定证书公钥的算法。subjectAlternativeNames12+Array<[GeneralName](#ZH-CN_TOPIC_0000002529285375__generalname12)>否是指定证书主体名称。matchAllSubjectAltNames12+boolean否是指定是否需要匹配证书主体名称。true为需要，false为不需要。authorityKeyIdentifier12+Uint8Array否是指定证书颁发机构密钥。minPathLenConstraint12+number否是指定证书CA路径长度。extendedKeyUsage12+Array<string>否是指定证书扩展用途。nameConstraints12+Uint8Array否是指定证书的使用者名称。certPolicy12+Array<string>否是指定证书策略。privateKeyValid12+string否是指定证书私钥有效期。subjectKeyIdentifier12+Uint8Array否是指定证书公钥。
...
名称类型只读可选说明issuerArray<Uint8Array>否是指定颁发者作为过滤条件, 至少要匹配到其中一个issuer。x509Cert[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)否是指定具体的证书对象作为过滤条件, 判断该证书是否在CRL列表中。updateDateTime12+string否是指定证书更新时间。maxCRL12+bigint否是指定CRL个数最大值。minCRL12+bigint否是指定CRL个数最小值。
...
名称类型只读可选说明certMatchParameters[X509CertMatchParameters](#ZH-CN_TOPIC_0000002529285375__x509certmatchparameters11)否否指定过滤条件。maxLengthnumber否是指定最终证书链中CA证书的最大长度。validationParameters[CertChainValidationParameters](#ZH-CN_TOPIC_0000002529285375__certchainvalidationparameters11)否否指定验证条件。
...
名称类型只读可选说明certChain[X509CertChain](#ZH-CN_TOPIC_0000002529285375__x509certchain11)是否生成的证书链对象。validationResult[CertChainValidationResult](#ZH-CN_TOPIC_0000002529285375__certchainvalidationresult11)是否指定最终证书链的最大长度。
...
名称类型只读可选说明CACert[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)否是信任的CA证书。如果配置了CACert，则校验证书链时只使用CACert，不再使用CAPubKey和CASubject。CAPubKeyUint8Array否是信任的CA证书公钥，DER格式。仅在未配置CACert时生效。CASubjectUint8Array否是信任的CA证书主题，DER格式。仅在配置了CAPubKey时生效。校验对象根据CAPubKey类型（自签或上级）决定是校验根证书的主题还是颁发者。nameConstraints12+Uint8Array否是名称约束，DER格式。只校验当前证书链的叶子证书。
...
名称值说明REVOCATION_CHECK_OPTION_PREFER_OCSP0

优先采用OCSP进行校验，默认采用CRL校验。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

REVOCATION_CHECK_OPTION_ACCESS_NETWORK1

支持通过访问网络获取CRL或OCSP响应进行吊销状态的校验，默认为关闭。仅支持从证书CDP扩展中获取第一个CRL分发点地址检查证书吊销状态，或从证书AIA扩展中获取第一个OCSP服务器地址检查证书吊销状态。必须声明ohos.permission.INTERNET权限。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

REVOCATION_CHECK_OPTION_FALLBACK_NO_PREFER2

当ACCESS_NETWORK选项打开时有效，如果优选的校验方法由于网络原因导致无法校验证书状态，则采用备选的方案进行校验。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

REVOCATION_CHECK_OPTION_FALLBACK_LOCAL3

当ACCESS_NETWORK选项打开时有效，如果在线获取CRL和OCSP响应都由于网络的原因导致无法校验证书状态，则采用本地设置的CRL和OCSP响应进行校验。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

REVOCATION_CHECK_OPTION_CHECK_INTERMEDIATE_CA_ONLINE22+4

当ACCESS_NETWORK选项打开时有效。如果开启了该能力，对终端实体证书OCSP或CRL校验失败，则会继续校验中间证书的吊销情况。默认关闭。

**元服务API：** 从API version 22开始，该接口支持在元服务中使用。

REVOCATION_CHECK_OPTION_LOCAL_CRL_ONLY_CHECK_END_ENTITY_CERT22+5
...
名称值说明VALIDATION_POLICY_TYPE_X5090默认值，不需要校验证书中的sslHostname或dNSName。VALIDATION_POLICY_TYPE_SSL1需要校验证书中的sslHostname或dNSName。
...
名称值说明KEYUSAGE_DIGITAL_SIGNATURE0证书持有者可以用证书中包含的私钥进行数字签名操作。KEYUSAGE_NON_REPUDIATION1证书持有者不可否认使用证书中包含的私钥进行的数字签名操作。KEYUSAGE_KEY_ENCIPHERMENT2证书持有者可以使用证书中包含的公钥进行密钥加密操作。KEYUSAGE_DATA_ENCIPHERMENT3证书持有者可以使用证书中包含的公钥进行数据加密操作。KEYUSAGE_KEY_AGREEMENT4证书持有者可以使用证书中包含的私钥进行密钥协商操作。KEYUSAGE_KEY_CERT_SIGN5证书持有者可以使用证书中包含的私钥对其他证书进行签名。KEYUSAGE_CRL_SIGN6证书持有者可以使用证书中包含的私钥对证书吊销列表（CRL）进行签名。KEYUSAGE_ENCIPHER_ONLY7证书持有者只能进行加密操作，不能进行解密操作。KEYUSAGE_DECIPHER_ONLY8证书持有者只能进行解密操作，不能进行加密操作。
...
名称类型只读可选说明ocspRequestExtensionArray<Uint8Array>否是表示发送OCSP请求的扩展字段。ocspResponderURIstring否是表示用于OCSP请求的备选服务器URL地址，支持HTTP/HTTPS，具体配置由与服务器协商决定。ocspResponderCert[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)否是表示用于OCSP响应的签名校验的签名证书。ocspResponsesUint8Array否是表示用于OCSP服务器响应的备选数据。crlDownloadURIstring否是表示用于CRL请求的备选下载地址。optionsArray<[RevocationCheckOptions](#ZH-CN_TOPIC_0000002529285375__revocationcheckoptions12)>否是表示证书吊销状态查询的策略组合。ocspDigeststring否是表示OCSP通信时创建证书ID使用的哈希算法。默认为SHA256，支持可配置MD5、SHA1、SHA224、SHA256、SHA384、SHA512算法。
...
名称类型只读可选说明datestring否是

表示需要校验证书的有效期。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

trustAnchorsArray<[X509TrustAnchor](#ZH-CN_TOPIC_0000002529285375__x509trustanchor11)>否否

表示信任锚列表。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

trustSystemCa20+boolean否是

表示是否使用系统预置CA证书校验证书链。true表示使用；false表示不使用。

**元服务API：** 从API version 20开始，该接口支持在元服务中使用。

certCRLsArray<[CertCRLCollection](#ZH-CN_TOPIC_0000002529285375__certcrlcollection11)>否是

表示需要校验证书是否在证书吊销列表中。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

revocationCheckParam12+[RevocationCheckParameter](#ZH-CN_TOPIC_0000002529285375__revocationcheckparameter12)否是

表示需要在线校验证证书吊销状态的参数对象。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

policy12+[ValidationPolicyType](#ZH-CN_TOPIC_0000002529285375__validationpolicytype12)否是

表示需要校验证书的策略类型。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

sslHostname12+string否是

表示需要校验证书中主机名，与policy配合使用。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

keyUsage12+Array<[KeyUsageType](#ZH-CN_TOPIC_0000002529285375__keyusagetype12)>否是
...
名称类型只读可选说明trustAnchor[X509TrustAnchor](#ZH-CN_TOPIC_0000002529285375__x509trustanchor11)是否表示信任锚。entityCert[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)是否表示实体证书。
...
名称类型只读可选说明privateKeystring | Uint8Array否是表示P12文件解析后的私钥。cert[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)否是表示P12文件解析后的证书。otherCertsArray<[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)>否是表示P12文件解析后的其他证书合集。
...
名称类型只读可选说明passwordstring否否表示P12文件的密码。needsPrivateKeyboolean否是

表示是否获取私钥。默认为true。

true为获取，返回PKCS8编码的私钥数据；false为不获取。

privateKeyFormat[EncodingBaseFormat](#ZH-CN_TOPIC_0000002529285375__encodingbaseformat18)否是

表示获取私钥的格式，当前支持PEM和DER格式。参数缺省时，默认为PEM格式。

**注意**：当needsPrivateKey值为true时，该参数生效。

needsCertboolean否是表示是否获取证书。默认为true。true为获取，false为不获取。needsOtherCertsboolean否是表示是否获取其他证书合集。默认为false。true为获取，false为不获取。
...
名称值说明AES_128_CBC0AES-128-CBC加密算法。AES_192_CBC1AES-192-CBC加密算法。AES_256_CBC2AES-256-CBC加密算法。
...
名称类型只读可选说明saltLennumber否是表示盐值长度。默认为16，最小值为8。iterationsnumber否是表示迭代次数。默认为2048。encryptionAlgorithm[PbesEncryptionAlgorithm](#ZH-CN_TOPIC_0000002529285375__pbesencryptionalgorithm21)否是表示PBES加密算法类型。默认为AES_256_CBC。
...
名称值说明SHA2560SHA256摘要算法。SHA3841SHA384摘要算法。SHA5122SHA512摘要算法。
...
名称类型只读可选说明passwordstring否否表示P12文件的密码。最小长度为4。keyEncParams[PbesParams](#ZH-CN_TOPIC_0000002529285375__pbesparams21)否是表示私钥加密的算法参数。encryptCertboolean否是表示是否加密证书。默认为true。true为加密，false为不加密。certEncParams[PbesParams](#ZH-CN_TOPIC_0000002529285375__pbesparams21)否是表示证书加密的算法参数。macSaltLennumber否是表示P12 MAC的盐值长度。最小值为8，默认为16。macIterationsnumber否是表示P12 MAC的迭代次数。默认为2048。macDigestAlgorithm[Pkcs12MacDigestAlgorithm](#ZH-CN_TOPIC_0000002529285375__pkcs12macdigestalgorithm21)否是表示P12 MAC的摘要算法。默认为SHA256。
...
名称值说明BINARY0表示二进制数据格式。TEXT1表示文本数据格式。
...
名称值说明PKCS1_PADDING0PKCS1填充方式。PKCS1_PSS_PADDING1PKCS1 PSS填充方式。
...
名称值说明SHA2560SHA256 算法SHA3841SHA384 算法SHA5122SHA512 算法
...
名称值说明AES_128_CBC0AES_128_CBC 算法AES_192_CBC1AES_192_CBC 算法AES_256_CBC2AES_256_CBC 算法AES_128_GCM3AES_128_GCM 算法AES_192_GCM4AES_192_GCM 算法AES_256_GCM5AES_256_GCM 算法
...
名称值说明SIGNER_CERTS0签名者证书ALL_CERTS1全部证书
...
名称类型只读可选说明keystring | Uint8Array否否未加密或加密的私钥，支持PEM或DER格式。passwordstring否是私钥的密码，如果私钥是加密的。
...
名称类型只读可选说明mdNamestring否否

消息摘要算法的名称，例如 "SHA384", 当前支持"SHA1"、"SHA256"、"SHA384"、"SHA512"。

**元服务API：** 从API version 18开始，该接口支持在元服务中使用。

rsaSignaturePadding22+[CmsRsaSignaturePadding](#ZH-CN_TOPIC_0000002529285375__cmsrsasignaturepadding22)否是
...
名称类型只读可选说明cert[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)否否RSA证书。
...
名称类型只读可选说明cert[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)否否EC证书。digestAlgorithm[CmsKeyAgreeRecipientDigestAlgorithm](#ZH-CN_TOPIC_0000002529285375__cmskeyagreerecipientdigestalgorithm22)否是KDF摘要算法，默认为SHA256。
...
名称类型只读可选说明keyTransInfo[CmsKeyTransRecipientInfo](#ZH-CN_TOPIC_0000002529285375__cmskeytransrecipientinfo22)否是keyTrans接收者信息。keyAgreeInfo[CmsKeyAgreeRecipientInfo](#ZH-CN_TOPIC_0000002529285375__cmskeyagreerecipientinfo22)否是keyAgree接收者信息。
...
名称类型只读可选说明contentDataFormat[CmsContentDataFormat](#ZH-CN_TOPIC_0000002529285375__cmscontentdataformat18)否是内容数据的格式。默认为CmsContentDataFormat.BINARY。outFormat[CmsFormat](#ZH-CN_TOPIC_0000002529285375__cmsformat18)否是Cms最终数据的输出格式。默认为DER。isDetachedboolean否是Cms最终数据是否不包含原始数据。默认为false。true为包含，false为不包含。
...
名称类型只读可选说明trustCertsArray<[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)>否否信任证书。signerCertsArray<[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)>否是签名证书。默认为空。contentDataUint8Array否是内容数据，如果是detached模式，则需要指定明文数据。attached模式可以不传。contentDataFormat[CmsContentDataFormat](#ZH-CN_TOPIC_0000002529285375__cmscontentdataformat18)否是内容数据的格式。默认为CmsContentDataFormat.BINARY。
...
名称类型只读可选说明keyInfo[PrivateKeyInfo](#ZH-CN_TOPIC_0000002529285375__privatekeyinfo18)否是私钥参数。默认为空。cert[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)否是公钥证书。默认为空。encryptedContentDataUint8Array否是加密的内容数据，如果CMS不包含指定数据。默认为空。contentDataFormat[CmsContentDataFormat](#ZH-CN_TOPIC_0000002529285375__cmscontentdataformat18)否是内容数据的格式。默认为CmsContentDataFormat.BINARY。
...
参数名类型必填说明inStream[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)是X509证书序列化数据。callbackAsyncCallback<[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)>是回调函数，表示X509证书对象。

**错误码：**

以下错误码的详细介绍请参见[证书错误码](../../errors/证书错误码.md)。
...
801this operation is not supported.19020001memory malloc failed.19030001crypto operation error.
...
参数名类型必填说明inStream[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)是X509证书序列化数据。

**返回值：**

类型说明Promise<[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)>表示X509证书对象。
...
参数名类型必填说明key[cryptoFramework.PubKey](@ohos.security.cryptoFramework (加解密算法库框架).md#ZH-CN_TOPIC_0000002497445368__pubkey)是用于验签的公钥对象。callbackAsyncCallback<void>是回调函数，使用AsyncCallback的第一个error参数判断是否验签成功，error为null表示成功，不为null表示失败。
...
19030001crypto operation error.
...
参数名类型必填说明key[cryptoFramework.PubKey](@ohos.security.cryptoFramework (加解密算法库框架).md#ZH-CN_TOPIC_0000002497445368__pubkey)是用于验签的公钥对象。
...
参数名类型必填说明callbackAsyncCallback<[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)>是回调函数，表示X509证书序列化数据。
...
19020001memory malloc failed.19020002
...
类型说明Promise<[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)>表示X509证书序列化数据。
...
类型说明[cryptoFramework.PubKey](@ohos.security.cryptoFramework (加解密算法库框架).md#ZH-CN_TOPIC_0000002497445368__pubkey)X509证书公钥对象：仅用于X509Cert的verify接口。

**错误码：**

以下错误码的详细介绍请参见[证书错误码](../../errors/证书错误码.md)。

错误码ID错误信息19020001memory malloc failed.19030001crypto operation error.
...
参数名类型必填说明datestring是日期，为ASN.1时间格式。
...
19020001memory malloc failed.19030001crypto operation error.19030003the certificate has not taken effect.19030004the certificate has expired.
...
从API version 9开始支持，从API version 10开始废弃，建议使用[getCertSerialNumber](#ZH-CN_TOPIC_0000002529285375__getcertserialnumber10)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明number表示X509证书序列号。
...
类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示X509证书颁发者名称。

**错误码：**

以下错误码的详细介绍请参见[证书错误码](../../errors/证书错误码.md)。

错误码ID错误信息19020001memory malloc failed.19020002
...
参数名类型必填说明encodingType[EncodingType](#ZH-CN_TOPIC_0000002529285375__encodingtype12)是表示编码类型。

**返回值**：

类型说明string表示X509证书颁发者名称，使用逗号分隔相对可分辨名称。
...
参数名类型必填说明encodingType[EncodingType](#ZH-CN_TOPIC_0000002529285375__encodingtype12)否
...
类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示X509证书主体名称，转化成字符串后使用逗号分隔相对可分辨名称。
...
类型说明string表示X509证书有效期起始时间，日期为ASN.1时间格式。
...
类型说明string表示X509证书有效期截止时间，日期为ASN.1时间格式。
...
类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示X509证书签名数据。
...
类型说明string表示X509证书签名算法对象标志符OID。若OID长度超过128字节，则会被截断。
...
类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示X509证书签名算法参数。

**错误码：**

以下错误码的详细介绍请参见[证书错误码](../../errors/证书错误码.md)。

错误码ID错误信息801this operation is not supported.19020001memory malloc failed.19020002
...
类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示X509证书密钥用途。
...
类型说明[DataArray](#ZH-CN_TOPIC_0000002529285375__dataarray)表示X509证书扩展密钥用途。
...
类型说明[DataArray](#ZH-CN_TOPIC_0000002529285375__dataarray)表示X509证书主体可选名称。
...
类型说明[DataArray](#ZH-CN_TOPIC_0000002529285375__dataarray)表示X509证书颁发者可选名称。
...
参数名类型必填说明itemType[CertItemType](#ZH-CN_TOPIC_0000002529285375__certitemtype10)是表示需要获取的证书字段。

**返回值**：

类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示X509证书对应的字段，返回值为DER格式。
...
参数名类型必填说明param[X509CertMatchParameters](#ZH-CN_TOPIC_0000002529285375__x509certmatchparameters11)是表示需要匹配的参数。

**返回值**：

类型说明boolean当参数匹配时，该方法返回true，否则返回false。
...
19020001memory malloc failed.19030001crypto operation error.
...
类型说明[DataArray](#ZH-CN_TOPIC_0000002529285375__dataarray)表示X509证书CRL的分发点统一资源标识符。
...
类型说明[X500DistinguishedName](#ZH-CN_TOPIC_0000002529285375__x500distinguishedname12)X509的可分辨对象。
...
类型说明[CertExtension](#ZH-CN_TOPIC_0000002529285375__certextension10)证书扩展域段类对象。
...
参数名类型必填说明inStream[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)是表示证书扩展域段序列化数据。callbackAsyncCallback<[CertExtension](#ZH-CN_TOPIC_0000002529285375__certextension10)>是回调函数，表示扩展域段对象。
...
参数名类型必填说明inStream[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)是表示证书扩展域段序列化数据。

**返回值**：

类型说明Promise<[CertExtension](#ZH-CN_TOPIC_0000002529285375__certextension10)>表示证书扩展域段对象。
...
类型说明[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)表示证书扩展域段序列化数据。
...
参数名类型必填说明valueType[ExtensionOidType](#ZH-CN_TOPIC_0000002529285375__extensionoidtype10)是表示证书扩展域段对象标识符类型。

**返回值**：

类型说明[DataArray](#ZH-CN_TOPIC_0000002529285375__dataarray)表示证书扩展域段对象标识符列表。
...
参数名类型必填说明valueType[ExtensionEntryType](#ZH-CN_TOPIC_0000002529285375__extensionentrytype10)是表示证书扩展域段获取的类型。oid[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)是表示证书扩展域段获取的对象标识符。

**返回值**：

类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示证书扩展域段对象的数据。
...
类型说明number当证书扩展域段中密钥用途包含签名用途，并且基本约束中cA字段为true时，表示证书为CA证书。如果不是CA，则返回-1；否则返回基本约束中的路径长度。如果证书是CA证书，但是基本约束中未给定路径长度，则返回-2，表示无路径长度限制。
...
类型说明boolean当存在不支持的关键扩展时，该方法返回true，否则返回false。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[cert.createX509CRL](#ZH-CN_TOPIC_0000002529285375__certcreatex509crl11)替代。

**系统能力：** SystemCapability.Security.Cert

**参数**：

参数名类型必填说明inStream[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)是表示证书吊销列表序列化数据。callbackAsyncCallback<[X509Crl](#ZH-CN_TOPIC_0000002529285375__x509crldeprecated)>是回调函数，表示证书吊销列表对象。
...
801this operation is not supported.19020001memory malloc failed.
...
从API version 9开始支持，从API version 11开始废弃，建议使用[cert.createX509CRL](#ZH-CN_TOPIC_0000002529285375__certcreatex509crl11-1)替代。

**系统能力：** SystemCapability.Security.Cert

**参数**：

参数名类型必填说明inStream[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)是表示证书吊销列表序列化数据。

**返回值**：

类型说明Promise<[X509Crl](#ZH-CN_TOPIC_0000002529285375__x509crldeprecated)>表示证书吊销列表对象。
...
参数名类型必填说明inStream[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)是表示证书吊销列表序列化数据。当前支持的数据长度不超过8192字节。callbackAsyncCallback<[X509CRL](#ZH-CN_TOPIC_0000002529285375__x509crl11)>是回调函数，表示证书吊销列表对象。
...
参数名类型必填说明inStream[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)是表示证书吊销列表序列化数据。当前支持的数据长度不超过8192字节。

**返回值**：

类型说明Promise<[X509CRL](#ZH-CN_TOPIC_0000002529285375__x509crl11)>表示证书吊销列表对象。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL](#ZH-CN_TOPIC_0000002529285375__x509crl11)替代。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.isRevoked](#ZH-CN_TOPIC_0000002529285375__isrevoked11)替代。

**系统能力：** SystemCapability.Security.Cert

**参数**：

参数名类型必填说明certX509Cert是表示被检查的证书对象。

**返回值**：

类型说明boolean表示证书吊销状态，true表示已吊销，false表示未吊销。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getType](#ZH-CN_TOPIC_0000002529285375__gettype11)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明string表示证书吊销列表类型。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getEncoded](#ZH-CN_TOPIC_0000002529285375__getencoded11)替代。

**系统能力：** SystemCapability.Security.Cert

**参数**：

参数名类型必填说明callbackAsyncCallback<[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)>是回调函数，表示X509证书吊销列表的序列化数据。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getEncoded](#ZH-CN_TOPIC_0000002529285375__getencoded11-1)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明Promise<[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)>表示X509证书吊销列表的序列化数据。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.verify](#ZH-CN_TOPIC_0000002529285375__verify11)替代。

**系统能力：** SystemCapability.Security.Cert

**参数**：

参数名类型必填说明key[cryptoFramework.PubKey](@ohos.security.cryptoFramework (加解密算法库框架).md#ZH-CN_TOPIC_0000002497445368__pubkey)是表示用于验签的公钥对象。callbackAsyncCallback<void>是回调函数,使用AsyncCallback的第一个error参数判断是否验签成功，error为null表示成功，error不为null表示失败。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.verify](#ZH-CN_TOPIC_0000002529285375__verify11-1)替代。

**系统能力：** SystemCapability.Security.Cert

**参数**：

参数名类型必填说明key[cryptoFramework.PubKey](@ohos.security.cryptoFramework (加解密算法库框架).md#ZH-CN_TOPIC_0000002497445368__pubkey)是表示用于验签的公钥对象。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getVersion](#ZH-CN_TOPIC_0000002529285375__getversion11)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明number表示获取X509证书吊销列表的版本号。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getIssuerName](#ZH-CN_TOPIC_0000002529285375__getissuername11)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示X509证书吊销列表颁发者名称。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getLastUpdate](#ZH-CN_TOPIC_0000002529285375__getlastupdate11)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明string表示X509证书吊销列表最后一次更新日期，日期为ASN.1时间格式。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getNextUpdate](#ZH-CN_TOPIC_0000002529285375__getnextupdate11)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明string表示X509证书吊销列表下一次更新的日期，日期为ASN.1时间格式。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getRevokedCert](#ZH-CN_TOPIC_0000002529285375__getrevokedcert11)替代。

**系统能力：** SystemCapability.Security.Cert

**参数**：

参数名类型必填说明serialNumbernumber是表示证书序列号。

**返回值**:

类型说明[X509CrlEntry](#ZH-CN_TOPIC_0000002529285375__x509crlentrydeprecated)表示被吊销X509证书对象。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getRevokedCertWithCert](#ZH-CN_TOPIC_0000002529285375__getrevokedcertwithcert11)替代。

**系统能力：** SystemCapability.Security.Cert

**参数**：

参数名类型必填说明cert[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)是表示证书对象。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getRevokedCerts](#ZH-CN_TOPIC_0000002529285375__getrevokedcerts11)替代。

**系统能力：** SystemCapability.Security.Cert

**参数**：

参数名类型必填说明callbackAsyncCallback<Array<[X509CrlEntry](#ZH-CN_TOPIC_0000002529285375__x509crlentrydeprecated)>>是回调函数，表示被吊销X509证书列表。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getRevokedCerts](#ZH-CN_TOPIC_0000002529285375__getrevokedcerts11-1)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**:

类型说明Promise<Array<[X509CrlEntry](#ZH-CN_TOPIC_0000002529285375__x509crlentrydeprecated)>>表示被吊销X509证书列表。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getTBSInfo](#ZH-CN_TOPIC_0000002529285375__gettbsinfo11)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**:

类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示证书吊销列表的tbsCertList信息。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getSignature](#ZH-CN_TOPIC_0000002529285375__getsignature11)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示X509证书吊销列表的签名数据。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getSignatureAlgName](#ZH-CN_TOPIC_0000002529285375__getsignaturealgname11)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明string表示X509证书吊销列表签名的算法名。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getSignatureAlgOid](#ZH-CN_TOPIC_0000002529285375__getsignaturealgoid11)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明string表示X509证书吊销列表签名算法的对象标志符OID。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRL.getSignatureAlgParams](#ZH-CN_TOPIC_0000002529285375__getsignaturealgparams11)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示X509证书吊销列表签名的算法参数。
...
参数名类型必填说明cert[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)是表示被检查的证书对象。
...
类型说明string表示X509证书吊销列表颁发者名称，使用逗号分隔相对可分辨名称。
...
参数名类型必填说明serialNumberbigint是表示证书序列号。

**返回值**:

类型说明[X509CRLEntry](#ZH-CN_TOPIC_0000002529285375__x509crlentry11)表示被吊销X509证书对象。
...
参数名类型必填说明callbackAsyncCallback<Array<[X509CRLEntry](#ZH-CN_TOPIC_0000002529285375__x509crlentry11)>>是回调函数，表示被吊销X509证书列表。
...
类型说明Promise<Array<[X509CRLEntry](#ZH-CN_TOPIC_0000002529285375__x509crlentry11)>>表示被吊销X509证书列表。
...
类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示X509CRL扩展用途。
...
参数名类型必填说明param[X509CRLMatchParameters](#ZH-CN_TOPIC_0000002529285375__x509crlmatchparameters11)是表示需要匹配的参数。
...
参数名类型必填说明algorithmstring是表示证书链校验器算法。当前仅支持输入“PKIX”。

**返回值**：

类型说明[CertChainValidator](#ZH-CN_TOPIC_0000002529285375__certchainvalidator)表示证书链校验器对象。
...
801this operation is not supported.19020001memory malloc failed.19020002
...
名称类型只读可选说明algorithmstring是否X509证书链校验器算法名称。
...
由于端侧系统时间不可信，证书链校验不包含对证书有效时间的校验。如果需要检查证书的时间有效性，可使用X509证书的[checkValidityWithDate](#ZH-CN_TOPIC_0000002529285375__checkvaliditywithdate)方法进行检查。详见[证书规格](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/certificate-framework-overview#证书规格)。
...
参数名类型必填说明certChain[CertChainData](#ZH-CN_TOPIC_0000002529285375__certchaindata)是表示X509证书链序列化数据。callbackAsyncCallback<void>是回调函数，使用AsyncCallback的第一个error参数判断是否校验成功，error为null表示成功，error不为null表示失败。
...
19030001crypto operation error.19030002the certificate signature verification failed.19030003the certificate has not taken effect.19030004the certificate has expired.19030005failed to obtain the certificate issuer.19030006the key cannot be used for signing a certificate.19030007the key cannot be used for digital signature.
...
参数名类型必填说明certChain[CertChainData](#ZH-CN_TOPIC_0000002529285375__certchaindata)是表示X509证书链序列化数据。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CrlEntry](#ZH-CN_TOPIC_0000002529285375__x509crlentry11)替代。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRLEntry.getEncoded](#ZH-CN_TOPIC_0000002529285375__getencoded11-2)替代。

**系统能力：** SystemCapability.Security.Cert

**参数**：

参数名类型必填说明callbackAsyncCallback<[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)>是回调函数，表示被吊销证书的序列化数据。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRLEntry.getEncoded](#ZH-CN_TOPIC_0000002529285375__getencoded11-3)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明Promise<[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)>表示被吊销证书的序列化数据。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRLEntry.getSerialNumber](#ZH-CN_TOPIC_0000002529285375__getserialnumber11)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明number表示被吊销证书的序列号。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRLEntry.getCertIssuer](#ZH-CN_TOPIC_0000002529285375__getcertissuer11)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示被吊销证书的颁发者信息。
...
从API version 9开始支持，从API version 11开始废弃，建议使用[X509CRLEntry.getRevocationDate](#ZH-CN_TOPIC_0000002529285375__getrevocationdate11)替代。

**系统能力：** SystemCapability.Security.Cert

**返回值**：

类型说明string表示证书被吊销的日期，日期为ASN.1时间格式。
...
类型说明string表示被吊销证书的颁发者信息，使用逗号分隔相对可分辨名称。
...
类型说明[DataBlob](#ZH-CN_TOPIC_0000002529285375__datablob)表示X509CRLEntry扩展用途。
...
类型说明boolean返回true则表示CRL Entry有扩展，返回false则表示无扩展。
...
参数名类型必填说明certsArray<[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)>是X509Cert数组。crlsArray<[X509CRL](#ZH-CN_TOPIC_0000002529285375__x509crl11)>否X509CRL数组。

**返回值**：

类型说明[CertCRLCollection](#ZH-CN_TOPIC_0000002529285375__certcrlcollection11)表示证书和证书吊销列表集合对象。
...
参数名类型必填说明param[X509CertMatchParameters](#ZH-CN_TOPIC_0000002529285375__x509certmatchparameters11)是表示证书需匹配的参数。

**返回值**：

类型说明Promise<Array<[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)>>Promise对象。表示匹配到的证书对象数组。
...
参数名类型必填说明param[X509CertMatchParameters](#ZH-CN_TOPIC_0000002529285375__x509certmatchparameters11)是表示证书需匹配的参数。callbackAsyncCallback<Array<[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)>>是回调函数，表示匹配到的证书对象数组。
...
参数名类型必填说明param[X509CRLMatchParameters](#ZH-CN_TOPIC_0000002529285375__x509crlmatchparameters11)是表示证书吊销列表需匹配的参数。

**返回值**：

类型说明Promise<Array<[X509CRL](#ZH-CN_TOPIC_0000002529285375__x509crl11)>>Promise对象，表示匹配到的证书吊销列表对象数组。
...
参数名类型必填说明param[X509CRLMatchParameters](#ZH-CN_TOPIC_0000002529285375__x509crlmatchparameters11)是表示证书吊销列表需匹配的参数对象。callbackAsyncCallback<Array<[X509CRL](#ZH-CN_TOPIC_0000002529285375__x509crl11)>>是回调函数，表示匹配到的证书吊销列表对象数组。
...
类型说明Promise<[X509CertChain](#ZH-CN_TOPIC_0000002529285375__x509certchain11)>表示X509证书链对象。
...
参数名类型必填说明inStream[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)是X509证书序列化数据。callbackAsyncCallback<[X509CertChain](#ZH-CN_TOPIC_0000002529285375__x509certchain11)>是回调函数，表示X509证书链对象。
...
参数名类型必填说明certsArray<[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)>是X509证书对象数组。

**返回值：**

类型说明[X509CertChain](#ZH-CN_TOPIC_0000002529285375__x509certchain11)表示X509证书链对象。
...
buildX509CertChain(param: [CertChainBuildParameters](#ZH-CN_TOPIC_0000002529285375__certchainbuildparameters12)): Promise<CertChainBuildResult>
...
参数名类型必填说明param[CertChainBuildParameters](#ZH-CN_TOPIC_0000002529285375__certchainbuildparameters12)是

构建证书链的参数对象。

[CertChainBuildParameters](#ZH-CN_TOPIC_0000002529285375__certchainbuildparameters12)中的maxLength要小于证书集合中证书数量。

**返回值：**

类型说明Promise<[CertChainBuildResult](#ZH-CN_TOPIC_0000002529285375__certchainbuildresult12)>表示X509证书链对象。
...
参数名类型必填说明dataUint8Array是P12文件，DER格式。config[Pkcs12ParsingConfig](#ZH-CN_TOPIC_0000002529285375__pkcs12parsingconfig18)是P12文件的解析配置。

**返回值：**

类型说明[Pkcs12Data](#ZH-CN_TOPIC_0000002529285375__pkcs12data18)表示P12文件解析后的证书、私钥及其他证书合集。
...
19030001crypto operation error.19030008maybe wrong password.
...
参数名类型必填说明dataUint8Array是Pkcs12文件，DER格式。passwordstring是Pkcs12的密码。

**返回值：**

类型说明Promise<[Pkcs12Data](#ZH-CN_TOPIC_0000002529285375__pkcs12data18)>Promise对象，返回Pkcs12文件解析后的证书、私钥及其他证书合集。返回的Pkcs12Data中的私钥采用PEM格式编码。
...
参数名类型必填说明data[Pkcs12Data](#ZH-CN_TOPIC_0000002529285375__pkcs12data18)是要打包的Pkcs12数据对象。config[Pkcs12CreationConfig](#ZH-CN_TOPIC_0000002529285375__pkcs12creationconfig21)是Pkcs12文件的创建配置。

**返回值：**

类型说明Promise<Uint8Array>Promise对象。表示创建的Pkcs12文件，DER格式。
...
参数名类型必填说明data[Pkcs12Data](#ZH-CN_TOPIC_0000002529285375__pkcs12data18)是要打包的P12数据对象。config[Pkcs12CreationConfig](#ZH-CN_TOPIC_0000002529285375__pkcs12creationconfig21)是P12文件的创建配置。

**返回值：**

类型说明Uint8Array表示创建的P12文件，DER格式。
...
createTrustAnchorsWithKeyStore(keystore: Uint8Array, pwd: string): Promise<Array<[X509TrustAnchor](#ZH-CN_TOPIC_0000002529285375__x509trustanchor11)>>

表示从P12文件中读取ca证书来构造[TrustAnchor](#ZH-CN_TOPIC_0000002529285375__x509trustanchor11)对象数组，并用Promise方式返回结果。
...
参数名类型必填说明keystoreUint8Array是P12文件，DER格式。pwdstring是P12文件的密码。

**返回值：**

类型说明Promise<Array<[X509TrustAnchor](#ZH-CN_TOPIC_0000002529285375__x509trustanchor11)>>表示X509TrustAnchor对象数组。
...
类型说明Array<[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)>X509证书数组。
...
参数名类型必填说明param[CertChainValidationParameters](#ZH-CN_TOPIC_0000002529285375__certchainvalidationparameters11)是表示校验X509证书链的参数。

**返回值**：

类型说明Promise<[CertChainValidationResult](#ZH-CN_TOPIC_0000002529285375__certchainvalidationresult11)>Promise对象，返回证书链校验结果。
...
参数名类型必填说明param[CertChainValidationParameters](#ZH-CN_TOPIC_0000002529285375__certchainvalidationparameters11)是表示校验X509证书链的参数。callbackAsyncCallback<[CertChainValidationResult](#ZH-CN_TOPIC_0000002529285375__certchainvalidationresult11)>是回调函数，返回证书链校验结果。
...
参数名类型必填说明keyInfo[PrivateKeyInfo](#ZH-CN_TOPIC_0000002529285375__privatekeyinfo18)是包含私钥跟口令的配置参数。config[CsrGenerationConfig](#ZH-CN_TOPIC_0000002529285375__csrgenerationconfig18)是包含生成CSR的配置参数。

**返回值：**

类型说明string | Uint8Array表示生成的CSR数据。
...
参数名类型必填说明nameStrstring是X509定义的string类型的Name字符串格式数据。

**返回值：**

类型说明Promise<[X500DistinguishedName](#ZH-CN_TOPIC_0000002529285375__x500distinguishedname12)>表示X509的可分辨对象。
...
参数名类型必填说明nameDerUint8Array是X509定义的Uint8Array类型的DER格式数据。
...
参数名类型必填说明typestring是指定类型的名称。

**返回值**：

类型说明Array<string>可分辨名的字符串数组。
...
类型说明string表示可分辨名的字符串，使用逗号分隔相对可分辨名称。
...
类型说明[EncodingBlob](#ZH-CN_TOPIC_0000002529285375__encodingblob)X509证书序列化数据。
...
参数名类型必填说明contentType[CmsContentType](#ZH-CN_TOPIC_0000002529285375__cmscontenttype18)是指定CMS内容类型。

**返回值：**

类型说明[CmsGenerator](#ZH-CN_TOPIC_0000002529285375__cmsgenerator18)CmsGenerator对象。
...
参数名类型必填说明cert[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)是指定X509证书。keyInfo[PrivateKeyInfo](#ZH-CN_TOPIC_0000002529285375__privatekeyinfo18)是指定私钥信息。config[CmsSignerConfig](#ZH-CN_TOPIC_0000002529285375__cmssignerconfig18)是指定签名者选项。
...
参数名类型必填说明cert[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)是要添加的X509证书。
...
参数名类型必填说明algorithm[CmsRecipientEncryptionAlgorithm](#ZH-CN_TOPIC_0000002529285375__cmsrecipientencryptionalgorithm22)是用于CMS封装数据的加密算法。
...
参数名类型必填说明recipientInfo[CmsRecipientInfo](#ZH-CN_TOPIC_0000002529285375__cmsrecipientinfo22)是接收者信息。
...
参数名类型必填说明dataUint8Array是Cms操作的内容。options[CmsGeneratorOptions](#ZH-CN_TOPIC_0000002529285375__cmsgeneratoroptions18)否Cms操作的配置选项。

**返回值：**

类型说明Promise<Uint8Array | string>返回Cms最终数据的Promise。
...
类型说明Uint8Array | string返回Cms最终数据。
...
类型说明Promise<Uint8Array>Promise对象, 返回加密的数据内容。
...
类型说明[CmsParser](#ZH-CN_TOPIC_0000002529285375__cmsparser22)CmsParser对象。
...
参数名类型必填说明dataUint8Array | string是CMS数据内容。cmsFormat[CmsFormat](#ZH-CN_TOPIC_0000002529285375__cmsformat18)是指定输入的CMS格式。
...
类型说明[CmsContentType](#ZH-CN_TOPIC_0000002529285375__cmscontenttype18)返回CMS数据类型。
...
参数名类型必填说明config[CmsVerificationConfig](#ZH-CN_TOPIC_0000002529285375__cmsverificationconfig22)是CMS验签配置内容。
...
19030001crypto operation error.19030003the certificate has not taken effect.19030004the certificate has expired.19030005failed to obtain the certificate issuer.
...
类型说明Promise<Uint8Array>Promise对象，返回CMS原始数据。
...
getCerts(type: CmsCertType): Promise<Array<[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)>>
...
参数名类型必填说明type[CmsCertType](#ZH-CN_TOPIC_0000002529285375__cmscerttype22)是从cms中获取证书的类型。

**返回值：**

类型说明Promise<Array<[X509Cert](#ZH-CN_TOPIC_0000002529285375__x509cert)>>Promise对象，返回证书集合。
...
参数名类型必填说明config[CmsEnvelopedDecryptionConfig](#ZH-CN_TOPIC_0000002529285375__cmsenvelopeddecryptionconfig22)是CMS解封装配置内容。

**返回值：**

类型说明Promise<Uint8Array>Promise对象，返回解封装结果。
