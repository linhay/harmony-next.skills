...
客户端：使用WebSocket建立服务器与客户端的双向连接，需要先通过[createWebSocket](#ZH-CN_TOPIC_0000002553361405__websocketcreatewebsocket)方法创建[WebSocket](#ZH-CN_TOPIC_0000002553361405__websocket)对象，然后通过[connect](#ZH-CN_TOPIC_0000002553361405__connect)方法连接到服务器。当连接成功后，客户端会收到[open](#ZH-CN_TOPIC_0000002553361405__onopen)事件的回调，之后客户端就可以通过[send](#ZH-CN_TOPIC_0000002553361405__send)方法与服务器进行通信。当服务器发信息给客户端时，客户端会收到[message](#ZH-CN_TOPIC_0000002553361405__onmessage)事件的回调。当客户端想要取消此连接时，通过调用[close](#ZH-CN_TOPIC_0000002553361405__close)方法主动断开连接后，客户端会收到[close](#ZH-CN_TOPIC_0000002553361405__onclose)事件的回调。若在上述任一过程中发生错误，客户端会收到[error](#ZH-CN_TOPIC_0000002553361405__onerror)事件的回调。

服务端：（从API version 23开始支持全设备使用，之前仅支持TV设备使用）使用WebSocket建立服务器与客户端的双向连接，需要先通过[createWebSocketServer](#ZH-CN_TOPIC_0000002553361405__websocketcreatewebsocketserver19)方法创建[WebSocketServer](#ZH-CN_TOPIC_0000002553361405__websocketserver19)对象，然后通过[start](#ZH-CN_TOPIC_0000002553361405__start19)方法启动服务器，监听客户端的申请建链的消息。当连接成功后，服务端会收到[connect](#ZH-CN_TOPIC_0000002553361405__onconnect19)事件的回调，之后服务端可以通过[send](#ZH-CN_TOPIC_0000002553361405__send19)方法与客户端进行通信，或者通过[listAllConnections](#ZH-CN_TOPIC_0000002553361405__listallconnections19)方法列举出当前与服务端建链的所有客户端信息。当客户端给服务端发消息时，服务端会收到[messageReceive](#ZH-CN_TOPIC_0000002553361405__onmessagereceive19)事件回调。当服务端想断开与某个客户端的连接时，可以通过调用[close](#ZH-CN_TOPIC_0000002553361405__close19)方法主动断开与某个客户端的连接，之后服务端会收到[close](#ZH-CN_TOPIC_0000002553361405__onclose19)事件的回调。当服务端想停止service时，可以调用[stop](#ZH-CN_TOPIC_0000002553361405__stop19)方法。若在上述任一过程中发生错误，服务端会收到[error](#ZH-CN_TOPIC_0000002553361405__onerror19)事件的回调。
...
在调用WebSocket的方法前，需要先通过[webSocket.createWebSocket](#ZH-CN_TOPIC_0000002553361405__websocketcreatewebsocket)创建一个WebSocket。
...
callback中返回的boolean值仅表示连接请求创建是否成功。如需感知WebSocket是否连接成功，需要在调用该接口前调用[on('open')](#ZH-CN_TOPIC_0000002553361405__onopen)订阅open事件。
...
以下错误码的详细介绍参见[webSocket错误码]([webSocket错误码](../../errors/webSocket错误码.md).md)和[通用错误码]([通用错误码](../../errors/通用错误码.md).md)。
...
订阅WebSocket的打开事件，使用callback异步回调。该事件用于指示WebSocket是否连接成功。该接口需要在调用[connect](#ZH-CN_TOPIC_0000002553361405__connect)发起连接请求前调用。
...
在调用WebSocketServer方法前，需要先通过[webSocket.createWebSocketServer](#ZH-CN_TOPIC_0000002553361405__websocketcreatewebsocketserver19)创建一个WebSocketServer。
