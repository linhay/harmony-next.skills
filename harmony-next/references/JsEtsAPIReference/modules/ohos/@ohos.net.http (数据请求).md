...
HTTP请求任务。在调用HttpRequest的方法前，需要先通过[createHttp()](#ZH-CN_TOPIC_0000002553361401__httpcreatehttp)创建一个任务。
...
(1) 此接口仅支持接收5MB以内的数据，如果需要接收超过5MB的数据，则需主动在[HttpRequestOptions](#ZH-CN_TOPIC_0000002553361401__httprequestoptions)的maxLimit中进行设置，或者使用[requestInStream](#ZH-CN_TOPIC_0000002553361401__requestinstream10)接口发起流式请求。
...
以下错误码的详细介绍参见[通用错误码]([通用错误码](../../errors/通用错误码.md).md)和[HTTP错误码]([HTTP错误码](../../errors/HTTP错误码.md).md)。
...
| callback | AsyncCallback<HttpResponse> | 是 | 回调函数。当请求成功时，回调内容是HttpResponse ，请求失败时为undefined。 |
...
从API version 6开始支持，从API version 8开始废弃，建议使用[on("headersReceive")](#ZH-CN_TOPIC_0000002553361401__onheadersreceive8)替代。
...
从API version 6开始支持，从API version 8开始废弃，建议使用[off("headersReceive")](#ZH-CN_TOPIC_0000002553361401__offheadersreceive8)替代。
...
| callback | Callback<DataReceiveProgressInfo> | 是 | 回调函数。当订阅成功时，回调内容是DataReceiveProgressInfo，订阅失败时为undefined。 |
...
| callback | Callback<DataSendProgressInfo> | 是 | 回调函数。当订阅成功时，回调内容是DataSendProgressInfo，订阅失败时为undefined。 |
...
| header | Object | 否 | 是 | HTTP请求头字段。当请求方式为"POST" "PUT" "DELETE" 或者""时，默认{'content-Type': 'application/json'}， 否则默认{'content-Type': 'application/x-www-form-urlencoded'}。 如果head中包含number类型的字段，最大支持int64的整数。 header字段支持JSON格式如 完整示例 和Record<string, string>格式输入。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
...
| usingProtocol9+ | HttpProtocol | 否 | 是 | 使用协议。默认值由系统自动指定。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
...
| multiFormDataList11+ | Array<MultiFormData> | 否 | 是 | 当'content-Type'为'multipart/form-data'时，则上传该字段定义的数据字段表单列表。 |
| certificatePinning12+ | CertificatePinning | CertificatePinning[] | 否 | 是 | 支持动态设置证书锁定配置，可以传入单个或多个证书PIN码。 |
| addressFamily15+ | AddressFamily | 否 | 是 | 支持解析目标域名时限定地址类型。 |
| remoteValidation18+ | RemoteValidation | 否 | 是 | 证书颁发机构（CA），用于验证远程服务器的身份。如果未设置此字段，系统CA将用于验证远程服务器的标识。 元服务API： 从API version 18开始，该接口支持在元服务中使用。 |
| tlsOptions18+ | TlsOptions | 否 | 是 | TLS配置。 元服务API： 从API version 18开始，该接口支持在元服务中使用。 |
| serverAuthentication18+ | ServerAuthentication | 否 | 是 | 安全连接期间的服务器身份验证配置。默认不认证。 元服务API： 从API version 18开始，该接口支持在元服务中使用。 |
| sslType20+ | SslType | 否 | 是 | 使用安全通信协议TLS（默认）或TLCP。如果使用TLCP，相关的选项（如caPath、clientCert和clientEncCert）必须赋有效值。 元服务API： 从API version 20开始，该接口支持在元服务中使用。 |
| clientEncCert20+ | ClientCert | 否 | 是 | 支持应用程序传入客户端证书，使服务器能够进行验证客户端的加密身份。 元服务API： 从API version 20开始，该接口支持在元服务中使用。 |
...
| resultType9+ | HttpDataType | 否 | 否 | 返回值类型。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
| responseCode | ResponseCode | number | 否 | 否 | 回调函数执行成功时，此字段为ResponseCode。若执行失败，错误码将会从AsyncCallback中的err字段返回。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
...
| performanceTiming11+ | PerformanceTiming | 否 | 否 | HTTP请求的各个阶段的耗时。 |
...
| certType | CertType | 否 | 是 | 证书类型，默认是PEM。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| dnsTiming | number | 否 | 否 | 从request请求到DNS解析完成耗时。 |
| tcpTiming | number | 否 | 否 | 从request请求到TCP连接完成耗时。 |
| tlsTiming | number | 否 | 否 | 从request请求到TLS连接完成耗时。 |
| firstSendTiming | number | 否 | 否 | 从request请求到开始发送第一个字节的耗时。 |
| firstReceiveTiming | number | 否 | 否 | 从request请求到接收第一个字节的耗时。 |
| totalFinishTiming | number | 否 | 否 | 从request请求到完成请求的耗时。 |
| redirectTiming | number | 否 | 否 | 从request请求到完成所有重定向步骤的耗时。 |
| responseHeaderTiming | number | 否 | 否 | 从request请求到header解析完成的耗时。 |
| responseBodyTiming | number | 否 | 否 | 从request请求到body解析完成的耗时。 |
| totalTiming | number | 否 | 否 | 从request请求回调到应用程序的耗时。 |
...
| filePath | string | 否 | 是 | 此参数将文件路径指向的文件内容设置为表单数据，如果未指定data内容，则必须设置filePath。 说明：需传入文件管理模块支持的格式，可以通过文件管理的access接口，验证文件是否存在且可访问。 |

#### http.createHttpResponseCache9+

createHttpResponseCache(cacheSize?: number): HttpResponseCache

创建一个HttpResponseCache对象，可用于存储HTTP请求的响应数据。对象中可调用[flush](#ZH-CN_TOPIC_0000002553361401__flush9)与[delete](#ZH-CN_TOPIC_0000002553361401__delete9)方法，cacheSize指定缓存大小。
...
存储HTTP访问请求响应的对象。在调用HttpResponseCache的方法前，需要先通过[createHttpResponseCache()](#ZH-CN_TOPIC_0000002553361401__httpcreatehttpresponsecache9)创建一个任务。
...
| cipherSuites | CipherSuite[] | 否 | 是 | 声明加密套件类型的数组。如果没有设置，默认携带全部支持的加密套件类型，加密套件类型参考TlsV13SpecificCipherSuite、TlsV12SpecificCipherSuite、TlsV10SpecificCipherSuite。 |
...
HTTP请求上下文数据。该对象实例在拦截器的[interceptorHandle](#ZH-CN_TOPIC_0000002553361401__interceptorhandle22)方法中作为参数传入，开发者可以通过该对象获取和修改HTTP请求的相关信息。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| interceptorType | InterceptorType | 否 | 否 | 拦截器类型，定义此拦截器何时被调用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| reqContext | HttpRequestContext | 是 | 经过HTTP拦截器的请求参数的上下文。 |
| rspContext | HttpResponse | 是 | 经过HTTP拦截器的返回结果的上下文。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<ChainContinue> | 继续HTTP处理或终止并返回HTTP响应。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| chain | HttpInterceptor[] | 是 | 拦截器实例组成的拦截链，支持传入单个或者多个不同类型的拦截器。 |
...
| 类型 | 说明 |
| --- | --- |
| HttpInterceptor[] | 返回通过addChain方法添加的所有拦截器实例。 |
...
将拦截器链附加到[HttpRequest](#ZH-CN_TOPIC_0000002553361401__httprequest)实例后，当该实例发起HTTP请求时，会触发已附加的拦截器链中相应类型的拦截器。

更多使用HTTP请求触发拦截器功能，可以参考[HTTP拦截器功能代码示例](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/http-request#http拦截器)。

HTTP拦截器相关能力仅支持[HttpRequest.request](#ZH-CN_TOPIC_0000002553361401__request)接口，目前暂不支持[HttpRequest.requestInStream](#ZH-CN_TOPIC_0000002553361401__requestinstream10)(流式传输)接口。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| httpRequest | HttpRequest | 是 | 要发起HTTP请求的HttpRequest。 |
