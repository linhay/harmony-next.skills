...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| typeId11+ | string | 否 | 否 | 标准化数据类型的ID（即UTD列表中对应的枚举值），也可以是自定义UTD。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 所指定的标准化数据类型（即UniformDataType中对应的枚举值）。 |
...
判断指定的标准化数据类型描述类对象的类型ID和当前标准化数据类型描述类对象的类型ID是否相同，即[TypeDescriptor](#ZH-CN_TOPIC_0000002522240636__typedescriptor11)对象的typeId。
