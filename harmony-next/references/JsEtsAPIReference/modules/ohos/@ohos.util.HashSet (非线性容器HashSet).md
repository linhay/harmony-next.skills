...
HashSet基于[HashMap](@ohos.util.HashMap (非线性容器HashMap).md)实现。在HashSet中，仅处理value对象。

HashSet和[TreeSet](@ohos.util.TreeSet (非线性容器TreeSet).md)相比，HashSet中的数据按Hash值排序，即存放元素的顺序和取出的顺序不一致，而TreeSet是有序存放。它们集合中的元素都不允许重复，HashSet允许插入null值，TreeSet不建议插入null值，可能会影响排序。
