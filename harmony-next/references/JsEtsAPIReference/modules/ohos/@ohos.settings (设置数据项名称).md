...
名称类型只读说明DEVICE_SHARED11+string是设备属性共享域，所有设置项不区分多用户。USER_PROPERTY11+string是为用户属性域，该域下所有配置区分多用户。
...
名称类型只读说明DATE_FORMAT(deprecated)string是
...
AUTO_GAIN_TIME(deprecated)string是
...
AUTO_GAIN_TIME_ZONE(deprecated)string是
...
名称类型只读说明FONT_SCALEstring是（domainName为USER_PROPERTY）字体的比例因子，值为固定浮点数。标准档位取值为1，其他档位包括0.85、1.15、1.3、1.45。关怀模式下，额外提供1.75、2、3.2档位。SCREEN_BRIGHTNESS_STATUSstring是屏幕亮度。取值范围:0到255。AUTO_SCREEN_BRIGHTNESSstring是
...
AUTO_SCREEN_BRIGHTNESS_MODEnumber是使用屏幕亮度自动调整时AUTO_SCREEN_BRIGHTNESS的值。MANUAL_SCREEN_BRIGHTNESS_MODEnumber是使用屏幕亮度手动调整时的AUTO_SCREEN_BRIGHTNESS值。SCREEN_OFF_TIMEOUTstring是设备在一段时间不活动后进入睡眠状态的等待时间（单位: ms）。DEFAULT_SCREEN_ROTATION(deprecated)string是

启用屏幕的自动旋转时，此属性无效。不启用自动旋转时，以下值可用:
...
ANIMATOR_DURATION_SCALE(deprecated)string是

动画持续时间的比例因子，影响所有此类动画的开始延迟和持续时间。

值为0，表示动画将立即结束。默认值为1。

TRANSITION_ANIMATION_SCALE(deprecated)string是

过渡动画的比例因子。

值为0，表示禁用过渡动画。

WINDOW_ANIMATION_SCALE(deprecated)string是

普通窗口动画的比例因子。

值为0，表示禁用窗口动画。

DISPLAY_INVERSION_STATUS(deprecated)string是
...
名称类型只读说明SETUP_WIZARD_FINISHED(deprecated)string是
...
END_BUTTON_ACTION(deprecated)string是

当用户不在呼叫中时，由用户按下呼叫结束按钮后会发生的情况。
...
- 值为3，表示显示主屏幕。如果用户已在主屏幕上，设备将进入睡眠状态。

ACCELEROMETER_ROTATION_STATUSstring是

是否启用加速计更改屏幕方向（是否启用自动旋转）。

- 值为1，表示启用加速计。

- 值为0，表示不启用加速计。

DEVICE_PROVISION_STATUS(deprecated)string是

是否预配设备。

在具有单个系统用户的多用户设备上，当值为true时，屏幕可能会被锁定。此外，其他功能无法在系统用户上启动，除非被标记在屏幕锁定上显示。

HDC_STATUS(deprecated)string是

是否启用USB设备上的HDC（硬盘控制器）。

- 值为true，表示启用HDC。

- 值为false，表示不启用HDC。

BOOT_COUNTING(deprecated)string是设备开机后的启动操作数。CONTACT_METADATA_SYNC_STATUS(deprecated)string是
...
DEVICE_NAMEstring是设备名称。USB_STORAGE_STATUS(deprecated)string是

是否启用USB大容量存储。

- 值为true，表示启用USB大容量存储。

- 值为false，表示不启用USB大容量存储。

DEBUGGER_WAITING(deprecated)string是

设备在启动应用程序进行调试时是否等待调试器进行调试。

- 值为1，表示设备等待调试器。

- 值为0，表示系统不会等待调试器，应用程序正常运行。

DEBUG_APP_PACKAGE(deprecated)string是要调试的应用程序的bundle name。ACCESSIBILITY_STATUS(deprecated)string是
...
ACTIVATED_ACCESSIBILITY_SERVICES(deprecated)string是已激活的辅助功能的列表。GEOLOCATION_ORIGINS_ALLOWED(deprecated)string是浏览器可以使用的默认地理位置。多个地理位置由空格分隔。SKIP_USE_HINTS(deprecated)string是

控制应用程序首次启动时是否跳过所有介绍性提示。适用于临时用户或熟悉环境的用户。

- 值为1，表示应用程序将在首次启动时跳过所有介绍性提示。

- 值为0，表示应用程序不会在首次启动时跳过所有介绍性提示。

TOUCH_EXPLORATION_STATUS(deprecated)string是
...
名称类型只读说明DEFAULT_INPUT_METHOD(deprecated)string是默认输入法及其ID。ACTIVATED_INPUT_METHOD_SUB_MODEstring是默认输入法键盘类型及其ID。ACTIVATED_INPUT_METHODS(deprecated)string是
...
用ima代表输入法ID，keyboardType代表键盘类型，示例格式是ima0:keyboardType0;keyboardType1;ima1:ima2:keyboardTypes0。

SELECTOR_VISIBILITY_FOR_INPUT_METHOD(deprecated)string是
...
AUTO_CAPS_TEXT_INPUT(deprecated)string是
...
AUTO_PUNCTUATE_TEXT_INPUT(deprecated)string是

是否为文本编辑器启用自动标点符号。自动标点符号使文本编辑器能够将两个空格转换为句点'.'和空格。

- 值为0，表示不启用自动标点符号。

- 值为1，表示启用自动标点符号。

AUTO_REPLACE_TEXT_INPUT(deprecated)string是

是否为文本编辑器启用自动更正。自动更正使文本编辑器能够更正拼写错误。

- 值为0，表示不启用自动更正。

- 值为1，表示启用自动更正。

SHOW_PASSWORD_TEXT_INPUT(deprecated)string是

是否在文本编辑器中启用密码显示。密码显示使文本编辑器能够在用户键入密码字符时显示密码字符。
...
名称类型只读说明DATA_ROAMING_STATUS(deprecated)string是
...
HTTP_PROXY_CFG(deprecated)string是全局HTTP代理的主机名和端口号。主机名和端口号由冒号':'分隔。NETWORK_PREFERENCE_USAGE(deprecated)string是要使用网络的用户首选项。
...
名称类型只读说明RTT_CALLING_STATUS(deprecated)string是

是否启用实时文本（RTT）呼叫。启用表示来电和去电在设备以及运营商支持时作为RTT呼叫应答。
...
名称类型只读说明VIBRATE_WHILE_RINGING(deprecated)string是

设备在来电响铃时是否振动。此属性由电话和设置应用程序使用。

该值是布尔类型，仅影响设备因来电而响铃的情况，不影响任何其他应用程序或场景。

DEFAULT_ALARM_ALERT(deprecated)string是系统默认告警的存储区域。DTMF_TONE_TYPE_WHILE_DIALING(deprecated)string是

拨号时播放的双音多频（DTMF）音的类型。

- 值为0，表示常规的短音效。

- 值为1，表示长音效。

DTMF_TONE_WHILE_DIALING(deprecated)string是
...
AFFECTED_MODE_RINGER_STREAMS(deprecated)string是音频流受振铃模式和请勿打扰（DND）模式更改的影响。要求特定的音频流受到振铃模式和DND模式变化的影响，将对应比特位设置为1。AFFECTED_MUTE_STREAMS(deprecated)string是受静音模式影响的音频流。若需在静音模式下保持特定音频流静音，将相应位设为1。DEFAULT_NOTIFICATION_SOUND(deprecated)string是系统默认通知音的存储区域。DEFAULT_RINGTONE(deprecated)string是系统默认铃声的存储区域。SOUND_EFFECTS_STATUS(deprecated)string是
...
VIBRATE_STATUS(deprecated)string是

设备是否为事件振动。该参数在系统内部使用。

- 值为1，表示设备会因事件而振动。

- 值为0，表示设备不因事件振动。

HAPTIC_FEEDBACK_STATUS(deprecated)string是
...
名称类型只读说明DEFAULT_TTS_PITCH(deprecated)string是

文本转语音(TTS)引擎的默认音高。

其中100=1x，该值设置为200，表示频率是正常声音频率的两倍。

DEFAULT_TTS_RATE(deprecated)string是

TTS引擎的默认语速。

其中100=1x。

DEFAULT_TTS_SYNTH(deprecated)string是默认TTS引擎。ENABLED_TTS_PLUGINS(deprecated)string是用于TTS的已激活插件包列表，多个插件包用空格分隔。
...
名称类型只读说明BLUETOOTH_DISCOVER_ABILITY_STATUS(deprecated)string是

设备是否可以被其他设备通过蓝牙发现或连接。
...
BLUETOOTH_DISCOVER_TIMEOUT(deprecated)string是

通过蓝牙发现设备的持续时间（以秒为单位）。

这段时间后，设备不可以被蓝牙搜寻到。

AIRPLANE_MODE_RADIOS(deprecated)string是

启用飞行模式时要禁用的无线电信号列表。

多个无线电信号用逗号(,)分隔。

取值包括以下常量：BLUETOOTH_RADIO、 CELL_RADIO、 NFC_RADIO、 WIFI_RADIO。

BLUETOOTH_RADIO(deprecated)string是常量，作为AIRPLANE_MODE_RADIOS的取值时表示蓝牙在飞行模式下禁用。CELL_RADIO(deprecated)string是常量，作为AIRPLANE_MODE_RADIOS的取值时表示蜂窝无线电在飞行模式下禁用。NFC_RADIO(deprecated)string是常量，作为AIRPLANE_MODE_RADIOS的取值时表示NFC在飞行模式下禁用。WIFI_RADIO(deprecated)string是常量，作为AIRPLANE_MODE_RADIOS的取值时表示Wi-Fi在飞行模式下禁用。BLUETOOTH_STATUS(deprecated)string是
...
OWNER_LOCKDOWN_WIFI_CFG(deprecated)string是

是否应锁定由设备所有者的应用程序创建的Wi-Fi配置。

- 值为true，表示Wi-Fi配置应该被锁定。

- 值为false，表示不应该被锁定。

WIFI_DHCP_MAX_RETRY_COUNT(deprecated)string是尝试从DHCP服务器获取IP地址的最大次数。WIFI_TO_MOBILE_DATA_AWAKE_TIMEOUT(deprecated)string是Wi-Fi连接断开后等待移动数据连接时保持唤醒锁的最长时间。WIFI_STATUS(deprecated)string是
...
WIFI_WATCHDOG_STATUS(deprecated)string是
...
应用上下文（仅支持UIAbilityContext和ExtensionContext）。
...
valuestring是数据项值。取值范围随业务变动。callbackAsyncCallback<boolean>是回调函数。返回true表示操作成功，返回false表示操作失败。
...
valuestring是数据项值。取值范围随业务变动。domainNamestring是
...
callbackAsyncCallback<string>是使用callback方式获取数据项的值。
...
类型说明Promise<string>Promise对象。返回获得的数据项的值。
...
defValuestring是默认值。由开发者设置，在数据库中查询不到该数据时，返回默认值。
...
defValuestring是默认值。由开发者设置，在数据库中查询不到该数据时，返回默认值。domainNamestring是
...
valuestring是数据项的具体数值。取值范围随业务变动。

**返回值**：

类型说明boolean返回设置数据项的值是否成功的结果。true表示设置成功，false表示设置失败。
...
observerAsyncCallback<void>是使用callback方式获取数据项的值。

**返回值**：

类型说明boolean返回注册观察者是否成功的结果。true表示注册成功，false表示注册失败。
...
类型说明boolean返回注销指定域名下指定键的监视器是否成功。true表示注销成功，false表示注销失败。
...
以下错误码详细介绍请参考[设置数据项错误码](../../errors/设置数据项错误码.md)。

错误码ID错误信息14800000Parameter error.14800010Original service error.
...
参数名类型必填说明enableboolean是是否开启飞行模式。true表示开启，false表示禁用。callbackAsyncCallback<void>是回调函数。
...
参数名类型必填说明enableboolean是是否开启飞行模式。true表示开启，false表示禁用。
...
callbackAsyncCallback<object>是回调函数。获取数据项的URI。
...
类型说明Promise<object>Promise对象。返回获取的数据项的URI。
...
从 API version 7开始支持，从API version 9开始废弃，建议使用[getValue()](#ZH-CN_TOPIC_0000002497605516__settingsgetvalue10)替代。
...
参数名类型必填说明dataAbilityHelper[DataAbilityHelper](../../topics/system-services/DataAbilityHelper.md)是数据管理辅助类。namestring是
...
callbackAsyncCallback<object>是使用callback方式获取数据项的值。
...
从 API version 7开始支持，从API version 9开始废弃，建议使用[getValue()](#ZH-CN_TOPIC_0000002497605516__settingsgetvalue10-1)替代。
...
类型说明Promise<object>Promise对象。返回获取的数据项的值。
...
从 API version 8开始支持，从API version 9开始废弃，建议使用[getValueSync()](#ZH-CN_TOPIC_0000002497605516__settingsgetvaluesync10)替代。
...
从 API version 8开始支持，从API version 9开始废弃，建议使用[setValueSync()](#ZH-CN_TOPIC_0000002497605516__settingssetvaluesync10)替代。
