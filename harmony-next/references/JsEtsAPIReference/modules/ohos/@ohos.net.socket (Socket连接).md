...
UDPSocket连接。在调用UDPSocket的方法前，需要先通过[socket.constructUDPSocketInstance](#ZH-CN_TOPIC_0000002522081482__socketconstructudpsocketinstance)创建UDPSocket对象。
...
发送数据前，需要先调用[UDPSocket.bind()](#ZH-CN_TOPIC_0000002522081482__bind)绑定IP地址和端口。该接口为耗时操作，请在Worker线程或taskpool线程调用该接口。
...
- [bind](#ZH-CN_TOPIC_0000002522081482__bind)方法调用成功后，才可调用此方法。

- bind异常、Socket已关闭（如调用close后）等异常情况下调用本接口会返回-1。

- 文件描述符的生命周期由系统管理，应用可以通过[close](#ZH-CN_TOPIC_0000002522081482__close)方法关闭Socket连接，避免直接操作文件描述符进行关闭。
...
UDPSocket连接的其他属性。继承自[ExtraOptionsBase](#ZH-CN_TOPIC_0000002522081482__extraoptionsbase)。
...
MulticastSocket连接。在调用MulticastSocket的方法前，需要先通过[socket.constructMulticastSocketInstance](#ZH-CN_TOPIC_0000002522081482__socketconstructmulticastsocketinstance11)创建MulticastSocket对象。
...
从已加入的多播组中退出，必须在加入多播组 [addMembership](#ZH-CN_TOPIC_0000002522081482__addmembership11) 之后退出才有效。
...
在调用 [addMembership](#ZH-CN_TOPIC_0000002522081482__addmembership11) 之后，调用此接口才有效。
...
TCPSocket连接。在调用TCPSocket的方法前，需要先通过[socket.constructTCPSocketInstance](#ZH-CN_TOPIC_0000002522081482__socketconstructtcpsocketinstance)创建TCPSocket对象。
...
- 文件描述符的生命周期由系统管理，应用可以通过[close](#ZH-CN_TOPIC_0000002522081482__close-2)方法关闭Socket连接，避免直接操作文件描述符进行关闭。
...
TCPSocket连接的其他属性。继承自[ExtraOptionsBase](#ZH-CN_TOPIC_0000002522081482__extraoptionsbase)。
...
TCPSocketServer连接。在调用TCPSocketServer的方法前，需要先通过[socket.constructTCPSocketServerInstance](#ZH-CN_TOPIC_0000002522081482__socketconstructtcpsocketserverinstance10)创建TCPSocketServer对象。
...
- [listen](#ZH-CN_TOPIC_0000002522081482__listen10)方法调用成功后，才可调用此方法。多次调用listen时，会获取最新监听端口绑定的文件描述符。

- 监听异常、Socket已关闭（如调用close后）等异常情况下调用本接口会返回-1。

- 文件描述符的生命周期由系统管理，应用可以通过[close](#ZH-CN_TOPIC_0000002522081482__close20)方法关闭Socket连接，避免直接操作文件描述符进行关闭。
...
TCPSocketServer停止监听并释放通过[listen](#ZH-CN_TOPIC_0000002522081482__listen10)方法绑定的端口。若多次调用[listen](#ZH-CN_TOPIC_0000002522081482__listen10)方法，再调用此方法时会释放TCPSocketServer的所有监听端口。使用Promise异步回调。


该方法不会关闭已有连接。如需关闭，请调用[TCPSocketConnection](#ZH-CN_TOPIC_0000002522081482__tcpsocketconnection10)的[close](#ZH-CN_TOPIC_0000002522081482__close10)方法。
...
- 文件描述符的生命周期由系统管理，应用可以通过[close](#ZH-CN_TOPIC_0000002522081482__close10)方法关闭Socket连接，避免直接操作文件描述符进行关闭。
...
LocalSocket连接。在调用LocalSocket的方法前，需要先通过[socket.constructLocalSocketInstance](#ZH-CN_TOPIC_0000002522081482__socketconstructlocalsocketinstance11)创建LocalSocket对象。
...
- 文件描述符的生命周期由系统管理，应用可以通过[close](#ZH-CN_TOPIC_0000002522081482__close11)方法关闭Socket连接，避免直接操作文件描述符进行关闭。
...
LocalSocketServer类。在调用LocalSocketServer的方法前，需要先通过[socket.constructLocalSocketServerInstance](#ZH-CN_TOPIC_0000002522081482__socketconstructlocalsocketserverinstance11)创建LocalSocketServer对象。
...
- [listen](#ZH-CN_TOPIC_0000002522081482__listen11)方法调用成功后，才可调用此方法。

- 监听异常、Socket已关闭（如调用close后）等异常情况下调用本接口会返回-1。

- 文件描述符的生命周期由系统管理，应用可以通过[close](#ZH-CN_TOPIC_0000002522081482__close20-1)方法关闭Socket连接，避免直接操作文件描述符进行关闭。
...
LocalSocketServer停止监听并释放通过[listen](#ZH-CN_TOPIC_0000002522081482__listen11)方法绑定的监听端口。使用Promise异步回调。


该方法不会关闭已有连接。如需关闭，请调用[LocalSocketConnection](#ZH-CN_TOPIC_0000002522081482__localsocketconnection11)的[close](#ZH-CN_TOPIC_0000002522081482__close11-1)方法。
...
- 文件描述符的生命周期由系统管理，应用可以通过[close](#ZH-CN_TOPIC_0000002522081482__close11-1)方法关闭Socket连接，避免直接操作文件描述符进行关闭。
...
TLSSocket连接。在调用TLSSocket的方法前，需要先通过[socket.constructTLSSocketInstance](#ZH-CN_TOPIC_0000002522081482__socketconstructtlssocketinstance9)创建TLSSocket对象。
...
getCertificate(callback: AsyncCallback<[X509CertRawData](#ZH-CN_TOPIC_0000002522081482__x509certrawdata9)>): void
...
getCertificate():Promise<[X509CertRawData](#ZH-CN_TOPIC_0000002522081482__x509certrawdata9)>
...
getRemoteCertificate(callback: AsyncCallback<[X509CertRawData](#ZH-CN_TOPIC_0000002522081482__x509certrawdata9)>): void
...
getRemoteCertificate():Promise<[X509CertRawData](#ZH-CN_TOPIC_0000002522081482__x509certrawdata9)>
...
- 文件描述符的生命周期由系统管理，应用可以通过[close](#ZH-CN_TOPIC_0000002522081482__close9)方法关闭Socket连接，避免直接操作文件描述符进行关闭。
...
TLSSocketServer连接。在调用TLSSocketServer的方法前，需要先通过[socket.constructTLSSocketServerInstance](#ZH-CN_TOPIC_0000002522081482__socketconstructtlssocketserverinstance10)创建TLSSocketServer对象。
...
- [listen](#ZH-CN_TOPIC_0000002522081482__listen10-3)方法调用成功后，才可调用此方法。多次调用listen时，会获取最新监听端口绑定的文件描述符。

- 监听异常、Socket已关闭（如调用close后）等异常情况下调用本接口会返回-1。

- 文件描述符的生命周期由系统管理，应用可以通过[close](#ZH-CN_TOPIC_0000002522081482__close20-2)方法关闭Socket连接，避免直接操作文件描述符进行关闭。
...
TLSSocketServer停止监听并释放通过[listen](#ZH-CN_TOPIC_0000002522081482__listen10-2)方法绑定的端口。使用Promise异步回调。


该方法不会关闭已有连接。如需关闭，请调用[TLSSocketConnection](#ZH-CN_TOPIC_0000002522081482__tlssocketconnection10)的[close](#ZH-CN_TOPIC_0000002522081482__close10-2)方法。
...
- 文件描述符的生命周期由系统管理，应用可以通过[close](#ZH-CN_TOPIC_0000002522081482__close10-2)方法关闭Socket连接，避免直接操作文件描述符进行关闭。
