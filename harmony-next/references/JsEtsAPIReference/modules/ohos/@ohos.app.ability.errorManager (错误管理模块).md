...
参数名类型必填说明typestring是填写'error'，表示错误观测器。observer[ErrorObserver](../../topics/misc/ErrorObserver.md)是错误观测器。

**返回值**：

类型说明number观测器的索引值，与观测器一一对应。可用于errorManager.off函数中的observerId参数。

**错误码**：

以下错误码详细介绍请参考[通用错误码](../../errors/通用错误码.md)和[元能力子系统错误码](../../errors/元能力子系统错误码.md)。

错误码ID错误信息401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified; 2.Incorrect parameter types; 3. Parameter verification failed.16000003The specified ID does not exist.
...
参数名类型必填说明typestring是填写'globalErrorOccurred'，表示错误观测器。observer[GlobalObserver](#ZH-CN_TOPIC_0000002497604610__globalobserver18)是自定义异常处理回调函数。
...
参数名类型必填说明typestring是填写'globalErrorOccurred'，表示错误观测器。observer[GlobalObserver](#ZH-CN_TOPIC_0000002497604610__globalobserver18)否由on方法注册的callback。建议使用该参数，缺省时默认清除所有通过on注册的相同env的callback，否则删除指定callback。

**错误码**：

以下错误码详细介绍请参考[通用错误码](../../errors/通用错误码.md)和[元能力子系统错误码](../../errors/元能力子系统错误码.md)。

错误码ID错误信息401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified; 2.Incorrect parameter types; 3. Parameter verification failed.16200001The caller has been released.16300004The observer does not exist.
...
参数名类型必填说明typestring是填写'error'，表示错误观测器。observerIdnumber是由on方法返回的观测器的index值。callbackAsyncCallback<void>是表示指定的回调方法。
...
参数名类型必填说明typestring是填写'error'，表示错误观测器。observerIdnumber是由on方法返回的观测器的index值。
...
参数名类型必填说明typestring是填写'loopObserver'，表示注册主线程消息处理耗时监听器。timeoutnumber是表示事件执行阈值（单位：毫秒）。 阈值必须大于0。observer[LoopObserver](../../topics/misc/LoopObserver.md)是注册主线程消息处理耗时监听器。
...
参数名类型必填说明typestring是填写'globalUnhandledRejectionDetected'，表示注册被拒绝promise监听器。observer[GlobalObserver](#ZH-CN_TOPIC_0000002497604610__globalobserver18)是注册被拒绝promise的callback。
...
参数名类型必填说明typestring是填写'unhandledRejection'，表示注册被拒绝promise监听器。observer[UnhandledRejectionObserver](#ZH-CN_TOPIC_0000002497604610__unhandledrejectionobserver12)是注册被拒绝promise监听器。
...
如果该回调函数执行时间超过1s，可能导致[AppRecovery](@ohos.app.ability.appRecovery (应用故障恢复).md)功能不可用。通过解析hilog日志中的begin与Freeze callback execution completed两者的时间差可以计算回调函数执行时长，如果超过1秒，可以尝试采用异步处理、减少阻塞操作、优化数据结构等方法优化回调逻辑，降低执行时长。
...
参数名类型必填说明typestring是填写'freeze'，表示应用主线程freeze观测器。observer[FreezeObserver](#ZH-CN_TOPIC_0000002497604610__freezeobserver18)是由on接口注册的freeze监听的callback。
...
参数名类型必填说明typestring是填写'loopObserver'，表示应用主线程观测器。observer[LoopObserver](../../topics/misc/LoopObserver.md)否应用主线程观测器标志。
...
参数名类型必填说明typestring是填写'globalUnhandledRejectionDetected'，表示注册被拒绝promise监听器。observer[GlobalObserver](#ZH-CN_TOPIC_0000002497604610__globalobserver18)否由on接口注册的被拒绝promise的callback。建议使用该参数，缺省时默认清除所有通过on注册的相同env的callback，否则删除指定callback。
...
参数名类型必填说明typestring是填写'unhandledRejection'，表示注册被拒绝promise监听器。observer[UnhandledRejectionObserver](#ZH-CN_TOPIC_0000002497604610__unhandledrejectionobserver12)否注册了被拒绝promise监听器。建议使用该参数，缺省时默认清除所有通过on注册的相同env的observer，否则删除指定observer。
...
参数名类型必填说明typestring是填写'freeze'，表示应用主线程freeze观测器。observer[FreezeObserver](#ZH-CN_TOPIC_0000002497604610__freezeobserver18)否由on接口注册的freeze监听的callback。建议使用该参数，如果参数不填会直接清空callback，否则删除指定的callback。

**错误码**：

以下错误码详细介绍请参考[通用错误码](../../errors/通用错误码.md)和[元能力子系统错误码](../../errors/元能力子系统错误码.md)。

错误码ID错误信息401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified; 2. Incorrect parameter types; 3. Parameter verification failed.16300004The observer does not exist.
...
参数名类型必填说明defaultHandler[ErrorHandler](#ZH-CN_TOPIC_0000002497604610__errorhandler21)否新注册的错误处理器，缺省时默认值为空。

**返回值**：

类型说明[ErrorHandler](#ZH-CN_TOPIC_0000002497604610__errorhandler21)返回上一次注册的错误处理器。

**错误码**：

以下错误码详细介绍请参考[元能力子系统错误码](../../errors/元能力子系统错误码.md)。

错误码ID错误信息16000205The API is not called in the main thread.
...
类型说明[_ErrorObserver.default](../../topics/misc/ErrorObserver.md)ErrorObserver模块。
...
类型说明[_LoopObserver](../../topics/misc/LoopObserver.md)LoopObserver模块。
...
参数名类型必填说明reasonError | any是通常是Error类型，表示被拒绝的理由。promisePromise<any>是被拒绝的promise。
...
定义异常监听，可以作为[errorManager.on('globalErrorOccurred')](#ZH-CN_TOPIC_0000002497604610__errormanageronglobalerroroccurred18)和[errorManager.on('globalUnhandledRejectionDetected')](#ZH-CN_TOPIC_0000002497604610__errormanageronglobalunhandledrejectiondetected18)的入参监听当前应用主线程事件处理事件。
...
参数名类型必填说明reason[GlobalError](#ZH-CN_TOPIC_0000002497604610__globalerror18)是有关异常事件名字、消息、错误堆栈信息、异常线程名称和类型的对象。
...
名称类型只读可选说明instanceNamestring否否
...
instanceType[InstanceType](#ZH-CN_TOPIC_0000002497604610__instancetype18)否否表示虚拟机的实例类型。
...
名称值说明MAIN0表示主虚拟机实例。WORKER1表示工作虚拟机实例。TASKPOOL2表示任务池虚拟机实例。CUSTOM3表示用户通过[napi_create_ark_runtime](../../topics/components/Node-API.md#ZH-CN_TOPIC_0000002529446145__napi_create_ark_runtime)从本机代码创建的虚拟机实例。
...
参数名类型必填说明errObjectError是有关异常事件名字、消息、错误堆栈信息的对象。