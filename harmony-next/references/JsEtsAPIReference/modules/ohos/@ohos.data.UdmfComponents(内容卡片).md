...
针对[ContentForm](@ohos.data.uniformDataStruct (标准化数据结构).md#ZH-CN_TOPIC_0000002553360561__contentform14)标准数据结构的内容卡片，支持设置标题（必选）、描述、应用图标、应用名称、跳转链接、内容图片。用户点击卡片时，执行传入的回调事件函数，若设置的跳转链接不为空，则跳转到指定的页面。
...
| 名称 | 类型 | 必填 | 装饰器类型 | 说明 |
| --- | --- | --- | --- | --- |
| contentFormData | uniformDataStruct.ContentForm | 是 | - | 内容卡片数据。 |
| formType | FormType | 是 | @Prop | 内容卡片类型，影响内容卡片的大小。 |
