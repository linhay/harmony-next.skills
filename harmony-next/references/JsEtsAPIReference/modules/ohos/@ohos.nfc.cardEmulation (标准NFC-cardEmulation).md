...
1. 声明aid（参考ISO/IEC 7816-4规范）时，name必须为payment-aid或者other-aid。填写错误会造成解析失败。
1. 声明权限时"requestPermissions"中的"name"字段的内容填写，必须是"ohos.permission.NFC_CARD_EMULATION"，不能更改。
1. 轻量级智能穿戴产品不同于其他设备，仅支持FA模型，属性配置和接口调用方式与其它设备有所区别，详见示例。
...
从 API version 6 开始支持，从 API version 9 开始废弃，建议使用[hasHceCapability](#ZH-CN_TOPIC_0000002497445444__cardemulationhashcecapability9)替代。

**系统能力：** SystemCapability.Communication.NFC.CardEmulation

名称值说明HCE0HCE 卡模拟。UICC1SIM 卡模拟。ESE2ESE 卡模拟。
...
名称值说明PAYMENT"payment"卡模拟应用所使用的业务是支付类型。OTHER"other"卡模拟应用所使用的业务是其他类型。
...
参数名类型必填说明featurenumber是卡模拟类型值，详细请见[FeatureType](#ZH-CN_TOPIC_0000002497445444__featuretypedeprecated)枚举值。

**返回值：**

**类型****说明**booleantrue: 支持该类型卡模拟， false: 不支持该类型卡模拟。
...
**类型****说明**booleantrue: 支持HCE， false: 不支持HCE。
...
参数名类型必填说明elementName[ElementName](../../topics/misc/ElementName.md)是所属应用声明NFC卡模拟能力的页面信息（至少包含bundleName、abilityName这两项的赋值），不可以为空。type[CardType](#ZH-CN_TOPIC_0000002497445444__cardtype9)是卡模拟业务类型。目前只支持默认支付应用查询。
...
**类型****说明**booleantrue: 是默认支付应用， false: 不是默认支付应用。
...
从 API version 8 开始支持，从 API version 9 开始废弃，建议使用[start](#ZH-CN_TOPIC_0000002497445444__start9)替代。
...
参数名类型必填说明aidListstring[]是动态注册卡模拟的AID列表。

**返回值：**

**类型****说明**booleantrue: 启动HCE功能或HCE已启动， false: 启动失败。

#### start9+

start(elementName: [ElementName](../../topics/misc/ElementName.md), aidList: string[]): void
...
参数名类型必填说明elementName[ElementName](../../topics/misc/ElementName.md)是所属应用声明NFC卡模拟能力的页面信息（至少包含bundleName、abilityName这两项的赋值），不可以为空。aidListstring[]是动态注册卡模拟的AID列表，允许为空。
...
801Capability not supported.3100301Card emulation running state is abnormal in service.
...
从 API version 8 开始支持，从 API version 9 开始废弃，建议使用[stop](#ZH-CN_TOPIC_0000002497445444__stop9)替代。
...
**类型****说明**booleantrue: 禁用HCE功能或HCE已禁用，false: 禁用失败。

**示例：**

示例请参见[on](#ZH-CN_TOPIC_0000002497445444__on8)接口的示例。

#### stop9+

stop(elementName: [ElementName](../../topics/misc/ElementName.md)): void
...
参数名类型必填说明elementName[ElementName](../../topics/misc/ElementName.md)是所属应用声明NFC卡模拟能力的页面信息（至少包含bundleName、abilityName这两项的赋值），不可以为空。
...
参数名类型必填说明typestring是要订阅的回调类型，固定填"hceCmd"字符串。callbackAsyncCallback<number[]>是回调函数，返回的是符合APDU协议的数据，每个number十六进制表示，范围是0x00~0xFF。

**错误码：**

以下错误码的详细介绍请参见[NFC错误码](../../errors/NFC错误码.md)。

错误码ID错误信息201Permission denied.401Invalid parameter.801Capability not supported.
...
参数名类型必填说明typestring是要取消订阅的事件类型，固定填"hceCmd"字符串。callbackAsyncCallback<number[]>否回调函数，返回的每个number十六进制表示，范围是0x00~0xFF。
...
从 API version 8 开始支持，从 API version 9 开始废弃，建议使用[transmit](#ZH-CN_TOPIC_0000002497445444__transmit9)替代。
...
参数名类型必填说明responseApdunumber[]是发送到对端读卡设备的符合APDU协议的数据，每个number十六进制表示，范围是0x00~0xFF。

#### transmit9+

transmit(response: number[]): Promise<void>

发送APDU数据到对端读卡设备，使用Promise异步回调。应用程序必须在[on](#ZH-CN_TOPIC_0000002497445444__on8)收到读卡设备发送的APDU数据后，才调用该接口响应数据。
...
参数名类型必填说明responsenumber[]是发送到对端读卡设备的符合APDU协议的数据，每个number十六进制表示，范围是0x00~0xFF。
...
发送APDU数据到对端读卡设备，应用程序必须在[on](#ZH-CN_TOPIC_0000002497445444__on8)收到读卡设备发送的APDU数据后，才调用该接口响应数据。使用Callback异步回调。
...
参数名类型必填说明responsenumber[]是发送到对端读卡设备的符合APDU协议的数据，每个number十六进制表示，范围是0x00~0xFF。callbackAsyncCallback<void>是回调函数。当发送APDU数据成功时，err为undefined，否则为错误对象。
