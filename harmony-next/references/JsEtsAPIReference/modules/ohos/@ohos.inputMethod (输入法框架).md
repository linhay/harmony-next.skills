...
获取客户端实例[InputMethodController](#ZH-CN_TOPIC_0000002522241258__inputmethodcontroller)。
...
getSystemInputMethodConfigAbility(): [ElementName](../../topics/misc/ElementName.md)
...
| 类型 | 说明 |
| --- | --- |
| [ElementName](../../topics/misc/ElementName.md) | 系统输入法设置界面Ability的ElementName。 |
...
获取客户端设置实例[InputMethodSetting](#ZH-CN_TOPIC_0000002522241258__inputmethodsetting8)。
...
从API version 6开始支持，从API version 9开始废弃，建议使用[getController](#ZH-CN_TOPIC_0000002522241258__inputmethodgetcontroller9)替代。
...
从API version 8开始支持，从API version 9开始废弃，建议使用[getSetting](#ZH-CN_TOPIC_0000002522241258__inputmethodgetsetting9)替代。
...
开发者可通过注册此对象来接收输入法应用发送的自定义通信数据，接收到自定义通信数据时会触发此对象中[onMessage](#ZH-CN_TOPIC_0000002522241258__onmessage15)回调函数。

此对象全局唯一，多次注册仅保留最后一次注册的对象及有效性，并触发上一个已注册对象的[onTerminated](#ZH-CN_TOPIC_0000002522241258__onterminated15)回调函数。

若取消注册全局已注册的对象时，会触发被取消对象中[onTerminated](#ZH-CN_TOPIC_0000002522241258__onterminated15)回调函数。
...
下列API示例中都需使用[getController](#ZH-CN_TOPIC_0000002522241258__inputmethodgetcontroller9)获取到InputMethodController实例，再通过实例调用对应方法。
...
当自绘控件所在窗口通过[setWindowFocusable](Interface (Window).md#ZH-CN_TOPIC_0000002522080752__setwindowfocusable9)设置为不可获焦窗口时，系统将无法保证自绘输入控件与输入法正常交互。若开发者希望在不可获焦窗口中绘制输入框，建议参考[不可获焦窗口中输入框与输入法交互指南](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/use-inputmethod-in-not-focusable-window)。
...
调用该接口不会解除与输入法的绑定，再次调用[showTextInput](#ZH-CN_TOPIC_0000002522241258__showtextinput10)时，可重新进入文本编辑状态。
...
[MessageHandler](#ZH-CN_TOPIC_0000002522241258__messagehandler15)对象全局唯一，多次注册仅保留最后一次注册的对象及有效性，并触发上一个已注册对象的[onTerminated](#ZH-CN_TOPIC_0000002522241258__onterminated15)回调函数。

未填写参数，则取消全局已注册的[MessageHandler](#ZH-CN_TOPIC_0000002522241258__messagehandler15)，并触发被取消注册对象中[onTerminated](#ZH-CN_TOPIC_0000002522241258__onterminated15)回调函数。
...
从API version 6开始支持，从API version 9开始废弃，建议使用[stopInputSession](#ZH-CN_TOPIC_0000002522241258__stopinputsession9)替代。
...
使用预览文本功能，需在调用[attach](#ZH-CN_TOPIC_0000002522241258__attach10)前订阅此事件，并和[on('finishTextPreview')](#ZH-CN_TOPIC_0000002522241258__onfinishtextpreview17)一起订阅。
...
使用预览文本功能，需在调用[attach](#ZH-CN_TOPIC_0000002522241258__attach10)前订阅此事件，并和[on('setPreviewText')](#ZH-CN_TOPIC_0000002522241258__onsetpreviewtext17)一起订阅。
...
下列API均需使用[getSetting](#ZH-CN_TOPIC_0000002522241258__inputmethodgetsetting9)获取到InputMethodSetting实例后，通过实例调用。
...
从API version 9开始支持，从API version 18开始废弃，建议使用[InputMethodListDialog](@ohos.inputMethodList (输入法切换列表控件).md#ZH-CN_TOPIC_0000002522081262__inputmethodlistdialog)替代。
...
从API version 8开始支持，从API version 9开始废弃，建议使用[getInputMethods](#ZH-CN_TOPIC_0000002522241258__getinputmethods9)替代。
...
从API version 8开始支持，从API version 9开始废弃，建议使用[getInputMethods](#ZH-CN_TOPIC_0000002522241258__getinputmethods9-1)替代。
...
从API version 8开始支持，从API version 9开始废弃，建议使用[InputMethodListDialog](@ohos.inputMethodList (输入法切换列表控件).md#ZH-CN_TOPIC_0000002522081262__inputmethodlistdialog)替代。
