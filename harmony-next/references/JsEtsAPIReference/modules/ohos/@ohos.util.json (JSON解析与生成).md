...
作为[JSON.parse](#ZH-CN_TOPIC_0000002553200669__jsonparse)函数的参数时，对象的每个成员将会调用此函数，允许在解析过程中对数据进行自定义处理或转换。

作为[JSON.stringify](#ZH-CN_TOPIC_0000002553200669__jsonstringify-1)函数的参数时，序列化时，每个属性都会经过该函数的转换处理。
...
检查ArkTS对象是否包含某种属性，可用于[JSON.parse](#ZH-CN_TOPIC_0000002553200669__jsonparse)解析JSON字符串之后。has接口仅支持最外层为字典形式（即大括号而非中括号包围）的合法json串。
...
从ArkTS对象中删除某种属性，可用于[JSON.parse](#ZH-CN_TOPIC_0000002553200669__jsonparse)解析JSON字符串之后。JSON.remove接口仅支持最外层为字典形式（即大括号而非中括号包围）的合法json串。
