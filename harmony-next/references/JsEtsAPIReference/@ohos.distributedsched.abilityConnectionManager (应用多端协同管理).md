...
参数名类型必填说明serviceNamestring是应用设置的服务名称（两端必须一致），最大长度为256字符。context[Context](Context (Stage模型的上下文基类).md)是表示应用上下文。peerInfo[PeerInfo](#ZH-CN_TOPIC_0000002529445403__peerinfo)是对端的协同信息。connectOptions[ConnectOptions](#ZH-CN_TOPIC_0000002529445403__connectoptions)是应用设置的连接选项。
