...
message[ResourceStr](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcestr)否否
...
levelMode15+[LevelMode](@ohos.promptAction (弹窗).md#ZH-CN_TOPIC_0000002529284783__levelmode15枚举说明)否是
...
immersiveMode15+[ImmersiveMode](@ohos.promptAction (弹窗).md#ZH-CN_TOPIC_0000002529284783__immersivemode15枚举说明)否是
...
类型说明[LevelOrder](@ohos.promptAction (弹窗).md#ZH-CN_TOPIC_0000002529284783__levelorder18)设置弹窗的显示顺序。
...
value10+[ResourceStr](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcestr)否否

Button的文本内容，若值为null，则该按钮不显示。

**元服务API：** 从API version 11开始，该接口支持在元服务中使用。

fontColor10+[ResourceColor](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcecolor)否是

Button的文本颜色。

**元服务API：** 从API version 11开始，该接口支持在元服务中使用。

backgroundColor10+[ResourceColor](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcecolor)否是

Button背景颜色。

**元服务API：** 从API version 11开始，该接口支持在元服务中使用。

action10+[VoidCallback](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__voidcallback12)否否
...
名称类型只读可选说明x[Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length)否是

弹窗遮蔽层区域相对于窗口左上角的x轴坐标。

默认值：0vp

y[Length](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__length)否是
...
名称类型只读可选说明dismissCallback<void>否否Dialog关闭回调函数。开发者需要退出时调用，不需要退出时无需调用。reason[DismissReason](Popup控制.md#ZH-CN_TOPIC_0000002529284839__dismissreason12枚举说明)否否Dialog无法关闭原因。根据开发者需要选择不同操作下，Dialog是否需要关闭。
...
名称类型只读可选说明wordBreak[WordBreak](枚举说明.md#ZH-CN_TOPIC_0000002529284967__wordbreak11)否是
...
从API version 7开始支持，从API version 18开始废弃，建议使用[showAlertDialog](Class (UIContext).md#ZH-CN_TOPIC_0000002529444749__showalertdialog)替代。showAlertDialog需先获取[UIContext](Class (UIContext).md)实例后再进行调用。

从API version 10开始，可以通过使用[UIContext](Class (UIContext).md)中的[showAlertDialog](Class (UIContext).md#ZH-CN_TOPIC_0000002529444749__showalertdialog)来明确UI的执行上下文。
...
直接使用AlertDialog可能导致[UI上下文不明确](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-global-interface)的问题，建议使用getUIContext()获取[UIContext](zh-cn_topic_0000002529444749.md)实例，并使用[showAlertDialog](Class (UIContext).md#ZH-CN_TOPIC_0000002529444749__showalertdialog)调用绑定实例的AlertDialog.show()。
