...
关于TLS1.2协议及规定内加密套件的信息，可以点击这里进行详细了解：[TLS协议及加密套件](公共说明.md#section1329115361128)。
...
Request Body传参，请务必遵循此请求头格式，否则可能导致请求失败，具体传参方式可参考[示例代码](#section14184218174113)。
...
在创建应用后，由AppGallery Connect（简称AGC）为应用分配的密钥（Client Secret）。参数取值详见[查看应用基本信息](https://developer.huawei.com/consumer/cn/doc/app/agc-help-appinfo-0000001100014694)中的**OAuth 2.0客户端ID(凭据)-Client Secret**参数。
...
Authorization Code可通过[华为账号一键登录（获取手机号和UnionID/OpenID）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-phone-unionid-login)、[华为账号登录（获取UnionID/OpenID）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-unionid-login)、[静默登录](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-silent-login)等场景获取。
...
Access Token，访问被权限管控资源的凭证。Access Token长度详见[Access Token和Refresh Token长度限制要求](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-faq-11)。
...
Refresh Token，用于刷新Access Token。Refresh Token有效期为180天，长度详见[Access Token和Refresh Token长度限制要求](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-faq-11)。
...
ID Token（JWT格式），详细信息请参见[验证ID Token有效性](验证ID Token有效性.md)中ID Token描述。
...
GroupUnionID是用户在关联主体账号组内的统一身份标识，使用场景详见[不同开发者的应用之间如何实现用户数据互通](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-faq-19)。当请求参数need_group_union_id不传或者为false时，该字段不返回。
...
业务响应主错误码，详见[错误码](#section1394417423453)。

sub_error

int

业务响应子错误码，详见[错误码](#section1394417423453)。
...
Java示例代码如下，运行前需要进行[示例代码环境配置](公共说明.md#section444143185314)（请将此示例代码与工具类CallUtils放于同一路径下，如不在同一路径，请手动添加import）
...
public class GetGroupUnionIDByCodeDemo {
    public static void main(String[] args) throws IOException {
        ...
        // 替换为您获取到的授权码（Authorization Code）
...
- 请求头的Content-Type为application/x-www-form-urlencoded，但实际代码调用时，未对请求body体进行URLEncode处理，可参考[示例代码](#section14184218174113)组装参数。
...
- “authorization_code”：该场景用于[获取用户级凭证](获取用户级凭证.md)。
- “refresh_token”： 该场景用于[刷新用户级凭证](刷新用户级凭证.md)。
- “client_credentials”：该场景用于[获取应用级凭证](获取应用级凭证.md)。
...
code被篡改或伪造的code导致，请排查code参数是否与获取到的code一致。
