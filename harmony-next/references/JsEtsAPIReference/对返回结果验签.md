...
1. 从header的x5c中获取证书链（依次是叶子证书、中间证书、根证书），使用[Huawei CBG Root CA G2](https://pki.consumer.huawei.com/ca/cer/RootCaG2Ecdsa.cer)证书对证书链进行验证。
1. 校验叶子证书的OID：1.3.6.1.4.1.2011.2.415.1.1。
1. 证书校验通过则从叶子证书获取到PublicKey。
1. 通过Header指定的算法和PublicKey进行JWT验签，可以使用各种开源库来验签，具体请参见[jwt.io](https://jwt.io/)。
...
public class JWSChecker {
    private static final String CA_CERT_FILE_PATH = "/path/to/cer/RootCaG2Ecdsa.cer";// TO-DO: Need to replace it with the actual value, please refer to [Huawei CBG Root CA G2](https://pki.consumer.huawei.com/ca/cer/RootCaG2Ecdsa.cer).
    ...
    private static PublicKey verifyChainAndGetPubKey(String[] certificates) throws Exception {
        ...
        try {
            ...
            // TO-DO: Need to do crl check if not ignore CRL
    ...
    private static PKIXParameters loadRootCAAndPKIX() throws Exception {
        ...
        // TO-DO: Under Java 8, Need to close the resource in the finally block.
