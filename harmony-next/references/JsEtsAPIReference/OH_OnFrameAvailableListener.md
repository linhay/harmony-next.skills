...
一个OH_NativeImage的监听者，通过[OH_NativeImage_SetOnFrameAvailableListener](native_image.h.md#ZH-CN_TOPIC_0000002497446038__oh_nativeimage_setonframeavailablelistener)接口注册该监听结构体，当有buffer可获取时，将触发回调给用户。
...
名称描述void* context用户自定义的上下文信息，会在回调触发时返回给用户。[OH_OnFrameAvailable](native_image.h.md#ZH-CN_TOPIC_0000002497446038__oh_onframeavailable) onFrameAvailable有buffer可获取时触发的回调函数。
