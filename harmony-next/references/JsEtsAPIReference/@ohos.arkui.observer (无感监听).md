...
名称类型只读可选说明navigationId[ResourceStr](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcestr)否否

包含NavDestination组件的Navigation组件的id。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

name[ResourceStr](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcestr)否否
...
mode15+[NavDestinationMode](NavDestination.md#ZH-CN_TOPIC_0000002529284873__navdestinationmode枚举说明11)否是
...
Navigation组件的uniqueId，可以通过[queryNavigationInfo](自定义组件内置方法.md#ZH-CN_TOPIC_0000002497444970__querynavigationinfo12)获取。
...
axis20+[Axis](枚举说明.md#ZH-CN_TOPIC_0000002529284967__axis)否是
...
名称类型只读可选说明context[UIAbilityContext](UIAbilityContext.md) | [UIContext](Class (UIContext).md)否否触发生命周期的routerPage页面对应的上下文信息。indexnumber否否
...
名称类型只读可选说明context[UIContext](Class (UIContext).md)否否屏幕像素密度变化时页面对应的上下文信息。densitynumber否否
...
名称类型只读可选说明context[UIAbilityContext](UIAbilityContext.md) | [UIContext](Class (UIContext).md)否否触发页面切换的Navigation对应的上下文信息。from[NavDestinationInfo](#ZH-CN_TOPIC_0000002529444737__navdestinationinfo) | [NavBar](zh-cn_topic_0000002497444902.html#ZH-CN_TOPIC_0000002497444902__navbar12)否否页面切换的源页面。to[NavDestinationInfo](#ZH-CN_TOPIC_0000002529444737__navdestinationinfo) | [NavBar](zh-cn_topic_0000002497444902.html#ZH-CN_TOPIC_0000002497444902__navbar12)否否页面切换的目的页面。operation[NavigationOperation](Navigation.md#ZH-CN_TOPIC_0000002497444902__navigationoperation11枚举说明)否否页面切换操作类型。
...
名称类型只读可选说明navigationId[ResourceStr](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcestr)否否指定需要监听的Navigation的ID。
...
最近一次聚焦的TabsContent组件的下标索引。索引从0开始。仅在[on('tabChange')](Class (UIObserver).md#ZH-CN_TOPIC_0000002497444806__ontabchange22)的回调函数中存在。
...
名称类型只读可选说明widthBreakpoint[WidthBreakpoint](枚举说明.md#ZH-CN_TOPIC_0000002529284967__widthbreakpoint13)是否窗口宽度所在的布局断点枚举。heightBreakpoint[HeightBreakpoint](枚举说明.md#ZH-CN_TOPIC_0000002529284967__heightbreakpoint13)是否窗口高度所在的布局断点枚举。
...
参数名类型必填说明typestring是监听事件，固定为'navDestinationUpdate'，即NavDestination组件的状态变化。options{ navigationId: [ResourceStr](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcestr) }是指定监听的Navigation的id。callbackCallback<[NavDestinationInfo](#ZH-CN_TOPIC_0000002529444737__navdestinationinfo)>是回调函数。返回当前的NavDestination组件状态。
...
参数名类型必填说明typestring是监听事件，固定为'navDestinationUpdate'，即NavDestination组件的状态变化。options{ navigationId: [ResourceStr](基础类型定义.md#ZH-CN_TOPIC_0000002497604974__resourcestr) }是指定监听的Navigation的id。callbackCallback<[NavDestinationInfo](#ZH-CN_TOPIC_0000002529444737__navdestinationinfo)>否回调函数。返回当前的NavDestination组件状态。
...
参数名类型必填说明typestring是监听事件，固定为'routerPageUpdate'，即router中page页面的状态变化。context[UIAbilityContext](UIAbilityContext.md) | [UIContext](Class (UIContext).md)是上下文信息，用以指定监听页面的范围。callbackCallback<[RouterPageInfo](#ZH-CN_TOPIC_0000002529444737__routerpageinfo)>是回调函数。携带pageInfo，返回当前的page页面状态。
...
参数名类型必填说明typestring是监听事件，固定为'routerPageUpdate'，即router中page页面的状态变化。context[UIAbilityContext](UIAbilityContext.md) | [UIContext](Class (UIContext).md)是上下文信息，用以指定监听页面的范围。callbackCallback<[RouterPageInfo](#ZH-CN_TOPIC_0000002529444737__routerpageinfo)>否需要被注销的回调函数。
...
参数名类型必填说明typestring是监听事件，固定为'densityUpdate'，即屏幕像素密度变化。context[UIContext](Class (UIContext).md)是上下文信息，用以指定监听页面的范围。callbackCallback<[DensityInfo](#ZH-CN_TOPIC_0000002529444737__densityinfo12)>是回调函数。携带densityInfo，返回变化后的屏幕像素密度。
...
参数名类型必填说明typestring是监听事件，固定为'densityUpdate'，即屏幕像素密度变化。context[UIContext](Class (UIContext).md)是上下文信息，用以指定监听页面的范围。callbackCallback<[DensityInfo](#ZH-CN_TOPIC_0000002529444737__densityinfo12)>否需要被注销的回调函数。若不指定具体的回调函数，则注销指定UIContext下所有densityUpdate事件监听。
...
参数名类型必填说明typestring是监听事件，固定为'willDraw'，即是否将要绘制。context[UIContext](Class (UIContext).md)是上下文信息，用以指定监听页面的范围。callbackCallback<void>是回调函数。
...
参数名类型必填说明typestring是监听事件，固定为'willDraw'，即是否将要绘制。context[UIContext](Class (UIContext).md)是上下文信息，用以指定监听页面的范围。callbackCallback<void>否需要被注销的回调函数。
...
参数名类型必填说明typestring是监听事件，固定为'didLayout'，即是否布局完成。context[UIContext](Class (UIContext).md)是上下文信息，用以指定监听页面的范围。callbackCallback<void>是回调函数。
...
参数名类型必填说明typestring是监听事件，固定为'didLayout'，即是否布局完成。context[UIContext](Class (UIContext).md)是上下文信息，用以指定监听页面的范围。callbackCallback<void>否需要被注销的回调函数。
...
参数名类型必填说明typestring是监听事件，固定为'navDestinationSwitch'，即Navigation的页面切换事件。context[UIAbilityContext](UIAbilityContext.md) | [UIContext](Class (UIContext).md)是上下文信息，用以指定监听页面切换事件的范围。callbackCallback<[NavDestinationSwitchInfo](#ZH-CN_TOPIC_0000002529444737__navdestinationswitchinfo12)>是回调函数。携带NavDestinationSwitchInfo，返回页面切换事件的信息。
...
参数名类型必填说明typestring是监听事件，固定为'navDestinationSwitch'，即Navigation的页面切换事件。context[UIAbilityContext](UIAbilityContext.md) | [UIContext](Class (UIContext).md)是上下文信息，用以指定监听页面切换事件的范围。callbackCallback<[NavDestinationSwitchInfo](#ZH-CN_TOPIC_0000002529444737__navdestinationswitchinfo12)>否需要被注销的回调函数。
...
监听TabContent页面的切换事件。相比[on('tabChange')](Class (UIObserver).md#ZH-CN_TOPIC_0000002497444806__ontabchange22)，本接口不支持监听Tabs组件初始化时，显示首个页签的事件。
...
监听指定Tabs组件id的TabContent页面切换事件。相比[on('tabChange')](Class (UIObserver).md#ZH-CN_TOPIC_0000002497444806__ontabchange22)，本接口不支持监听Tabs组件初始化时，显示首个页签的事件。
