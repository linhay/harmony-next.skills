...
参数名类型必填说明colorsArrayBuffer是默认按照BGRA_8888格式处理的颜色数组。options[image.InitializationOptions](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__initializationoptions8)是创建像素的属性，包括透明度，尺寸，缩略值，像素格式和是否可编辑。
...
根据Surface ID和区域信息创建一个PixelMap对象。该区域的大小由[Region](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__region8).size指定。使用Promise异步回调。
...
参数名类型必填说明colorsArrayBuffer是BGRA_8888格式的颜色数组。options[image.InitializationOptions](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__initializationoptions8)是创建像素的属性，包括透明度，尺寸，缩略值，像素格式和是否可编辑。
...
参数名类型必填说明pixelmap[image.PixelMap](Interface (PixelMap).md)是image下的非sendable的PixelMap。
...
以下错误码的详细介绍请参见[通用错误码说明文档](通用错误码.md)和[Image错误码](Image错误码.md)。
...
由于图片占用内存较大，所以当PixelMap对象使用完成后，应主动调用[release](Interface (PixelMap).md#ZH-CN_TOPIC_0000002497605846__release7)方法及时释放内存。释放时应确保该对象的所有异步方法均执行完成，且后续不再使用该对象。
...
类型说明[image.PixelMap](Interface (PixelMap).md)成功同步返回image下的非sendable的PixelMap对象，失败抛出异常。
...
参数名类型必填说明area[image.PositionArea](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__positionarea7)是区域大小，根据区域读取。
...
参数名类型必填说明area[image.PositionArea](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__positionarea7)是区域，根据区域写入。
...
类型说明Promise<[image.ImageInfo](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__imageinfo)>Promise实例，用于异步获取图像像素信息，失败时返回错误信息。
...
类型说明[image.ImageInfo](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__imageinfo)图像像素信息。
...
参数名类型必填说明region[image.Region](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__region8)是裁剪的尺寸。
...
当前支持格式有：.jpg .png .gif .bmp .webp .dng [SVG](Functions.md#ZH-CN_TOPIC_0000002529445805__svg标签说明) .ico。
...
参数名类型必填说明size[image.Size](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__size)是图像的默认大小。format[image.ImageFormat](Enums.md#ZH-CN_TOPIC_0000002529285837__imageformat9)是图像格式，取值为image.ImageFormat常量，目前仅支持 ImageFormat:JPEG。capacitynumber是同时访问的最大图像数。该参数仅作为期望值，实际capacity由设备硬件决定。
...
参数名类型必填说明options[image.DecodingOptions](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__decodingoptions7)否解码参数。
...
参数名类型必填说明componentType[image.ComponentType](Enums.md#ZH-CN_TOPIC_0000002529285837__componenttype9)是图像的组件类型。

**返回值：**

类型说明Promise<[image.Component](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__component9)>Promise实例，用于异步返回组件缓冲区。
...
名称类型只读可选说明size[image.Size](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__size)是否图片大小。capacitynumber是否同时访问的图像数。该参数仅作为期望值，实际capacity由设备硬件决定。format[image.ImageFormat](Enums.md#ZH-CN_TOPIC_0000002529285837__imageformat9)是否图像格式。
