...
名称类型只读可选说明albumType[AlbumType](Enums.md#ZH-CN_TOPIC_0000002529445919__albumtype)是否相册类型。albumSubtype[AlbumSubtype](Enums.md#ZH-CN_TOPIC_0000002529445919__albumsubtype)是否相册子类型。albumNamestring否否相册名称。预置相册不可写，用户相册可写。albumUristring是否相册uri。countnumber是否相册中文件数量。coverUristring是否封面文件uri。
