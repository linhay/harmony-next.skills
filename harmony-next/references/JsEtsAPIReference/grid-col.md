...
spannumber1否在未设置明确断点时，默认占用列数。offsetnumber0否未设置具体分辨率模式下偏移时，当前元素沿容器布局方向，默认偏移的列数。
...
grid-[columns|rows]-gap<length>0否用于设置行与行的间距或者列与列的间距，也可以支持通过grid-gap设置相同的行列间距，仅当display为grid时生效。grid-row-[start|end]number-否用于设置当前元素在网格布局中的起止行号，仅当父组件display样式为grid时生效（仅div支持display样式设置为grid）。grid-column-[start|end]number-否用于设置当前元素在网格布局中的起止列号，仅当父组件display样式为grid时生效（仅div支持display样式设置为grid）。
