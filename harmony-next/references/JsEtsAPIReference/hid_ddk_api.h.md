...
参数项描述[Hid_Device](Hid_Device.md) *hidDevice创建设备需要的基本信息，包括设备名、厂商ID、产品ID等。[Hid_EventProperties](Hid_EventProperties.md) *hidEventProperties创建的设备关注的事件，包括事件类型、按键事件属性、绝对坐标事件属性、相对坐标事件属性等。
...
参数项描述uint64_t deviceId操作设备的ID。uint8_t interfaceIndex接口索引，对应HID设备的接口。[Hid_DeviceHandle](Hid_DeviceHandle.md) **dev设备操作句柄。
...
参数项描述[Hid_DeviceHandle](Hid_DeviceHandle.md) **dev设备操作句柄。
...
参数项描述[Hid_DeviceHandle](Hid_DeviceHandle.md) *dev设备操作句柄。uint8_t *data待写入的数据。uint32_t length写入数据的字节长度，最大不超过[HID_MAX_REPORT_BUFFER_SIZE](hid_ddk_types.h.md#ZH-CN_TOPIC_0000002529445575__hid_max_report_buffer_size)。uint32_t *bytesWritten实际写入的数据字节数。
...
参数项描述[Hid_DeviceHandle](Hid_DeviceHandle.md) *dev设备操作句柄。uint8_t *data存放读取数据的缓冲区。uint32_t bufSize存放读取数据的缓冲区大小，最大不超过[HID_MAX_REPORT_BUFFER_SIZE](hid_ddk_types.h.md#ZH-CN_TOPIC_0000002529445575__hid_max_report_buffer_size)。int timeout超时时间（毫秒）或-1表示阻塞等待。uint32_t *bytesRead读取的字节数。
...
参数项描述[Hid_DeviceHandle](Hid_DeviceHandle.md) *dev设备操作句柄。uint8_t *data存放读取数据的缓冲区。uint32_t bufSize存放读取数据的缓冲区大小，最大不超过[HID_MAX_REPORT_BUFFER_SIZE](hid_ddk_types.h.md#ZH-CN_TOPIC_0000002529445575__hid_max_report_buffer_size)。uint32_t *bytesRead读取的字节数。
...
参数项描述[Hid_DeviceHandle](Hid_DeviceHandle.md) *dev设备操作句柄。int nonBlock
...
参数项描述[Hid_DeviceHandle](Hid_DeviceHandle.md) *dev设备操作句柄。[Hid_RawDevInfo](Hid_RawDevInfo.md) *rawDevInfo设备原始信息，包含供应商ID、产品ID和总线类型。
...
参数项描述[Hid_DeviceHandle](Hid_DeviceHandle.md) *dev设备操作句柄。char *data存放读取数据的缓冲区。uint32_t bufSize存放读取数据的缓冲区大小，最大不超过[HID_MAX_REPORT_BUFFER_SIZE](hid_ddk_types.h.md#ZH-CN_TOPIC_0000002529445575__hid_max_report_buffer_size)。
...
参数项描述[Hid_DeviceHandle](Hid_DeviceHandle.md) *dev设备操作句柄。uint8_t *data存放读取数据的缓冲区。uint32_t bufSize存放读取数据的缓冲区大小，最大不超过[HID_MAX_REPORT_BUFFER_SIZE](hid_ddk_types.h.md#ZH-CN_TOPIC_0000002529445575__hid_max_report_buffer_size)。
...
参数项描述[Hid_DeviceHandle](Hid_DeviceHandle.md) *dev设备操作句柄。uint8_t *buf存放描述符的缓冲区。uint32_t bufSize缓冲区的字节大小，最大不超过[HID_MAX_REPORT_BUFFER_SIZE](hid_ddk_types.h.md#ZH-CN_TOPIC_0000002529445575__hid_max_report_buffer_size)。uint32_t *bytesRead读取的字节数。
