...
参数名类型必填说明secureDnsMode[SecureDnsMode](Enums.md#ZH-CN_TOPIC_0000002529445167__securednsmode10)是使用HTTPDNS的模式。secureDnsConfigstring是HTTPDNS server的配置，必须是https协议并且只允许配置一个server。
...
参数名类型必填说明urlstring | Resource是需要加载的 URL。headersArray<[WebHeader](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__webheader)>否
...
调整当前网页的缩放比例，[zoomAccess](属性.md#ZH-CN_TOPIC_0000002497605206__zoomaccess)需为true。
...
异步查找网页中所有匹配关键字'searchString'的内容并高亮，结果通过[onSearchResultReceive](事件.md#ZH-CN_TOPIC_0000002497445228__onsearchresultreceive9)异步返回。
...
类型说明Array<[WebMessagePort](Interface (WebMessagePort).md)>web消息端口列表。
...
完整示例代码参考[onMessageEventExt](Interface (WebMessagePort).md#ZH-CN_TOPIC_0000002497445222__onmessageeventext10)。
...
参数名类型必填说明namestring是要发送的消息名称。portsArray<[WebMessagePort](Interface (WebMessagePort).md)>是要发送的消息端口。uristring是接收该消息的URI。
...
类型说明image.[PixelMap](Interface (PixelMap).md)页面favicon图标的PixelMap对象。
...
[onLoadIntercept](事件.md#ZH-CN_TOPIC_0000002497445228__onloadintercept10)在加载开始的时候触发，该时刻还未生成历史节点，所以在onLoadIntercept中调用getBackForwardEntries拿到的历史栈不包括当前正在加载中的跳转。

**系统能力：** SystemCapability.Web.Webview.Core

**返回值：**

类型说明[BackForwardList](Interface (BackForwardList).md)当前Webview的历史信息列表。
...
1.对文件的操作需要导入文件管理模块，详情请参考[文件管理](@ohos.file.fs (文件管理).md)。
...
对Web内核赋予自定义协议url的跨域请求与fetch请求的权限。当Web在跨域fetch自定义协议url时，该fetch请求可被[onInterceptRequest](事件.md#ZH-CN_TOPIC_0000002497445228__oninterceptrequest9)事件接口所拦截，从而开发者可以进一步处理该请求。建议在任何Web组件初始化之前调用该接口。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明schemesArray<[WebCustomScheme](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__webcustomscheme)>是自定义协议配置，最多支持同时配置10个自定义协议。
...
参数名类型必填说明schemesArray<[WebCustomScheme](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__webcustomscheme)>是自定义协议配置，最多支持同时配置10个自定义协议。lazyInitWebEngineboolean是
...
获取当前网站的证书信息。使用Web组件加载https网站，会进行SSL证书校验，该接口会通过Promise异步返回当前网站的X509格式证书（X509Cert证书类型定义见[X509Cert](@ohos.security.cert (证书模块).md#ZH-CN_TOPIC_0000002529285375__x509cert)定义），便于开发者展示网站证书信息。
...
获取当前网站的证书信息。使用Web组件加载https网站，会进行SSL证书校验，该接口会通过AsyncCallback异步返回当前网站的X509格式证书（X509Cert证书类型定义见[X509Cert定义](@ohos.security.cert (证书模块).md)），便于开发者展示网站证书信息。
...
参数名类型必填说明urlstring是预加载的url。additionalHeadersArray<[WebHeader](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__webheader)>否
...
参数名类型必填说明request[RequestInfo](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__requestinfo12)是预获取请求的信息。additionalHeadersArray<[WebHeader](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__webheader)>否预获取请求的附加HTTP请求头。cacheKeystring否用于后续查询预获取资源缓存的key。仅支持字母和数字，未传入或传入空则取默认值url作为key。cacheValidTimenumber否
...
参数名类型必填说明mode[RenderProcessMode](Enums.md#ZH-CN_TOPIC_0000002529445167__renderprocessmode12)是
...
类型说明[RenderProcessMode](Enums.md#ZH-CN_TOPIC_0000002529445167__renderprocessmode12)
...
类型说明print.[PrintDocumentAdapter](@ohos.print (打印).md#ZH-CN_TOPIC_0000002529285499__printdocumentadapter11)返回打印文档的适配器。
...
类型说明[SecurityLevel](Enums.md#ZH-CN_TOPIC_0000002529445167__securitylevel11)当前网页的安全级别，具体值为NONE、SECURE、WARNING、DANGEROUS。
...
type[ScrollType](Enums.md#ZH-CN_TOPIC_0000002529445167__scrolltype12)否
...
通过[registerJavaScriptProxy](#ZH-CN_TOPIC_0000002497605192__registerjavascriptproxy)或者[javaScriptProxy](zh-cn_topic_0000002497605206.html#ZH-CN_TOPIC_0000002497605206__javascriptproxy)注入JavaScript对象到window对象中。该接口可以获取最后一次调用注入的对象的frame的url。
...
类型说明[MediaPlaybackState](Enums.md#ZH-CN_TOPIC_0000002529445167__mediaplaybackstate12)当前网页的播放状态，具体值为NONE、PLAYING、PAUSED、STOPPED。
...
参数名类型必填说明urlstring是本地JavaScript文件对应的网络地址，即业务网页请求该文件的服务器版本时使用的网络地址。网络地址仅支持http或https协议，长度不超过2048。如果该网络地址对应的缓存失效，则业务网页将通过网络请求对应的资源。scriptstring | Uint8Array是本地JavaScript的文本内容。内容不能为空。cacheOptions[CacheOptions](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__cacheoptions12)是用于控制字节码缓存更新。
...
JavaScript资源的获取方式也可通过[网络请求](@ohos.net.http (数据请求).md)的方式获取，但此方法获取到的http响应头非标准HTTP响应头格式，需额外将响应头转换成标准HTTP响应头格式后使用。如通过网络请求获取到的响应头是e-tag，则需要将其转换成E-Tag后使用。
...
注册回调函数，开启[应用接管网页媒体播放功能](属性.md#ZH-CN_TOPIC_0000002497605206__enablenativemediaplayer12)后，当网页中有播放媒体时，触发注册的回调函数。
...
参数名类型必填说明callback[CreateNativeMediaPlayerCallback](Types.md#ZH-CN_TOPIC_0000002497605202__createnativemediaplayercallback12)是接管网页媒体播放的回调函数。
...
参数名类型必填说明info[SnapshotInfo](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__snapshotinfo12)是全量绘制结果入参。callbackAsyncCallback<[SnapshotResult](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__snapshotresult12)>是全量绘制回调结果。
...
injectOfflineResources(resourceMaps: Array<[OfflineResourceMap](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__offlineresourcemap12)>): void
...
参数名类型必填说明resourceMapsArray<[OfflineResourceMap](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__offlineresourcemap12)>是本地离线资源配置对象，单次调用最大支持注入30个资源，单个资源最大支持10Mb。
...
设置一个路径列表，当file协议访问该路径列表中的资源时，允许跨域访问本地文件，也允许跨域访问其他在线资源。此外，当设置了路径列表时，file协议仅允许访问路径列表中的资源（[fileAccess](属性.md#ZH-CN_TOPIC_0000002497605206__fileaccess)的行为将会被此接口行为覆盖）。路径列表中的路径必须满足以下路径格式之一：

1.应用文件目录的子目录（应用文件目录通过Ability Kit中的[Context.filesDir](Context (Stage模型的上下文基类).md#ZH-CN_TOPIC_0000002529444589__context)获取），例如：

- /data/storage/el2/base/files/example
- /data/storage/el2/base/haps/entry/files/example

2.应用资源目录及其子目录（应用资源目录通过Ability Kit中的[Context.resourceDir](Context (Stage模型的上下文基类).md#ZH-CN_TOPIC_0000002529444589__context)获取），例如：

- /data/storage/el1/bundle/entry/resource/resfile
- /data/storage/el1/bundle/entry/resource/resfile/example

3.从API version 21开始，还包括了应用缓存目录及其子目录（应用缓存目录通过Ability Kit中的[Context.cacheDir](Context (Stage模型的上下文基类).md#ZH-CN_TOPIC_0000002529444589__context)获取），例如：

- /data/storage/el2/base/cache
- /data/storage/el2/base/haps/entry/cache/example
- 设置的目录路径中，不允许包含cache/web，否则会抛出异常码401。如果设置目录路径是cache，cache/web也不允许访问。

4.从API version 21开始，还包括了应用临时目录及其子目录（应用临时目录通过Ability Kit中的[Context.tempDir](Context (Stage模型的上下文基类).md#ZH-CN_TOPIC_0000002529444589__context)获取），例如：

- /data/storage/el2/base/temp
- /data/storage/el2/base/haps/entry/temp/example

当路径列表中有其中一个路径不满足以上条件之一，则会抛出异常码401，并且设置路径列表失败。当设置的路径列表为空，则file协议可访问范围以[fileAccess](属性.md#ZH-CN_TOPIC_0000002497605206__fileaccess)的行为为准。
...
参数名类型必填说明level[PressureLevel](Enums.md#ZH-CN_TOPIC_0000002529445167__pressurelevel14)是需要清理内存的内存等级。
...
参数名类型必填说明configuration[PdfConfiguration](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__pdfconfiguration14)是生成PDF所需参数。callbackAsyncCallback<[PdfData](Class (PdfData).md)>是回调返回网页PDF数据流。
...
参数名类型必填说明configuration[PdfConfiguration](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__pdfconfiguration14)是生成PDF所需参数。
...
类型说明[ScrollOffset](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__scrolloffset13)网页当前的滚动偏移量（包含过滚动偏移量）。
...
类型说明[ScrollOffset](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__scrolloffset13)网页当前的滚动偏移量（不包含过滚动偏移量）。
...
类型说明[HitTestValue](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__hittestvalue)点击区域的元素信息。
...
类型说明[ControllerAttachState](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__controllerattachstate20)WebViewController与Web组件的绑定状态。
...
参数名类型必填说明typestring是表示注册WebViewController绑定状态事件，固定为"controllerAttachStateChange"。callbackCallback<[ControllerAttachState](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__controllerattachstate20)>是WebViewController绑定状态改变时的回调函数。
...
参数名类型必填说明typestring是表示注册WebViewController绑定状态事件，固定为"controllerAttachStateChange"。callbackCallback<[ControllerAttachState](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__controllerattachstate20)>否WebViewController绑定状态发生改变时的回调函数，默认情况下不填写回调函数。如果填写了Callback，将仅取消注册该特定的回调。如果不填写Callback，将取消注册所有回调。
...
异步等待WebViewController与Web组件绑定完成，绑定完成或超时触发回调，通过Promise方式返回当前[ControllerAttachState](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__controllerattachstate20)状态。
...
类型说明Promise<[ControllerAttachState](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__controllerattachstate20)>Promise实例，返回当前[ControllerAttachState](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__controllerattachstate20)状态。
...
类型说明[WebHitTestType](Enums.md#ZH-CN_TOPIC_0000002529445167__webhittesttype)被点击区域的元素类型。
...
- 该接口高度设置为0时，Web网页内容可恢复，键盘弹起避让模式将使用[keyboardAvoidMode()](属性.md#ZH-CN_TOPIC_0000002497605206__keyboardavoidmode12)声明的模式。
...
在当前接口设置为true时如果页面加载发生错误将触发[onOverrideErrorPage](事件.md#ZH-CN_TOPIC_0000002497445228__onoverrideerrorpage20)回调，可在该回调接口中设置自定义的错误展示页面。
...
获取页面首屏加载预测信息（详细说明见[BlanklessInfo](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__blanklessinfo20)），并开始本次加载过渡帧生成，应用根据此信息确定是否需要启用无白屏加载。必须与[setBlanklessLoadingWithKey](#ZH-CN_TOPIC_0000002497605192__setblanklessloadingwithkey20)接口配套使用，并且必须在触发加载页面的接口之前或在onLoadIntercept中调用。需在WebViewController与Web组件绑定后才能使用。
...
- 如果发现快照相似度（即[BlanklessInfo](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__blanklessinfo20)中的similarity）极低，请确认key值是否传递正确。
...
类型说明[BlanklessInfo](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__blanklessinfo20)页面首屏加载预测信息，主要包括首屏相似度预测值，首屏加载耗时预测值，应用需根据此信息来决策是否启用无白屏加载插帧。
...
类型说明[WebBlanklessErrorCode](Enums.md#ZH-CN_TOPIC_0000002529445167__webblanklesserrorcode20)返回接口调用是否成功，具体见[WebBlanklessErrorCode](Enums.md#ZH-CN_TOPIC_0000002529445167__webblanklesserrorcode20)定义。
...
设置Web组件的销毁模式。当Web组件销毁时，销毁模式会影响Web内核资源释放的时机，例如JavaScript运行上下文、渲染上下文等。默认值：[WebDestroyMode.NORMAL_MODE](Enums.md#ZH-CN_TOPIC_0000002529445167__webdestroymode20)（普通模式），由系统决定销毁时机。应用可设置[WebDestroyMode.FAST_MODE](Enums.md#ZH-CN_TOPIC_0000002529445167__webdestroymode20)（快速模式），以立即销毁资源，从而提升特定场景的性能。

[WebDestroyMode.FAST_MODE](Enums.md#ZH-CN_TOPIC_0000002529445167__webdestroymode20)（快速模式）会改变Web组件销毁时机，应用需关注依赖Web组件销毁时机的错误实现，例如：Web组件销毁后仍调用WebviewController的未定义行为，与[WebDestroyMode.NORMAL_MODE](Enums.md#ZH-CN_TOPIC_0000002529445167__webdestroymode20)（普通模式）相比，销毁时机提前，有更高的几率触发未关联绑定的异常（17100001），建议应用捕捉异常，或者通过[getAttachState](#ZH-CN_TOPIC_0000002497605192__getattachstate20)方法查询是否绑定状态，来避免稳定性问题。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明mode[WebDestroyMode](Enums.md#ZH-CN_TOPIC_0000002529445167__webdestroymode20)是
...
在Web页面场景，设置全局滚动条模式。不显式调用时，默认为[ScrollbarMode.OVERLAY_LAYOUT_SCROLLBAR ](Enums.md#ZH-CN_TOPIC_0000002529445167__scrollbarmode23)（非常驻滚动条）。

- 根据滚动条模式，改变当前应用所有web滚动条模式为常驻滚动条或非常驻滚动条。
- 若[forceDisplayScrollBar](属性.md#ZH-CN_TOPIC_0000002497605206__forcedisplayscrollbar14)接口与当前接口同时设置，forceDisplayScrollBar接口设置不生效。
...
参数名类型必填说明scrollbarMode[ScrollbarMode](Enums.md#ZH-CN_TOPIC_0000002529445167__scrollbarmode23)是滚动条模式。
...
参数名类型必填说明engineVersion[ArkWebEngineVersion](Enums.md#ZH-CN_TOPIC_0000002529445167__arkwebengineversion20)是ArkWeb内核版本。
...
类型说明[ArkWebEngineVersion](Enums.md#ZH-CN_TOPIC_0000002529445167__arkwebengineversion20)返回由[ArkWebEngineVersion](Enums.md#ZH-CN_TOPIC_0000002529445167__arkwebengineversion20)所定义的当前使用的ArkWeb内核版本。
...
类型说明[SiteIsolationMode](Enums.md#ZH-CN_TOPIC_0000002529445167__siteisolationmode21)
...
对于仅加载可信网页的第三方应用，可以关闭此功能，以提升性能并减少内存占用，同时减少跨域访问的拦截。默认值根据不同的设备而定，PC/Table采用严格站点隔离[SiteIsolationMode.STRICT](Enums.md#ZH-CN_TOPIC_0000002529445167__siteisolationmode21)，Phone默认部分站点隔离[SiteIsolationMode.PARTIAL](Enums.md#ZH-CN_TOPIC_0000002529445167__siteisolationmode21)。[坚盾守护模式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/web-secure-shield-mode)下采用严格站点隔离。
...
参数名类型必填说明mode[SiteIsolationMode](Enums.md#ZH-CN_TOPIC_0000002529445167__siteisolationmode21)是
...
参数名类型必填说明mode[WebSoftKeyboardBehaviorMode](Enums.md#ZH-CN_TOPIC_0000002529445167__websoftkeyboardbehaviormode22)是Web软键盘自动控制模式。
