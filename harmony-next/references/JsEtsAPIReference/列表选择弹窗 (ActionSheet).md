...
本模块功能依赖UI的执行上下文，不可在[UI上下文不明确](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-global-interface)的地方使用，参见[UIContext](zh-cn_topic_0000002529444749.md)说明。
...
levelOrder18+[LevelOrder](@ohos.promptAction (弹窗).md#ZH-CN_TOPIC_0000002529284783__levelorder18)否是
...
类型说明[LevelMode](@ohos.promptAction (弹窗).md#ZH-CN_TOPIC_0000002529284783__levelmode15枚举说明)设置弹窗的显示层级。
...
类型说明[ImmersiveMode](@ohos.promptAction (弹窗).md#ZH-CN_TOPIC_0000002529284783__immersivemode15枚举说明)设置页面内弹窗的蒙层效果。
...
style10+[DialogButtonStyle](枚举说明.md#ZH-CN_TOPIC_0000002529284967__dialogbuttonstyle10)否是
...
从API version 8开始支持，从API version 18开始废弃，建议使用[showActionSheet](Class (UIContext).md#ZH-CN_TOPIC_0000002529444749__showactionsheet)替代。showActionSheet需先获取[UIContext](Class (UIContext).md)实例后再进行调用。

从API version 10开始，可以通过使用[UIContext](Class (UIContext).md)中的[showActionSheet](Class (UIContext).md#ZH-CN_TOPIC_0000002529444749__showactionsheet)来明确UI的执行上下文。
...
直接使用ActionSheet可能导致实例不明确的问题，建议使用getUIContext()获取[UIContext](Class (UIContext).md)实例，并使用[showActionSheet](Class (UIContext).md#ZH-CN_TOPIC_0000002529444749__showactionsheet)调用绑定实例的ActionSheet.show()。
