...
通过[获取用户级凭证](获取用户级凭证.md)获取的Access Token。获取凭证时Authorization Code需包含profile scope的授权。
...
用户昵称，该字段返回场景详见[获取头像昵称](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-get-avatar-nickname)。

- 请求参数“getNickName”为0或不传时，返回匿名化账号。
- 请求参数“getNickName”为1时，返回昵称，没有昵称时返回匿名化账号。  说明：

调用成功后响应中若不包含用户昵称displayName，请确认获取的Authorization Code是否携带了profile（昵称和头像）scope。参考[客户端开发](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-get-avatar-nickname#section154801110677)
...
调用成功后响应中若不包含昵称类型displayNameFlag，请确认获取的Authorization Code是否携带了profile（昵称和头像）scope。参考[客户端开发](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-get-avatar-nickname#section154801110677)
...
用户头像，该字段返回场景详见[获取头像昵称](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-get-avatar-nickname)。用户未设置头像时不返回。
...
- 未对access_token进行URLEncode处理，可参考[示例代码](#section1888515151299)组装参数。
