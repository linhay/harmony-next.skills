...
参数项描述fiber指向要初始化的纤程的指针, 具体可参考[ffrt_fiber_t](ffrt_fiber_t.md)。func纤程切换后所要执行的方法。void* arg纤程切换后所要执行方法的入参。void* stack纤程堆栈内存指针。size_t stack_size纤程堆栈大小, 具体可参考[ffrt_storage_size_t](type_def.h.md#ZH-CN_TOPIC_0000002497605556__ffrt_storage_size_t)。
...
参数项描述[ffrt_fiber_t](ffrt_fiber_t.md)* from将要保存的纤程指针。[ffrt_fiber_t](ffrt_fiber_t.md)* to将要恢复的纤程指针。