...
AppServiceExtensionContext模块是[AppServiceExtensionAbility](@ohos.app.ability.AppServiceExtensionAbility (应用后台服务扩展组件).md)的上下文环境，继承自[ExtensionContext](ExtensionContext.md)。
...
参数名类型必填说明want[Want](@ohos.app.ability.Want (Want).md)是Want类型参数，传入需要启动的Ability的信息，如Ability名称、Bundle名称等。options[StartOptions](@ohos.app.ability.StartOptions (startAbility的可选参数).md)否启动Ability所携带的参数。
...
参数名类型必填说明want[Want](@ohos.app.ability.Want (Want).md)是Want类型参数，传入需要连接的Ability的信息，如Ability名称，Bundle名称等。callback[ConnectOptions](ConnectOptions.md)是ConnectOptions类型的回调函数，返回服务连接成功、连接失败、断开的信息。
