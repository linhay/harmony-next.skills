...
MediaAssetChangeRequest implements [MediaChangeRequest](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285945__mediachangerequest11).
...
参数名类型必填说明asset[PhotoAsset](Interface (PhotoAsset).md)是需要变更的资产。
...
指定待创建资产的数据来源，可参考[FileUri](@ohos.file.fileuri (文件URI).md)。
...
参数名类型必填说明context[Context](Context (Stage模型的上下文基类).md)是传入Ability实例的上下文。fileUristring是图片资产的数据来源，在应用沙箱下的uri。示例fileUri：'file://com.example.temptest/data/storage/el2/base/haps/entry/files/test.jpg'。
...
参数名类型必填说明context[Context](Context (Stage模型的上下文基类).md)是传入Ability实例的上下文。fileUristring是视频资产的数据来源，在应用沙箱下的uri。示例fileUri：'file://com.example.temptest/data/storage/el2/base/haps/entry/files/test.mp4'。
...
参数名类型必填说明context[Context](Context (Stage模型的上下文基类).md)是传入Ability实例的上下文。photoType[PhotoType](Enums.md#ZH-CN_TOPIC_0000002529445919__phototype)是待创建的文件类型，IMAGE或者VIDEO类型。extensionstring是文件扩展名，例如：'jpg'。options[CreateOptions](Interfaces (其他).md#ZH-CN_TOPIC_0000002529285945__createoptions)否
...
参数名类型必填说明context[Context](Context (Stage模型的上下文基类).md)是传入Ability实例的上下文。assetsArray<[PhotoAsset](Interface (PhotoAsset).md)>是待删除的媒体文件数组，数组中元素个数不超过300个。
...
参数名类型必填说明context[Context](Context (Stage模型的上下文基类).md)是传入Ability实例的上下文。uriListArray<string>是待删除的媒体文件uri数组，数组中元素个数不超过300个。
...
对于创建资产的变更请求，在调用接口[applyChanges](Interface (PhotoAccessHelper).md#ZH-CN_TOPIC_0000002529445917__applychanges11)的提交生效之前，该接口会返回null。
...
类型说明[PhotoAsset](Interface (PhotoAsset).md)返回当前资产变更请求中的资产。
...
通过[fileUri](@ohos.file.fileuri (文件URI).md)从应用沙箱添加资源。
...
参数名类型必填说明type[ResourceType](Enums.md#ZH-CN_TOPIC_0000002529445919__resourcetype11)是待添加资源的类型。fileUristring是待添加资源的数据来源，在应用沙箱下的uri。示例fileUri：'file://com.example.temptest/data/storage/el2/base/haps/entry/files/test.jpg'。
...
参数名类型必填说明type[ResourceType](Enums.md#ZH-CN_TOPIC_0000002529445919__resourcetype11)是待添加资源的类型。dataArrayBuffer是待添加资源的数据。
...
参数名类型必填说明imageFileType[ImageFileType](Enums.md#ZH-CN_TOPIC_0000002529445919__imagefiletype13)是需要保存的类型。
