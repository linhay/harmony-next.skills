...
参数名类型必填说明context[UIAbilityContext](UIAbilityContext.md)是需要进入kiosk模式的UIAbility的上下文。
...
参数名类型必填说明context[UIAbilityContext](UIAbilityContext.md)是需要退出kiosk模式的UIAbility的上下文。
...
类型说明[_KioskStatus](KioskStatus (Kiosk状态信息).md#ZH-CN_TOPIC_0000002497604626__kioskstatus)表示Kiosk状态信息，包括系统是否处于Kiosk模式以及该模式下的应用信息。