...
类型说明[NavDestinationInfo](@ohos.arkui.observer (无感监听).md#ZH-CN_TOPIC_0000002529444737__navdestinationinfo)返回NavDestinationInfo实例对象。
...
类型说明[NavigationInfo](@ohos.arkui.observer (无感监听).md#ZH-CN_TOPIC_0000002529444737__navigationinfo12)返回NavigationInfo实例对象。
...
类型说明[RouterPageInfo](@ohos.arkui.observer (无感监听).md#ZH-CN_TOPIC_0000002529444737__routerpageinfo)返回RouterPageInfo实例对象。
...
自定义弹窗控制器，可以控制当前自定义弹窗，具体控制能力包括关闭弹窗等，详见[promptAction.DialogController](@ohos.promptAction (弹窗).md#ZH-CN_TOPIC_0000002529284783__dialogcontroller18)。

**元服务API：** 从API version 18开始，该接口支持在元服务中使用。

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

类型说明[promptAction.DialogController](@ohos.promptAction (弹窗).md#ZH-CN_TOPIC_0000002529284783__dialogcontroller18)表示对象类型为promptAction.DialogController实例对象。