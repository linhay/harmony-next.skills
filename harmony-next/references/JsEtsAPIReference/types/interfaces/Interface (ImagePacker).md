...
在调用ImagePacker的方法前，需要先通过[image.createImagePacker](Functions.md#ZH-CN_TOPIC_0000002522241966__imagecreateimagepacker)构建一个ImagePacker实例。

编码期间，请避免修改或释放作为输入的ImageSource/PixelMap/Picture对象，以免出现crash或其他未定义行为。

由于图片占用内存较大，所以当ImagePacker实例使用完成后，应主动调用[release](#ZH-CN_TOPIC_0000002522241968__release)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。
...
从API version 6开始支持，从API version 13开始废弃，建议使用[packToData](#ZH-CN_TOPIC_0000002522241968__packtodata13)代替。
...
从API version 8开始支持，从API version 13开始废弃，建议使用[packToData](#ZH-CN_TOPIC_0000002522241968__packtodata13)代替。
