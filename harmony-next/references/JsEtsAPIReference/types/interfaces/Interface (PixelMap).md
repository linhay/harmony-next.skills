...
图像像素类，用于读取或写入图像数据以及获取图像信息。在调用PixelMap的方法前，需要先通过[image.createPixelMap](../../topics/misc/Functions.md#ZH-CN_TOPIC_0000002529445805__imagecreatepixelmap8)创建一个PixelMap实例。目前pixelmap序列化大小最大128MB，超过会送显失败。大小计算方式为(宽*高*每像素占用字节数)。

从API version 11开始，PixelMap支持通过worker跨线程调用。当PixelMap通过[Worker](../../modules/ohos/@ohos.worker (启动一个Worker).md)跨线程后，原线程的PixelMap的所有接口均不能调用，否则将报错501 服务器不具备完成请求的功能。

在调用PixelMap的方法前，可以通过[image.createPixelMap](../../topics/misc/Functions.md#ZH-CN_TOPIC_0000002529445805__imagecreatepixelmap8)传入像素数据创建一个PixelMap对象，也可以通过[ImageSource](Interface (ImageSource).md)进行图片解码创建PixelMap对象。
...
参数名类型必填说明xnumber是宽度的缩放倍数。ynumber是高度的缩放倍数。level[AntiAliasingLevel](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529285837__antialiasinglevel12)是采用的缩放算法。
...
参数名类型必填说明xnumber是宽度的缩放倍数。ynumber是高度的缩放倍数。level[AntiAliasingLevel](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529285837__antialiasinglevel12)否采用的缩放算法。
...
参数名类型必填说明region[Region](Interfaces (其他).md#ZH-CN_TOPIC_0000002497445866__region8)是裁剪的区域。取值范围不能超过图片的宽和高（单位：像素）。xnumber是宽度的缩放倍数。不能为0。ynumber是高度的缩放倍数。不能为0。level[AntiAliasingLevel](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529285837__antialiasinglevel12)否采用的缩放算法。默认值是AntiAliasingLevel.NONE。
...
类型说明[colorSpaceManager.ColorSpaceManager](../../modules/ohos/@ohos.graphics.colorSpaceManager (色彩管理).md#ZH-CN_TOPIC_0000002497445992__colorspacemanager)图像广色域信息。
...
参数名类型必填说明colorSpace[colorSpaceManager.ColorSpaceManager](../../modules/ohos/@ohos.graphics.colorSpaceManager (色彩管理).md#ZH-CN_TOPIC_0000002497445992__colorspacemanager)是图像广色域信息。
...
参数名类型必填说明targetColorSpace[colorSpaceManager.ColorSpaceManager](../../modules/ohos/@ohos.graphics.colorSpaceManager (色彩管理).md#ZH-CN_TOPIC_0000002497445992__colorspacemanager)是目标色彩空间，支持SRGB、DCI_P3、DISPLAY_P3、ADOBE_RGB_1998。callbackAsyncCallback<void>是回调函数。当对图像像素颜色进行色彩空间转换成功，err为undefined，否则为错误对象。
...
参数名类型必填说明targetColorSpace[colorSpaceManager.ColorSpaceManager](../../modules/ohos/@ohos.graphics.colorSpaceManager (色彩管理).md#ZH-CN_TOPIC_0000002497445992__colorspacemanager)是目标色彩空间，支持SRGB、DCI_P3、DISPLAY_P3、ADOBE_RGB_1998。
...
参数名类型必填说明key[HdrMetadataKey](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529285837__hdrmetadatakey12)是HDR元数据的关键字，可用于查询对应值。

**返回值：**

类型说明[HdrMetadataValue](../../topics/misc/Types.md#ZH-CN_TOPIC_0000002529445811__hdrmetadatavalue12)返回元数据的值。
...
参数名类型必填说明key[HdrMetadataKey](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529285837__hdrmetadatakey12)是HDR元数据的关键字，用于设置对应值。value[HdrMetadataValue](../../topics/misc/Types.md#ZH-CN_TOPIC_0000002529445811__hdrmetadatavalue12)是元数据的值。
...
从MessageSequence中获取PixelMap，如需使用同步方式创建PixelMap可使用：[createPixelMapFromParcel](../../topics/misc/Functions.md#ZH-CN_TOPIC_0000002529445805__imagecreatepixelmapfromparcel11)。
...
参数名类型必填说明targetPixelFormat[PixelMapFormat](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529285837__pixelmapformat7)是目标像素格式，用于YUV和RGB类型互转，或者ASTC_4x4类型转为RGBA_8888类型。目前仅支持NV12/NV21与RGB888/RGBA8888/RGB565/BGRA8888/RGBAF16互转，YCRCB_P010/YCBCR_P010与RGBA1010102互转，ASTC_4x4转为RGBA_8888。
