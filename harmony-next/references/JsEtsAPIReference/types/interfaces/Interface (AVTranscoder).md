...
视频转码管理类，用于视频转码。在调用AVTranscoder的方法前，需要先通过[createAVTranscoder()](Functions.md#ZH-CN_TOPIC_0000002522082030__mediacreateavtranscoder12)构建一个AVTranscoder实例。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| fdSrc12+ | AVFileDescriptor | 否 | 否 | 源媒体文件描述，通过该属性设置数据源。 使用示例： 假设一个连续存储的媒体文件，地址偏移：0，字节长度：100。其文件描述为AVFileDescriptor{ fd = 资源句柄; offset = 0; length = 100; }。 说明： - 将资源句柄（fd）传递给AVTranscoder实例之后，请不要通过该资源句柄做其他读写操作，包括但不限于将同一个资源句柄传递给多个AVPlayer/AVMetadataExtractor/AVImageGenerator/AVTranscoder。同一时间通过同一个资源句柄读写文件时存在竞争关系，将导致视频转码数据获取异常。 元服务API： 从API version 22开始，该接口支持在元服务中使用。 |
| fdDst12+ | number | 否 | 否 | 目标媒体文件描述，通过该属性设置数据输出。在创建AVTranscoder实例后，必须设置fdSrc和fdDst属性。 说明： - 将资源句柄（fd）传递给AVTranscoder实例之后，请不要通过该资源句柄做其他读写操作，包括但不限于将同一个资源句柄传递给多个AVPlayer/AVMetadataExtractor/AVImageGenerator/AVTranscoder。同一时间通过同一个资源句柄读写文件时存在竞争关系，将导致视频转码数据获取异常。 元服务API： 从API version 22开始，该接口支持在元服务中使用。 |
...
需要[prepare()](#ZH-CN_TOPIC_0000002522242028__prepare12)事件成功触发后，才能调用start方法。
...
需要[start()](#ZH-CN_TOPIC_0000002522242028__start12)事件成功触发后，才能调用pause方法，可以通过调用[resume()](#ZH-CN_TOPIC_0000002522242028__resume12)接口来恢复转码。
...
需要在[pause()](#ZH-CN_TOPIC_0000002522242028__pause12)事件成功触发后，才能调用resume方法。
...
需要在[prepare()](#ZH-CN_TOPIC_0000002522242028__prepare12)、[start()](#ZH-CN_TOPIC_0000002522242028__start12)、[pause()](#ZH-CN_TOPIC_0000002522242028__pause12)或[resume()](#ZH-CN_TOPIC_0000002522242028__resume12)事件成功触发后，才能调用cancel方法。
...
注册AVtranscoder的错误事件，该事件仅用于错误提示。如果AVTranscoder上报error事件，开发者需要通过[release()](#ZH-CN_TOPIC_0000002522242028__release12)退出转码操作。使用callback异步回调。
...
当AVTranscoder上报complete事件时，当前转码操作已完成，开发者需要通过[release()](#ZH-CN_TOPIC_0000002522242028__release12)退出转码操作。
