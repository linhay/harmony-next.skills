...
以下API需先使用UIContext中的[getMeasureUtils()](Class (UIContext).md#ZH-CN_TOPIC_0000002529444749__getmeasureutils12)方法获取MeasureUtils实例，再通过此实例调用对应方法。

-

如需更多测算文本参数，建议使用图形对应测算接口[Paragraph](../../modules/ohos/@ohos.graphics.text (文本模块).md#ZH-CN_TOPIC_0000002497605988__paragraph)接口。

-

调用文本计算接口时，不推荐同时用[ApplicationContext.setFontSizeScale](../../topics/graphics/ApplicationContext (应用上下文).md#ZH-CN_TOPIC_0000002529284613__applicationcontextsetfontsizescale13)设置应用字体大小缩放比例。为了确保时序正确性，建议开发者自行监听字体缩放变化，以保证测算结果的准确性。

-

在测算裁剪后的文本时，由于某些Unicode字符（如emoji）的码位长度大于1，直接按字符串长度裁剪会导致不准确的结果。建议基于Unicode码点进行迭代处理，避免错误截断字符，确保测算结果准确，请参考[measureTextSize方法的示例2](#ZH-CN_TOPIC_0000002497604780__measuretextsize12)。
...
measureText接口的计算结果始终是单行文本的宽度，入参options中配置的布局约束（如constraintWidth、maxLines）对measureText的结果没有影响。如果需要计算布局约束下的宽度，请使用[measureTextSize](#ZH-CN_TOPIC_0000002497604780__measuretextsize12)方法。
...
参数名类型必填说明options[MeasureOptions](../../modules/ohos/@ohos.measure (文本计算).md#ZH-CN_TOPIC_0000002529444755__measureoptions)是被计算文本描述信息。
...
通过MeasureUtils的measureTextSize方法和unicode码点计算，手动实现文本截断。与设置[maxLines](../../topics/graphics/Text.md#ZH-CN_TOPIC_0000002497444914__maxlines)、[textOverflow](../../topics/graphics/Text.md#ZH-CN_TOPIC_0000002497444914__textoverflow)实现同样的效果。
...
将属性字符串根据文本布局选项转换成对应的[Paragraph](../../modules/ohos/@ohos.graphics.text (文本模块).md#ZH-CN_TOPIC_0000002497605988__paragraph)数组。

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

**参数：**

参数名类型必填说明styledString[StyledString](../../topics/misc/属性字符串.md#ZH-CN_TOPIC_0000002497444920__styledstring)是待转换的属性字符串。options[TextLayoutOptions](../../topics/misc/文本组件公共接口.md#ZH-CN_TOPIC_0000002529444865__textlayoutoptions对象说明20)否文本布局选项。

**返回值：**

类型说明Array<[Paragraph](../../modules/ohos/@ohos.graphics.text (文本模块).md#ZH-CN_TOPIC_0000002497605988__paragraph)>[Paragraph](../../modules/ohos/@ohos.graphics.text (文本模块).md#ZH-CN_TOPIC_0000002497605988__paragraph)的数组。
