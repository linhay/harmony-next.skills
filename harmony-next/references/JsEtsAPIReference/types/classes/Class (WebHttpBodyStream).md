...
POST、PUT请求的数据体，支持BYTES、FILE、BLOB、CHUNKED类型的数据。注意本类中其他接口需要在[initialize](#ZH-CN_TOPIC_0000002529285187__initialize12)成功后才能调用。
...
类型说明Promise<void>Promise实例，用于获取WebHttpBodyStream是否初始化成功。

**错误码：**

以下错误码的详细介绍请参见[Webview错误码](../../errors/Webview错误码.md)。

错误码ID错误信息17100022Failed to initialize the HTTP body stream.
...
参数名类型必填说明sizenumber是读取WebHttpBodyStream中的字节数。

**返回值：**

类型说明Promise<ArrayBuffer>Promise实例，用于获取WebHttpBodyStream中读取的数据。
...
完整示例代码参考[initialize](#ZH-CN_TOPIC_0000002529285187__initialize12)。
...
类型说明number获取WebHttpBodyStream中的数据大小。
...
类型说明numberWebHttpBodyStream中当前的读取位置。
...
类型说明booleanWebHttpBodyStream是否采用分块传输，如果采用分块传输则返回true，否则返回false。
