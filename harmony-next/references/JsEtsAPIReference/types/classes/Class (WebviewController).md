...
参数名类型必填说明webTagstring否指定了 Web 组件的名称。
...
参数名类型必填说明secureDnsMode[SecureDnsMode](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__securednsmode10)是使用HTTPDNS的模式。secureDnsConfigstring是HTTPDNS server的配置，必须是https协议并且只允许配置一个server。
...
参数名类型必填说明webDebuggingAccessboolean是
...
参数名类型必填说明urlstring | Resource是需要加载的 URL。headersArray<[WebHeader](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__webheader)>否
...
错误码ID错误信息17100001Init error. The WebviewController must be associated with a Web component.17100002URL error. The webpage corresponding to the URL is invalid, or the URL length exceeds 2048.17100003Invalid resource path or file type.401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameter types. 3.Parameter verification failed.
...
参数名类型必填说明datastring是按照"base64"或者"URL"编码后的一段字符串。mimeTypestring是媒体类型（MIME）。encodingstring是编码类型，具体为"base64"或者"URL"编码。baseUrlstring否指定的一个URL路径（"http"/"https"/"data"协议），并由Web组件赋值给window.origin。当加载大量html文件时，需设置为"data"。historyUrlstring否用作历史记录所使用的URL。非空时，历史记录以此URL进行管理。当baseUrl为空时，此属性无效。

**错误码：**

以下错误码的详细介绍请参见[Webview错误码](../../errors/Webview错误码.md)、[通用错误码](../../errors/通用错误码.md)。

错误码ID错误信息17100001Init error. The WebviewController must be associated with a Web component.401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameter types. 3.Parameter verification failed.
...
可以结合使用[getBackForwardEntries](#ZH-CN_TOPIC_0000002497605192__getbackforwardentries)来获取当前WebView的历史信息列表，以及使用[accessStep](#ZH-CN_TOPIC_0000002497605192__accessstep)来判断是否可以按照给定的步数前进或后退。
...
错误码ID错误信息17100001Init error. The WebviewController must be associated with a Web component.
...
在Web组件首次加载过程中调用[setCustomUserAgent](#ZH-CN_TOPIC_0000002497605192__setcustomuseragent10)，可能会导致在当前存在多个历史节点的情况下，获取的accessBackward实际为false，即没有后退节点。建议先调用setCustomUserAgent方法设置UserAgent，再通过loadUrl加载具体页面。

该现象是由于在Web组件首次加载时，调用[setCustomUserAgent](#ZH-CN_TOPIC_0000002497605192__setcustomuseragent10)会导致组件重新加载并保持初始历史节点的状态。随后新增的节点将替换初始历史节点，不会生成新的历史节点，导致accessBackward为false。

**系统能力：** SystemCapability.Web.Webview.Core

**返回值：**

类型说明boolean当前页面可以后退返回true,否则返回false。
...
按照历史栈，后退一个页面。一般结合[accessBackward](#ZH-CN_TOPIC_0000002497605192__accessbackward)一起使用。
...
此状态下会尽可能的暂停任何可以安全暂停的内容，例如动画和地理位置。但不会暂停JavaScript，要全局暂停JavaScript，请使用[pauseAllTimers](#ZH-CN_TOPIC_0000002497605192__pausealltimers12)。要重新激活Web组件，请调用[onActive](#ZH-CN_TOPIC_0000002497605192__onactive)。
...
namestring是注册对象的名称，与window中调用的对象名一致。注册后window对象可以通过此名字访问应用侧JavaScript对象。methodListArray<string>是参与注册的应用侧JavaScript对象的同步方法。asyncMethodList12+Array<string>否参与注册的应用侧JavaScript对象的异步方法，默认为空。异步方法无法获取返回值。permission12+string否
...
参数名类型必填说明scriptstring是JavaScript脚本。callbackAsyncCallback<string>是回调执行JavaScript脚本结果。JavaScript脚本若执行失败或无返回值时，返回null。
...
参数名类型必填说明scriptstring是JavaScript脚本。

**返回值：**

类型说明Promise<string>Promise实例，返回脚本执行的结果，执行失败返回null。
...
参数名类型必填说明scriptstring | ArrayBuffer12+是JavaScript脚本。callbackAsyncCallback<[JsMessageExt](Class (JsMessageExt).md)>是回调执行JavaScript脚本结果。
...
参数名类型必填说明scriptstring | ArrayBuffer12+是JavaScript脚本。

**返回值：**

类型说明Promise<[JsMessageExt](Class (JsMessageExt).md)>Promise实例，返回脚本执行的结果。
...
删除通过registerJavaScriptProxy注册到window上的指定name的应用侧JavaScript对象。删除后，须调用[refresh](#ZH-CN_TOPIC_0000002497605192__refresh)接口。
...
错误码ID错误信息17100001Init error. The WebviewController must be associated with a Web component.17100008Failed to delete JavaScriptProxy because it does not exist.401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameter types. 3.Parameter verification failed.
...
调整当前网页的缩放比例，[zoomAccess](../../topics/misc/属性.md#ZH-CN_TOPIC_0000002497605206__zoomaccess)需为true。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明factornumber是
...
错误码ID错误信息17100001Init error. The WebviewController must be associated with a Web component.17100004Function not enabled.401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameter types. 3.Parameter verification failed.
...
异步查找网页中所有匹配关键字'searchString'的内容并高亮，结果通过[onSearchResultReceive](../../topics/misc/事件.md#ZH-CN_TOPIC_0000002497445228__onsearchresultreceive9)异步返回。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明searchStringstring是查找的关键字。
...
清除所有通过[searchAllAsync](#ZH-CN_TOPIC_0000002497605192__searchallasync)匹配到的高亮字符查找结果。
...
加载的html文件，请参考[searchAllAsync](#ZH-CN_TOPIC_0000002497605192__searchallasync)接口下加载的html文件。
...
参数名类型必填说明isExtentionType10+boolean否
...
类型说明Array<[WebMessagePort](../interfaces/Interface (WebMessagePort).md)>web消息端口列表。
...
完整示例代码参考[onMessageEventExt](../interfaces/Interface (WebMessagePort).md#ZH-CN_TOPIC_0000002497445222__onmessageeventext10)。
...
参数名类型必填说明namestring是要发送的消息名称。portsArray<[WebMessagePort](../interfaces/Interface (WebMessagePort).md)>是要发送的消息端口。uristring是接收该消息的URI。
...
错误码ID错误信息17100001Init error. The WebviewController must be associated with a Web component.17100004Function not enabled.
...
参数名类型必填说明baseNamestring是生成的离线网页存储位置，该值不能为空。autoNameboolean是

决定是否自动生成文件名。

false表示按baseName的文件名存储，true表示根据当前Url自动生成文件名，并按baseName的文件目录存储。

callbackAsyncCallback<string>是返回文件存储路径，保存网页失败会返回null。

**错误码：**

以下错误码的详细介绍请参见[Webview错误码](../../errors/Webview错误码.md)、[通用错误码](../../errors/通用错误码.md)。

错误码ID错误信息401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameter types. 3. Parameter verification failed.17100001Init error. The WebviewController must be associated with a Web component.17100003Invalid resource path or file type.
...
类型说明Promise<string>Promise实例，保存成功返回文件路径，保存失败返回null。
...
参数名类型必填说明stepnumber是需要前进或后退的步长。
...
风险提示：如果想获取url来做JavascriptProxy通信接口认证，请使用[getLastJavascriptProxyCallingFrameUrl12+](#ZH-CN_TOPIC_0000002497605192__getlastjavascriptproxycallingframeurl12)

**系统能力：** SystemCapability.Web.Webview.Core

**返回值：**

类型说明string当前页面的原始url地址。
...
类型说明image.[PixelMap](../interfaces/Interface (PixelMap).md)页面favicon图标的PixelMap对象。
...
错误码ID错误信息17100001Init error. The WebviewController must be associated with a Web component.401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameter types.
...
错误码ID错误信息17100001Init error. The WebviewController must be associated with a Web component.401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified.
...
参数名类型必填说明clearRomboolean是设置为true时同时清除ROM和RAM中的缓存，设置为false时只清除RAM中的缓存。
...
[onLoadIntercept](../../topics/misc/事件.md#ZH-CN_TOPIC_0000002497445228__onloadintercept10)在加载开始的时候触发，该时刻还未生成历史节点，所以在onLoadIntercept中调用getBackForwardEntries拿到的历史栈不包括当前正在加载中的跳转。

**系统能力：** SystemCapability.Web.Webview.Core

**返回值：**

类型说明[BackForwardList](../interfaces/Interface (BackForwardList).md)当前Webview的历史信息列表。
...
类型说明Uint8Array当前Webview的页面状态历史记录序列化后的数据。
...
1.对文件的操作需要导入文件管理模块，详情请参考[文件管理](../../modules/ohos/@ohos.file.fs (文件管理).md)。
...
参数名类型必填说明stateUint8Array是页面状态历史记录序列化数据。
...
对Web内核赋予自定义协议url的跨域请求与fetch请求的权限。当Web在跨域fetch自定义协议url时，该fetch请求可被[onInterceptRequest](../../topics/misc/事件.md#ZH-CN_TOPIC_0000002497445228__oninterceptrequest9)事件接口所拦截，从而开发者可以进一步处理该请求。建议在任何Web组件初始化之前调用该接口。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明schemesArray<[WebCustomScheme](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__webcustomscheme)>是自定义协议配置，最多支持同时配置10个自定义协议。

**错误码：**

以下错误码的详细介绍请参见[Webview错误码](../../errors/Webview错误码.md)、[通用错误码](../../errors/通用错误码.md)。

错误码ID错误信息401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameter types.17100020Failed to register custom schemes.
...
参数名类型必填说明schemesArray<[WebCustomScheme](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__webcustomscheme)>是自定义协议配置，最多支持同时配置10个自定义协议。lazyInitWebEngineboolean是
...
错误码ID错误信息401Parameter error. Possible causes: 1. The length of the schemes array is greater than 10. 2. The character length of the scheme is greater than 32. 3. The character in the scheme is not within the allowed range of lowercase English letters, numbers, and the symbols ".", "+", "-".17100020Failed to register custom schemes.
...
获取当前网站的证书信息。使用Web组件加载https网站，会进行SSL证书校验，该接口会通过Promise异步返回当前网站的X509格式证书（X509Cert证书类型定义见[X509Cert](../../modules/ohos/@ohos.security.cert (证书模块).md#ZH-CN_TOPIC_0000002529285375__x509cert)定义），便于开发者展示网站证书信息。

**系统能力：** SystemCapability.Web.Webview.Core

**返回值：**

类型说明Promise<Array<cert.X509Cert>>Promise实例，用于获取当前加载的https网站的X509格式证书数组。
...
获取当前网站的证书信息。使用Web组件加载https网站，会进行SSL证书校验，该接口会通过AsyncCallback异步返回当前网站的X509格式证书（X509Cert证书类型定义见[X509Cert定义](../../modules/ohos/@ohos.security.cert (证书模块).md)），便于开发者展示网站证书信息。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明callbackAsyncCallback<Array<cert.X509Cert>>是通过AsyncCallback异步返回当前网站的X509格式证书。
...
错误码ID错误信息401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameter types.17100001Init error. The WebviewController must be associated with a Web component.
...
参数名类型必填说明urlstring是预加载的url。additionalHeadersArray<[WebHeader](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__webheader)>否

url的附加HTTP请求头。

默认值： []

prefetchOptions[PrefetchOptions](Class (PrefetchOptions).md)否用来自定义预取行为的相关选项。

**错误码：**

以下错误码的详细介绍请参见[Webview错误码](../../errors/Webview错误码.md)。

错误码ID错误信息17100001Init error. The WebviewController must be associated with a Web component.17100002URL error. The webpage corresponding to the URL is invalid, or the URL length exceeds 2*1024*1024.
...
根据指定的请求信息和附加的http请求头去预获取资源请求，存入内存缓存，并指定其缓存key和有效期，以加快加载速度。目前仅支持Content-Type为application/x-www-form-urlencoded的post请求。最多可以预获取6个post请求。如果要预获取第7个，请通过[clearPrefetchedResource](#ZH-CN_TOPIC_0000002497605192__clearprefetchedresource12)清除不需要的post请求缓存，否则会自动清除最早预获取的post缓存。如果要使用预获取的资源缓存，开发者需要在正式发起的post请求的请求头中增加键值“ArkWebPostCacheKey”，其内容为对应缓存的cacheKey。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明request[RequestInfo](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__requestinfo12)是预获取请求的信息。additionalHeadersArray<[WebHeader](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__webheader)>否预获取请求的附加HTTP请求头。cacheKeystring否用于后续查询预获取资源缓存的key。仅支持字母和数字，未传入或传入空则取默认值url作为key。cacheValidTimenumber否
...
错误码ID错误信息17100002URL error. The webpage corresponding to the URL is invalid, or the URL length exceeds 2*1024*1024.
...
根据指定的缓存key列表清除对应的预获取资源缓存。入参中的缓存key必须是[prefetchResource](#ZH-CN_TOPIC_0000002497605192__prefetchresource12)指定预获取到的资源缓存key。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明cacheKeyListArray<string>是用于后续查询预获取资源缓存的key。仅支持字母和数字，未传入或传入空则取默认值url作为key。
...
参数名类型必填说明urlstring是预连接的url。preconnectableboolean是是否进行预连接。如果preconnectable为true，则对url进行DNS解析，socket建链预连接；如果preconnectable为false，则不做任何预连接操作。numSocketsnumber是要预连接的socket数。socket数目连接需要大于0，最多允许6个连接。

**错误码：**

以下错误码的详细介绍请参见[Webview错误码](../../errors/Webview错误码.md)。

错误码ID错误信息17100002URL error. The webpage corresponding to the URL is invalid, or the URL length exceeds 2*1024*1024.17100013The number of preconnect sockets is invalid.
...
参数名类型必填说明userAgentstring是用户自定义代理信息。建议先使用[getUserAgent](#ZH-CN_TOPIC_0000002497605192__getuseragent)获取当前默认用户代理，在此基础上追加自定义用户代理信息。
...
当需要设置应用级自定义用户代理时，建议在Web组件创建前调用setAppCustomUserAgent方法设置User-Agent，再创建指定src的Web组件或通过[loadUrl](#ZH-CN_TOPIC_0000002497605192__loadurl)加载具体页面。
...
参数名类型必填说明userAgentstring是用户自定义代理信息。建议先使用[getDefaultUserAgent](#ZH-CN_TOPIC_0000002497605192__getdefaultuseragent14)获取当前默认用户代理，在此基础上追加自定义用户代理信息。
...
当需要对特定网站设置自定义用户代理时，建议在Web组件创建前调用setUserAgentForHosts方法设置User-Agent，再创建指定src的Web组件或通过[loadUrl](#ZH-CN_TOPIC_0000002497605192__loadurl)加载具体页面。
...
参数名类型必填说明userAgentstring是用户自定义代理信息。建议先使用[getDefaultUserAgent](#ZH-CN_TOPIC_0000002497605192__getdefaultuseragent14)获取当前默认用户代理，在此基础上追加自定义用户代理信息。hostsArray<string>是用户自定义代理的相关域名列表，每次调用时仅保留最新传入的列表，并限制最大条目数为两万，超出部分自动截断。
...
参数名类型必填说明timeoutnumber是socket连接超时时间，以秒为单位，必须为大于0的整数。
...
参数名类型必填说明urlstring是需要预热ServiceWorker的url。
...
本功能默认不生效，HarmonyOS只提供恶意网址拦截页WebUI，网址风险检测以及显示WebUI的功能由Vendor实现。推荐在WebContentsObserver中监听跳转[DidStartNavigation](https://gitcode.com/HarmonyOS-tpc/chromium_src/blob/master/content/public/browser/web_contents_observer.h)、[DidRedirectNavigation](https://gitcode.com/HarmonyOS-tpc/chromium_src/blob/master/content/public/browser/web_contents_observer.h)进行检测。
...
错误码ID错误信息17100001Init error. The WebviewController must be associated with a Web component.401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameter types.801Capability not supported.
...
错误码ID错误信息17100001Init error. The WebviewController must be associated with a Web component.801Capability not supported.
...
参数名类型必填说明hostListArray<string>是绕过智能防跟踪功能的域名列表。
...
类型说明stringArkWeb默认User-Agent字符串。
...
错误码ID错误信息17100001Init error. The WebviewController must be associated with a Web component.401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Parameter string is too long. 3.Parameter verification failed.801Capability not supported.
...
当Web组件使能广告过滤功能后，默认所有页面都是开启广告过滤的，支持通过[addAdsBlockDisallowedList](Class (AdsBlockManager).md#ZH-CN_TOPIC_0000002497605186__addadsblockdisallowedlist12)指定域名禁用广告过滤。

**系统能力：** SystemCapability.Web.Webview.Core

**返回值：**

类型说明boolean返回true代表此网页已开启广告过滤，返回false代表当前网页已关闭广告过滤。
...
参数名类型必填说明mode[RenderProcessMode](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__renderprocessmode12)是

渲染子进程模式。

可以先调用[getRenderProcessMode()](#ZH-CN_TOPIC_0000002497605192__getrenderprocessmode12)查看当前设备的ArkWeb渲染子进程模式，枚举值0为单子进程模式，枚举值1为多子进程模式。
...
类型说明[RenderProcessMode](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__renderprocessmode12)
...
使用"POST"方法加载带有postData的url。如果url不是网络url，则会使用[loadUrl](#ZH-CN_TOPIC_0000002497605192__loadurl)方法加载url，忽略postData参数。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明urlstring是需要加载的 URL。postDataArrayBuffer是使用"POST"方法传递数据。 该请求必须采用"application/x-www-form-urlencoded"编码。

**错误码：**

以下错误码的详细介绍请参见[Webview错误码](../../errors/Webview错误码.md)、[通用错误码](../../errors/通用错误码.md)。

错误码ID错误信息17100001Init error. The WebviewController must be associated with a Web component.17100002URL error. The webpage corresponding to the URL is invalid, or the URL length exceeds 2048.401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameter types.
...
参数名类型必填说明jobNamestring是需要打印的文件名。

**返回值：**

类型说明print.[PrintDocumentAdapter](../../modules/ohos/@ohos.print (打印).md#ZH-CN_TOPIC_0000002529285499__printdocumentadapter11)返回打印文档的适配器。

**错误码：**

以下错误码的详细介绍请参见[Webview错误码](../../errors/Webview错误码.md)、[通用错误码](../../errors/通用错误码.md)。

错误码ID错误信息401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameter types. 3.Parameter verification failed.17100001Init error. The WebviewController must be associated with a Web component.
...
类型说明[SecurityLevel](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__securitylevel11)当前网页的安全级别，具体值为NONE、SECURE、WARNING、DANGEROUS。
...
type[ScrollType](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__scrolltype12)否
...
通过[registerJavaScriptProxy](#ZH-CN_TOPIC_0000002497605192__registerjavascriptproxy)或者[javaScriptProxy](../../topics/misc/属性.md#ZH-CN_TOPIC_0000002497605206__javascriptproxy)注入JavaScript对象到window对象中。该接口可以获取最后一次调用注入的对象的frame的url。

**系统能力：** SystemCapability.Web.Webview.Core

**返回值：**

类型说明string最后一次调用注入的对象的frame的url。
...
类型说明[MediaPlaybackState](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__mediaplaybackstate12)当前网页的播放状态，具体值为NONE、PLAYING、PAUSED、STOPPED。
...
为当前Web组件设置[WebSchemeHandler](Class (WebSchemeHandler).md), [WebSchemeHandler](Class (WebSchemeHandler).md)类用于拦截指定scheme的请求。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明schemestring是要拦截的协议。handler[WebSchemeHandler](Class (WebSchemeHandler).md)是拦截此协议的拦截器。

**错误码：**

以下错误码的详细介绍请参见[Webview错误码](../../errors/Webview错误码.md)、[通用错误码](../../errors/通用错误码.md)。

错误码ID错误信息401Parameter error. Possible causes: 1. Incorrect parameter types.17100001Init error. The WebviewController must be associated with a Web component.
...
完整示例代码参考[startCamera](#ZH-CN_TOPIC_0000002497605192__startcamera12)。
...
参数名类型必填说明urlstring是本地JavaScript文件对应的网络地址，即业务网页请求该文件的服务器版本时使用的网络地址。网络地址仅支持http或https协议，长度不超过2048。如果该网络地址对应的缓存失效，则业务网页将通过网络请求对应的资源。scriptstring | Uint8Array是本地JavaScript的文本内容。内容不能为空。cacheOptions[CacheOptions](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__cacheoptions12)是用于控制字节码缓存更新。

**返回值：**

类型说明Promise<number>生成字节码缓存的错误码，0表示无错误，-1表示内部错误。

**错误码：**

以下错误码的详细介绍请参见[Webview错误码](../../errors/Webview错误码.md)、[通用错误码](../../errors/通用错误码.md)。

错误码ID错误信息401Invalid input parameter.Possible causes: 1. Mandatory parameters are left unspecified.2. Incorrect parameter types.3. Parameter verification failed.17100001Init error. The WebviewController must be associated with a Web component.
...
JavaScript资源的获取方式也可通过[网络请求](../../modules/ohos/@ohos.net.http (数据请求).md)的方式获取，但此方法获取到的http响应头非标准HTTP响应头格式，需额外将响应头转换成标准HTTP响应头格式后使用。如通过网络请求获取到的响应头是e-tag，则需要将其转换成E-Tag后使用。
...
注册回调函数，开启[应用接管网页媒体播放功能](../../topics/misc/属性.md#ZH-CN_TOPIC_0000002497605206__enablenativemediaplayer12)后，当网页中有播放媒体时，触发注册的回调函数。
...
参数名类型必填说明callback[CreateNativeMediaPlayerCallback](../../topics/misc/Types.md#ZH-CN_TOPIC_0000002497605202__createnativemediaplayercallback12)是接管网页媒体播放的回调函数。
...
参数名类型必填说明info[SnapshotInfo](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__snapshotinfo12)是全量绘制结果入参。callbackAsyncCallback<[SnapshotResult](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__snapshotresult12)>是全量绘制回调结果。
...
injectOfflineResources(resourceMaps: Array<[OfflineResourceMap](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__offlineresourcemap12)>): void
...
参数名类型必填说明resourceMapsArray<[OfflineResourceMap](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__offlineresourcemap12)>是本地离线资源配置对象，单次调用最大支持注入30个资源，单个资源最大支持10Mb。
...
参数名类型必填说明hostNamestring是要添加DNS记录的主机域名。addressstring是主机域名解析地址（支持IPv4，IPv6）。aliveTimenumber是缓存有效时间（秒）。
...
请参考[clearHostIP](#ZH-CN_TOPIC_0000002497605192__clearhostip12)。
...
参数名类型必填说明hostNamestring是要清除DNS记录的主机域名。
...
字段参数类型必填参数描述schemestring否可选参数，不设置即不匹配该项，支持协议：http、https。hoststring是必选参数，精准匹配，即url的host字段和规则字段完全一致才会放行，可允许同一host多条规则同时生效。portnumber否可选字段，不设置即不匹配该项。pathstring否可选字段，不设置即不匹配该项，匹配方式为前缀匹配，以"pathA/pathB/pathC"为例：pathA/pathB/pathC三级目录下全部允许访问，其中pathC必须是完整的目录名或者文件名，不允许部分匹配。

**错误码：**

以下错误码的详细介绍请参见[Webview错误码](../../errors/Webview错误码.md)、[通用错误码](../../errors/通用错误码.md)。

错误码ID错误信息401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified.2. Parameter string is too long.3. Parameter verification failed.17100001Init error. The WebviewController must be associated with a Web component.
...
设置一个路径列表，当file协议访问该路径列表中的资源时，允许跨域访问本地文件，也允许跨域访问其他在线资源。此外，当设置了路径列表时，file协议仅允许访问路径列表中的资源（[fileAccess](../../topics/misc/属性.md#ZH-CN_TOPIC_0000002497605206__fileaccess)的行为将会被此接口行为覆盖）。路径列表中的路径必须满足以下路径格式之一：

1.应用文件目录的子目录（应用文件目录通过Ability Kit中的[Context.filesDir](../../topics/graphics/Context (Stage模型的上下文基类).md#ZH-CN_TOPIC_0000002529444589__context)获取），例如：

- /data/storage/el2/base/files/example
- /data/storage/el2/base/haps/entry/files/example

2.应用资源目录及其子目录（应用资源目录通过Ability Kit中的[Context.resourceDir](../../topics/graphics/Context (Stage模型的上下文基类).md#ZH-CN_TOPIC_0000002529444589__context)获取），例如：

- /data/storage/el1/bundle/entry/resource/resfile
- /data/storage/el1/bundle/entry/resource/resfile/example

3.从API version 21开始，还包括了应用缓存目录及其子目录（应用缓存目录通过Ability Kit中的[Context.cacheDir](../../topics/graphics/Context (Stage模型的上下文基类).md#ZH-CN_TOPIC_0000002529444589__context)获取），例如：

- /data/storage/el2/base/cache
- /data/storage/el2/base/haps/entry/cache/example
- 设置的目录路径中，不允许包含cache/web，否则会抛出异常码401。如果设置目录路径是cache，cache/web也不允许访问。

4.从API version 21开始，还包括了应用临时目录及其子目录（应用临时目录通过Ability Kit中的[Context.tempDir](../../topics/graphics/Context (Stage模型的上下文基类).md#ZH-CN_TOPIC_0000002529444589__context)获取），例如：

- /data/storage/el2/base/temp
- /data/storage/el2/base/haps/entry/temp/example

当路径列表中有其中一个路径不满足以上条件之一，则会抛出异常码401，并且设置路径列表失败。当设置的路径列表为空，则file协议可访问范围以[fileAccess](../../topics/misc/属性.md#ZH-CN_TOPIC_0000002497605206__fileaccess)的行为为准。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明pathListArray<string>是路径列表
...
需要在[initializeWebEngine()](#ZH-CN_TOPIC_0000002497605192__initializewebengine)初始化内核之前调用。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明features[BackForwardCacheSupportedFeatures](Class (BackForwardCacheSupportedFeatures).md)是允许使用特定的页面进入前进后退缓存中。
...
参数名类型必填说明options[BackForwardCacheOptions](Class (BackForwardCacheOptions).md)是用来控制Web组件前进后退缓存相关选项。
...
参数名类型必填说明level[PressureLevel](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__pressurelevel14)是需要清理内存的内存等级。

**错误码：**

以下错误码的详细介绍请参见[通用错误码](../../errors/通用错误码.md)。

错误码ID错误信息401Parameter error. Possible causes: 1. Mandatory parameters are left unspecified. 2. Parameter string is too long. 3.Parameter verification failed.
...
参数名类型必填说明configuration[PdfConfiguration](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__pdfconfiguration14)是生成PDF所需参数。callbackAsyncCallback<[PdfData](Class (PdfData).md)>是回调返回网页PDF数据流。

**错误码：**

以下错误码的详细介绍请参见[Webview错误码](../../errors/Webview错误码.md)、[通用错误码](../../errors/通用错误码.md)。

错误码ID错误信息401Invalid input parameter.17100001Init error. The WebviewController must be associated with a Web component.
...
参数名类型必填说明configuration[PdfConfiguration](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__pdfconfiguration14)是生成PDF所需参数。

**返回值：**

类型说明Promise<[PdfData](Class (PdfData).md)>Promise实例，返回网页数据流。
...
类型说明[ScrollOffset](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__scrolloffset13)网页当前的滚动偏移量（包含过滚动偏移量）。
...
类型说明[ScrollOffset](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__scrolloffset13)网页当前的滚动偏移量（不包含过滚动偏移量）。
...
类型说明[HitTestValue](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__hittestvalue)点击区域的元素信息。
...
类型说明[ControllerAttachState](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__controllerattachstate20)WebViewController与Web组件的绑定状态。
...
参数名类型必填说明typestring是表示注册WebViewController绑定状态事件，固定为"controllerAttachStateChange"。callbackCallback<[ControllerAttachState](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__controllerattachstate20)>是WebViewController绑定状态改变时的回调函数。

**示例：**

请参考[off](#ZH-CN_TOPIC_0000002497605192__offcontrollerattachstatechange20)。
...
参数名类型必填说明typestring是表示注册WebViewController绑定状态事件，固定为"controllerAttachStateChange"。callbackCallback<[ControllerAttachState](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__controllerattachstate20)>否WebViewController绑定状态发生改变时的回调函数，默认情况下不填写回调函数。如果填写了Callback，将仅取消注册该特定的回调。如果不填写Callback，将取消注册所有回调。
...
异步等待WebViewController与Web组件绑定完成，绑定完成或超时触发回调，通过Promise方式返回当前[ControllerAttachState](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__controllerattachstate20)状态。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明timeoutnumber是
...
类型说明Promise<[ControllerAttachState](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__controllerattachstate20)>Promise实例，返回当前[ControllerAttachState](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__controllerattachstate20)状态。
...
- 当没有指定端口port时，该接口等同于[setWebDebuggingAccess](#ZH-CN_TOPIC_0000002497605192__setwebdebuggingaccess)接口，ArkWeb会启动一个本地domain socket监听。
...
指定devtools服务的tcp端口号。如果没有指定port，那么该接口等同于[setWebDebuggingAccess](#ZH-CN_TOPIC_0000002497605192__setwebdebuggingaccess)接口。
...
错误码ID错误信息17100023The port number is not within the allowed range.
...
从API version11开始支持，从API version 18开始废弃。建议使用[getLastHitTest](#ZH-CN_TOPIC_0000002497605192__getlasthittest18)替代。

**系统能力：** SystemCapability.Web.Webview.Core

**返回值：**

类型说明[WebHitTestType](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__webhittesttype)被点击区域的元素类型。
...
- 该接口高度设置为0时，Web网页内容可恢复，键盘弹起避让模式将使用[keyboardAvoidMode()](../../topics/misc/属性.md#ZH-CN_TOPIC_0000002497605206__keyboardavoidmode12)声明的模式。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明avoidHeightnumber是
...
在当前接口设置为true时如果页面加载发生错误将触发[onOverrideErrorPage](../../topics/misc/事件.md#ZH-CN_TOPIC_0000002497445228__onoverrideerrorpage20)回调，可在该回调接口中设置自定义的错误展示页面。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明enableboolean是表示是否启用默认错误页。true表示启用，false表示不启用。
...
参数名类型必填说明enableboolean是是否启用私有网络访问检查功能开关。true表示启用，false表示禁用。
...
类型说明boolean返回Web组件是否启用了私有网络访问检查功能。true表示已启用；false表示已禁用。
...
获取页面首屏加载预测信息（详细说明见[BlanklessInfo](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__blanklessinfo20)），并开始本次加载过渡帧生成，应用根据此信息确定是否需要启用无白屏加载。必须与[setBlanklessLoadingWithKey](#ZH-CN_TOPIC_0000002497605192__setblanklessloadingwithkey20)接口配套使用，并且必须在触发加载页面的接口之前或在onLoadIntercept中调用。需在WebViewController与Web组件绑定后才能使用。

- 持久缓存容量：默认大小为30MB（约30页），可以通过接口[setBlanklessLoadingCacheCapacity](#ZH-CN_TOPIC_0000002497605192__setblanklessloadingcachecapacity20)设置缓存容量，具体见该接口说明。超过容量时根据LRU（Least Recently Used，淘汰不常用缓存的策略）机制更新缓存。自动清理超过7天的持久缓存数据，缓存清除后第三次加载页面开始有优化效果。
- 如果发现快照相似度（即[BlanklessInfo](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__blanklessinfo20)中的similarity）极低，请确认key值是否传递正确。
...
类型说明[BlanklessInfo](../interfaces/Interfaces (其他).md#ZH-CN_TOPIC_0000002529285193__blanklessinfo20)页面首屏加载预测信息，主要包括首屏相似度预测值，首屏加载耗时预测值，应用需根据此信息来决策是否启用无白屏加载插帧。
...
设置无白屏加载是否启用，本接口必须与[getBlanklessInfoWithKey](#ZH-CN_TOPIC_0000002497605192__getblanklessinfowithkey20)接口配套使用。

- 需在触发页面加载的接口之后调用，其他约束同[getBlanklessInfoWithKey](#ZH-CN_TOPIC_0000002497605192__getblanklessinfowithkey20)。
- 页面加载必须在调用本接口的组件中进行。
...
类型说明[WebBlanklessErrorCode](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__webblanklesserrorcode20)返回接口调用是否成功，具体见[WebBlanklessErrorCode](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__webblanklesserrorcode20)定义。
...
参数名类型必填说明keysArray<string>否

清除Blankless优化方案页面的key值列表，key值为[getBlanklessInfoWithKey](#ZH-CN_TOPIC_0000002497605192__getblanklessinfowithkey20)中指定过的。

默认值：所有Blankless优化方案缓存的页面key列表。

合法取值范围：长度不超过2048，key列表长度<=100。key和加载页面时输入给ArkWeb的相同。

非法值设置行为：key长度超过2048时该key不生效；长度超过100时，取前100个；当为空时，使用默认值。
...
设置Web组件的销毁模式。当Web组件销毁时，销毁模式会影响Web内核资源释放的时机，例如JavaScript运行上下文、渲染上下文等。默认值：[WebDestroyMode.NORMAL_MODE](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__webdestroymode20)（普通模式），由系统决定销毁时机。应用可设置[WebDestroyMode.FAST_MODE](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__webdestroymode20)（快速模式），以立即销毁资源，从而提升特定场景的性能。

[WebDestroyMode.FAST_MODE](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__webdestroymode20)（快速模式）会改变Web组件销毁时机，应用需关注依赖Web组件销毁时机的错误实现，例如：Web组件销毁后仍调用WebviewController的未定义行为，与[WebDestroyMode.NORMAL_MODE](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__webdestroymode20)（普通模式）相比，销毁时机提前，有更高的几率触发未关联绑定的异常（17100001），建议应用捕捉异常，或者通过[getAttachState](#ZH-CN_TOPIC_0000002497605192__getattachstate20)方法查询是否绑定状态，来避免稳定性问题。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明mode[WebDestroyMode](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__webdestroymode20)是
...
在Web页面场景，设置全局滚动条模式。不显式调用时，默认为[ScrollbarMode.OVERLAY_LAYOUT_SCROLLBAR ](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__scrollbarmode23)（非常驻滚动条）。

- 根据滚动条模式，改变当前应用所有web滚动条模式为常驻滚动条或非常驻滚动条。
- 若[forceDisplayScrollBar](../../topics/misc/属性.md#ZH-CN_TOPIC_0000002497605206__forcedisplayscrollbar14)接口与当前接口同时设置，forceDisplayScrollBar接口设置不生效。
...
参数名类型必填说明scrollbarMode[ScrollbarMode](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__scrollbarmode23)是滚动条模式。
...
在HarmonyOS 6.0及以后，使用遗留内核时，部分ArkWeb接口不会生效，参考[M114内核在HarmonyOS6.0系统上的适配指导](https://gitcode.com/HarmonyOS-tpc/chromium_src/blob/132_trunk/web/ReleaseNote/CompatibleWithLegacyWebEngine.md)。
...
参数名类型必填说明engineVersion[ArkWebEngineVersion](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__arkwebengineversion20)是ArkWeb内核版本。
...
类型说明[ArkWebEngineVersion](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__arkwebengineversion20)返回由[ArkWebEngineVersion](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__arkwebengineversion20)所定义的当前使用的ArkWeb内核版本。

**示例：**

请参考[setActiveWebEngineVersion](#ZH-CN_TOPIC_0000002497605192__setactivewebengineversion20)。
...
需要在[initializeWebEngine()](#ZH-CN_TOPIC_0000002497605192__initializewebengine)初始化内核或者创建Web组件之前调用。

**系统能力：** SystemCapability.Web.Webview.Core

**参数：**

参数名类型必填说明enabledboolean是是否启用Web内核自动预连接的开关。true表示启用，false表示禁用。
...
如果没有使用[setAutoPreconnect](#ZH-CN_TOPIC_0000002497605192__setautopreconnect21)设置Web内核自动预连接的状态，则默认启用自动预连接，返回true。

**系统能力：** SystemCapability.Web.Webview.Core

**返回值：**

类型说明boolean返回Web内核是否启用了自动预连接。true表示已启用；false表示已禁用。
...
类型说明[SiteIsolationMode](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__siteisolationmode21)
...
对于仅加载可信网页的第三方应用，可以关闭此功能，以提升性能并减少内存占用，同时减少跨域访问的拦截。默认值根据不同的设备而定，PC/Table采用严格站点隔离[SiteIsolationMode.STRICT](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__siteisolationmode21)，Phone默认部分站点隔离[SiteIsolationMode.PARTIAL](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__siteisolationmode21)。[坚盾守护模式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/web-secure-shield-mode)下采用严格站点隔离。
...
参数名类型必填说明mode[SiteIsolationMode](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__siteisolationmode21)是
...
错误码ID错误信息17100001Init error. Possible causes: 1. Site Isolation mode is already set by the developer. 2. Site Isolation mode cannot be strict in single-render-process mode. 3. Site Isolation mode cannot be changed while Secure Shield mode is active.
...
参数名类型必填说明mode[WebSoftKeyboardBehaviorMode](../../topics/misc/Enums.md#ZH-CN_TOPIC_0000002529445167__websoftkeyboardbehaviormode22)是Web软键盘自动控制模式。
