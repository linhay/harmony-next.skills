...
参数名类型必填说明admin[Want](@ohos.app.ability.Want (Want).md)是企业设备管理扩展组件。Want中必须包含企业设备管理扩展能力的abilityName和所在应用的bundleName。certificate[CertBlob](#ZH-CN_TOPIC_0000002529445557__certblob)是证书信息。证书文件应放在应用沙箱路径等应用有权限访问的路径下。accountIdnumber是用户ID，指定具体用户，取值范围：大于等于0。accountId可以通过@ohos.account.osAccount中的[getOsAccountLocalId](zh-cn_topic_0000002529285493.html#ZH-CN_TOPIC_0000002529285493__getosaccountlocalid9-1)等接口来获取。
...
参数名类型必填说明admin[Want](@ohos.app.ability.Want (Want).md)是企业设备管理扩展组件。Want中必须包含企业设备管理扩展能力的abilityName和所在应用的bundleName。tokenIdnumber是目标应用的身份标识。可通过[bundleManager.getApplicationInfo](ApplicationInfo.md)获取accessTokenId。当前只支持最多100个tokenId被保存策略。policy[ClipboardPolicy](#ZH-CN_TOPIC_0000002529445557__clipboardpolicy)是剪贴板策略。
...
参数名类型必填说明admin[Want](@ohos.app.ability.Want (Want).md)是企业设备管理扩展组件。Want中必须包含企业设备管理扩展能力的abilityName和所在应用的bundleName。tokenIdnumber否目标应用的身份标识。可通过[bundleManager.getApplicationInfo](ApplicationInfo.md)获取accessTokenId。当前只支持最多100个tokenId被保存策略。
...
参数名类型必填说明admin[Want](@ohos.app.ability.Want (Want).md)是企业设备管理扩展组件。Want中必须包含企业设备管理扩展能力的abilityName和所在应用的bundleName。bundleNamestring是被设置水印的应用包名。sourcestring | [image.PixelMap](Interface (PixelMap).md)是
